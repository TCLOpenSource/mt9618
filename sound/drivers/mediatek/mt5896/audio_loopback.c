...
static unsigned int audio_loopback_get_capture_dsp_offset(struct mtk_snd_loopback *loopback)
{
	...
	if (loopback->loopback_pcm_sw_running == 1) {
		if (loopback->loopback_pcm_sw_running_stopped == 1) {
			...
			loopback->dsp_dma_buf.r_buf.sw_w_offset =
				(loopback->dsp_dma_buf.r_buf.sw_w_offset + offset_bytes) %
				(loopback->dsp_dma_buf.r_buf.size);

			offset  = loopback->dsp_dma_buf.r_buf.sw_w_offset;
		} else {
			offset = loopback->dsp_pb_dma_buf.r_buf.sw_w_offset;
			loopback->dsp_dma_buf.r_buf.sw_w_offset = offset;
...
static int audio_loopback_dma_start(struct mtk_snd_loopback *loopback)
{
	...
	if (loopback->dsp_mixing_mode &&
		...
		timespec_add_ns(&_stTimeStamp, DIGITAL_DELAY_NS_UNIT);
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (!loopback->dsp_mixing_mode) {
			...
			period_size = INTERVAL_UNIT *
				(loopback->dsp_dma_buf.sample_rate/RATE_16000);
			...
			new_w_ptr = loopback->dsp_dma_buf.r_buf.addr +
				((loopback->dsp_dma_buf.r_buf.w_ptr -
				loopback->dsp_dma_buf.r_buf.addr + offset_bytes) %
				loopback->dsp_dma_buf.r_buf.size);
			loopback->pcm_capture_ts.tv_sec = _stTimeStamp.tv_sec;
			loopback->pcm_capture_ts.tv_nsec = _stTimeStamp.tv_nsec;
			timespec_add_ns(&_stTimeStamp, PERIOD_NS_UNIT);
	...
	struct platform_device *pdev, struct mtk_snd_loopback *loopback)
{
	...
	if (voc_np) {
		...
		if (ret == 0
			&& (cap_source != NULL && cap_source[0] == 'S' && cap_source[1] == 'W'))
			sw_loop = 1;
	}

	if (sw_loop & mixing_mode) {
		loopback->i2s_input_channel = CH_2;
		loopback->dsp_capture_source = LOOPBACK_FROM_SPEAKER;
	} else {
		switch (loopback->i2s_input_channel) {
		case CH_2:
			loopback->dsp_capture_source = LOOPBACK_FROM_I2S_RX_2CH;
			break;
		case CH_4:
			loopback->dsp_capture_source = LOOPBACK_FROM_I2S_RX_4CH;
			break;
		case CH_8:
			loopback->dsp_capture_source = LOOPBACK_FROM_I2S_RX_8CH;
			break;
		default:
