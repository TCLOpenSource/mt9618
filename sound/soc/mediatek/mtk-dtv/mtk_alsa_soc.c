...
#define DEC_PLAY_CMD	0x9500AC
#define time_interval	10

#define REG_VIVALDI05_BASE	0x951500
#define REG_AUDIO_CH_M1_CFG	(REG_VIVALDI05_BASE + 0x00) // [4:0]
#define REG_AUDIO_CH_M2_CFG	(REG_VIVALDI05_BASE + 0x01)
#define REG_AUDIO_CH_M3_CFG	(REG_VIVALDI05_BASE + 0x02)
#define REG_AUDIO_CH_M4_CFG	(REG_VIVALDI05_BASE + 0x03)
#define REG_AUDIO_CH_M5_CFG	(REG_VIVALDI05_BASE + 0x04)
#define REG_AUDIO_CH_M6_CFG	(REG_VIVALDI05_BASE + 0x05)
#define REG_AUDIO_CH_M7_CFG	(REG_VIVALDI05_BASE + 0x06)
#define REG_AUDIO_CH_M8_CFG	(REG_VIVALDI05_BASE + 0x07)
#define REG_AUDIO_CH_M9_CFG	(REG_VIVALDI05_BASE + 0x08)
#define REG_AUDIO_CH_M10_CFG	(REG_VIVALDI05_BASE + 0x09)
#define REG_AUDIO_CH_M11_CFG	(REG_VIVALDI05_BASE + 0x0A)
#define REG_AUDIO_CH_M12_CFG	(REG_VIVALDI05_BASE + 0x0B)
#define REG_AUDIO_CH_M13_CFG	(REG_VIVALDI05_BASE + 0x0C)
#define REG_AUDIO_CH_M14_CFG	(REG_VIVALDI05_BASE + 0x0D)
#define REG_AUDIO_CH_M15_CFG	(REG_VIVALDI05_BASE + 0x0E)
#define REG_AUDIO_CH_M16_CFG	(REG_VIVALDI05_BASE + 0x0F)

#define REG_AUDIO_CH5_CFG	(REG_VIVALDI05_BASE + 0x10)
#define REG_AUDIO_CH6_CFG	(REG_VIVALDI05_BASE + 0x11)
#define REG_AUDIO_CH7_CFG	(REG_VIVALDI05_BASE + 0x12)
#define REG_AUDIO_CH8_CFG	(REG_VIVALDI05_BASE + 0x13)
#define REG_AUDIO_CH9_CFG	(REG_VIVALDI05_BASE + 0x14)
#define REG_AUDIO_CH10_CFG	(REG_VIVALDI05_BASE + 0x15)

struct timer_list timer_charge;

static int hw_reboot_cycle_cntr;
static int hw_reboot_state;
...
static const struct clk_type mux_clk_table[] = {
	...
	/* set mux clk : viva_audma_v2_r1_256fs_mux_int_ck from dtsi to 0 */
	{MUX_AUDMA_V2_R1_256FS_MUX, 0},
	...
	/* set mux clk : viva_audma_v2_r2_256fs_int_ck from dtsi to 0 */
	{MUX_AUDMA_V2_R2_256FS, 0},
	...
	{MUX_SPDIF_RX_DG, 1},
	/* set mux clk : viva_spdif_rx_synth_ref_int_ck from dtsi to 1 */
	{MUX_SPDIF_RX_SYNTH_REF, 1},
	...
	/* set mux clk : viva_i2s_tdm_1x16tx32b_bclk_int_ck from dtsi to 2 */
	{MUX_I2S_TDM_1X16TX32B_BCLK, 2},
	/* set mux clk : viva_i2s_tdm_1x16tx32b_fifo_int_ck from dtsi to 0 */
	{MUX_I2S_TDM_1X16TX32B_FIFO, 0},
	/* set mux clk : viva_i2s_tdm_1x16tx32b_bclk_to_pad_int_ck from dtsi to 0 */
	{MUX_I2S_TDM_1X16TX32B_BCLK_TO_PAD, 0},
...
struct mtk_soc_priv {
	...
	const char	*ini_path;
	char		pre_init_table[MAX_STRING_LEN];
	char		init_table[MAX_STRING_LEN];
	char		depop_table[MAX_STRING_LEN];
	char		earc_table[MAX_STRING_LEN];
	struct clk	**gate_clk;
	struct clk	**mux_clk;
	struct clk	**clk;
...
static unsigned short mtk_spdif_clk_flag;
static unsigned short mtk_arc_earc_clk_flag;
...
{
	if (__clk_is_enabled(priv->mux_clk[clk_name]))
...
int mtk_earc_clk_set_enable(int format)
{
	...
	/* set mux clk : viva_spdif_tx2_128fs_int_ck from dtsi to 1 */
	ret = mtk_alsa_mux_clock_setting_by_name("viva_earc_tx_source_256fs_int_ck", 1);
	if (ret)
		return ret;
	ret = mtk_alsa_mux_clock_enable_by_name("viva_earc_tx_source_256fs_int_ck");
	if (ret)
		return ret;
	...
	case 0:
		/* set mux clk : spdif_tx2_128fs_mux_int_ck from dtsi to 0 */
		ret = mtk_alsa_mux_clock_setting_by_name("earc_tx_source_256fs_mux_int_ck", 0);

		if (ret)
			break;
		ret = mtk_alsa_mux_clock_enable_by_name("earc_tx_source_256fs_mux_int_ck");
		if (ret)
			break;

		/* set mux clk : spdif_tx2_128fs_mux_int_ck from dtsi to 0 */
		ret = mtk_alsa_mux_clock_setting_by_name("earc_tx_gpa_256fs_mux_int_ck", 0);
		if (ret)
			break;
		ret = mtk_alsa_mux_clock_enable_by_name("earc_tx_gpa_256fs_mux_int_ck");
		if (ret)
			break;
		//wriu     0x10265a             0x02
		ret = mtk_alsa_mux_clock_setting_by_name("earc_tx_pll_256fs_ref_mux_int_ck", 0);
		if (ret)
			break;
	...
	case 1:
		/* set mux clk : spdif_tx2_128fs_mux_int_ck from dtsi to 0 */
		ret = mtk_alsa_mux_clock_setting_by_name("earc_tx_source_256fs_mux_int_ck", 2);
		if (ret)
			break;
		ret = mtk_alsa_mux_clock_enable_by_name("earc_tx_source_256fs_mux_int_ck");
		if (ret)
	...
	case 2:
		/* set mux clk : spdif_tx2_128fs_mux_int_ck from dtsi to 0 */
		ret = mtk_alsa_mux_clock_setting_by_name("earc_tx_source_256fs_mux_int_ck", 3);
		if (ret)
			break;
		ret = mtk_alsa_mux_clock_enable_by_name("earc_tx_source_256fs_mux_int_ck");
		if (ret)
		...
		//wriu     0x10265a             0x02
		ret = mtk_alsa_mux_clock_setting_by_name("earc_tx_pll_256fs_ref_mux_int_ck", 3);
		if (ret)
			break;
		ret = mtk_alsa_mux_clock_enable_by_name("earc_tx_pll_256fs_ref_mux_int_ck");
		if (ret)
	...
	default:
		apr_err(MTK_ALSA_LEVEL_ERROR,
			"[ALSA SOC]Err! un-supported earc clk format!!!\n");
		break;
...
void mtk_src_data_mux_init(void)
{
	/* CH-M1 sel to Decoder1(1,2) DMA Reader */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH_M1_CFG, 0x1F, 0x00);
	/* CH-M2 sel to Decoder1(3,4) DMA Reader */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH_M2_CFG, 0x1F, 0x00);
	/* CH-M3 sel to Decoder1(5,6) DMA Reader */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH_M3_CFG, 0x1F, 0x00);
	/* CH-M4 sel to Decoder1(7,8) DMA Reader */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH_M4_CFG, 0x1F, 0x00);
	/* CH-M5 sel to Decoder1(9,10) DMA Reader */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH_M5_CFG, 0x1F, 0x00);
	/* CH-M6 sel to Decoder1(11,12) DMA Reader */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH_M6_CFG, 0x1F, 0x00);
	/* CH-M7 sel to Decoder4(1,2) input */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH_M7_CFG, 0x1F, 0x03);
	/* CH-M8 sel to I2S_RX2(1,2) input */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH_M8_CFG, 0x1F, 0x0F);
	/* CH-M9 sel to Decoder3(1,2) DMA Reader */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH_M9_CFG, 0x1F, 0x02);
	/* CH-M10 sel to Decoder3(3,4) DMA Reader */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH_M10_CFG, 0x1F, 0x02);
	/* CH-M11 sel to Decoder3(5,6) DMA Reader */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH_M11_CFG, 0x1F, 0x02);
	/* CH-M12 sel to Decoder3(7,8) DMA Reader */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH_M12_CFG, 0x1F, 0x02);
	/* CH-M13 sel to Decoder3(9,10) DMA Reader */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH_M13_CFG, 0x1F, 0x02);
	/* CH-M14 sel to Decoder3(11,12) DMA Reader */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH_M14_CFG, 0x1F, 0x02);
	/* CH-M15 sel to ADC2(1,2) DMA Reader */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH_M15_CFG, 0x1F, 0x09);
	/* CH-M16 sel to ADC1(1,2) DMA Reader */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH_M16_CFG, 0x1F, 0x08);
	/* CH5 sel to Decoder5(5,6) DMA Reader */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH5_CFG, 0x1F, 0x0A);
	/* CH6 sel to Decoder5(7,8) DMA Reader */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH6_CFG, 0x1F, 0x0A);
	/* CH7 sel to Decoder5(1,2) DMA Reader */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH7_CFG, 0x1F, 0x0A);
	/* CH8 sel to Decoder5(3,4) DMA Reader */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH8_CFG, 0x1F, 0x0A);
	/* CH9 sel to Decoder5(9,10) DMA Reader */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH9_CFG, 0x1F, 0x0A);
	/* CH10 sel to Decoder5(11,12) DMA Reader */
	mtk_alsa_write_reg_mask_byte(REG_AUDIO_CH10_CFG, 0x1F, 0x0A);
}
...
int mtk_alsa_read_chip(void)
{
	...
	if (priv->ip_version == 0x0100) {
		apr_info(MTK_ALSA_LEVEL_INFO, "[ALSA MTK] this is m6L\n");
		chip_num = 1;
	} else {
		apr_info(MTK_ALSA_LEVEL_INFO, "[ALSA MTK] this is m6\n");
		chip_num = 0;
	}
	return chip_num;
}
...
int mtk_alsa_set_hw_reboot(int cycle_cntr, int state)
{
	hw_reboot_cycle_cntr = cycle_cntr;
	hw_reboot_state = state;

	return 0;
}
EXPORT_SYMBOL_GPL(mtk_alsa_set_hw_reboot);

int mtk_alsa_get_hw_reboot_flag(int device_cycle_cntr)
{
	if ((device_cycle_cntr < hw_reboot_cycle_cntr) || (hw_reboot_state))
		return 1;
	else
		return 0;
}
EXPORT_SYMBOL_GPL(mtk_alsa_get_hw_reboot_flag);

int mtk_alsa_get_hw_reboot_cycle(void)
{
	if (hw_reboot_state)
		return -1;
	else
		return hw_reboot_cycle_cntr;
}
EXPORT_SYMBOL_GPL(mtk_alsa_get_hw_reboot_cycle);
...
static void mtk_charge_monitor(struct timer_list *timer)
{
	...
	if (timer == NULL) {
		apr_err(MTK_ALSA_LEVEL_ERROR, "[%s] timer is NULL.\n", __func__);
		return;
	}
	...
	if (chg_finished == 1) {
		...
		timer->expires = jiffies + msecs_to_jiffies(time_interval);
		add_timer(timer);
	}
}
...
int mtk_init_comp_resume(struct snd_soc_component *component)
{
	...
	if (clock_write(CLK_NONPM, 0x1BAC, 1, 0, 0) < 0)
	...
	// do src data mux init
	mtk_src_data_mux_init();

	mtk_io_power_disable();

	/* Charge timer setup */
	timer_setup(&timer_charge, mtk_charge_monitor, 0);
	timer_charge.expires = jiffies + msecs_to_jiffies(time_interval);
...
static int mtk_init_probe(struct snd_soc_component *component)
{
	...
	if (ret)
		dev_err(dev, "[AUDIO][ERROR][%s]sysfs_create_groups fail !!!\n", __func__);
	...
	for_each_card_rtds(component->card, rtd) {
		...
		if ((!strncmp(dai_link->cpus->dai_name, "HDMI-RX2", strlen("HDMI-RX2"))) &&
			...
			if (ret)
				dev_err(dev, "[AUDIO][%s]sysfs_merge_groups fail!\n", __func__);
		}
	}
...
int mtk_soc_dev_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < priv->bank_num; i++) {
		...
		if ((res->start == pu32array[IOMAP_1]) || (res->start == pu32array[IOMAP_2]) ||
			(res->start == pu32array[IOMAP_3]) || (res->start == pu32array[IOMAP_4]) ||
			(res->start == pu32array[IOMAP_5])) {
			priv->reg_base[i] = of_iomap(node, i);
		} else
			priv->reg_base[i] = devm_ioremap_resource(&pdev->dev, res);

		if (IS_ERR((const void *)priv->reg_base[i])) {
			dev_err(dev, "[ALSA MTK]err reg_base=%lx\n",
	...
	/* stop play anyway */
	mtk_alsa_write_reg(DEC_PLAY_CMD, 0x0);

	// do src data mux init
	mtk_src_data_mux_init();

	/* get ini path */
	ret = of_property_read_string(node, "ini_path", &name);
	if (ret) {
		adev_err(dev, MTK_ALSA_LEVEL_ERROR,
			"[ALSA MTK]can't get audio ini path\n");
		return -EINVAL;
	}
	priv->ini_path = name;
	strncpy(priv->pre_init_table, name, strlen(name));
	strncpy(priv->init_table, name, strlen(name));
	strncpy(priv->depop_table, name, strlen(name));
	strncpy(priv->earc_table, name, strlen(name));

	/* get audio depop off table */
	ret = of_property_read_string(node, "depop_table", &name);
	if (ret) {
		dev_err(dev, "[ALSA MTK]can't get audio depop table path\n");
		return -EINVAL;
	}
	strncat(priv->depop_table, name, strlen(name));
	...
	if (mtk_alsa_read_reg_byte(0x951040) & 0x4) {
		// if depop is on then do depop off to prevent do twice
		ret = mtk_alsa_init(2);
		if (ret < 0) {
			dev_err(dev, "[ALSA MTK]can't load auto depop off table\n");
			return -EINVAL;
		}
	}
	...
	strncat(priv->pre_init_table, name, strlen(name));
	...
	strncat(priv->init_table, name, strlen(name));
	...
	strncat(priv->earc_table, name, strlen(name));
...
int mtk_soc_pm_resume(struct device *dev)
{
	...
	/* Charge timer has been set up in mtk_init_comp_resume*/
