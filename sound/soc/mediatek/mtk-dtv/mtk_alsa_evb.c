...
SND_SOC_DAILINK_DEFS(fe_tdm_playback,
	DAILINK_COMP_ARRAY(COMP_CPU("TDM-TX")),
	DAILINK_COMP_ARRAY(COMP_DUMMY()),
	DAILINK_COMP_ARRAY(COMP_PLATFORM("mtk-sound-playback-platform")));
...
SND_SOC_DAILINK_DEFS(be_i2s_tx2,
	DAILINK_COMP_ARRAY(COMP_CPU("BE_I2S_TX2")),
	DAILINK_COMP_ARRAY(COMP_DUMMY()),
	DAILINK_COMP_ARRAY(COMP_PLATFORM("mtk-sound-playback-platform")));
...
SND_SOC_DAILINK_DEFS(amp_dev_1,
	DAILINK_COMP_ARRAY(COMP_CPU("AMPLIFIER_1")),
	DAILINK_COMP_ARRAY(COMP_DUMMY()));

SND_SOC_DAILINK_DEFS(amp_dev_2,
	DAILINK_COMP_ARRAY(COMP_CPU("AMPLIFIER_2")),
	DAILINK_COMP_ARRAY(COMP_DUMMY()));

SND_SOC_DAILINK_DEFS(amp_dev_3,
	DAILINK_COMP_ARRAY(COMP_CPU("AMPLIFIER_3")),
	DAILINK_COMP_ARRAY(COMP_DUMMY()));

SND_SOC_DAILINK_DEFS(amp_dev_4,
	DAILINK_COMP_ARRAY(COMP_CPU("AMPLIFIER_4")),
	DAILINK_COMP_ARRAY(COMP_DUMMY()));

#define DAI_LINK_MULTI_AMP(x) \
{ \
	.name = "MULTI_"#x, \
	.stream_name = "MULTI_"#x, \
	.ignore = 1, \
	.no_pcm = 1, \
	SND_SOC_DAILINK_REG(x), \
}

#define IDX_MAX		18	/* codec-dai-name-x"\0" */
...
static struct snd_soc_dai_link mtk_snd_dais[] = {
	...
	[DAI_LINK_TDM_PLAYBACK] = {
		...
		.id = DAI_LINK_TDM_PLAYBACK,
		.dynamic = 1,
		.dpcm_playback = 1,
		.playback_only = 1,
		SND_SOC_DAILINK_REG(fe_tdm_playback),
	},
	/* Digital out links */
	[DAI_LINK_SPDIF_PLAYBACK] = {
	...
	[DAI_LINK_I2S_TX2_BE] = {
		.name = "I2S-TX2",
		.no_pcm = 1,
		.id = DAI_LINK_I2S_TX2_BE,
		.dpcm_playback = 1,
		.playback_only = 1,
		SND_SOC_DAILINK_REG(be_i2s_tx2),
	},
	...
	/* multiple amp */
	[DAI_LINK_AMP_1] = DAI_LINK_MULTI_AMP(amp_dev_1),
	[DAI_LINK_AMP_2] = DAI_LINK_MULTI_AMP(amp_dev_2),
	[DAI_LINK_AMP_3] = DAI_LINK_MULTI_AMP(amp_dev_3),
	[DAI_LINK_AMP_4] = DAI_LINK_MULTI_AMP(amp_dev_4),
};
...
static int mtk_parse_duplex_capability(struct device_node *np)
{
	...
	if (!of_property_read_bool(np, "src3"))
		...
		ret = of_property_read_string(np, "src3", &device_status);
...
static int mtk_parse_playback_capability(struct device_node *np)
{
	...
	if (!of_property_read_bool(np, "hfp"))
		...
		ret = of_property_read_string(np, "hfp", &device_status);
	...
	if (!of_property_read_bool(np, "tdm-tx2"))
		mtk_snd_dais[DAI_LINK_TDM_PLAYBACK].ignore = 1;
	else {
		ret = of_property_read_string(np, "tdm-tx2", &device_status);

		if (ret == 0) {
			if (!strncmp(device_status, "disable", strlen("disable")))
				mtk_snd_dais[DAI_LINK_TDM_PLAYBACK].ignore = 1;
		} else
			pr_err("%s[%d] parser tdm dto error\n", __func__, __LINE__);
	}
	...
	struct device_node *np)
{
	...
	if (!of_property_read_bool(np, "hdmi-rx2"))
		...
		ret = of_property_read_string(np, "hdmi-rx2", &device_status);
	...
	if (!of_property_read_bool(np, "i2s-rx2"))
		...
		ret = of_property_read_string(np, "i2s-rx2", &device_status);
	...
	if (!of_property_read_bool(np, "loopback"))
		...
		ret = of_property_read_string(np, "loopback", &device_status);
			...
			struct device_node *codec_node, const char *name)
{
	...
	if (sink != NULL) {
		if (!strncmp(sink, "DAC0", strlen("DAC0")))
			...
			codec_index = DAI_LINK_I2S_TX2_BE;
	}
...
static int mtk_snd_machine_dev_probe(struct platform_device *pdev)
{
	...
	for (i = 1; i <= (DAI_LINK_AMP_4 - DAI_LINK_AMP_1 + 1) ; i++) {
		codec_dai_name = NULL;
		ret = scnprintf(idx_str, sizeof(idx_str), "codec-dai-name-%d", i);
		if (ret < 0) {
			pr_err("[ALSA EVB]scnprintf fail\n");
			return -1;
		}

		ret = of_property_read_string(node, idx_str, &codec_dai_name);
		if (ret == 0) {
			mtk_snd_dais[DAI_LINK_AMP_1 + i - 1].cpus->dai_name = codec_dai_name;
			mtk_snd_dais[DAI_LINK_AMP_1 + i - 1].ignore = 0;
		}
	}
#endif
