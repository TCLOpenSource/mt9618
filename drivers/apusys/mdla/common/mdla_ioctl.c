...
static int (*mdla_ioctl_perf)(struct file *file, u32 command,
		unsigned long arg, bool need_pwr_on);

static int ioctl_to_comm(struct comm_kmem *mem, void *data, int svp)
{
...
static int comm_to_ioctl(struct comm_kmem *mem, void *data, int svp)
{
	struct ioctl_malloc *malloc;
	struct ioctl_malloc_svp *malloc_svp;
	u32 type;
...
static struct comm_user *ioctl_user(struct file *file)
{
	static struct comm_user *kernel_user;

	if (file == NULL) {
		if (kernel_user == NULL) {
			if (comm_util_get_cb()->create_user(&kernel_user)) {
				mdla_err("%s: create kernel user fail\n", __func__);
				return NULL;
			}
		}
		return kernel_user;
	...
	if (file->private_data)
		return (struct comm_user *)file->private_data;

	return NULL;
}

static long mdla_ioctl_run_cmd(struct file *file, void *ioctl_cmd, int ioctl_svp)
{
	...
	if (!ioctl_svp) {
		...
		memcpy(&cmd_ut, ioctl_cmd, sizeof(struct ioctl_run_cmd_sync_svp));
	}

	cmd_ut.cmd_info.cmd_user = ioctl_user(file);

	return mdla_cmd_ops_get()->ut_run_sync(&cmd_ut,
			mdla_get_device(0));
}

static int mdla_ioctl_mem_alloc(struct file *file, void *ioctl_data, int from_user, int svp)
{
	struct comm_kmem mem;
	struct comm_user *u = ioctl_user(file);

	memset(&mem, 0, sizeof(mem));

	if (ioctl_to_comm(&mem, ioctl_data, svp))
		return -EINVAL;

	if (u && comm_util_get_cb()->alloc_user_mem(u, &mem)) {
		mdla_err("%s: fail, size:%d, dev:%p\n", __func__, mem.size, u);
		return -EFAULT;
	}
	...
	if (comm_to_ioctl(&mem, ioctl_data, svp))
		return -EINVAL;

	return 0;
}

static int mdla_ioctl_mem_free(struct file *file, void *ioctl_data, int svp)
{
	struct comm_kmem mem;
	struct comm_user *u = ioctl_user(file);
	...
	if (u && comm_util_get_cb()->free_user_mem(u, &mem)) {
		mdla_err("%s: fail, iova:%llx size:%d dev:%p\n", __func__,
								mem.iova,
								mem.size,
								u);
		return -EFAULT;
...
static long mdla_ioctl_entry(struct file *file,
			...
			unsigned int from_user)
{
	...
	case IOCTL_MALLOC:
		...
		if (mdla_ioctl_mem_alloc(file, &malloc_data, from_user, 0))
			return -EFAULT;
		...
		if (copy_to_ioctl((void *)arg, &malloc_data,
					sizeof(struct ioctl_malloc),
					from_user)) {
			mdla_ioctl_mem_free(file, &malloc_data, 0);
	...
	case IOCTL_FREE:
		...
		if (mdla_ioctl_mem_free(file, &malloc_data, 0))
			return -EFAULT;
	...
	case IOCTL_RUN_CMD_SYNC:
		...
		ret = mdla_ioctl_run_cmd(file, &cmd, 0);
		break;
	case IOCTL_MALLOC_SVP:
		...
		if (mdla_ioctl_mem_alloc(file, &malloc_svp, from_user, 1))
			return -EFAULT;
	...
	case IOCTL_FREE_SVP:
		...
		ret = mdla_ioctl_mem_free(file, &malloc_svp, 1);
		break;
	case IOCTL_RUN_CMD_SYNC_SVP:
		...
		ret = mdla_ioctl_run_cmd(file, &cmd_svp, 1);
		break;
	default:
		ret = -EINVAL;
	}
...
static long mdla_ioctl(struct file *file, unsigned int command, unsigned long arg)
{
	return mdla_ioctl_entry(file, command, arg, 1);
}

long mdla_kernel_ioctl(unsigned int command, void *arg)
{
	return mdla_ioctl_entry(NULL, command, (unsigned long)arg, 0);
}
...
static int mdla_open(struct inode *inodep, struct file *file)
{
	struct comm_user *u;

	if (comm_util_get_cb()->create_user(&u)) {
		mdla_err("%s: create user fail\n", __func__);
		return -EFAULT;
	}

	file->private_data = u;
	mdla_drv_debug("%s(): device(%p) has been opened\n", __func__, u);

	return 0;
}

static int mdla_release(struct inode *inodep, struct file *file)
{
	struct comm_user *u;

	u = file->private_data;
	if (comm_util_get_cb()->delete_user(u)) {
		mdla_err("%s(): delete user(%p) fail\n", __func__, u);
		return -EBADF;
	}

	mdla_drv_debug("%s(): device(%p) successfully closed\n", __func__, u);
