...
static u32 mreg_cmd_size = 0x180;
...
static void mdla_plat_dump_reg(u32 core_id, struct seq_file *s)
{
	...
	for (i = 0x000; i < 0x1000; i = i + 0x004)
		mdla_timeout_debug("apu_mdla_cmde_mreg_top+%.4x: %.8x\n",
...
static int mdla_dts_map(struct platform_device *pdev)
{
	...
	dev_info(dev, "Device Tree Probing, pdev:%p\n", pdev);
...
static void mdla_sw_multi_devices_deinit(void)
{
	...
	for (i = 0; i < nr_core_ids; i++) {
		...
		if (mdla_plat_devices[i].cmd_buf_len)
			kvfree(mdla_plat_devices[i].cmd_buf_dmp);
	}
...
static int mdla_print_time_stamp(u32 core, struct command_entry *ce)
{
	int i;
	ktime_t start, end;

	if (ce && ce->tv_count > 0) {
		mdla_qos_debug("[TIMEVAL_LOG] cmd id=0x%x\n", ce->fin_cid);
		for (i = 0; i < ce->tv_count; i++) {
			if (i > 0) {
				start = ce->tv[i-1];
				end = ce->tv[i];
				mdla_qos_debug("\t\t|-intervel %d: %lld us\n",
						i, ktime_to_ns(ktime_sub(end, start)>>10));
			}
			mdla_qos_debug("\t[%04x] %s\n", i, ce->tv_str[i]?ce->tv_str[i]:"none");
		}

		start = ce->tv[0];
		end = ce->tv[ce->tv_count - 1];
		mdla_qos_debug("\ttotal spend %lld us\n",
				ktime_to_ns(ktime_sub(end, start)>>10));
	}
	return 0;
}

static int mdla_add_time_stamp(u32 core, struct command_entry *ce, const char *str)
{
	if (ce && ce->tv_count < MDLA_TIMEVAL_NUM) {
		ce->tv[ce->tv_count] = ktime_get();
		ce->tv_str[ce->tv_count] = str;
		ce->tv_count++;
	}
...
int mdla_v2_0_init(struct platform_device *pdev)
{
	...
	if (mdla_v2_0_sched_init(drv_data))
		goto err_sched;
	...
	/* setup debug execute time */
	cmd_cb->print_time_stamp	= mdla_print_time_stamp;
	cmd_cb->add_time_stamp		= mdla_add_time_stamp;
