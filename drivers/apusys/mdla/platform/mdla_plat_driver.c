...
int mdla_plat_init(struct platform_device *pdev)
{
	...
	if (of_property_read_u32(dev->of_node, "core_num", &nr_core_ids))
		dev_info(dev, "parsing core_num null\n");
	...
	mdla_dbg_write_u32(FS_SECURE_DRV_EN,
			!!(drv->sw_cfg & BIT(CFG_DTV_SECURE_DRV_RDY)));
	mdla_dbg_write_u32(FS_KLOG_LVL, MDLA_KLOG_LVL_INFO);
