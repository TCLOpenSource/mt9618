...
#define redirect_output(...)						\
	do {								\
		u32 log_lvl = mdla_dbg_read_u32(FS_KLOG_LVL);		\
		if (log_lvl == MDLA_KLOG_LVL_INFO)			\
			pr_info(MDLA_TAG "[INFO]" __VA_ARGS__);		\
		if (log_lvl == MDLA_KLOG_LVL_WARN)			\
			pr_warn(MDLA_TAG "[INFO]" __VA_ARGS__);		\
		if (log_lvl == MDLA_KLOG_LVL_CRIT)			\
			pr_crit(MDLA_TAG "[INFO]" __VA_ARGS__);		\
	} while (0)
