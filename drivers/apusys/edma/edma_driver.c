...
int edma_initialize(struct edma_device *edma_device)
{
	...
	for (sub_id = 0; sub_id < edma_device->edma_sub_num; sub_id++) {
		...
		ret = snprintf(edma_sub->sub_name, EDMA_SUB_NAME_SIZE, "edma%d", edma_sub->sub);
		if (ret < 0) {
			pr_err("edma(sub_id %d) copy string fail\n", sub_id);
			break;
		}
		pr_notice("edma initialize name = %s\n", edma_sub->sub_name);

		ret = edma_init_queue_task(edma_sub);

		//edma_test_init(edma_sub->base_addr);
	}
...
int edma_send_cmd(int cmd, void *hnd, struct apusys_device *adev)
{
	...
	case APUSYS_CMD_EXECUTE:{
			...
			edma_ext = (struct edma_ext *)(uintptr_t)cmd_hnd->kva;
...
static int mtk_edma_sub_probe(struct platform_device *pdev)
{
	...
	pr_notice("mtk edma_sub_probe name = %s done\n", edma_sub->sub_name);
...
static const struct of_device_id edma_of_ids[] = {
	{.compatible = "mtk,mt5896-edma",},
	{.compatible = "mtk,mt5896_e3-edma",},
	{.compatible = "mtk,mt5897-edma",},
	{}
};
...
static int __init edma_init(void)
{
	...
	pr_info("platform_driver_register done! \r\n");

	return ret;

err_unreg_edma_sub:
	pr_info("err_unreg_edma_sub done! \r\n");
