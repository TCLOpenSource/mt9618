...
static inline int lock_command(struct edma_sub *edma_sub,
				struct edma_request *req,
				u32 ioctl)
{
	if (edma_lock_buf(req, ioctl))
		return -EINVAL;
				...
				struct edma_request *req)
{
	...
	if (req->ext_reg_addr != 0) {
		ext_reg = (unsigned int *) (uintptr_t)
			edma_map_buf(NULL, req->ext_reg_addr, 0, req->desp_iommu_en);
...
int edma_get_available_edma(struct edma_device *edma_device)
{
	...
	for (i = 0; i < edma_device->edma_sub_num; i++) {
		...
		#ifdef APUSYS_ALWAYS_POWER_ON
		if (!edma_sub)
			continue;

		if (edma_read_reg32(edma_sub->base_addr, APU_EDMA2_CTL_0)
				& DMA_IDLE_MASK)
			break;
		#else
		if (edma_sub)
						...
						struct edma_request *req)
{
	...
	if (lock_command(edma_sub, req, 0))
		goto lock_cmd_fail;
...
int edma_ext_mode(struct edma_sub *edma_sub, struct edma_request *req)
{
	...
	if (lock_command(edma_sub, req, 1))
		goto lock_cmd_fail;
...
#define POWER_RDY_WAIT_LOOP 10

int edma_power_on(struct edma_sub *edma_sub)
{
	struct edma_device *edma_device;
	int retry = POWER_RDY_WAIT_LOOP;
	int ret = 0;
	...
	if (timer_pending(&edma_device->power_timer)) {
		...
		if (!ret) {
			...
			if (!retry) {
				LOG_ERR("%s(): fail, power on not ready\n",
								__func__);
