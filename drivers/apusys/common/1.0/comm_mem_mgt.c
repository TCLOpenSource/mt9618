...
int comm_mem_free(struct comm_kmem *mem, u32 closefd)
{
	...
	case APU_COMM_MEM_DRAM_DMA:
		...
		list_for_each_safe(list_ptr, tmp, &g_mem_mgr.list) {
			...
			if (mr->kmem.iova == mem->iova &&
				...
				// prevent free in-used memory
				if (mr->kmem.refcount) {
					mutex_unlock(&g_mem_mgr.list_mtx);
					comm_err("%s(): iova:%llx is in use, count:%x\n",
						__func__, mem->iova, mr->kmem.refcount);
					return -EBUSY;
				}
	...
	case APU_COMM_MEM_VLM:
	default:
		comm_err("%s(): mem type %x not support\n", __func__, mem->mem_type);
		break;
	}

	if (ret)
		comm_err("%s(): iova:%llx, fd:%d, close:%u\n",
...
int comm_mem_map_kva(struct comm_kmem *mem)
{
	...
	list_for_each_safe(list_ptr, tmp, &g_mem_mgr.list) {
		...
		if ((iova | auto_ofs) >= mr->kmem.iova &&
			...
			((iova | auto_ofs) < (iova | auto_ofs) + size)) {
...
static int comm_mem_query_by_iova(struct comm_kmem *mem, int (*cb)(struct comm_kmem *))
{
	struct mem_record *mr = NULL;
	struct list_head *tmp = NULL, *list_ptr = NULL;
	u64 iova;
...
static int comm_mem_query_by_kva(struct comm_kmem *mem, int (*cb)(struct comm_kmem *))
{
	struct mem_record *mr = NULL;
	struct list_head *tmp = NULL, *list_ptr = NULL;
	int ret = -EFAULT;
...
static int comm_mem_query_by_fd(struct comm_kmem *mem, int (*cb)(struct comm_kmem *))
{
	struct mem_record *mr = NULL;
	struct list_head *tmp = NULL, *list_ptr = NULL;
	int ret = -EFAULT;
...
int comm_mem_query_mem(struct comm_kmem *mem, u32 type, int (*cb)(struct comm_kmem *))
{
	int ret = -EFAULT;

	switch (type) {
	case COMM_QUERY_BY_FD:
		ret = comm_mem_query_by_fd(mem, cb);
		break;
	case COMM_QUERY_BY_IOVA:
		ret = comm_mem_query_by_iova(mem, cb);
		break;
	case COMM_QUERY_BY_KVA:
		ret = comm_mem_query_by_kva(mem, cb);
		break;
	default:
		break;
	}
