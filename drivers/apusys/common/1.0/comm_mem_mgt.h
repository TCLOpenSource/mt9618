...
static inline int comm_mem_get_locked(struct comm_kmem *mem)
{
	mem->refcount++;
	return 0;
}

static inline int comm_mem_get(struct comm_kmem *mem)
{
	mutex_lock(&mem->mtx);
	comm_mem_get_locked(mem);
	mutex_unlock(&mem->mtx);
	return 0;
}

static inline int comm_mem_put_locked(struct comm_kmem *mem)
{
	if (mem->refcount)
		mem->refcount--;
	return 0;
...
static inline int comm_mem_put(struct comm_kmem *mem)
{
	mutex_lock(&mem->mtx);
	comm_mem_put_locked(mem);
	mutex_unlock(&mem->mtx);
	return 0;
}
