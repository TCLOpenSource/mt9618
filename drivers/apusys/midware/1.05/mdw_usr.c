...
int mdw_usr_mem_free(struct apusys_mem *um, struct mdw_usr *u)
{
	...
	//add for fix kernel panic issue in tv
	mutex_lock(&u_mgr.mtx);
	mutex_lock(&u->mtx);

	ret = mdw_usr_mem_delete(mm);
...
int mdw_usr_mem_flush(struct apusys_mem *um)
{
	int ret = 0;
	struct mdw_mem *mm = NULL;
...
int mdw_usr_mem_invalidate(struct apusys_mem *um)
{
	int ret = 0;
	struct mdw_mem *mm = NULL;
