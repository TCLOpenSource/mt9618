...
int apu_device_power_suspend(enum DVFS_USER user, int is_suspend)
{
	...
	if (IS_ERR_OR_NULL(ad)) {
		pr_err("[%s] called by no-exist user (%d)\n",
				__func__, ori_usr);
		ret = -EFAULT;
		goto out;
	}
 ...
 *	return pwr_status;
 */
	ad = apu_find_device(MDLA0);
	if (IS_ERR(ad))
		return false;

	return ad->plat_ops->power_status() ? true : false;
}
