	...
	for_each_child_of_node(node, child) {
		for_each_child_of_node(child, gChild) {
			...
			if (fixed_divs[index].id) {
				pr_err("fatal error: duplicated ID[%d]%s %d\n",
						index, child->name, fixed_divs[index].id);
				continue;
			}
...
static void __init of_mtk_dtv_mg(struct device_node *node)
{
	...
	if (base) {
		...
		pr_info("[%s]%s reg base : 0x%x -> 0x%pK\n",
...
static void __init of_mtk_dtv_gate_en(struct device_node *node)
{
	...
	for_each_child_of_node(node, child) {
		...
		if (!IS_ERR_OR_NULL(clk_data->clks[index])) {
			pr_err("fatal error: duplicated ID[%d]%s %p\n",
						index, child->name, clk_data->clks[index]);
			continue;
		}
 ...
 */
static int clock_merak_read(enum dtv_regbank_type bank_type, unsigned int reg_addr,
	unsigned int start, unsigned int end)
{
	...
	CLK_GNL_RW_INFO("read 0x%x, mask:%x value: %x\n", reg_addr, mask, value);
 ...
 */
static int clock_merak_write(enum dtv_regbank_type bank_type, unsigned int reg_addr, u16 value,
		unsigned int start, unsigned int end)
{
	struct dtv_clk_reg_access *target = NULL;
	unsigned long flags;
	...
	CLK_GNL_RW_INFO("write mask:%x value: %x reg_addr: %x\n",
				mask, _value, reg_addr);
...
static void __init of_mtk_dtv_clk_general(struct device_node *node)
{
	...
	mtk_dtvclk_register(clock_merak_write, clock_merak_read);
	CLK_BOOT_INFO("register merak R/W:%lx %lx\n",
		(unsigned long)clock_merak_write, (unsigned long)clock_merak_read);
