...
module_param_named(clk, _clk_source, ulong, 0644);
		...
		u32 *indexbuf, u32 indexes)
{
	...
	while (head < indexes) {
		...
		if (_wait_ready) {
			...
			if (_verbose) {
				te = ktime_get();
				pr_info("check done flag in %d us, loop %d time(s)\n",
						(int)ktime_us_delta(te, ts), timeout);
			}
