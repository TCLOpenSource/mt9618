				  ...
				  unsigned int timeout)
{
	...
	mtk_writew_relaxed((mtktv_wdt->wdt_base + REG_0194_L4_MAIN), (wdt_cnt >> WORD_SHIFT));
				    ...
				    unsigned int timeout)
{
	...
	mtk_writew_relaxed((mtktv_wdt->wdt_base + REG_018C_L4_MAIN), (wdt_cnt >> WORD_SHIFT));
...
static int mtktv_wdt_probe(struct platform_device *pdev)
{
	...
	if (err) {
		...
		mtktv_wdt->wdt_ping_self = 1;
	}

	err = of_property_read_u32(pdev->dev.of_node,
				"pre-timeout-isr", &mtktv_wdt->wdt_pre_timeout);
	if (err) {
		dev_info(&pdev->dev, "could not get pingself resource\n");
		mtktv_wdt->wdt_pre_timeout = 0;
	}

	mtktv_wdt->wdt_dev.info = &mtktv_wdt_pretimeout_info;
	mtktv_wdt->irq_num = platform_get_irq(pdev, 0);
	if (mtktv_wdt->wdt_pre_timeout == 1) {
		if (res > 0)
	...
	watchdog_init_timeout(&mtktv_wdt->wdt_dev, WDT_MAX_TIMEOUT, &pdev->dev);
	...
	if (mtktv_wdt->wdt_ping_self == 1) {
		...
		mod_timer(&mtktv_wdt->timer, jiffies + WDT_HEART_BEATS_HZ);
		mtktv_wdt_start(&mtktv_wdt->wdt_dev);
		mtktv_wdt_set_timeout(&mtktv_wdt->wdt_dev, WDT_MAX_TIMEOUT);
	}

	if (mtktv_wdt->wdt_pre_timeout == 1)
		mtktv_wdt_set_pretimeout(&mtktv_wdt->wdt_dev, WDT_PRETIMEOUT);
