 ...
 * @dma_direct_map: support dma_(un)map_single API, no need DMA bounce buffer
...
static int _dtv_i2c_prepare_dma_buf(struct dtv_i2c_dev *i2c_dev)
{
	...
	i2c_dev->dma_direct_map = false;
	ret = of_reserved_mem_device_init(i2c_dev->dev);
	if (ret) {
		dev_notice(i2c_dev->dev, "reserved memory presetting failed\n");
		if (i2c_dev->support_dma)
			// use dma_(un)map_single for I2C DMA when no reserved memory in dts
			i2c_dev->dma_direct_map = true;
	}
					 ...
					 dma_addr_t *dma_pa_ptr)
{
	...
	if (!i2c_dev->dma_direct_map && (msg->len > i2c_dev->fifo_depth) && i2c_dev->support_dma) {
...
static inline void _dtv_i2c_put_dma_buf(struct dtv_i2c_dev *i2c_dev,
					...
					dma_addr_t dma_pa)
{
	if (!i2c_dev->dma_direct_map && dma_va)
		/* free bounce buffer */
...
static int ___dtv_i2c_wait_int(struct dtv_i2c_dev *i2c_dev)
{
	...
	if (i2c_dev->irq <= 0) {
		...
		if (i2c_dev->use_dma)
			while (time_left != 0) {
				if (mtk_readb(i2c_dev->base + HWI2C_REG_0090_MIIC0))
					break;
...
static int _dtv_i2c_prepare_dma_xfer(struct dtv_i2c_dev *i2c_dev, struct i2c_msg *msg,
				     void **dma_va, dma_addr_t dma_pa, dma_addr_t *dma_relloc_pa)
{
	if (msg->len == 0)
	...
	if (i2c_dev->support_dma && i2c_dev->dma_direct_map) {
		// support dma direct mapping, can use dma_map_single()

		// ensure msg. data buffer is dma safe
		*dma_va = i2c_get_dma_safe_msg_buf(msg, i2c_dev->fifo_depth);
		if (IS_ERR_OR_NULL(*dma_va)) {
			// allocate dma buffer fail, can not use I2C DMA
			i2c_log(i2c_dev, MTK_I2C_LOG_WARN,
				"fail to allocate dma direct mapping buffer!\n");
			*dma_va = NULL;
			*dma_relloc_pa = 0;
			return -1;
		}

		*dma_relloc_pa = dma_map_single(i2c_dev->dev, *dma_va, msg->len,
						(msg->flags & I2C_M_RD) ?
							DMA_FROM_DEVICE : DMA_TO_DEVICE);
		if (dma_mapping_error(i2c_dev->dev, *dma_relloc_pa)) {
			// dma_map_single() error, can not use I2C DMA
			i2c_log(i2c_dev, MTK_I2C_LOG_WARN,
				"fail to map dma direct mapping buffer!\n");
			//need free memory allocated by i2c_get_dma_safe_msg_buf()
			i2c_put_dma_safe_msg_buf(*dma_va, msg, false);
			return -1;
		}

		// adjust dma_pa in i2c dma xfer
		*dma_relloc_pa -= i2c_dev->dma_phy_off;
...
static void _dtv_i2c_finish_dma_xfer(struct dtv_i2c_dev *i2c_dev, struct i2c_msg *msg, void *dma_va,
				     dma_addr_t dma_relloc_pa, bool xferred)
{
	if (i2c_dev->support_dma && i2c_dev->dma_direct_map && (msg->len > i2c_dev->fifo_depth)) {
		// support dma direct mapping, can use dma_unmap_single(), need adjust dma addr
		dma_unmap_single(i2c_dev->dev, dma_relloc_pa + i2c_dev->dma_phy_off, msg->len,
				 (msg->flags & I2C_M_RD) ? DMA_FROM_DEVICE : DMA_TO_DEVICE);
		// restore dma safe msg. data buffer
		i2c_put_dma_safe_msg_buf(dma_va, msg, xferred);
		return;
	}

	if (!xferred)
		// nothing todo if I2C transfer failed
		return;
			...
			int num)
{
	...
	for (i = 0; ret >= 0 && i < num; i++) {
		 ...
		 *    DRAM
		 * 4. Some chip(s) no need bounce buffer and can use dma_(un)map_single() API.
		 *
		 * Try allocate DMA bounce buffer if needed,
		 * dma_va != NULL indicate bounce buffer is available,
		 * use riu mode if allocate bounce buffer failed.
		 * This function is similar as memory allocation part of i2c_get_dma_safe_msg_buf().
		 * _dtv_i2c_prepare_dma_xfer() will execute memcpy part if i2c is write transfer.
		 *
		 * If chip(s) is support direct dma mapping, _dtv_i2c_prepare_dma_xfer() will direct
		 * use i2c_get_dma_safe_msg_buf() and dma_map_single().
		 */
		...
		if (msgs[i].flags & I2C_M_NOSTART) {
			i2c_log(i2c_dev, MTK_I2C_LOG_DEBUG,
				"NOSTART msg %d: chip=0x%X, len=%d, va %p\n",
				i, addr_fld, msgs[i].len, msgs[i].buf);
			...
			ret = _dtv_i2c_nostart_data(i2c_dev, i == 0, &(msgs[i]), stop);
						     ...
						     &dma_relloc_pa) >= 0) {
			i2c_log(i2c_dev, MTK_I2C_LOG_DEBUG,
				"dma msg %d: chip=0x%X, len=%d, va %p, pa %llX, pa_relloc %llX\n",
				i, addr_fld, msgs[i].len, dma_va, (u64)dma_pa, (u64)dma_relloc_pa);
			// may use dma xfer
			...
			/*
			 * memcpy part of i2c_put_dma_safe_msg_buf() if i2c is read transfer.
			 *
			 * If chip(s) is support direct dma mapping, direct using
			 * i2c_put_dma_safe_msg_buf() and dma_unmap_single().
			 */
			_dtv_i2c_finish_dma_xfer(i2c_dev, &msgs[i], dma_va, dma_relloc_pa,
						 !!(ret >= 0));
		} else {
			i2c_log(i2c_dev, MTK_I2C_LOG_DEBUG,
				"byte msg %d: chip=0x%X, len=%d, va %p\n",
				i, addr_fld, msgs[i].len, msgs[i].buf);
		...
		if (ret < 0)
			i2c_log(i2c_dev, i2c_dev->nak_log_level,
				"msg %d: chip=0x%X, len=%d, buf %p, va %p, pa_relloc %llX ret=%d\n",
				i, addr_fld, msgs[i].len, msgs[i].buf, dma_va, (u64)dma_relloc_pa,
				ret);
		else
			i2c_log(i2c_dev, MTK_I2C_LOG_DEBUG,
				"msg %d: chip=0x%X, len=%d, buf %p, va %p, pa_relloc %llX ret=%d\n",
