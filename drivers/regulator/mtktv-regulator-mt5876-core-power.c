...
static enum E_REGULATOR_TYPE type = E_REGULATOR_NONE;
...
static int mtktv_regulator_parse(void)
{
	...
	REGULATOR_FIND_BY_NAME(NULL, "buck_core_power", node);
...
{
	int ret = 0;
	enum PWM_ChNum num = E_PVR_PWM_CH1;
	u32 detect_vol = 0;
	u32 index = 0, total = 0;
	u16 val = 0;
...
static struct regulator_desc mtktv_regulator_desc = {
	...
	.owner          = THIS_MODULE,
	.of_match = of_match_ptr("buck_core_power"),
...
static int mt5876_regulator_probe(struct platform_device *pdev)
{
	...
	drvdata->CPUCorePower = devm_iio_channel_get(dev, "core_power_adc");
	...
	drvdata->pwm = devm_pwm_get(dev, "pwmadc1");
...
MODULE_DESCRIPTION("Regulator Driver for MediaTek MT5876 core power PMIC");
MODULE_LICENSE("GPL");
