...
static void __iomem *thermal_pm_base3;
#define BASE_ADDRESS                    (thermal_pm_base  + 0x200)
#define BASE_ADDRESS_2                   (thermal_pm_base2 + 0x200)
#define BASE_ADDRESS_PM_DMY              (thermal_pm_base3 + 0xA0C)
...
#define M6L_VBE_CODE_FT_MULTIPLIER     (740)
...
static struct iio_channel *SAR_Thermal;
static int chip_id;
...
int get_gpu_temperature_m6(void)
{
	int temperature = 0;
...
int get_gpu_temperature_m6l(void)
{
	int temperature = 0;
	...
	if (tsensor_ref == MOKONA_DEFAULT_T_SENSOR_REF) {
		...
		temperature = tsensor_shift + ((tsensor_ref - temperature)
			* M6L_VBE_CODE_FT_MULTIPLIER);
	}
	temperature /= TEMP_DIV_1000;
	return temperature;
}

int get_gpu_temperature_mokona(void)
{
	int temperature = 0;
	u16 pm_dmy_reg;
	...
	pm_dmy_reg = readw((void __iomem *)(BASE_ADDRESS_PM_DMY)) & PM_DMY_MASK | PM_DMY_GPU;
	writew(pm_dmy_reg, (void __iomem *)BASE_ADDRESS_PM_DMY);
...
int get_pm_temperature_mokona(void)
{
	int temperature = 0;
	u16 pm_dmy_reg;
	/*
	...
	mutex_lock(&temp_mutex);
	pm_dmy_reg = readw((void __iomem *)(BASE_ADDRESS_PM_DMY)) & PM_DMY_MASK;
	writew(pm_dmy_reg, (void __iomem *)BASE_ADDRESS_PM_DMY);
...
int get_gpu_temperature_miffy(void)
{
	int temperature = 0;
...
int get_gpu_temperature_moka(void)
{
	int temperature = 0;
...
int get_gpu_temperature_bysar(void)
{
	...
	if (chip_id == M6_ID)
		temperature = get_gpu_temperature_m6();
	else if (chip_id == M6L_ID)
		temperature = get_gpu_temperature_m6l();
	else if (chip_id == MOKONA_ID)
		temperature = get_gpu_temperature_mokona();
	else if (chip_id == MIFFY_ID)
		temperature = get_gpu_temperature_miffy();
	else if (chip_id == MOKA_ID)
		temperature = get_gpu_temperature_moka();
	else
		temperature = 0;

	return temperature;
}
EXPORT_SYMBOL(get_gpu_temperature_bysar);

int get_pm_temperature_bysar(void)
{
	int temperature = 0;

	if (chip_id == MOKONA_ID)
		temperature = get_pm_temperature_mokona();
	else
		temperature = 0;

	return temperature;
}
EXPORT_SYMBOL(get_pm_temperature_bysar);


static int get_temp(void *thermal, int *temp)
{
	*temp = get_gpu_temperature_bysar() * TRANS;
	return 0;
}
...
static struct thermal_zone_of_device_ops mtktv_gpu_thermal_of_ops = {
	.get_temp = get_temp,
};

static int mtktv_gpu_thermal_probe(struct platform_device *pdev)
{
	...
	if (thermal_pm_base == NULL) {
		...
		thermal_pm_base3 = of_iomap(pdev->dev.of_node, BASE_NUM);
		if (!thermal_pm_base || !thermal_pm_base2 || !thermal_pm_base3) {
			...
			dev_err(dev, "failed to map thermal_pm_base3 (%ld)\n",
					PTR_ERR(thermal_pm_base3));
			dev_err(dev, "node:%s reg base error\n", node->name);
			return -EINVAL;
		}
		of_property_read_u32_array(node, "reg", tmp, BASE_NUM);
	}

	SAR_Thermal = devm_iio_channel_get(dev, "gpu_adc");
	...
	tzdev = devm_thermal_zone_of_sensor_register(&pdev->dev
		, 0, NULL, &mtktv_gpu_thermal_of_ops);
...
static int mtktv_gpu_thermal_remove(struct platform_device *pdev)
{
	return 0;
}

static const struct of_device_id mtktv_gpu_thermal_of_match[] = {
	{ .compatible = "gpu-thermal" },
	{},
};
MODULE_DEVICE_TABLE(of, mtktv_gpu_thermal_of_match);

static struct platform_driver mtktv_gpu_thermal_platdrv = {
	.driver = {
		.name       = "gpu-thermal",
		.owner      = THIS_MODULE,
		.of_match_table = mtktv_gpu_thermal_of_match,
	},
	.probe  = mtktv_gpu_thermal_probe,
	.remove = mtktv_gpu_thermal_remove,
};


static int __init mtktv_gpu_thermal_init(void)
{
	platform_driver_register(&mtktv_gpu_thermal_platdrv);
	return 0;
}

late_initcall(mtktv_gpu_thermal_init);

MODULE_DESCRIPTION("MediaTek TV GPU Thermal driver");
