...
#define IR_RESUME_SEND_WAKEUPKEY     1// 1
...
#define STR_LEN		6
#define KEYMAP_GROUP	2

static unsigned int profile_num;
struct key_map_list *key_map_list;
...
static int mtk_ir_get_keymap_from_dts(struct device_node *np)
{
	...
	unsigned int malloc_keymap_len = 0;

	IR_PRINT("#####mtk_ir : get_keymap_from_dts#####\n");
	...
	malloc_keymap_len = (sizeof(struct key_map_list) * MAX_PROFILE_NUM);

	key_map_list = kzalloc(malloc_keymap_len, GFP_KERNEL);
	if (key_map_list == NULL) {
		IRDBG_ERR("%s: kzalloc key_map_list fail!\n", __func__);
	...
	for (profile_index = 0; profile_index < MAX_PROFILE_NUM; profile_index++) {
		...
		//get IR keymap and keymap size from DTS
		keymap_size = of_property_read_variable_u32_array(np_ir_key, "ir_key_table",
							ir_keymap, KEYMAP_GROUP, MAX_KEYMAP_SIZE);
		keymap_size = keymap_size/KEYMAP_GROUP;
		if (keymap_size == 0) {
		...
		malloc_len = (sizeof(struct key_map_table) * keymap_size);
		table[profile_index] = kmalloc(malloc_len, GFP_KERNEL);
		...
		for (i = 0; i < keymap_size ; i++) {
			table[profile_index][i].keycode = ir_keymap[i*KEYMAP_GROUP];
			table[profile_index][i].scancode = ir_keymap[i*KEYMAP_GROUP+1];
			IR_PRINT("==>[table] [%d](0x%x,0x%x)\n", i,
				table[profile_index][i].keycode, table[profile_index][i].scancode);
		}

		key_map_list[profile_index].map.scan = table[profile_index];
		key_map_list[profile_index].map.headcode = u32headcode_Tmp;
		key_map_list[profile_index].map.size = keymap_size;
		key_map_list[profile_index].map.name = dts_str;
		...
		MIRC_Map_Register(&key_map_list[profile_index]);
	...
	size_t len)
{
	...
	if (level > IR_DBG_ALL)
		level = IR_DBG_ALL;
	IR_PRINT("Set IR Debug Level :%ld\n", level);
...
#define CUR_SPD_LV_SZ 30
	ssize_t ret = 0;
	enum IR_SPEED_LEVEL_e level = IR_SPEED_FAST_H;
	int ret_flag = 0;
