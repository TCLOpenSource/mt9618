...
static int ir_toshiba_decode(struct mtk_ir_dev *dev, struct ir_raw_data ev)
{
	...
	case STATE_BIT_PULSE:
		...
		{
			if ((prevsc->scancode_protocol == (1<<IR_TYPE_TOSHIBA))
			&&(((u32)(MIRC_Get_System_Time()-KeyTime)) <= TOSHIBA_REPEAT_TIMEOUT)) {
			//repeat
				...
				if (RepeatCheckFlag == FALSE) {
					...
					RepeatCheckTimer.timer.expires = 
					jiffies + msecs_to_jiffies(REPEAT_CHECK_TIMEOUT);
	...
	case STATE_BIT_SPACE:
		...
		if (data->u8BitCount == TOSHIBA_NBITS)
		{
			...
			if ((0 != ir_clsoe_wb_toshiba) &&
			(u8Cmd  == (ir_clsoe_wb_toshiba & 0xff))) {
				MIRC_Set_WhiteBalance_Enable(false);
				IRDBG_INFO(
				"[Toshiba] u8Cmd = %x, MIRC_Set_WhiteBalance_Enable(false);\n",
				u8Cmd);
			}
...
#ifdef CONFIG_MTK_TV_MIRC_INPUT_DEVICE
							sc->scancode = 
							(u8Addr<<16) | (u8AddrInv<<8) | u8Cmd;
							sc->scancode_protocol =
							(1<<IR_TYPE_TOSHIBA);
