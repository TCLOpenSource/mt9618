...
// ( 3(start + toggle) + 5(system) + 8*4(Command data) ) * 2 - 3 = 77 bits,
//minus 3 is because we don't need to count the pulse of first start bits.
#define FACTORY_IR_RC5_DATA_BITS	77 

static u32 g_u32FactoryCmd ;
	...
	case STATE_BIT_DATA:
		...
		if(g_bFactory_RC == TRUE) {
			if(data->u8BitCount >= FACTORY_IR_RC5_DATA_BITS) {
				if(g_u32FactoryCmd == 0) {
					u32Data = databits_to_data(data->u64DataBits
					, data->u8BitCount, 0);
...
#if FACTORY_IR_CTRL
/* start(second '1' bit), toggle(1 bit) and system bit(5 bit). from Beko spec. */
			if((u32Data & 0x1FC0) == 0x17C0) {
