...
static int ir_panasonic_decode(struct mtk_ir_dev *dev, struct ir_raw_data ev)
{
	...
	case STATE_BIT_SPACE:
		...
		(ev.duration,PANASONIC_BIT_0_SPACE_LWB,PANASONIC_BIT_0_SPACE_UPB))
			break;
		data->u8BitCount++;
				...
				if (dev->support_ir[i].eIRType == IR_TYPE_PANASONIC)
				{
					...
					&& (dev->support_ir[i].u8Enable == TRUE)) {
						if ((u8Data ^ (u8Custom_K + 0x80)) == u8Verify) {
							...
							if (prevsc->scancode_protocol ==
							(1<<IR_TYPE_PANASONIC) && 
							((u32)KeyTime_repeat <=
							PANASONIC_REPEAT_TINEOUT)) {
								if (((speed != 0)
								&&( data->u8RepeatTimes
								>= (speed - 1)))
								|| ((speed == 0)
