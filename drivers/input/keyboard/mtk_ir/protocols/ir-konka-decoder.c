...
static int ir_konka_decode(struct mtk_ir_dev *dev, struct ir_raw_data ev)
{
	...
	case STATE_BIT_TRAILER:
		...
		for (i= 0;i<dev->support_num;i++) {
			if ((dev->support_ir[i].eIRType == IR_TYPE_KONKA)
			&&(dev->support_ir[i].u8Enable == TRUE)) {
				if (u8Cuscode == dev->support_ir[i].u32HeadCode) {
					...
					speed = dev->support_ir[i].u32IRSpeed;
#endif
					if (prevsc->scancode_protocol== (1<<IR_TYPE_KONKA)
					&& ((u32)(MIRC_Get_System_Time()- KeyTime) 
					<= KONKA_REPEAT_TIMEOUT)) {
						if (((speed != 0)
						&&( data->u8RepeatTimes >= (speed - 1)))
						|| ((speed == 0)
