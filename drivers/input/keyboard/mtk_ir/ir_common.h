...
typedef struct IR_Toshiba_Spec_s {
	...
	u8 u8RepeatTimes;		//Toshiba repeat filter times [if u8RepeatTimes == speed ,
					//return repeat flag=1 return true , otherwise return false]
...
typedef struct IR_Metz_Spec_s {
	...
	u8 u8RepeatTimes;		//Metz repeat filter times [if u8RepeatTimes == speed ,
					//return repeat flag=1 return true , otherwise return false]
	u8 u8PrevToggle;
...
typedef struct IR_KONKA_Spec_s {
	...
	u8 u8RepeatTimes;		//konka repeat filter times [if u8RepeatTimes == speed ,
					//return repeat flag=1 return true , otherwise return false]
...
typedef struct IR_PANASONIC_Spec_s {
	...
	u8 u8RepeatTimes;	//panasonic repeat filter times
				//[if u8RepeatTimes == speed , return repeat flag=1 &return true ,
...
//low/up bound Coefficient
//[Calculate The Specific Timming Data According To The Differ Coefficient]
