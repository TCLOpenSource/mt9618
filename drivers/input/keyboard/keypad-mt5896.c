...
#define KEYPAD_RESUME_SEND_WAKEUPKEY			1
#ifdef KEYPAD_RESUME_SEND_WAKEUPKEY
...
struct mtk_keypad {
	...
	unsigned char	_downtrig;
	unsigned int	keystored_t;
	unsigned char	_uprelease;
	ktime_t			_start_time;
	struct task_struct *t_keypad_tsk;
};
			...
			unsigned int *row_state)
{
	...
	for (scancnt = 0; scancnt < (keypad->key_num); scancnt++) {
		if (keypad->key_adc < keypad->low_bound) {
			if ((keypad->key_adc) < (keypad->scanmap[scancnt].keythresh_vol)) {
				row_state[keypad->ch_detected] = keypad->scanmap[scancnt].keycode;
				break;
			}
		}
	}
...
static void t_keypad_thread_op(struct mtk_keypad *keypad_t)
{
	struct input_dev *input_dev_t = keypad_t->input_dev;
	if (keypad_t->_downtrig) {
		if ((keypad_t->keystored_t != RELEASE_KEY) && (keypad_t->_uprelease)) {
			keypad_t->prekey[keypad_t->ch_detected] = keypad_t->keystored_t;
			keypad_t->pressed[keypad_t->ch_detected] = 1;
			keypad_t->_uprelease = 0;
			input_event(input_dev_t, EV_KEY, keypad_t->keystored_t, 1);
			input_sync(keypad_t->input_dev);
			keypad_t->_start_time = ktime_get();
			}
		}
		if (keypad_t->_uprelease == 0) {
			if ((keypad_t->keystored_t == keypad_t->prekey[keypad_t->ch_detected])
				&& (keypad_t->pressed[keypad_t->ch_detected])
				&& (keypad_t->keystored_t != RELEASE_KEY)) {
				if (keypad_t->no_autorepeat == 0) {
					if (ktime_ms_delta(ktime_get(), keypad_t->_start_time) >
						(keypad_t->debounce_time)) {
						input_event(input_dev_t, EV_KEY,
						keypad_t->keystored_t, MAGIC_NUMBER_TWO);
						input_sync(keypad_t->input_dev);
						keypad_t->_start_time = ktime_get();
					}
				}
				keypad_t->repeat_count++;
					...
					if ((keypad_t->keystored_t == RELEASE_KEY)
						&& (keypad_t->pressed[keypad_t->ch_detected])) {
						...
						input_sync(keypad_t->input_dev);
						keypad_t->prekey[keypad_t->ch_detected] = 0;
						keypad_t->_uprelease = 1;
						keypad_t->_downtrig = 0;
					}
				}
			/*Key Long Pressed reset*/
		if ((keypad_t->reset_timeout) && (keypad_t->_uprelease == 0)) {
			if (ktime_ms_delta(ktime_get(),
				keypad_t->_start_time) > (keypad_t->reset_timeout)) {
		...
		if (keypad_t->no_autorepeat == 0) {
			if ((ktime_ms_delta(ktime_get(), keypad_t->_start_time) >
			(keypad_t->debounce_time)) && keypad_t->_uprelease == 0) {
				keypad_t->repeat_count = 0;
				keypad_t->pressed[keypad_t->ch_detected] = 0;
				input_event(input_dev_t, EV_KEY,
				keypad_t->prekey[keypad_t->ch_detected], 0);
...
static int t_keypad_thread(void *arg)
{
	...
	while (1) {
		wait_event_freezable(keypad_t->wait, keypad_t->_downtrig == 1);
		...
		ch_offset_t = (keypad_t->ch_detected) << MAGIC_NUMBER_TWO;
		...
		if ((row_state_t[keypad_t->ch_detected] == RELEASE_KEY)
			&& (keypad_t->_uprelease == 1)) {
			keypad_t->_downtrig = 0;
			keypad_t->_uprelease = 1;
			keypad_t->prekey[keypad_t->ch_detected] = 0;
		...
		keypad_t->keystored_t = row_state_t[keypad_t->ch_detected];
		t_keypad_thread_op(keypad_t);
	}
	return 0;
}
...
static irqreturn_t mtk_keypad_irq_thread(int irq, void *dev_id)
{
	...
	while (int_ststmp) {
		...
		if (chanelcnt != keypad->chanel)
			return IRQ_NONE; //next...
		int_ststmp >>= 1;
		...
		if (adc_val < RELEASE_ADC && chanelcnt < 4) {
			if (adc_val < keypad->low_bound) {
				keypad->_downtrig = 1;
				wake_up(&keypad->wait);//wakeup
			}
		}
...
static int mtk_keypad_probe(struct platform_device *pdev)
{
	...
	keypad->t_keypad_tsk = kthread_create(t_keypad_thread, keypad, "Keypad_Check");
	if (IS_ERR(keypad->t_keypad_tsk)) {
		error = PTR_ERR(keypad->t_keypad_tsk);
		keypad->t_keypad_tsk = NULL;
		goto err_handle;
	} else
		wake_up_process(keypad->t_keypad_tsk);
	/*patch for add_timer issue*/
...
static int mtk_keypad_suspend(struct device *dev)
{
	...
	dev_info(dev, "##### KEYPAD Driver Suspend #####\n");
#ifdef IR_RESUME_SEND_WAKEUPKEY
...
static int mtk_keypad_resume(struct device *dev)
{
	...
	dev_info(dev, "KEYPAD:WakeUp(name=%s, id=0x%02X,key=0x%02X).\n", pm_get_wakeup_reason_str(),
									 wakeup_src, wakeup_key);
	if (wakeup_src == PM_WK_SAR) {
		/* if (wakeup_key == 0) //Fix it later. */
        if (wakeup_key == 0x80AA) {
            wakeup_key = KEY_PROG1;//upgrade event
        }
        else{
            wakeup_key = KEY_POWER;//power event
        }
		input_set_capability(input_dev, EV_KEY, wakeup_key);

		input_report_key(keypad->input_dev, wakeup_key, 1);
		input_sync(keypad->input_dev);
		input_report_key(keypad->input_dev, wakeup_key, 0);
		input_sync(keypad->input_dev);
	}
#endif
