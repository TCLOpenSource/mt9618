...
enum input_cmd_t {
	...
	INPUT_SVCMSG_GET_CONFIG_WAKEUP = 6,
	INPUT_SVCMSG_GET_ID_CONFIG = 7,
	//...
	INPUT_SVCMSG_MAX,
};

#define INPUT_DEVICE_NAME_SIZE			(32)

#define INPUT_CLIMSG_ACK				(0x0)
#define INPUT_CLIMSG_KEY_EVENT				(0x1)
#define INPUT_CLIMSG_GETKEY_EVENT			(0x2)

#define INPUT_SVCMSG_PAYLOAD_SIZE			(19 + INPUT_DEVICE_NAME_SIZE)
#define INPUT_CLIMSG_PAYLOAD_SIZE			(19 + INPUT_DEVICE_NAME_SIZE)
...
struct input_svcmsg_get_config_wakeup {
		uint8_t  msg_id;		//=INPUT_CLIMSG_ACK
		uint8_t  keyindex;
		uint32_t ret;
		uint32_t keycode;
};
build_assert(sizeof(struct input_svcmsg_get_config_wakeup) <= sizeof(struct input_svcmsg));

struct input_svcmsg_get_id_config {
		uint8_t  msg_id;		//=INPUT_CLIMSG_ACK
		uint32_t vendor_id;
		uint32_t product_id;
		uint32_t version;
...
build_assert(sizeof(struct input_svcmsg_get_id_config) <= sizeof(struct input_svcmsg));
...
struct input_climsg_get_config_wakeup {
		uint8_t  msg_id;		//=INPUT_CLIMSG_ACK
		uint8_t  keyindex;
		uint32_t ret;
		uint32_t keycode;
};
build_assert(sizeof(struct input_climsg_get_config_wakeup) <= sizeof(struct input_climsg));
...
struct input_climsg_get_id_config {
		uint8_t  msg_id;		//=INPUT_CLIMSG_ACK
		uint32_t vendor_id;
		uint32_t product_id;
		uint32_t version;
...
build_assert(sizeof(struct input_climsg_get_id_config) <= sizeof(struct input_climsg));

// keep sizeof(struct input_climsg) <= (20 + INPUT_DEVICE_NAME_SIZE) for save memory
build_assert(sizeof(struct input_climsg) <= (20 + INPUT_DEVICE_NAME_SIZE));
#endif
