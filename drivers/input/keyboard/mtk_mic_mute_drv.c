...
#define MUTE_STR_200		200
#define MUTE_STR_5		5
#define MUTE_STR_0x0100		0x0100
...
static void mic_mute_gpio_work_func(struct work_struct *work)
{
	...
	queue_delayed_work(mwq, &bdata->work, MUTE_STR_200);
}
...
mic_mute_get_devtree_pdata(struct device *dev)
{
	...
	device_for_each_child_node(dev, child) {
		...
		if (fwnode_property_read_u32(child, "debounce-interval",
			...
			button->debounce_interval = MUTE_STR_5;
...
static int mic_mute_probe(struct platform_device *pdev)
{
	...
	input->id.version = MUTE_STR_0x0100;
