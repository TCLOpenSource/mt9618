...
static struct gpio_leds_priv *gpio_leds_create(struct platform_device *pdev)
{
	...
	device_for_each_child_node(dev, child) {
		...
		fwnode_property_read_u32_array(child, "gpios",
						&led_dat->gpio, 1);
		fwnode_property_read_u32_array(child, "led_pwm",
					(uint32_t *)&led_dat->led_pwm, 1);
		fwnode_property_read_u32_array(child, "invert",
					(uint32_t *)&led_dat->invert, 1);
		pr_err("gpio=%d ledpwm=%d invert=%d\n",led_dat->gpio,
					led_dat->led_pwm, led_dat->invert);

		led.gpiod = gpio_to_desc(led_dat->gpio);
		...
		gpiod_set_consumer_name(led.gpiod,
...
static int breath_led_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_string(pdev->dev.of_node, "pmu_type", &pmu_name);

	if (ret) {
		pr_err("get pmu_type fail 0x%x\n", ret);
		return ret;
	}
	if (!strcmp(pmu_name, "pm51")) {
		pr_err("strcmp pm51\n");
		led_bus_dev->pmu_is_cm4 = false;
	} else if (!strcmp(pmu_name, "cm4")) {
		pr_err("strcmp cm4\n");
		led_bus_dev->pmu_is_cm4 = true;
	} else {
		pr_err("strcmp fail\n");
		return -EINVAL;
	}

	if (!led_bus_dev->pmu_is_cm4) {
	...
	led_bus_dev->gpio = priv->leds[0].gpio;
   	led_bus_dev->mchan = priv->mchan;
...
int mtk_rpmsg_led_txn(void *data, size_t len)
{
	...
	struct led_proxy_cmd_msg msg;
	int ret, i;

	if (list_empty(&led_chip_dev_list))
		return -EINVAL;
	list_for_each_entry_safe(led_bus_dev, n, &led_chip_dev_list, list) {
		if (led_bus_dev != NULL) { //find the first led device
			break;
		}
	}
...
#endif
	if (led_bus_dev->pmu_is_cm4) {
		if (len >= LED_PROXY_MSG_SIZE)
			return -EINVAL;
		led_bus_dev->rpmsg_dev->txn_result = -EIO;
		for (i = 0; i < len; i++) {
			if (i == 0)
				msg.cmd = *((u8 *)data + i);
			else
				msg.payload[i-1] = *((u8 *)data + i);
		}
		ret = rpmsg_send(led_bus_dev->rpmsg_dev->rpdev->ept, &msg, sizeof(msg));
		if (ret) {
			rpmsg_log(led_bus_dev->rpmsg_dev, MTK_LED_LOG_ERR,
					"rpmsg send failed, ret=%d\n", ret);
			return ret;
		}
		// wait for ack
		if (wait_for_completion_interruptible(&led_bus_dev->rpmsg_dev->ack))
			return -EBUSY;
		dev_info(led_bus_dev->rpmsg_dev->dev, "%s return\n", __func__);
		return led_bus_dev->rpmsg_dev->txn_result;
	}
...
static int mtk_rpmsg_led_probe(struct rpmsg_device *rpdev)
{
	...
	if (rpmsg_led->led_dev->pmu_is_cm4) {
		send_box[0] = LED_CMD_SET_GPIO;
		send_box[1] = rpmsg_led->led_dev->gpio;
		rpmsg_led->txn(&send_box, sizeof(u16));
	}
	return 0;
}
					...
					const struct kernel_param *kp)
{
	...
	for (i = 0; i < PREPH_PARAM_COUNT; i++) {
		if (tokens[i] != NULL) {
			ret = kstrtou8(tokens[i], 0, &send_box[i]);
			pr_err("send_box[%d]:%d\n", i, (int)send_box[i]);
			if (ret < 0) {
