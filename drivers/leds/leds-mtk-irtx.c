			   ...
			   struct led_mtk_irtx *led, struct device_node *child)
{
	...
	led_data->cdev.flags = 0;// no LED_CORE_SUSPENDRESUME;
	led_data->priv = priv;
...
static int led_mtk_irtx_resume(struct platform_device *dev)
{
	struct led_mtk_irtx_priv *priv = platform_get_drvdata(dev);
	struct led_classdev *led_cdev = &priv->leds[priv->num_leds-1].cdev;
	struct led_mtk_irtx_data *led_dat =
		container_of(led_cdev, struct led_mtk_irtx_data, cdev);
