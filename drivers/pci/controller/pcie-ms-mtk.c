...
#define VERSION	"20220614"
#define M6_PHY
#define POWER_CYCLE_TIME		20
...
void mtk_pcie_port_power_control(struct mtk_pcie_port *port)
{
	struct mtk_pcie *pcie = port->pcie;

	if (port->pwrctl_gpio) {
		gpiod_set_value(port->pwrctl_gpio, false);
		dev_info(pcie->dev, "Power Control.\n");
		msleep(POWER_CYCLE_TIME);
...
static int mtk_pcie_startup_port(struct mtk_pcie_port *port)
{
	...
	/* power off -10ms  power on */
	mtk_pcie_port_power_control(port);
