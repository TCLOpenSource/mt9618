...
#define RPROC_VAL_RSC_TABLE_DONE	(0xFF)
...
static int mtk_pmu_parse_dt(struct platform_device *pdev)
{
	...
	pmu->rsc_table_dummy = of_iomap(pdev->dev.of_node, 0);
	if (!pmu->rsc_table_dummy) {
...
static int mtk_pmu_rproc_probe(struct platform_device *pdev)
{
	...
	writeb(RPROC_VAL_RSC_TABLE_DONE, pmu->rsc_table_dummy);
