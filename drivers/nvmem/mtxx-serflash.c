...
((BIT(((1)?_bits_)+1)-BIT(((0)?_bits_))) & (_val_<<((0)?_bits_)))
...
#define FSP_WRITE_MASK(nor,  addr,  val,  mask)\
WRITE_WORD_MASK(nor->base + ((addr)<<2), (val), (mask))
...
static MS_BOOL _HAL_FSP_WaitDone(struct mtk_nor *mtk_nor)
{
	...
	do {
		if ((FSP_READ(mtk_nor,  REG_FSP_DONE_FLAG)
			& REG_FSP_DONE_FLAG_MASK) == REG_FSP_DONE) {
			bRet = TRUE;
			break;
		}
...
static MS_U8 HAL_FSP_CheckFlashBusy(struct mtk_nor *mtk_nor, MS_U32 u32Delay)
{
	...
	do {
		...
		FSP_WRITE_MASK(mtk_nor, REG_FSP_WBF_SIZE,
						REG_FSP_WBF_SIZE0(1), REG_FSP_WBF_SIZE0_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_RBF_SIZE,
						REG_FSP_RBF_SIZE0(1), REG_FSP_RBF_SIZE0_MASK);
...
static MS_BOOL HAL_FSP_Write(struct mtk_nor *mtk_nor, MS_U32 u32Addr,
							MS_U32 u32Size, MS_U8 *pu8Data)
{
	MS_BOOL bRet = TRUE;
	MS_U32 u32I, u32J;
	MS_U32 u32quotient;
	...
	for (u32J = 0; u32J <= u32quotient; u32J++) {
		...
		FSP_WRITE_MASK(mtk_nor, REG_FSP_WBF_SIZE,
					REG_FSP_WBF_SIZE0(1), REG_FSP_WBF_SIZE0_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_WBF_SIZE,
					REG_FSP_WBF_SIZE1((4+u32SizeTmp)), REG_FSP_WBF_SIZE1_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_WBF_SIZE,
					REG_FSP_WBF_SIZE2(0), REG_FSP_WBF_SIZE2_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_RBF_SIZE,
					REG_FSP_RBF_SIZE0(0), REG_FSP_RBF_SIZE0_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_RBF_SIZE,
					REG_FSP_RBF_SIZE1(0), REG_FSP_RBF_SIZE1_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_RBF_SIZE,
					REG_FSP_RBF_SIZE2(0), REG_FSP_RBF_SIZE2_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_CTRL, REG_FSP_ENABLE, REG_FSP_ENABLE_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_CTRL, REG_FSP_NRESET, REG_FSP_RESET_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_CTRL, REG_FSP_INT, REG_FSP_INT_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_CTRL, REG_FSP_2NDCMD_ON, REG_FSP_2NDCMD_MASK);
...
static MS_BOOL HAL_FSP_Read(struct mtk_nor *mtk_nor, MS_U32 u32Addr,
								MS_U32 u32Size,  MS_U8 *pu8Data)
{
	MS_BOOL bRet = TRUE;
	MS_U32 u32Idx,  u32J;
	...
	for (u32J = 0; u32J <= u32quotient; u32J++) {
		...
		FSP_WRITE_MASK(mtk_nor, REG_FSP_WBF_SIZE,
					REG_FSP_WBF_SIZE0(4), REG_FSP_WBF_SIZE0_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_WBF_SIZE,
					REG_FSP_WBF_SIZE1(0), REG_FSP_WBF_SIZE1_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_WBF_SIZE,
					REG_FSP_WBF_SIZE2(0), REG_FSP_WBF_SIZE2_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_RBF_SIZE,
					REG_FSP_RBF_SIZE0(u32SizeTmp), REG_FSP_RBF_SIZE0_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_RBF_SIZE,
					REG_FSP_RBF_SIZE1(0), REG_FSP_RBF_SIZE1_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_RBF_SIZE,
					REG_FSP_RBF_SIZE2(0), REG_FSP_RBF_SIZE2_MASK);
				...
				u8 *buf,  int len)
{
	...
	if (mtk_nor->type) {
		...
		FSP_WRITE_MASK(mtk_nor, REG_FSP_WBF_SIZE,
					REG_FSP_WBF_SIZE0(1), REG_FSP_WBF_SIZE0_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_WBF_SIZE,
					REG_FSP_WBF_SIZE0(len), REG_FSP_WBF_SIZE1_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_RBF_SIZE,
					REG_FSP_RBF_SIZE0(0), REG_FSP_RBF_SIZE0_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_CTRL, REG_FSP_ENABLE, REG_FSP_ENABLE_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_CTRL, REG_FSP_NRESET, REG_FSP_RESET_MASK);
				...
				u8 *buf,  int len)
{
	...
	if (mtk_nor->type) {
		...
		FSP_WRITE_MASK(mtk_nor, REG_FSP_WBF_SIZE,
						REG_FSP_WBF_SIZE0(1), REG_FSP_WBF_SIZE0_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_RBF_SIZE,
						REG_FSP_RBF_SIZE0(len), REG_FSP_RBF_SIZE0_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_CTRL, REG_FSP_ENABLE, REG_FSP_ENABLE_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_CTRL, REG_FSP_NRESET, REG_FSP_RESET_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_CTRL, REG_FSP_INT, REG_FSP_INT_MASK);
		FSP_WRITE_MASK(mtk_nor, REG_FSP_CTRL, REG_FSP_2NDCMD_OFF, REG_FSP_2NDCMD_MASK);
...
static int mtk_nor_drv_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_err(&pdev->dev,  "failed to locate the chip\n");
