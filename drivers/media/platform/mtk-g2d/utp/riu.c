...
//  Local Variables
//-------------------------------------------------------------------------------------------------
static uint64_t _gRIU_VBase;
static uint32_t _gRIU_PBase;
//-------------------------------------------------------------------------------------------------
//  Local Functions
//-------------------------------------------------------------------------------------------------
	...
	uint32_t u32Value)
{
	...
	writel(u32Value, (void *)((uintptr_t)VA_Offset));
}
	...
	uint32_t fld)
{
	...
	tmp = __u4IO32AccessFld(
		1,
		readl((const void *)((uintptr_t)VA_Offset)),
		u32Value,
		fld);
	writel(tmp, (void *)((uintptr_t)VA_Offset));
}
	...
	uint32_t u32P_Addr)
{
	...
	return readl((const void *)((uintptr_t)VA_Offset));
}
