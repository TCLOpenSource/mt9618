...
enum v4l2_g2d_interface gGeInterface;
//-------------------------------------------------------------------------------------------------
//  Debug Functions
//-------------------------------------------------------------------------------------------------
...
MS_U16 GE_CODA_ReadReg(GE_CTX_HAL_LOCAL *pGEHalLocal, uint32_t addr)
{
	...
	if (gGeInterface < G2D_INTERFACE_GE0 || gGeInterface >= G2D_INTERFACE_MAX)
		return 0;
	else if (gGeInterface == G2D_INTERFACE_GE0)
		u32Base = 0;
	else if (gGeInterface == G2D_INTERFACE_GE1)
		u32Base = GE1_OFFSET;
	...
	if (u16NoFIFOMask == 0)
		return pGEHalLocal->u16RegGETable[gGeInterface][(addr-GE0_2410_BASE)/ADDR_MUTI];

	return (mtk_read2byte(addr + u32Base) & u16NoFIFOMask) |
		(pGEHalLocal->u16RegGETable[gGeInterface][(addr-GE0_2410_BASE)/ADDR_MUTI] &
		~u16NoFIFOMask);
}
...
void GE_CODA_WriteReg(GE_CTX_HAL_LOCAL *pGEHalLocal, uint32_t addr, uint32_t value)
{
	...
	if (gGeInterface < G2D_INTERFACE_GE0 || gGeInterface >= G2D_INTERFACE_MAX)
		return;
	if (gGeInterface == G2D_INTERFACE_GE0)
		u32Base = 0;
	else if (gGeInterface == G2D_INTERFACE_GE1)
		u32Base = GE1_OFFSET;

	// CMDQ special command
	if ((addr-GE0_2410_BASE)/ADDR_MUTI < GE_TABLE_REGNUM) {
		pGEHalLocal->u16RegGETable[gGeInterface][(addr-GE0_2410_BASE)/ADDR_MUTI] = value;
	} else {
	...
	mtk_write2byte((addr + u32Base), value);
}
...
void GE_Init_RegImage(GE_CTX_HAL_LOCAL *pGEHalLocal)
{
	...
	for (addr = 0; addr < GE_TABLE_REGNUM; addr++) {
		pGEHalLocal->u16RegGETable[G2D_INTERFACE_GE0][addr]
			= mtk_read2byte((addr*ADDR_MUTI)+GE0_2410_BASE);
		pGEHalLocal->u16RegGETable[G2D_INTERFACE_GE1][addr]
			= mtk_read2byte((addr*ADDR_MUTI)+GE0_2410_BASE+GE1_OFFSET);
	}
...
void GE_Init(GE_CTX_HAL_LOCAL *pGEHalLocal, GE_Config *cfg)
{
	...
	for (i = G2D_INTERFACE_GE0 ; i < G2D_INTERFACE_MAX ; i++) {
		gGeInterface = (enum v4l2_g2d_interface)i;
	...
	gGeInterface = G2D_INTERFACE_GE0;
...
int GE_SetInterface(enum v4l2_g2d_interface eInterface)
{
	gGeInterface = eInterface;

	if (eInterface == G2D_INTERFACE_GE1)
		mtk_write2bytemask(REG_0004_GE2 + GE1_OFFSET, 1, REG_EN_GE_CHK_CLR_CRC);
	else
		mtk_write2bytemask(REG_0004_GE2 + GE1_OFFSET, 0, REG_EN_GE_CHK_CLR_CRC);
...
GE_Result HAL_GE_STR_RestoreReg(GE_CTX_HAL_LOCAL *pGEHalLocal, GE_STR_SAVE_AREA *pGFX_STRPrivate)
{
	...
	for (i = 0; _GE_Reg_Backup[i] < REG_GE_INVALID; ++i) {
		mtk_write2byte((_GE_Reg_Backup[i]*ADDR_MUTI) + GE0_2410_BASE,
			pGFX_STRPrivate->GE0_Reg[_GE_Reg_Backup[i]]);
		mtk_write2byte((_GE_Reg_Backup[i]*ADDR_MUTI) + GE0_2410_BASE + GE1_OFFSET,
			pGFX_STRPrivate->GE1_Reg[_GE_Reg_Backup[i]]);
	}
