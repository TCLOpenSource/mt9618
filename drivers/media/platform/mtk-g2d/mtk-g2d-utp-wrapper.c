...
int g2d_resource(u32 type)
{
	...
	case V4L2_CID_EXT_G2D_GET_RESOURCE:
		MApi_GFX_BeginDraw();
		GE_SetInterface(G2D_INTERFACE_GE0);
		break;
...
int g2d_render(struct v4l2_ext_control *ext_ctrl)
{
	...
	case V4L2_G2D_DRAW_LINE:
		ret = _g2d_drawline(render.line, render.dst.fmt);
		break;
	case V4L2_G2D_RECTFILL:
		ret = _g2d_rectfill(render.rect, render.dst.fmt);
		break;
	case V4L2_G2D_BITBLT:
		ret = _g2d_bitblt(render.bitblt);
		break;
	case V4L2_G2D_DRAW_OVAL:
		ret = _g2d_drawoval(render.oval);
		break;
	case V4L2_G2D_SPANFILL:
		ret = _g2d_spanfill(render.spanfill, render.dst.fmt);
		break;
	case V4L2_G2D_TRIFILL:
		ret = _g2d_trifill(render.tri, render.dst.fmt);
		break;
	case V4L2_G2D_CLEAR_FRAME:
		if (MApi_GFX_ClearFrameBuffer(render.frame.addr, render.frame.length,
				  (MS_U8) render.frame.value) != GFX_SUCCESS)
			ret = -1;
		break;
	case V4L2_G2D_CLEAR_FRAME_BY_WORD:
		if (MApi_GFX_ClearFrameBufferByWord(render.frame.addr, render.frame.length,
					render.frame.value) != GFX_SUCCESS)
			ret = -1;
		break;
	default:
		break;
...
int g2d_get_render_info(struct v4l2_ext_control *ext_ctrl)
{
	...
	memset(&stSrcBuffInfo, 0, sizeof(GFX_BufferInfo));
	memset(&stDstBuffInfo, 0, sizeof(GFX_BufferInfo));
...
int g2d_set_interface(struct v4l2_ext_control *ext_ctrl)
{
	enum v4l2_g2d_interface eInterface;

	if (copy_from_user(&eInterface, (void __user *)ext_ctrl->ptr,
						sizeof(enum v4l2_g2d_interface))) {
		G2D_ERROR("copy_from_user fail\n");
		return -1;
	}

	G2D_DEBUG("Set interface = %d\n", eInterface);
	GE_SetInterface(eInterface);
