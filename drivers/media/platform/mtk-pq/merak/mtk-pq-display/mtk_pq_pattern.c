...
#define DEFAULT_VALUE_2 (2)
#define DEFAULT_VALUE_11 (11)
...
char *position_str[PQ_PATTERN_POSITION_MAX] = {
	...
	" VIP_IN ", " VIP_OUT", " HDR10  ", "B2R_PRE1",
	"B2R_PRE2", "B2R_POST1", "B2R_POST2", "   MDW  ",
	"DW_HW5POST", "XVYCC"};
...
//  Local enums
//-------------------------------------------------------------------------------------------------
enum pq_pattern_type {
	E_PQ_PATTERN_TYPE_FILL_PURE_COLOR = 0,
	E_PQ_PATTERN_TYPE_FILL_RAMP,
	E_PQ_PATTERN_TYPE_FILL_PURE_COLORBAR,
	E_PQ_PATTERN_TYPE_FILL_GRADIENT_COLORBAR,
	E_PQ_PATTERN_TYPE_FILL_BLACK_WHITE_COLORBAR,
	E_PQ_PATTERN_TYPE_FILL_CROSS,
	E_PQ_PATTERN_TYPE_FILL_PIP_WINDOW,
	E_PQ_PATTERN_TYPE_FILL_DOT_MATRIX,
	E_PQ_PATTERN_TYPE_FILL_CROSSHATCH,
	E_PQ_PATTERN_TYPE_FILL_BOX = DEFAULT_VALUE_11,
	E_PQ_PATTERN_TYPE_FILL_HV_RAMP_MISC,
	E_PQ_PATTERN_TYPE_FILL_RGB_PURE_COLORBAR,
	E_PQ_PATTERN_TYPE_FILL_MAX
};
	...
	unsigned short *h_size, unsigned short *v_size)
{
	...
	if ((win_id >= PQ_WIN_MAX_NUM) ||
	    (position < 0) ||
	    (position >= PQ_PATTERN_POSITION_MAX)) {
	...
	struct pq_pattern_info *pattern_info)
{
	...
	if ((window < 0) || (window >=  pqdev->xc_win_num)) {
	...
	find = _mtk_pq_pattern_get_value_from_string(buf, "force_timing", len, &value);
	if (find)
		pattern_info->force_timing.enable = true;
	else
		pattern_info->force_timing.enable = false;

	find = _mtk_pq_pattern_get_value_from_string(buf, "force_width", len, &value);
	if (find)
		...
		pattern_info->force_timing.force_h_size = h_size;
	}

	find = _mtk_pq_pattern_get_value_from_string(buf, "force_height", len, &value);
	if (find)
		pattern_info->force_timing.force_v_size = value;
	else {
		ret = _mtk_pq_pattern_get_size(pattern_info->win_id,
			pattern_info->position, &h_size, &v_size);
		pattern_info->force_timing.force_v_size = v_size;
	}

	find = _mtk_pq_pattern_get_value_from_string(buf, "force_rgb", len, &value);
	if (find)
		pattern_info->force_rgb = value;
	else
		pattern_info->force_rgb = true;
	...
	unsigned int len, struct pq_pattern_info *pattern_info)
{
	...
	_mtk_pq_pattern_get_value_from_string(buf, "reset", len, &value);
	pure_colorbar->shift_reset = value;
	...
	unsigned int len, struct pq_pattern_info *pattern_info)
{
	...
	if (!find)
		cross->cross_color.red = DEFAULT_COLOR_VALUE;
	else
		cross->cross_color.red = value;

	find = _mtk_pq_pattern_get_value_from_string(buf, "r_end", len, &value);
	if (!find)
		cross->border_color.red = DEFAULT_COLOR_VALUE;
	else
...
static int _mtk_pq_pattern_fill_pip_window(char *buf,
	unsigned int len, struct pq_pattern_info *pattern_info)
{
	int ret = 0;
	int value = 0;
	...
	unsigned short h_size = 0;
	unsigned short v_size = 0;
	struct pq_pattern_pip_window *pip_window = NULL;
	...
	find = _mtk_pq_pattern_get_value_from_string(buf, "g_st", len, &value);
	if (!find)
		pip_window->pip_color.green = DEFAULT_COLOR_VALUE;
	else
		pip_window->pip_color.green = value;

	find = _mtk_pq_pattern_get_value_from_string(buf, "b_st", len, &value);
	if (!find)
		pip_window->pip_color.blue = DEFAULT_COLOR_VALUE;
	else
		pip_window->pip_color.blue = value;

	_mtk_pq_pattern_get_value_from_string(buf, "r_end", len, &value);
	...
	find = _mtk_pq_pattern_get_value_from_string(buf, "x", len, &value);
	if (!find)
		pip_window->window.x = (h_size >> DEFAULT_VALUE_2);
	else
		pip_window->window.x = value;

	find = _mtk_pq_pattern_get_value_from_string(buf, "y", len, &value);
	if (!find)
		pip_window->window.y = (v_size >> DEFAULT_VALUE_2);
	else
	...
	char s, unsigned char index, struct pq_pattern_dot_matrix *dot_matrix)
{
	...
	for (count = 0; count < 8; count++) {
		...
		if (addr >= MATRIX_MAX_ADDR) {
			STI_PQ_LOG(STI_PQ_LOG_LEVEL_PATTERN,
				"Matrix array is not enough, need to fix\n");
			break;
		}
	...
	struct pq_pattern_dot_matrix *dot_matrix)
{
	...
	if (snprintf(content, sizeof(content), "%s", string) < 0)
		return -EINVAL;
...
static int _mtk_pq_pattern_fill_crosshatch(char *buf,
	unsigned int len, struct pq_pattern_info *pattern_info)
{
	int ret = 0;
	int value = 0;
	...
	unsigned short h_size = 0;
	unsigned short v_size = 0;
	struct pq_pattern_crosshatch *crosshatch = NULL;
	...
	find = _mtk_pq_pattern_get_value_from_string(buf, "h_line_interval", len, &value);
	if (!find)
		crosshatch->h_line_interval = DEFAULT_LINE_INTERVAL;
	else
		crosshatch->h_line_interval = value;

	find = _mtk_pq_pattern_get_value_from_string(buf, "v_line_interval", len, &value);
	if (!find)
		crosshatch->v_line_interval = DEFAULT_LINE_INTERVAL;
	else
		crosshatch->v_line_interval = value;

	find = _mtk_pq_pattern_get_value_from_string(buf, "h_line_width", len, &value);
	if (!find)
		crosshatch->h_line_width = DEFAULT_LINE_WIDTH;
	else
		crosshatch->h_line_width = value;

	find = _mtk_pq_pattern_get_value_from_string(buf, "v_line_width", len, &value);
	if (!find)
		crosshatch->v_line_width = DEFAULT_LINE_WIDTH;
	else
		crosshatch->v_line_width = value;

	find = _mtk_pq_pattern_get_value_from_string(buf, "h_position_st", len, &value);
	if (!find)
		crosshatch->h_position_st = DEFAULT_POSITION_ST;
	else
		crosshatch->h_position_st = value;

	find = _mtk_pq_pattern_get_value_from_string(buf, "v_position_st", len, &value);
	if (!find)
...
static int _mtk_pq_pattern_fill_box(char *buf,
	unsigned int len, struct pq_pattern_info *pattern_info)
{
	int ret = 0;
	int value = 0;
	...
	unsigned short h_size = 0;
	unsigned short v_size = 0;
	struct pq_pattern_box *box = NULL;
...
static int _mtk_pq_pattern_fill_hv_ramp_misc(char *buf,
	unsigned int len, struct pq_pattern_info *pattern_info)
{
	int ret = 0;
	int value = 0;
...
static int _mtk_pq_pattern_fill_rgb_pure_colorbar(char *buf,
	unsigned int len, struct pq_pattern_info *pattern_info)
{
	int ret = 0;
	int value = 0;
	...
	unsigned short h_size = 0;
	unsigned short v_size = 0;
	struct pq_pattern_rgb_pure_colorbar *rgb_pure_colorbar = NULL;
	...
	find = _mtk_pq_pattern_get_value_from_string(buf, "width", len, &value);
	if (!find)
		rgb_pure_colorbar->colorbar_h_size = DEFAULT_H_SIZE;
	else
		rgb_pure_colorbar->colorbar_h_size = value;

	find = _mtk_pq_pattern_get_value_from_string(buf, "height", len, &value);
	if (!find)
		rgb_pure_colorbar->colorbar_v_size = DEFAULT_V_SIZE;
	else
...
static int _mtk_pq_pattern_fill_type(enum pq_pattern_type ePatternType, char *cmd,
	unsigned int len, struct pq_pattern_info *pattern_info)
{
	int ret = 0;

	if (!cmd) {
		STI_PQ_LOG(STI_PQ_LOG_LEVEL_ERROR, "cmd is NULL!\n");
		return -EINVAL;
	}

	if (!pattern_info) {
		STI_PQ_LOG(STI_PQ_LOG_LEVEL_ERROR, "pattern_info is NULL!\n");
		return -EINVAL;
	}

	switch (ePatternType) {
	case E_PQ_PATTERN_TYPE_FILL_PURE_COLOR:
		...
		break;
	case E_PQ_PATTERN_TYPE_FILL_RAMP:
		...
		break;
	case E_PQ_PATTERN_TYPE_FILL_PURE_COLORBAR:
		...
		break;
	case E_PQ_PATTERN_TYPE_FILL_GRADIENT_COLORBAR:
		...
		break;
	case E_PQ_PATTERN_TYPE_FILL_BLACK_WHITE_COLORBAR:
		...
		break;
	case E_PQ_PATTERN_TYPE_FILL_CROSS:
		...
		break;
	case E_PQ_PATTERN_TYPE_FILL_PIP_WINDOW:
		ret = _mtk_pq_pattern_fill_pip_window(cmd, len, pattern_info);
		if (ret) {
			STI_PQ_LOG(STI_PQ_LOG_LEVEL_ERROR,
				"_mtk_pq_pattern_fill_pip_window fail, ret = %d!\n", ret);
			goto exit;
		}
		break;
	case E_PQ_PATTERN_TYPE_FILL_DOT_MATRIX:
		...
		break;
	case E_PQ_PATTERN_TYPE_FILL_CROSSHATCH:
		ret = _mtk_pq_pattern_fill_crosshatch(cmd, len, pattern_info);
		if (ret) {
			STI_PQ_LOG(STI_PQ_LOG_LEVEL_ERROR,
				"_mtk_pq_pattern_fill_crosshatch fail, ret = %d!\n", ret);
			goto exit;
		}
		break;
	case E_PQ_PATTERN_TYPE_FILL_BOX:
		ret = _mtk_pq_pattern_fill_box(cmd, len, pattern_info);
		if (ret) {
			STI_PQ_LOG(STI_PQ_LOG_LEVEL_ERROR,
				"_mtk_pq_pattern_fill_box fail, ret = %d!\n", ret);
			goto exit;
		}
		break;
	case E_PQ_PATTERN_TYPE_FILL_HV_RAMP_MISC:
		ret = _mtk_pq_pattern_fill_hv_ramp_misc(cmd, len, pattern_info);
		if (ret) {
			STI_PQ_LOG(STI_PQ_LOG_LEVEL_ERROR,
				"_mtk_pq_pattern_fill_hv_ramp_misc fail, ret = %d!\n",
				ret);
			goto exit;
		}
		break;
	case E_PQ_PATTERN_TYPE_FILL_RGB_PURE_COLORBAR:
		ret = _mtk_pq_pattern_fill_rgb_pure_colorbar(cmd, len, pattern_info);
		if (ret) {
			STI_PQ_LOG(STI_PQ_LOG_LEVEL_ERROR,
				"_mtk_pq_pattern_fill_rgb_pure_colorbar fail, ret = %d!\n",
				ret);
			goto exit;
		}
		break;
	...
	char *buf, ssize_t used_count, enum pq_pattern_position position)
{
	...
	if ((position < 0) || (position >= PQ_PATTERN_POSITION_MAX)) {
		STI_PQ_LOG(STI_PQ_LOG_LEVEL_ERROR, "Invalid position(%u)\n", position);
		return 0;
	}
...
static void _mtk_pq_pattern_copy_pattern_info(
	struct mtk_pq_platform_device *pqdev, struct pq_pattern_info *pattern_info)
{
	int win_num = 0;
	struct mtk_pq_device *pq_dev = NULL;
	struct mtk_pq_device *pq_dev0 = NULL;

	pq_dev0 = pqdev->pq_devs[pattern_info->win_id];

	for (win_num = 1; win_num < PQ_PAT_WIN; win_num++) {
		pq_dev = pqdev->pq_devs[win_num];
		if (!pq_dev) {
			STI_PQ_LOG(STI_PQ_LOG_LEVEL_PATTERN, "pq_dev is NULL!\n");
			continue;
		}
		if (!pattern_info->enable)
			pq_dev->pattern_status.pattern_enable_position = INVALID_PATTERN_POSITION;
		else {
			pq_dev->pattern_status.pattern_enable_position =
				pq_dev0->pattern_status.pattern_enable_position;
			pq_dev->pattern_status.pattern_type = pq_dev0->pattern_status.pattern_type;
		}
	...
	struct device *dev, enum pq_pattern_position position)
{
	...
	STI_PQ_LOG(STI_PQ_LOG_LEVEL_PATTERN, "count = %zd!\n", total_count);
	...
	struct device *dev, enum pq_pattern_position position)
{
	...
	if (snprintf(cmd, len + 1, "%s", buf) < 0) {
		STI_PQ_LOG(STI_PQ_LOG_LEVEL_ERROR,
			"Fail to print string to buffer\n");
		return -EINVAL;
	}
	...
	if (!pattern_info.enable) {
		...
		ret = _mtk_pq_pattern_fill_type((enum pq_pattern_type)type,
			cmd, len, &pattern_info);
		if (ret) {
			STI_PQ_LOG(STI_PQ_LOG_LEVEL_ERROR,
				"_mtk_pq_pattern_fill_type fail, ret = %d, type = %d!\n",
				ret, (enum pq_pattern_type)type);
			goto exit;
		}
	...
	_mtk_pq_pattern_copy_pattern_info(pqdev, &pattern_info);

	if (pq_dev->stream_meta.pq_mode == meta_pq_mode_legacy) {
		struct msg_pq_pattern_info msg_pattern_info = {0};

		msg_pattern_info.force_rgb = pattern_info.force_rgb;
		msg_pattern_info.position = pattern_info.position;
		msg_pattern_info.pattern_type = pattern_info.pattern_type;
		msg_pattern_info.win_id = pattern_info.win_id;
		msg_pattern_info.enable_ts = pattern_info.enable_ts;
		msg_pattern_info.enable = pattern_info.enable;
		msg_pattern_info.force_timing_enable = pattern_info.force_timing.enable;
		msg_pattern_info.force_h_size = pattern_info.force_timing.force_h_size;
		msg_pattern_info.force_v_size = pattern_info.force_timing.force_v_size;
		memcpy(&(msg_pattern_info.m), &(pattern_info.m), sizeof(pattern_info.m));

		ret = mtk_pq_common_set_pattern_info(pq_dev, NULL, (void *)&msg_pattern_info);
	}
exit:
...
int mtk_pq_pattern_store_only_enable(const char *buf,
	struct device *dev, enum pq_pattern_position position)
{
	int ret = 0;
	int len = 0;
...
void mtk_pq_pattern_init(struct mtk_pq_device *pq_dev)
{
	pq_dev->trigger_pattern = true;
}
