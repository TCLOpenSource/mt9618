...
#define PQ_ABF_CUTR_MODE1_W_SIZE	(960)
#define PQ_ABF_CUTR_MODE2_W_SIZE	(1920)
#define PQ_ABF_CUTR_MODE3_W_SIZE	(3840)
#define PQ_ABF_CUTR_MODE1_H_SIZE	(1023)
#define PQ_ABF_CUTR_MODE2_H_SIZE	(2047)
#define PQ_ABF_CUTR_MODE3_H_SIZE	(4095)
#define PQ_ABF_CUTR_MODE1_HV_SIZE (120*68*64)
#define PQ_ABF_CUTR_MODE2_HV_SIZE (120*68*256)
...
struct display_device {
	...
	uint8_t pq_iommu_window;
	uint8_t pq_iommu_idx_dip;
	uint8_t di_align;
	uint8_t pre_hvsp_align;
	uint8_t hvsp_align;
	uint8_t post_hvsp_align;
	uint16_t h_max_size;
	uint16_t v_max_size;
	...
	uint16_t hvsp_12tap_size;
	uint16_t spf_vtap_size;
	uint16_t extraframe_p;
	uint16_t extraframe_i;
	uint16_t pq_cap_width_max_size;
	uint16_t pq_cap_height_max_size;
};
...
struct output_meta_compare_info {
	...
	struct window_info displayBase;
	struct window_info realDisplaySize;
	uint8_t u8Interlace;
	uint32_t u32OriginalInputFps;
	uint8_t u8MuteAction;
	uint8_t u8PqMuteAction;
	uint8_t u8SignalStable;
	uint8_t u8DotByDotType;
	uint32_t u32RefreshRate;
	uint32_t u32HdrApplyType;
	bool bMuteChange;
	bool bFdMaskBypass;
