...
bool _GetFmtBpp(u32 fourcc, u32 *pu32Dividend, u32 *pu32Divisor)
{
	u16 u16Dividend = 0;
	u16 u16Divisor = 0;
...
u32 MapByteToPixel(u32 fourcc, u32 u32Byte)
{
	u32 u32Dividend = 0;
	u32 u32Divisor = 0;
	u32 pixel = 0;
	...
	if ((u32Dividend == 0) || (u32Divisor == 0)) {
		DIP_SVC_ERR("pitch byte transfer to pixel fail\n");
		return 0;
	}

	//bpp = u32Dividend/u32Divisor;
	pixel = (u32Byte*u32Divisor)/u32Dividend;
...
int mtk_dip_svc_enable(struct dip_ctx *ctx, u8 u8Enable)
{
	...
	if (ctx->secure_info.u32BufCnt == 0) {
		DIP_SVC_ERR("[%s]Error SDC BufCnt is Zero\n", __func__);
		return -EINVAL;
	}
	...
	stSvcInfo.u32BufCnt = SDC_FB_CNT;
	stSvcInfo.u32InputSize[0] = stSvcInfo.u64InputAddr[ctx->secure_info.u32BufCnt-1] -
		stSvcInfo.u64InputAddr[0] +
		ctx->secure_info.u32InputSize[ctx->secure_info.u32BufCnt-1];

	if ((enSrc == E_DIP_SRC_DIPR) || (enSrc == E_DIP_SRC_B2R)) {
		if (ctx->dev->dipcap_dev.u32SDCCap == SDC_CAP_B2R)
			stSvcInfo.enSrc = E_DIP_SRC_B2R;
		else if (ctx->dev->dipcap_dev.u32SDCCap == SDC_CAP_DIPR)
			stSvcInfo.enSrc = E_DIP_SRC_DIPR;
		else
		...
		stSvcInfo.u32InputPitch = MapByteToPixel(ctx->in.fmt->fourcc,
			ctx->in.u32BytesPerLine[0]);
	} else {
		stSvcInfo.enSrc = enSrc;
	...
	for (u32Idx = 0; u32Idx < IN_PLANE_MAX; u32Idx++) {
		DIP_SVC_INFO("[%s] u32InputSize[%d]:0x%x\n", __func__,
	...
	DIP_SVC_INFO("[%s] u32InputPitch:%d\n", __func__, stSvcInfo.u32InputPitch);
