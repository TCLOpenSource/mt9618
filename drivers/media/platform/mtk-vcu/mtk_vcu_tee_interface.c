...
static int _vdec_tee_cmd_handle_task(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		wait_event(cmd_task->queue,
			   kthread_should_stop() ||
			   (cmd_task->exec_cmd_msg != NULL));

		if (kthread_should_stop())
			break;

		cmd_msg = (struct mtk_vcu_tee_cmd_msg *)cmd_task->exec_cmd_msg;
		exec_cmd = cmd_msg->cmd;
		exec_param = cmd_msg->param;

		switch (exec_cmd) {
		case VCU_TEE_CMD_OPEN_CONTEXT:
			ret = _vcu_tee_open_context(exec_param, true);
			break;
		case VCU_TEE_CMD_CLOSE_CONTEXT:
			ret = _vcu_tee_close_context(exec_param, true);
			break;
		case VCU_TEE_CMD_INVOKE_COMMAND: {
			struct invoke_hdl_param *param = exec_param;
		...
		case VCU_TEE_CMD_REGISTER_SHM: {
			struct register_shm_hdl_param *param = exec_param;
	...
	void *param)
{
	...
	// Lock TEE CMD mutex
	mutex_lock(&cmd_task->cmd_lock);

	// prepare cmd and param
	tee_handler->cmd_msg.cmd = cmd;
	tee_handler->cmd_msg.param = param;

	// set exec_cmd_msg to trigger execution in cmd handle task
	cmd_task->exec_cmd_msg = &tee_handler->cmd_msg;

	wake_up(&cmd_task->queue);
	wait_for_completion(&cmd_task->completion);

	// keep execution result
	exec_ret = tee_handler->cmd_msg.ret;

	// Unlock TEE CMD mutex
	mutex_unlock(&cmd_task->cmd_lock);
