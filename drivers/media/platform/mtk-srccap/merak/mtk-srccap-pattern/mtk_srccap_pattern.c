...
#define PATTERN_REG_NUM_MAX		(100)
	...
	unsigned short *h_size, unsigned short *v_size)
{
	...
	if (position < 0 || position >= SRCCAP_PATTERN_POSITION_MAX) {
	...
	struct srccap_pattern_info *pattern_info, struct hwregOut *pattern_reg)
{
	...
	data.common.color_space =
		(enum hwreg_srccap_pattern_color_space)pattern_env[position].color_space;
	...
	struct srccap_pattern_info *pattern_info)
{
	...
	pattern_reg.reg = MALLOC(sizeof(struct reg_info) * PATTERN_REG_NUM_MAX);
	if (pattern_reg.reg == NULL) {
		PATTERN_LOG(PATTERN_LOG_ERROR, "malloc pattern reg fail!\n");
		return -EINVAL;
	}
...
exit:
	FREE_MEM(pattern_reg.reg, sizeof(struct reg_info) * PATTERN_REG_NUM_MAX);
	return ret;
}
	...
	struct device *dev, enum srccap_pattern_position position)
{
	...
	PATTERN_LOG(PATTERN_LOG_INFO, "count = %ld!\n", (unsigned long)total_count);
	...
	struct device *dev, enum srccap_pattern_position position)
{
	...
	cmd = MALLOC(sizeof(char) * 0x1000);
	if (cmd == NULL) {
		PATTERN_LOG(PATTERN_LOG_ERROR, "Malloc cmd fail!\n");
		return -EINVAL;
	}
	...
	char_num = snprintf(cmd, len + 1, "%s", buf);
	if (char_num < 0 || char_num >= (len + 1)) {
		SRCCAP_MSG_ERROR("invalid string size\n");
		FREE_MEM(cmd, sizeof(char) * 0x1000);
		return -EPERM;
	}
	...
	memcpy(&srccap_dev->pattern_info, &pattern_info, sizeof(struct srccap_pattern_info));
...
void mtk_srccap_pattern_resume(struct srccap_pattern_info *pattern_info)
{
	int ret = 0;
