...
static MS_U8 _mtk_srccap_hdmirx_pkt_get_raw_data(
	...
	MS_U32 u32_buf_len)
{
	return (MS_U8)KDrv_SRCCAP_HDMIRx_pkt_get_gnl(
			enInputPortType,
			enPacketType,
...
static MS_BOOL _mtk_srccap_hdmirx_pkt_valid(
	...
	enum v4l2_ext_hdmi_packet_state enPacketType)
{
	MS_U32 u32_pkt_recv_flag = 0;
	E_MUX_INPUTPORT ePort = INPUT_PORT_NONE_PORT;

	ePort = mtk_hdmirx_to_muxinputport(src);

	if (ePort != INPUT_PORT_NONE_PORT) {
		u32_pkt_recv_flag = MDrv_HDMI_Get_PacketStatus(ePort);
	...
	MS_U32 u32_buf_len)
{
	...
	static struct hdmi_avi_packet_info pre_ptr;
	...
	if (!_mtk_srccap_hdmirx_pkt_valid(src, V4L2_EXT_HDMI_PKT_AVI)) {
		// keep previous data when no AVI packet received
		memcpy(ptr, &pre_ptr, sizeof(struct hdmi_avi_packet_info));
		return 0;
	}

	if (KDrv_SRCCAP_HDMIRx_pkt_get_avi(
		mtk_hdmirx_to_muxinputport(src),
		&pkt_avi, 0)) {
		_mtk_srccap_hdmirx_ParseAVIInfoFrame(&pkt_avi, ptr);
		memcpy(&pre_ptr, ptr, sizeof(struct hdmi_avi_packet_info));
		return 1;
	}
	return 0;
}
	...
	MS_U32 u32_buf_len)
{
	...
	if (KDrv_SRCCAP_HDMIRx_pkt_get_gcp(
		mtk_hdmirx_to_muxinputport(src),
		&pkt_buf, 0)) {
	...
	MS_U32 u32_buf_len)
{
	...
	if (KDrv_SRCCAP_HDMIRx_pkt_get_hdrif(
		mtk_hdmirx_to_muxinputport(src),
		&pkt_buf, 0)) {
	...
	struct v4l2_ext_hdmi_pkt_report_s *buf_ptr)
{
	...
	if (!KDrv_SRCCAP_HDMIRx_pkt_get_report(
		mtk_hdmirx_to_muxinputport(src),
		_mtk_v4l2src_packet_state_to_hdmirx(enPacketType),
		&my_report_buf))
		return false;
...
bool mtk_srccap_hdmirx_pkt_is_dsc(
	enum v4l2_srccap_input_source src)
{
	return KDrv_SRCCAP_HDMIRx_pkt_is_dsc(
		mtk_hdmirx_to_muxinputport(src));
}
...
bool mtk_srccap_hdmirx_pkt_is_vrr(
	enum v4l2_srccap_input_source src)
{
	return KDrv_SRCCAP_HDMIRx_pkt_is_vrr(
		mtk_hdmirx_to_muxinputport(src));
}
	...
	bool mute_en)
{
	...
	enum HDMI_AUDIO_PATH_TYPE audio_mux_n  =
		KDrv_SRCCAP_HDMIRx_mux_query_audio_inverse(InputPort);

	if (audio_mux_n != HDMI_AUDIO_PATH_NONE) {
		return KDrv_SRCCAP_HDMIRx_pkt_audio_mute_en(
...
bool mtk_srccap_hdmirx_pkt_audio_status_clr(
	enum v4l2_srccap_input_source src)
{
	return KDrv_SRCCAP_HDMIRx_pkt_audio_mute_status_clr(
		mtk_hdmirx_to_muxinputport(src));
}
	...
	enum v4l2_srccap_input_source src)
{
	E_MUX_INPUTPORT enInputPortType
		= mtk_hdmirx_to_muxinputport(src);
	MS_U8 u8_active_src
		= KDrv_SRCCAP_HDMIRx_mux_query_video_inverse(
			enInputPortType);
	...
	u32_audio_fs_hz = MDrv_HDMI_Get_Audio_Fs(
		mtk_hdmirx_to_muxinputport(src));
