...
#define DV_LEGACY_NEG_COUNT (10)
#define DV_TRANS_COUNT_BASE (26)
#define DV_TRANS_COUNT_EXT  (10)
...
#define DV_CTRL_EVENT_TRANSITION  (1)
#define DV_UDELAY_1000000 (1000000)
#define DV_UDELAY_1100000 (1100000)
#define DV_UDELAY_50000   (50000)
#define DV_UDELAY_51000   (51000)

#define IS_LEGACY_MODE(srccap_dev)  \
...
enum srccap_dv_descrb_neg_action {
	SRCCAP_DV_DESCRB_NEG_ACTION_RESUME = 0,
	SRCCAP_DV_DESCRB_NEG_ACTION_FREEZE,
	SRCCAP_DV_DESCRB_NEG_ACTION_MUTE,
};
...
DECLARE_WAIT_QUEUE_HEAD(dv_ctrl_wait);
static uint32_t s_dv_ctrl_event_flag;

//-----------------------------------------------------------------------------
// Local Functions
//-----------------------------------------------------------------------------
static int dv_control_task(void *data)
{
	int ret = 0;
	static int trans_mute_count;
	...
	while (!kthread_should_stop()) {
		...
		if (srccap_dev->dv_info.descrb_info.buf.in_mute == FALSE) {
			wait_event_interruptible(dv_ctrl_wait, ((s_dv_ctrl_event_flag &
				DV_CTRL_EVENT_TRANSITION) != 0x00000000));
			s_dv_ctrl_event_flag = 0;

			srccap_dev->dv_info.descrb_info.buf.in_mute = TRUE;
			trans_mute_count = DV_TRANS_COUNT_BASE;

			SRCCAP_DV_DEFAULT_LOG_TRACE("[TRA] set video mute\n");

			ret = drv_hwreg_srccap_dv_descrb_set_video_mute(srccap_dev->dev_id, true,
				true, NULL, NULL);
			if (ret < 0)
				SRCCAP_DV_LOG_TRACE(SRCCAP_DV_DBG_LEVEL_ERR, "set mute failed\n");
		}

		if (srccap_dev->dv_info.descrb_info.buf.in_mute) {
			if (srccap_dev->dv_info.descrb_info.buf.in_transition) {
				// not stable
				SRCCAP_DV_LOG_TRACE(SRCCAP_DV_DBG_LEVEL_DEBUG,
					"[TRA] in transition, keep in mute\n");
				if (trans_mute_count < DV_TRANS_COUNT_EXT)
					trans_mute_count = DV_TRANS_COUNT_EXT;
			}

			if (trans_mute_count > 0) {
				trans_mute_count--;
				SRCCAP_DV_LOG_TRACE(SRCCAP_DV_DBG_LEVEL_DEBUG,
					"[TRA] transition mute count = %d\n", trans_mute_count);
				usleep_range(DV_UDELAY_50000, DV_UDELAY_51000);
				continue;
			}
		}

		SRCCAP_DV_DEFAULT_LOG_TRACE("[TRA] set video unmute\n");

		ret = drv_hwreg_srccap_dv_descrb_set_video_mute(srccap_dev->dev_id, false,
			true, NULL, NULL);
		if (ret < 0)
			SRCCAP_DV_LOG_TRACE(SRCCAP_DV_DBG_LEVEL_ERR, "set unmute failed\n");

		srccap_dev->dv_info.descrb_info.buf.in_mute = FALSE;
	}

	return ret;
}

static bool dv_descrb_determine_crc_12b_444_en(struct mtk_srccap_dev *srccap_dev, u32 hw_ver)
{
	bool en = FALSE;
	...
	if (hw_ver >= DV_SRCCAP_HW_TAG_V4)
		en = FALSE;
	else if (srccap_dev->dv_info.descrb_info.pkt_info.color_format
...
static int dv_descrb_set_hw_r_enable(
	struct mtk_srccap_dev *srccap_dev)
{
	...
	en = dv_descrb_determine_crc_12b_444_en(srccap_dev, hw_ver);
	riu = TRUE;
	ret = drv_hwreg_srccap_dv_descrb_set_crc_12b_444_enable(
			dev_id, en, riu, NULL, NULL);
	if (ret < 0) {
		SRCCAP_DV_LOG_CHECK_RETURN(ret, -EINVAL);
		goto exit;
	}

	SRCCAP_DV_LOG_TRACE(SRCCAP_DV_DBG_LEVEL_HDMI_DESC,
		"descramble IRQ DV Srccap HW Version=%d, crc_12b_444_en=%d\n", hw_ver, en);
...
static void dv_descrb_update_event(
	struct mtk_srccap_dev *srccap_dev,
	struct srccap_dv_descrb_state *state)
{
	int ret = 0;

	if ((srccap_dev == NULL) || (state == NULL)) {
		SRCCAP_DV_LOG_CHECK_POINTER(-EINVAL);
		return;
	}
	...
	if (state->interface !=
		...
		srccap_dev->dv_info.descrb_info.common.event_changed |= true;
		state->interface =
			srccap_dev->dv_info.descrb_info.common.interface;
	}
	/* game */
	if (state->dv_game_mode !=
			srccap_dev->dv_info.descrb_info.common.dv_game_mode) {
		srccap_dev->dv_info.descrb_info.common.event_changed |= true;
		state->dv_game_mode =
			srccap_dev->dv_info.descrb_info.common.dv_game_mode;
	}
	/* hdmi 422 pack */
	if (state->hdmi_422_pack_en !=
			srccap_dev->dv_info.descrb_info.common.hdmi_422_pack_en) {
		srccap_dev->dv_info.descrb_info.common.event_changed |= true;
		state->hdmi_422_pack_en =
				srccap_dev->dv_info.descrb_info.common.hdmi_422_pack_en;
	}
	/* transition */
	if (state->in_transition !=
			srccap_dev->dv_info.descrb_info.buf.in_transition) {
		srccap_dev->dv_info.descrb_info.common.event_changed |= true;
		state->in_transition =
				srccap_dev->dv_info.descrb_info.buf.in_transition;
	}
	/* neg */
	if (state->neg_ctrl !=
			srccap_dev->dv_info.descrb_info.buf.neg_ctrl) {
		srccap_dev->dv_info.descrb_info.common.event_changed |= true;
		state->neg_ctrl =
				srccap_dev->dv_info.descrb_info.buf.neg_ctrl;
	}
	...
	struct srccap_dv_init_out *out)
{
	...
	u8 pkt_num = 0;
	bool riu = FALSE;
	struct task_struct *control_task = NULL;
	int hw_ver = 0;
	...
	for (i = 0; i < SRCCAP_DV_DESCRB_BUF_NUM; i++) {
		if (in->dev->dv_info.descrb_info.buf.irq_info[i].data == NULL)
			in->dev->dv_info.descrb_info.buf.irq_info[i].data =
				devm_kzalloc(in->dev->dev,
					(DV_DESCRB_PKT_LENGTH * DV_DESCRB_MAX_HW_SUPPORT_PKT),
					GFP_KERNEL);
		if (!in->dev->dv_info.descrb_info.buf.irq_info[i].data)
			return -ENOMEM;
		in->dev->dv_info.descrb_info.buf.irq_info[i].crc = FALSE;
		in->dev->dv_info.descrb_info.buf.irq_info[i].data_length = 0;
	}
	in->dev->dv_info.descrb_info.buf.irq_wptr = 0;
	in->dev->dv_info.descrb_info.buf.irq_rptr = 0;
	memset(&in->dev->dv_info.descrb_info.state, 0, sizeof(struct srccap_dv_descrb_state));
	in->dev->dv_info.descrb_info.common.event_changed = false;
	...
	if (dev_id == 0) {
		control_task = kthread_run(dv_control_task, in->dev, "dv control task");
		if (!control_task) {
			ret = -ECHILD;
			goto exit;
		}
	...
	struct srccap_dv_streamon_out *out)
{
	...
	in->dev->dv_info.descrb_info.buf.irq_wptr = 0;
	in->dev->dv_info.descrb_info.buf.irq_rptr = 0;
	in->dev->dv_info.descrb_info.buf.neg_cnt = 0;
	memset(&in->dev->dv_info.descrb_info.state, 0, sizeof(struct srccap_dv_descrb_state));
	in->dev->dv_info.descrb_info.common.event_changed = false;
	...
	struct srccap_dv_streamoff_out *out)
{
	...
	//reset mute and freeze status
	hw_ver = in->dev->srccap_info.cap.u32DV_Srccap_HWVersion;
	if ((hw_ver >= DV_SRCCAP_HW_TAG_V4) && (hw_ver <= DV_SRCCAP_HW_TAG_V5)) {

		if ((in->dev->dv_info.descrb_info.buf.in_mute == false)
			&& (in->dev->dv_info.descrb_info.buf.neg_cnt > 0)) {

			SRCCAP_DV_DEFAULT_LOG_TRACE("[TRA] reset mute\n");

			s_dv_ctrl_event_flag |= DV_CTRL_EVENT_TRANSITION;
			wake_up(&dv_ctrl_wait);
		}

		if (in->dev->dv_info.descrb_info.buf.in_freeze == true) {

			SRCCAP_DV_DEFAULT_LOG_TRACE("[TRA] reset freeze\n");

			ret = drv_hwreg_srccap_dv_descrb_set_video_freeze(in->dev->dev_id, false,
					true, NULL, NULL);
			if (ret < 0)
				SRCCAP_DV_LOG_TRACE(SRCCAP_DV_DBG_LEVEL_ERR,
				"set unfreeze failed\n");

			in->dev->dv_info.descrb_info.buf.in_freeze = false;
		}
	}
...
int mtk_dv_descrb_set_mute_and_freeze(struct mtk_srccap_dev *srccap_dev,
	enum srccap_dv_descrb_interface interface)
{
	struct srccap_dv_descrb_info *descrb_info = NULL;
	int hw_ver = 0;
	int ret = 0;
	...
	hw_ver = srccap_dev->srccap_info.cap.u32DV_Srccap_HWVersion;
	descrb_info = &(srccap_dev->dv_info.descrb_info);

	if (IS_LEGACY_MODE(srccap_dev) && (descrb_info->common.interface != interface))
		dv_mute = true;

	if (((descrb_info->common.interface != interface)
		|| mtk_common_check_color_format_change(srccap_dev, M_HDMI_COLOR_DV_DESCRB))
		&& (srccap_dev->dv_info.descrb_info.buf.in_mute == false)) {

		s_dv_ctrl_event_flag |= DV_CTRL_EVENT_TRANSITION;
		wake_up(&dv_ctrl_wait);

		SRCCAP_DV_DEFAULT_LOG_TRACE(
			"[TRA] set video mute, dv_mute = %d, ori interface=%d, new interface=%d\n",
			dv_mute, descrb_info->common.interface, interface);
		dv_mute = true;
	}

	if (IS_LEGACY_MODE(srccap_dev)) {
		enum srccap_dv_descrb_neg_action neg_action = SRCCAP_DV_DESCRB_NEG_ACTION_RESUME;
		bool video_mute = FALSE;

		ret = drv_hwreg_srccap_dv_descrb_get_video_mute(srccap_dev->dev_id, &video_mute);

		/* determine NEG action */
		switch (descrb_info->buf.neg_ctrl) {
		case M_DV_CRC_STATE_FAIL_TRANSITION:
			if (video_mute || dv_mute)
				neg_action = SRCCAP_DV_DESCRB_NEG_ACTION_MUTE;
			else
				neg_action = SRCCAP_DV_DESCRB_NEG_ACTION_FREEZE;
			SRCCAP_DV_DEFAULT_LOG_TRACE(
				"[TRA] neg_ctrl=%d, video_mute=%d, dv_mute=%d --> action=%d\n",
				descrb_info->buf.neg_ctrl, video_mute, dv_mute, neg_action);
			break;

		case M_DV_CRC_STATE_FAIL_STABLE:
			neg_action = SRCCAP_DV_DESCRB_NEG_ACTION_MUTE;
			break;

		default:
			neg_action = SRCCAP_DV_DESCRB_NEG_ACTION_RESUME;
			break;
		}

		if (neg_action == SRCCAP_DV_DESCRB_NEG_ACTION_FREEZE) {
			/* freeze */
			if (descrb_info->buf.in_freeze == false) {
				ret = drv_hwreg_srccap_dv_descrb_set_video_freeze(
					srccap_dev->dev_id, true, true, NULL, NULL);

				SRCCAP_DV_DEFAULT_LOG_TRACE("[TRA] NEG:set video freeze\n");
				descrb_info->buf.in_freeze = true;
			}
			descrb_info->buf.neg_cnt = DV_LEGACY_NEG_COUNT;
		} else if (neg_action == SRCCAP_DV_DESCRB_NEG_ACTION_MUTE) {
			/* mute */
			ret = drv_hwreg_srccap_dv_descrb_set_video_mute(srccap_dev->dev_id, true,
				true, NULL, NULL);

			SRCCAP_DV_DEFAULT_LOG_TRACE("[TRA] NEG:set video mute\n");
			descrb_info->buf.neg_cnt = DV_LEGACY_NEG_COUNT;

			/* unfreeze */
			if (descrb_info->buf.in_freeze == true) {
				ret = drv_hwreg_srccap_dv_descrb_set_video_freeze(
					srccap_dev->dev_id, false, true, NULL, NULL);

				SRCCAP_DV_DEFAULT_LOG_TRACE(
					"[TRA] NEG:set video unfreeze (in mute)\n");
				descrb_info->buf.in_freeze = false;
			}
		} else  if (descrb_info->buf.neg_cnt > 0) {
			descrb_info->buf.neg_cnt--;
			SRCCAP_DV_LOG_TRACE(SRCCAP_DV_DBG_LEVEL_DEBUG,
				"[TRA] NEG:neg_cnt = %d\n", descrb_info->buf.neg_cnt);
			if (descrb_info->buf.neg_cnt == 0) {
				/* unmute */
				if (descrb_info->buf.in_mute == false) {
					ret = drv_hwreg_srccap_dv_descrb_set_video_mute(
						srccap_dev->dev_id, false, true, NULL, NULL);

					SRCCAP_DV_DEFAULT_LOG_TRACE("[TRA] NEG:set video unmute\n");
				}
				...
				if (descrb_info->buf.in_freeze == true) {
					...
					SRCCAP_DV_DEFAULT_LOG_TRACE(
						"[TRA] NEG:set video unfreeze\n");
					descrb_info->buf.in_freeze = false;
				}
			}
		}

		if (descrb_info->buf.neg_ctrl != M_DV_CRC_STATE_OK) {
			SRCCAP_DV_DEFAULT_LOG_TRACE("[TRA] in_freeze:%d, in_mute:%d\n",
				descrb_info->buf.in_freeze, descrb_info->buf.in_mute);
			// because transition is handled here already for legacy,
			// neg_ctrl should be set as M_DV_CRC_STATE_OK to skip action of C2
			descrb_info->buf.neg_ctrl = M_DV_CRC_STATE_OK;
		}
	...
	struct srccap_dv_dqbuf_out *out)
{
	...
	bool crc = FALSE;
	struct mtk_srccap_dev *srccap_dev = NULL;
	struct srccap_dv_descrb_irq_info *irq_info = NULL;
	int hw_ver = 0;
	struct srccap_dv_descrb_state *state = NULL;

	SRCCAP_DV_LOG_TRACE(SRCCAP_DV_DBG_LEVEL_INFO, "descramble dequeue buffer\n");

	if ((in == NULL) || (out == NULL) || (in->dev == NULL) || (in->avi == NULL)) {
		SRCCAP_DV_LOG_CHECK_POINTER(-EINVAL);
		goto exit;
	}
	...
	irq_info = &(in->dev->dv_info.descrb_info.buf.irq_info[index]);
	crc = irq_info->crc;
	in->dev->dv_info.descrb_info.common.dv_game_mode = FALSE;
	state = &(in->dev->dv_info.descrb_info.state);
	/* reset event */
	srccap_dev->dv_info.descrb_info.common.event_changed = false;
	...
	if (!is_vsem_received && !is_drm_received) {
		ret = dv_descrb_get_vsif(in, crc, &interface, color_format, &is_vsif_received);
		if (ret < 0)
			goto exit;
	...
	ret = mtk_dv_descrb_set_mute_and_freeze(srccap_dev, interface);
	if (ret < 0)
		goto exit;


	if ((interface != SRCCAP_DV_DESCRB_INTERFACE_NONE)
		&& (srccap_dev->dv_info.descrb_info.common.interface != interface))
		SRCCAP_DV_DEFAULT_LOG_TRACE("DV Content, vsif:%d, emp:%d, drm:%d\n",
		is_vsif_received, is_vsem_received, is_drm_received);
	...
	if ((hw_ver >= DV_SRCCAP_HW_TAG_V4) && (hw_ver <= DV_SRCCAP_HW_TAG_V5)
		&& (interface != SRCCAP_DV_DESCRB_INTERFACE_NONE)) {
		srccap_dev->dv_info.descrb_info.common.hdmi_422_pack_en = TRUE;
	} else {
		srccap_dev->dv_info.descrb_info.common.hdmi_422_pack_en =
	...
	if ((interface != SRCCAP_DV_DESCRB_INTERFACE_NONE) &&
		((srccap_dev->timingdetect_info.data.vrr_type == SRCCAP_TIMINGDETECT_VRR_TYPE_VRR)
		|| (srccap_dev->hdmirx_info.u8ALLMMode[srccap_dev->dev_id] == 1))) {
		SRCCAP_DV_LOG_TRACE(SRCCAP_DV_DBG_LEVEL_INFO,
		"dev id:%d, vrr=%d, allm=%d, ori dv game mode=%d\n",
		srccap_dev->dev_id,
		srccap_dev->timingdetect_info.data.vrr_type,
		srccap_dev->hdmirx_info.u8ALLMMode[in->dev->dev_id],
		srccap_dev->dv_info.descrb_info.common.dv_game_mode);
		srccap_dev->dv_info.descrb_info.common.dv_game_mode = true;
	}

	/* debug mode */
	if (in->dev->dv_info.debug_mode.force_game_mode != SRCCAP_DV_DEBUG_FORCE_GAME_MODE_NONE) {
		if (in->dev->dv_info.debug_mode.force_game_mode ==
			SRCCAP_DV_DEBUG_FORCE_GAME_MODE_ON)
			in->dev->dv_info.descrb_info.common.dv_game_mode = TRUE;
		else
			in->dev->dv_info.descrb_info.common.dv_game_mode = FALSE;
	}

	/* update event change */
	dv_descrb_update_event(srccap_dev, state);
	...
	SRCCAP_DV_LOG_TRACE(SRCCAP_DV_DBG_LEVEL_DEBUG,
		"index=%d, crc=%d, DoViCtr=%d, NEG=%d, game=%d(force=%d), hdmi422=%d\n",
		index,
		crc,
		...
		in->dev->dv_info.debug_mode.force_game_mode,
		srccap_dev->dv_info.descrb_info.common.hdmi_422_pack_en);
...
static void _mtk_dv_descrb_update_color_format(
	struct mtk_srccap_dev *srccap_dev)
{
	struct hdmi_avi_packet_info avi;

	if (mtk_srccap_hdmirx_pkt_get_AVI(srccap_dev->srccap_info.src,
			&avi, sizeof(struct hdmi_avi_packet_info)) == 0) {
	...
	void *param)
{
	...
	dev_id = (u8)srccap_dev->dev_id;
	hw_ver = srccap_dev->srccap_info.cap.u32DV_Srccap_HWVersion;
	index = srccap_dev->dv_info.descrb_info.buf.irq_wptr;
	irq_info = &(srccap_dev->dv_info.descrb_info.buf.irq_info[index]);

	// Get color format
	_mtk_dv_descrb_update_color_format(srccap_dev);
	...
	if (mtk_dv_is_descrb_control()) {
		ret = dv_descrb_set_hw_r_enable(srccap_dev);
		if (ret < 0) {
			SRCCAP_DV_LOG_CHECK_RETURN(ret, -EINVAL);
			goto exit;
		}
	...
	irq_info->data_length = metadata_len;
	...
	if (!crc) {
		...
		irq_info->data_length = 0;
	}

	srccap_dev->dv_info.descrb_info.buf.irq_rptr = index;
	srccap_dev->dv_info.descrb_info.buf.irq_wptr = ((index + 1) % SRCCAP_DV_DESCRB_BUF_NUM);
