...
#define CHECKAVDOPEN()               \
	do {                        \
		if (u32AVDopen == 0) {    \
			pr_info("[HWREG][mdbgin_avd][%s:%d]Init first, after use AVD API.\r\n",  \
			__func__, __LINE__);    \
		}             \
	} while (0)
...
enum AVD_Result Api_AVD_Init(struct VD_INITDATA *pVD_InitData, MS_U32 u32InitDataLen)
{
	...
	if (bAVDCMAInit == 0) {
		if (MApi_CMA_Pool_Init(&AVD_CMA_Pool_Init_PARAM) == TRUE) {
			bAVDCMAInit = 1;
			STI_LOGI("AVD", "[%s,%d] get pool_handle_id is %u.\n",
				__PRETTY_FUNCTION__, __LINE__,
				AVD_CMA_Pool_Init_PARAM.pool_handle_id);
			STI_LOGI("AVD", "[%s,%d] get miu is %u.\n",
				__PRETTY_FUNCTION__, __LINE__,
				AVD_CMA_Pool_Init_PARAM.miu);
			STI_LOGI("AVD", "[%s,%d] get heap_miu_start_offset is 0x%lX.\n",
				__PRETTY_FUNCTION__, __LINE__,
				AVD_CMA_Pool_Init_PARAM.heap_miu_start_offset);
			STI_LOGI("AVD", "[%s,%d] get heap_length is 0x%lX.\n",
				__PRETTY_FUNCTION__, __LINE__,
				AVD_CMA_Pool_Init_PARAM.heap_length);
		} else {
			bAVDCMAInit = 0;
			STI_LOGI("AVD", "[%s,%d], CMA_POOL_INIT ERROR!!.\n",
				__PRETTY_FUNCTION__, __LINE__);
			return FALSE;
		}
	}
	...
	if (u32AVDopen == 1) {
		...
		if ((bAVDCMAInit == 1) && (bAVDGetMA == 1)) {
			if (MApi_CMA_Pool_PutMem(&AVD_CMA_Free_PARAM) == FALSE) {
				STI_LOGI("AVD", "[%s,%d] CMA_Pool_PutMem ERROR!!\n.",
					__PRETTY_FUNCTION__, __LINE__);
				bAVDGetMA = 1;
			} else {
				bAVDGetMA = 0;
...
MS_BOOL Api_AVD_DspReset(void)
{
	MS_BOOL ret = FALSE;

	CHECKAVDOPEN();
	if (u32AVDopen == 1)
		ret = Drv_AVD_ResetDSP(pAVDResoucePrivate);
	return ret;
}
...
MS_BOOL Api_AVD_SetInput(enum AVD_InputSourceType eSource, MS_U8 u8ScartFB)
{
	...
	if ((bAVDCMAInit == 1) && (bAVDGetMA == 0)) {
		...
		if (MApi_CMA_Pool_GetMem(&AVD_CMA_Alloc_PARAM) == FALSE) {
			bAVDGetMA = 0;
			STI_LOGI("AVD", "[%s,%d] CMA_Pool_GetMem ERROR!!.\n",
				__PRETTY_FUNCTION__, __LINE__);
		} else {
			bAVDGetMA = 1;
			STI_LOGI("AVD", "[%s,%d] get from heap_id %d.\n",
				__PRETTY_FUNCTION__, __LINE__,
				AVD_CMA_Alloc_PARAM.pool_handle_id);
			STI_LOGI("AVD", "[%s,%d] ask offset 0x%lX.\n",
				__PRETTY_FUNCTION__, __LINE__,
				AVD_CMA_Alloc_PARAM.offset_in_pool);
			STI_LOGI("AVD", "[%s,%d] ask length 0x%lX.\n",
				__PRETTY_FUNCTION__, __LINE__,
				AVD_CMA_Alloc_PARAM.length);
			STI_LOGI("AVD", "[%s,%d] return va is 0x%lX.\n",
				__PRETTY_FUNCTION__, __LINE__,
				AVD_CMA_Alloc_PARAM.virt_addr);
...
void Api_AVD_75IRE_Setup_Cancel(MS_BOOL bEnable)
{
	CHECKAVDOPEN();
	if (u32AVDopen == 1)
		Drv_AVD_Set_75IRE_Setup_Cancel(bEnable, pAVDResoucePrivate);
}
...
void Api_AVD_GetFactoryPara(enum AVD_Factory_Para FactoryPara, MS_U8 *u8Value)
{
	CHECKAVDOPEN();
	if (u32AVDopen == 1)
		Drv_AVD_GetFactoryPara(FactoryPara, u8Value, pAVDResoucePrivate);
}
...
MS_U32 Api_AVD_SetPowerState(EN_POWER_MODE u16PowerState)
{
	...
	CHECKAVDOPEN();
	if (u32AVDopen == 1)
		ret = Drv_AVD_SetPowerState(u16PowerState, NULL);
	return ret;
}
EXPORT_SYMBOL(Api_AVD_SetPowerState);
...
MS_BOOL SYMBOL_WEAK Api_AVD_GetCGMSDetect(void)
{
	MS_BOOL ret = FALSE;
...
MS_BOOL SYMBOL_WEAK Api_AVD_GetMacroVisionDetect(void)
{
	MS_BOOL ret = FALSE;
