...
{
	enum AVD_Result eReturn = E_AVD_FAIL;

	if (u32PowerState == E_POWER_SUSPEND) {
		STI_LOGI("AVD", "[PowerState-SUSPEND]Start.\n");
		#if	defined(MSTAR_STR)
		if (bInitSTR == 1) {
		...
		#endif
		STI_LOGI("AVD", "[PowerState-SUSPEND]End.\n");
		eReturn = E_AVD_OK;
	} else if (u32PowerState == E_POWER_RESUME) {
		STI_LOGI("AVD", "[PowerState-RESUME]Start.\n");
#if	defined(MSTAR_STR)
		if (bInitSTR == 1) {
			vd_str_init();
			...
			if ((eSource_Str == E_INPUT_SOURCE_CVBS1) ||
				...
				if (eVideoStandard_Str != etemp_VideoStandard)
					STI_LOGI("AVD", "timing change - YES.\n");
				else
					STI_LOGI("AVD", "timing change - NO.\n");
			}
		}
#endif
		STI_LOGI("AVD", "[PowerState-RESUME]End.\n");
		eReturn = E_AVD_OK;
	} else
		eReturn = E_AVD_FAIL;

	return (MS_U32)eReturn;
}

static void	vd_str_store_reg(void)
{
	HWREG_AVD_STR_STORE_REG();
}
...
static void	vd_str_setinput(void)
{
	HWREG_AVD_AFEC_SetInput((int)eSource_Str, u8ScartFB_Str,
			(int)eDemodType_Str, u32XTAL_Clock_Str);
...
static void	_Drv_AVD_VDMCU_Init(struct AVD_RESOURCE_PRIVATE	*pResource)
{
	...
	if (pResource->_u8AVDDbgLevel == AVD_DBGLV_DEBUG)
		STI_LOGI("AVD", "_MDrv_AVD_VDMCU_Init\n");
	#endif
	...
	#endif
	{
		...
		#if	AVD_DRV_DEBUG
		if (pResource->_u8AVDDbgLevel	== AVD_DBGLV_DEBUG)
			STI_LOGI("AVD", "[AVD_LOAD_CODE_BYTE_WRITE]F/W ADDR = 0x%X\n",
		...
		#if	AVD_DRV_DEBUG
		if (pResource->_u8AVDDbgLevel == AVD_DBGLV_DEBUG)
			STI_LOGI("AVD", "[AVD_LOAD_CODE_BDMA_FROM_SPI]F/W ADDR = 0x%X 0x%X\n",
		...
		#if	AVD_DRV_DEBUG
		if (pResource->_u8AVDDbgLevel == AVD_DBGLV_DEBUG)
			STI_LOGI("AVD", "[AVD_LOAD_CODE_BDMA_FROM_DRAM]F/W ADDR = 0x%X\n",
			(unsigned int)(pResource->g_VD_InitData.pu8VD_DSP_Code_Address));
		#endif
		HWREG_AVD_VDMCU_SetFreeze(ENABLE);
		...
		if (pResource->_u8AVDDbgLevel >= AVD_DBGLV_ERR)
			STI_LOGI("AVD", "_MDrv_AVD_VDMCU_Init eLoadCodeType Invalid\r\n");
		#endif
	}
...
static void	_Drv_AVD_3DCombSpeedup(struct	AVD_RESOURCE_PRIVATE *pResource)
{
	...
	#if	AVD_DRV_DEBUG
	if (pResource->_u8AVDDbgLevel == AVD_DBGLV_DEBUG)
		STI_LOGI("AVD", "_Api_AVD_3DCombSpeedup Enable %d\n",
...
static void	_Drv_AVD_ShifClk_Monitor(struct AVD_RESOURCE_PRIVATE *pResource)
{
		...
		default:
		 ...
		 HWREG_AVD_ShiftClk((int)pResource->gShiftMode, (int)eVideoStandard,
...
void _Drv_AVD_COMB_StillImage(struct AVD_RESOURCE_PRIVATE	*pResource)
{
	...
	if ((pResource->_eVDInputSource == E_INPUT_SOURCE_ATV) ||
		...
		if (check_counter < 10) {
			...
			if (pResource->g_stillImageParam.bMessageOn) {
				#if AVD_DRV_DEBUG
				STI_LOGI("AVD", "[AVD]AVG noise mag = %d\n",
					(int)u16Value);
				#endif
			}
			...
			if (u16Value <= pResource->g_stillImageParam.u8Threshold1) {
				...
				if (pResource->g_stillImageParam.bMessageOn) {
					#if AVD_DRV_DEBUG
					STI_LOGI("AVD", "[AVD]Thread =%d\n\n",
						u16Value);
					STI_LOGI("AVD", "[1]Thread1: u16Value = 0x%x\n\n",
						u16Value);
					#endif
				}
				...
				if (status == 1) {
					if (pResource->g_stillImageParam.bMessageOn) {
						#if AVD_DRV_DEBUG
						STI_LOGI("AVD", "[4]Thread1: u16Value = 0x%x\n\n",
							u16Value);
						#endif
					}
					HWREG_AVD_COMB_SET_StillImage(
					...
					if (pResource->g_stillImageParam.bMessageOn) {
						#if AVD_DRV_DEBUG
						STI_LOGI("AVD", "[5]Thread2: u16Value = 0x%x\n\n",
							u16Value);
						#endif
					}
					 HWREG_AVD_COMB_SET_StillImage(
					...
					if (pResource->g_stillImageParam.bMessageOn) {
						#if AVD_DRV_DEBUG
						STI_LOGI("AVD", "[6]Thread3: u16Value = 0x%x\n\n",
							u16Value);
						#endif
					}
					 status = 2;
				...
				if (pResource->g_stillImageParam.bMessageOn) {
					#if AVD_DRV_DEBUG
					STI_LOGI("AVD", "[2]Thread2: u16Value = 0x%x\n\n",
						u16Value);
					#endif
				}
				HWREG_AVD_COMB_SET_StillImage(
				...
				if (status == 1) {
					if (pResource->g_stillImageParam.bMessageOn) {
						#if AVD_DRV_DEBUG
						STI_LOGI("AVD", "[7]Thread1: u16Value = 0x%x\n\n",
							u16Value);
						#endif
					}
					status = 2;
					...
					if (pResource->g_stillImageParam.bMessageOn) {
						#if AVD_DRV_DEBUG
						STI_LOGI("AVD", "[8]Thread2: u16Value = 0x%x\n\n",
							u16Value);
						#endif
					}
					 HWREG_AVD_COMB_SET_StillImage(
					...
					if (pResource->g_stillImageParam.bMessageOn) {
						#if AVD_DRV_DEBUG
						STI_LOGI("AVD", "[9]Thread3: u16Value = 0x%x\n\n",
							u16Value);
						#endif
					}
					 HWREG_AVD_COMB_SET_StillImage(
				...
				if (pResource->g_stillImageParam.bMessageOn) {
					#if AVD_DRV_DEBUG
					STI_LOGI("AVD", "=====>3Thread3: u16Value = 0x%x\n\n",
						u16Value);
					#endif
				}
				HWREG_AVD_COMB_SET_StillImage(
...
struct AVD_RESOURCE_PRIVATE *pResource)
{
	...
	if (strncpy(pAVD_Mutex_String, "Mutex VD", 10) == NULL) {
		#if AVD_DRV_DEBUG
		if (pResource->_u8AVDDbgLevel == AVD_DBGLV_DEBUG)
			STI_LOGI("AVD", "Api_AVD_Init strcpy Fail\n");
		#endif
		return E_AVD_FAIL;
	}
#if	!defined STI_PLATFORM_BRING_UP
...
MS_BOOL	Drv_AVD_ResetMCU(struct	AVD_RESOURCE_PRIVATE *pResource)
{
	...
	#if	AVD_DRV_DEBUG
	if (pResource->_u8AVDDbgLevel == AVD_DBGLV_DEBUG)
		STI_LOGI("AVD", "MDrv_AVD_ResetMCU\n");
	#endif
...
MS_BOOL Drv_AVD_ResetDSP(struct AVD_RESOURCE_PRIVATE *pResource)
{
	AVD_LOCK(pResource->_s32AVD_Mutex, pResource->_u8AVDDbgLevel);
	#if AVD_DRV_DEBUG
	if (pResource->_u8AVDDbgLevel == AVD_DBGLV_DEBUG)
		STI_LOGI("AVD", "MDrv_AVD_ResetDSP\n");
	#endif

	HWREG_AVD_RESET_DSP_Code();
	...
	struct AVD_RESOURCE_PRIVATE *pResource)
{
	...
	#if	AVD_DRV_DEBUG
	if (pResource->_u8AVDDbgLevel == AVD_DBGLV_DEBUG)
		STI_LOGI("AVD", "Api_AVD_ForceVideoStandard %d\n", eVideoStandardType);
	#endif
...
void Drv_AVD_SetRegFromDSP(struct	AVD_RESOURCE_PRIVATE *pResource)
{
		...
		if (pResource->g_VD_InitData.u32VDPatchFlag	& AVD_PATCH_FINE_TUNE_FH_DOT) {
			...
			if (pResource->_u8AVDDbgLevel == AVD_DBGLV_DEBUG)
				if (pResource->u32VDPatchFlagStatus & AVD_PATCH_FINE_TUNE_FH_DOT)
					STI_LOGI("AVD", "LatchH,HStart,Debouncer = %d %d %d\n",
...

	{
		...
		if (abs(u16StandardVtotal - HWREG_AVD_AFEC_GetVTotal()) >= 9)
		 ...
		 eSource = (int)pResource->_eVDInputSource;
	...
	{ //burst
		...
		if (pResource->_u8Comb10Bit3Flag) {
			...
			#endif
			{
				...
				#if	AVD_DRV_DEBUG
				if (pResource->_u8AVDDbgLevel == AVD_DBGLV_DEBUG)
					STI_LOGI("AVD", "Drv_AVD_3DCombSpeedup Disable\n");
				#endif
			}
			#if	defined(STELLAR_FLAG)
	...
	if (pResource->_u8AVDDbgLevel == AVD_DBGLV_DEBUG)
		if (pResource->u32PreVDPatchFlagStatus != pResource->u32VDPatchFlagStatus) {
			STI_LOGI("AVD", "u32VDPatchFlagStatus %x\n",
	...
	struct AVD_RESOURCE_PRIVATE *pResource) //SCART_FB eFB)
{
	...
	#if	AVD_DRV_DEBUG
	if (pResource->_u8AVDDbgLevel == AVD_DBGLV_DEBUG)
		STI_LOGI("AVD", "Api_AVD_SetInput %d\n", eSource);
	#endif
	...
	#endif
	HWREG_AVD_AFEC_SetInput((int)eSource, u8ScartFB, (int)pResource->g_VD_InitData.eDemodType,
	pResource->g_VD_InitData.u32XTAL_Clock);
	//HWREG_AVD_COMB_SetMemoryRequest(ENABLE);
	#if	defined(MSTAR_FLAG)
 ...
 */
	...
	if (eSource	== E_INPUT_SOURCE_ATV) {
		if (pResource->g_VD_InitData.bRFGainSel == VD_USE_FIX_GAIN) {
			...
			if (pResource->g_VD_InitData.eDemodType == DEMODE_NORMAL) {
				#if	defined(STELLAR_FLAG)
	...
	struct AVD_RESOURCE_PRIVATE *pResource)
{
	...
	#if	AVD_DRV_DEBUG
	if (pResource->_u8AVDDbgLevel == AVD_DBGLV_DEBUG)
		STI_LOGI("AVD", "Api_AVD_SetVideoStandard %d\n", eStandard);
	#endif
...
void Drv_AVD_SetChannelChange(struct AVD_RESOURCE_PRIVATE	*pResource)
{
	...
	#if	AVD_DRV_DEBUG
	if (pResource->_u8AVDDbgLevel == AVD_DBGLV_DEBUG)
		STI_LOGI("AVD", "Drv_AVD_SetChannelChange\n");
	#endif
...
void Drv_AVD_SetHsyncDetectionForTuning(MS_BOOL	bEnable, struct	AVD_RESOURCE_PRIVATE *pResource)
{
	...
	#if	AVD_DRV_DEBUG
	if (pResource->_u8AVDDbgLevel == AVD_DBGLV_DEBUG)
		STI_LOGI("AVD", "Api_AVD_SetHsyncDetectionForTuning %d\n", bEnable);
	#endif
...
void Drv_AVD_Set_75IRE_Setup_Cancel(MS_BOOL bEnable, struct AVD_RESOURCE_PRIVATE *pResource)
{
	AVD_LOCK(pResource->_s32AVD_Mutex, pResource->_u8AVDDbgLevel);
	HWREG_AVD_Set_75IRE_Setup_Cancel(bEnable);
	AVD_UNLOCK(pResource->_s32AVD_Mutex, pResource->_u8AVDDbgLevel);
}
	...
	struct AVD_RESOURCE_PRIVATE *pResource)
{
	...
	case E_FACTORY_PARA_AFEC_D4:
		...
		pResource->_u8AfecD4Factory = u8Value;
		break;
	...
	case E_FACTORY_PARA_AFEC_D7_LOW:
		...
		break;
	default:
		STI_LOGI("AVD", "ERROR!!! Invalid Factory Parameter!\n");
		break;
	}
}

void Drv_AVD_GetFactoryPara(enum AVD_Factory_Para FactoryPara, MS_U8 *u8Value,
	struct AVD_RESOURCE_PRIVATE *pResource)
{
	switch (FactoryPara) {
	case E_FACTORY_PARA_AFEC_D4:
		*u8Value = HWREG_AVD_AFEC_GET_PACTH_D4();
		break;
	case E_FACTORY_PARA_AFEC_D8:
		*u8Value = HWREG_AVD_AFEC_GET_PACTH_D8();
		break;
	case E_FACTORY_PARA_AFEC_D5_BIT2:
		*u8Value = HWREG_AVD_AFEC_GET_FORCE_K1K2();
		break;
	case E_FACTORY_PARA_AFEC_D9_BIT0:
		*u8Value = HWREG_AVD_AFEC_GET_RF_LOW_HSYNC_DETECTION();
		break;
	case E_FACTORY_PARA_AFEC_A0:
		*u8Value = HWREG_AVD_AFEC_GET_DPL_K1();
		break;
	case E_FACTORY_PARA_AFEC_A1:
		*u8Value = HWREG_AVD_AFEC_GET_DPL_K2();
		break;
	case E_FACTORY_PARA_AFEC_66_BIT67:
		*u8Value = HWREG_AVD_AFEC_GET_ENABLE_SLICE_LEVEL();
		break;
	case E_FACTORY_PARA_AFEC_6E_BIT7654:
		*u8Value = HWREG_AVD_AFEC_GET_V_SLICE_LEVEL();
		break;
	case E_FACTORY_PARA_AFEC_6E_BIT3210:
		*u8Value = HWREG_AVD_AFEC_GET_H_SLICE_LEVEL();
		break;
	case E_FACTORY_PARA_AFEC_43:
		*u8Value = HWREG_AVD_AFEC_GET_FIX_GAIN();
		break;
	case E_FACTORY_PARA_AFEC_44:
		*u8Value = HWREG_AVD_AFEC_GET_AGC_UPDATE();
		break;
	case E_FACTORY_PARA_AFEC_CB:
		*u8Value = HWREG_AVD_AFEC_GET_Standard_Detection_Flag();
		break;
	case E_FACTORY_PARA_AFEC_CF_BIT2:
		*u8Value = HWREG_AVD_AFEC_GET_RF_MODE();
		break;
	case E_FACTORY_PARA_AFEC_D5_BIT3:
		*u8Value = HWREG_AVD_AFEC_GET_JILIN_PATCH();
		break;
	case E_FACTORY_PARA_AFEC_D7_HIGH:
		*u8Value = pResource->g_VD_InitData.u8ColorKillHighBound;
		break;
	case E_FACTORY_PARA_AFEC_D7_LOW:
		*u8Value = pResource->g_VD_InitData.u8ColorKillLowBound;
		break;
	default:
		STI_LOGI("AVD", "ERROR!!! Invalid Factory Parameter!\n");
		break;
...
struct AVD_RESOURCE_PRIVATE *pResource)
{
	...
	#if	AVD_DRV_DEBUG
	if (pResource->_u8AVDDbgLevel == AVD_DBGLV_DEBUG)
		STI_LOGI("AVD", "Api_AVD_GetCaptureWindow %ld %d\n",
...
{
	MS_U32 u32Return = 1;
	u32Return = AVDStr(u16PowerState, NULL);
	return u32Return;
}
...
MS_BOOL	Drv_AVD_IsAVDAlive(void)
{
	...
	if (u8DeadCnt == 0) {
		STI_LOGI("AVD", "MDrv_AVD_IsAVDAlive Status : Alive\n");
		ret = TRUE;
	} else {
		STI_LOGI("AVD", "MDrv_AVD_IsAVDAlive Status : Dead\n");
		ret = FALSE;
	}
	return ret;
}
