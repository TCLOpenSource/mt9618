...
static MS_U32 gu32_unmute_debounce_ms;
static MS_U32 gu32_mute_msk;
static MS_BOOL gu32_dither_value[HDMI_PORT_MAX_NUM] = {FALSE, FALSE, FALSE, FALSE};
...
// time_diff = t1 - t2,
static inline MS_U32 _mdrv_hdmi_time_diff_u32(MS_U32 t1, MS_U32 t2)
{
	return t1 > t2 ? (t1 - t2) : t1 + HDMI_DWORD_FUL_MSK - t2 + 1;
}
...
void _mdrv_HDMI_ParseMultiVSInfoFrame(void *pPacket, stVS_PARSING_INFO *pstVSParsingInfo)
{
	MS_U8 ucpktcnt = 0;
	MS_U8 ucInfoFrame[30] = {0x00};
	MS_U8 i = 0;

	ucpktcnt = *(MS_U8 *)(pPacket + 120); // multi vs cnt

	for (i = 0; i < ucpktcnt; i++) {
		memcpy(ucInfoFrame, (MS_U8 *)(pPacket + 30*i), 30);
		// 0: HB1
		// 1: HB2
		// 2: PB1
		// 3: PB0
		...
		//parsing process
		pstVSParsingInfo->u8Version = ucInfoFrame[0];
		pstVSParsingInfo->u8Length = ucInfoFrame[1] & BMASK(4:0);

		if ((ucInfoFrame[5] == 0x00) && (ucInfoFrame[4] == 0x0C) &&
			(ucInfoFrame[3] == 0x03)) {
			pstVSParsingInfo->enHDMIVideoFormat =
			(E_HDMI_ADDITIONAL_VIDEO_FORMAT)
			(ucInfoFrame[6] & BMASK(7:5)) >> 5;

			if (pstVSParsingInfo->enHDMIVideoFormat ==
			E_HDMI_4Kx2K_FORMAT) {
				pstVSParsingInfo->u8HDMIVIC = ucInfoFrame[7];
			} else if (pstVSParsingInfo->enHDMIVideoFormat ==
			E_HDMI_3D_FORMAT) {
				pstVSParsingInfo->u83DStructure =
				(ucInfoFrame[7] & BMASK(7:4)) >> 4;
				pstVSParsingInfo->u83DExtData =
				(ucInfoFrame[8] & BMASK(7:4)) >> 4;
			}
		} else if ((ucInfoFrame[5] == 0xC4) &&
			(ucInfoFrame[4] == 0x5D) && (ucInfoFrame[3] == 0xD8)) {
			if (ucInfoFrame[7] & BIT(1))
				pstVSParsingInfo->u8ALLMMode = 1;
		} else if ((ucInfoFrame[5] == 0x1A) &&
			(ucInfoFrame[4] == 0xBB) && (ucInfoFrame[3] == 0xFB)) {
			if (ucInfoFrame[7] == 0x0 && ucInfoFrame[6] == 0x1)
				pstVSParsingInfo->u8FilmmakerMode = 1;
		}

		switch (pstVSParsingInfo->u8HDMIVIC) {
		case 0x1:
			pstVSParsingInfo->en4Kx2KVICCode =
				E_VIC_4Kx2K_30Hz;
			break;
		case 0x2:
			pstVSParsingInfo->en4Kx2KVICCode =
		...
		case 0x0:
			pstVSParsingInfo->en3DStructure =
			E_XC_3D_INPUT_FRAME_PACKING;
			break;
		case 0x1:
			pstVSParsingInfo->en3DStructure =
			E_XC_3D_INPUT_FIELD_ALTERNATIVE;
			break;
		case 0x2:
			pstVSParsingInfo->en3DStructure =
			E_XC_3D_INPUT_LINE_ALTERNATIVE;
			break;
		case 0x3:
			pstVSParsingInfo->en3DStructure =
			E_XC_3D_INPUT_SIDE_BY_SIDE_FULL;
			break;
		case 0x4:
			pstVSParsingInfo->en3DStructure =
			E_XC_3D_INPUT_L_DEPTH;
			break;
		case 0x5:
			pstVSParsingInfo->en3DStructure =
			E_XC_3D_INPUT_L_DEPTH_GRAPHICS_GRAPHICS_DEPTH;
		...
		case 0x6:
			pstVSParsingInfo->en3DStructure =
			E_XC_3D_INPUT_TOP_BOTTOM;
			break;
		case 0x8:
			pstVSParsingInfo->en3DStructure =
			E_XC_3D_INPUT_SIDE_BY_SIDE_HALF;
			break;
		default:
			pstVSParsingInfo->en3DStructure =
			E_XC_3D_INPUT_MODE_NONE;
			break;
		};

		switch (pstVSParsingInfo->u83DExtData) {
		case 0x0:
			pstVSParsingInfo->en3DExtendedData =
...
void _mdrv_HDMI_CheckHDCP14RiProc(void)
{
	...
	for (enPortIndex = INPUT_PORT_DVI0; enPortIndex <= INPUT_PORT_DVI_END;
		...
		if (pHDMIRes->usHDCP14RiCount[ucPortIndex] < HDMI_HDCP14_RI_COUNT)
			...
			// [13] Ri
			if (KDrv_SRCCAP_HDMIRx_hdcp_getstatus(enPortIndex) & HDMI_BIT13) {
				pHDMIRes->bHDCP14RiFlag[ucPortIndex] = TRUE;
				KDrv_SRCCAP_HDMIRx_hdcp_clearstatus(enPortIndex, HDMI_BIT5);
			} else {
...
void _mdrv_HDMI_CheckPacketReceiveProc(void)
{
	...
	for (u8HDMIInfoSource = 0; u8HDMIInfoSource < HDMI_PORT_MAX_NUM;
		...
		if (pHDMIRes->ucReceiveIntervalCount[u8HDMIInfoSource] <
			...
			u32PacketStatus = KDrv_SRCCAP_HDMIRx_pkt_packet_info(
			...
			pHDMIRes->u8PacketReceiveCount[u8HDMIInfoSource] =
				KDrv_SRCCAP_HDMIRx_pkt_getpacketreceivercount(
					u8HDMIInfoSource,
					pHDMIRes->u8PacketReceiveCount[u8HDMIInfoSource],
...
static int _mdrv_HDMI_PollingTask(void *punused __attribute__((unused)))
{
	if (pHDMIRes != NULL) {
		...
		while (pHDMIRes->bHDMITaskProcFlag) {
			if (pHDMIRes->bHDMITaskProcFlag_Park == FALSE) {
				KDrv_SRCCAP_HDMIRx_mac_proc_stablepolling(
...
static void _mdrv_HDMI_hdcp_init(E_MUX_INPUTPORT enInputPortType)
{
	...
	pHDMIRes->stHDMIPollingInfo[ucPortIndex].ucHDMIInfoSource =
		KDrv_SRCCAP_HDMIRx_mux_inport_2_src(enInputPortType);

	if (pHDMIRes->bHDCP22EnableFlag[ucPortIndex]) {
		KDrv_SRCCAP_HDMIRx_hdcp_22_init(ucPortIndex);
		KDrv_SRCCAP_HDMIRx_hdcp_writedone_irq_enable(enInputPortType, TRUE);

		pHDMIRes->bHDCP22IRQEnableFlag[ucPortIndex] = TRUE;
	}
	if (pHDMIRes->bHDCP14R0IRQEnable[ucPortIndex])
		KDrv_SRCCAP_HDMIRx_hdcp_readr0_irq_enable(enInputPortType, TRUE);
}
...
static inline void _mdrv_HDMI_suspend(void)
{
	...
	for (enInputPortType = INPUT_PORT_DVI0; enInputPortType <= INPUT_PORT_DVI_END;
		...
		if (KDrv_SRCCAP_HDMIRx_query_dvi_wakeup_support() == 0) {
			// Clear all CLK and DATA R-term
			KDrv_SRCCAP_HDMIRx_phy_set_clkpulllow(
				TRUE, enInputPortType);
			KDrv_SRCCAP_HDMIRx_phy_set_datarterm_ctrl(
		...
		if (pHDMIRes->bHDCP22EnableFlag[ucPortIndex]) {
			...
			KDrv_SRCCAP_HDMIRx_hdcp_22_init(ucPortIndex);
			KDrv_SRCCAP_HDMIRx_hdcp_writedone_irq_enable(enInputPortType,
					FALSE);

			// clear status
			KDrv_SRCCAP_HDMIRx_hdcp_22_polling_writedone(ucPortIndex);
			KDrv_SRCCAP_HDMIRx_hdcp_22_polling_readdone(ucPortIndex);

			pHDMIRes->bHDCP22IRQEnableFlag[ucPortIndex] = FALSE;
		}
		if (pHDMIRes->bHDCP14R0IRQEnable[ucPortIndex]) {
			KDrv_SRCCAP_HDMIRx_hdcp_readr0_irq_enable(enInputPortType, FALSE);
...
static inline void _mdrv_HDMI_resume(stHDMIRx_Bank *pstHDMIRxBank)
{
	...
	for (enInputPortType = INPUT_PORT_DVI0; enInputPortType <= INPUT_PORT_DVI_END;
		...
		pHDMIRes->stHDMIPollingInfo[ucPortIndex].ucHDMIInfoSource =
			KDrv_SRCCAP_HDMIRx_mux_inport_2_src(enInputPortType);
		pHDMIRes->stHDMIPollingInfo[ucPortIndex].u32MuteEvent = 0;
		pHDMIRes->stHDMIPollingInfo[ucPortIndex].u32MuteHwStop_t1 = 0;
		pHDMIRes->stHDMIPollingInfo[ucPortIndex].u8MutedebounceFlag = 0;
		pHDMIRes->stHDMIPollingInfo[ucPortIndex].u16DEStableCnt = 0;
	}
...
{
	if (KDrv_SRCCAP_HDMIRx_pkt_check_additional_format(
			u8HDMIInfoSource, &pHDMIRes->stHDMIPollingInfo[u8HDMIInfoSource]) ==
		E_HDMI_3D_FORMAT) {
		KDrv_SRCCAP_HDMIRx_pkt_get_3d_meta_field(
							...
							MS_U8 *ucHDCPR0ReadDoneIndex)
{
	...
	if (KDrv_SRCCAP_HDMIRx_hdcp_22_polling_writedone(ucPortIndex)) {
	...
	if (KDrv_SRCCAP_HDMIRx_hdcp_22_polling_readdone(ucPortIndex)) {
		if (pHDMIRes->ucHDCP22RecvIDListSend[ucPortIndex]) {
			// clear ready bit after source read re
			KDrv_SRCCAP_HDMIRx_hdcp_22_setready_bit(ucPortIndex, FALSE);
	...
	if (KDrv_SRCCAP_HDMIRx_hdcp_14_polling_r0_readdone(ucPortIndex)) {
							...
							stHDMIRx_Bank *pstHDMIRxBank)
{
	...
	if (!KDrv_SRCCAP_HDMIRx_attach(hw_version, TRUE)) {
		ret_val &= FALSE;
...
{
	if (!KDrv_SRCCAP_HDMIRx_attach(hw_version, FALSE)) {
		HDMIRX_MSG_ERROR("%s,%d\n", __func__, __LINE__);
		return FALSE;
	}
	return TRUE;
...
static inline MS_BOOL _mdrv_HDMI_hw_module_init_1(
	...
	if (!KDrv_SRCCAP_HDMIRx_mac_init(
		bImmeswitchSupport, ucMHLSupportPath)) {
		ret_val &= FALSE;
		HDMIRX_MSG_ERROR("\r\n");
	}
	// r2 init,
	if (!KDrv_SRCCAP_HDMIRx_phy_init()) {
		ret_val &= FALSE;
		HDMIRX_MSG_ERROR("\r\n");
	}
	// phy port init
	...
	for (enInputPortType = INPUT_PORT_DVI0;
		enInputPortType <= INPUT_PORT_DVI_END;
		 enInputPortType++) {
		if (!KDrv_SRCCAP_HDMIRx_phy_init_p(enInputPortType)) {
			ret_val &= FALSE;
			HDMIRX_MSG_ERROR("\r\n");
		}
		if (!KDrv_SRCCAP_HDMIRx_tmds_init(enInputPortType)) {
			ret_val &= FALSE;
			HDMIRX_MSG_ERROR("\r\n");
		}

		if (KDrv_SRCCAP_HDMIRx_a_query_2p1_port(enInputPortType))
			if (!KDrv_SRCCAP_HDMIRx_frl_init(
				enInputPortType)) {
				ret_val &= FALSE;
				HDMIRX_MSG_ERROR("%s,%d\n",
				__func__, __LINE__);
		...
		if (!KDrv_SRCCAP_HDMIRx_pkt_init(enInputPortType)) {
			ret_val &= FALSE;
			HDMIRX_MSG_ERROR("\r\n");
		}

		if (!KDrv_SRCCAP_HDMIRx_hdcp_init(
			(MS_U8)(enInputPortType - INPUT_PORT_DVI0))) {
			ret_val &= FALSE;
			HDMIRX_MSG_ERROR("\r\n");
		}

		if (!KDrv_SRCCAP_HDMIRx_hpd_init(enInputPortType)) {
			ret_val &= FALSE;
			HDMIRX_MSG_ERROR("\r\n");
		}
	}
	...
	for (edid_n = E_XC_PROG_DVI0_EDID;
		...
		if (!KDrv_SRCCAP_HDMIRx_edid_init(edid_n)) {
			ret_val &= FALSE;
			HDMIRX_MSG_ERROR("\r\n");
		}
	}
...
static inline MS_BOOL _mdrv_HDMI_hw_module_init_2(
	...
	for (u32_i = 0 ; u32_i < KDrv_SRCCAP_HDMIRx_query_audio_n();
		u32_i++) {
		if (!KDrv_SRCCAP_HDMIRx_pkt_audio_init_setting(
	...
	for (u32_i = 0 ; u32_i < KDrv_SRCCAP_HDMIRx_query_dsc_n();
		u32_i++) {
		if (!KDrv_SRCCAP_HDMIRx_dsc_init(E_DSC_D0 + u32_i)) {
			ret_val &= FALSE;
			HDMIRX_MSG_ERROR("%s,%d\n",
			__func__, __LINE__);
		}
	...
	if (!KDrv_SRCCAP_HDMIRx_mux_init(INPUT_PORT_DVI0)) {
		ret_val &= FALSE;
		HDMIRX_MSG_ERROR("\r\n");
	}

	if (KDrv_SRCCAP_HDMIRx_a_query_dsc_port(enInputPortType))
		// P0 owns the dsc decoder.
		if (!KDrv_SRCCAP_HDMIRx_mux_sel_dsc(
			INPUT_PORT_DVI0, E_DSC_D0)) {
			ret_val &= FALSE;
			HDMIRX_MSG_ERROR("\r\n");
		}

	if (!KDrv_SRCCAP_HDMIRx_phy_initMBXPara()) {
		ret_val &= FALSE;
		HDMIRX_MSG_ERROR("\r\n");
	}

	if (KDrv_SRCCAP_HDMIRx_query_dvi2miu_n()) {
#define DVI2MIU_BUF_LEN (0x2000)
		if (DVI2MIU_BUF_LEN
			* KDrv_SRCCAP_HDMIRx_query_dvi2miu_n() <=
		(KDrv_SRCCAP_HDMIRx_query_dram_ofs_end()
		- KDrv_SRCCAP_HDMIRx_query_dram_ofs_dvi2miu())) {

			phy_addr_tmp = (phys_addr_t)(pstHDMIRxBank->pExchangeDataPA
				+ KDrv_SRCCAP_HDMIRx_query_dram_ofs_dvi2miu());

			for (u32_i = 0;
			u32_i < KDrv_SRCCAP_HDMIRx_query_dvi2miu_n();
			u32_i++) {
				if (!KDrv_SRCCAP_HDMIRx_dvi2miu_init(
					DVI2MIU_S0 + u32_i,
					phy_addr_tmp,
					DVI2MIU_BUF_LEN)) {
					ret_val &= FALSE;
	...
	for (enInputPortType = INPUT_PORT_DVI0;
		enInputPortType <= INPUT_PORT_DVI_END;
		 enInputPortType++) {
		KDrv_SRCCAP_HDMIRx_mux_sel_timingdet(
			enInputPortType,
			HDMI_SOURCE_VERSION_HDMI14);

		KDrv_SRCCAP_HDMIRx_mux_sel_clk(
			E_CLK_MUX_TMDS_FRL,
			enInputPortType,
			E_SIG_TMDS,
			0);
...
_mdrv_HDMI_hw_module_de_init(int hw_version, const stHDMIRx_Bank *pstHDMIRxBank)
{
	...
	if (!KDrv_SRCCAP_HDMIRx_mux_deinit(INPUT_PORT_DVI0)) {
		HDMIRX_MSG_ERROR("\r\n");
		return FALSE;
	}

	if (KDrv_SRCCAP_HDMIRx_a_query_dsc_port(enInputPortType))
		if (!KDrv_SRCCAP_HDMIRx_dsc_deinit(E_DSC_D0)) {
			HDMIRX_MSG_ERROR("\r\n");
			return FALSE;
		}

	for (edid_n = E_XC_PROG_DVI0_EDID; edid_n <= E_XC_PROG_DVI3_EDID; edid_n++) {
		if (!KDrv_SRCCAP_HDMIRx_edid_deinit(edid_n))
			HDMIRX_MSG_ERROR("\r\n");
	}
	...
	for (enInputPortType = INPUT_PORT_DVI0; enInputPortType <= INPUT_PORT_DVI_END;
		...
		if (!KDrv_SRCCAP_HDMIRx_hpd_deinit(enInputPortType)) {
			HDMIRX_MSG_ERROR("\r\n");
			return FALSE;
		}

		if (!KDrv_SRCCAP_HDMIRx_hdcp_deinit(
				(MS_U8)(enInputPortType - INPUT_PORT_DVI0))) {
			HDMIRX_MSG_ERROR("\r\n");
			return FALSE;
		}

		if (!KDrv_SRCCAP_HDMIRx_pkt_deinit(enInputPortType)) {
			HDMIRX_MSG_ERROR("\r\n");
			return FALSE;
		}
		if (KDrv_SRCCAP_HDMIRx_a_query_2p1_port(enInputPortType))
			if (!KDrv_SRCCAP_HDMIRx_frl_deinit(
				enInputPortType)) {
				HDMIRX_MSG_ERROR("%s,%d\n",
				__func__, __LINE__);
				return FALSE;
			}
		if (!KDrv_SRCCAP_HDMIRx_tmds_deinit(enInputPortType)) {
			HDMIRX_MSG_ERROR("\r\n");
			return FALSE;
		}
		if (!KDrv_SRCCAP_HDMIRx_phy_deinit_p(enInputPortType)) {
			HDMIRX_MSG_ERROR("\r\n");
			return FALSE;
		}
	}
	...
	// r2 init,
	if (!KDrv_SRCCAP_HDMIRx_phy_deinit()) {
		HDMIRX_MSG_ERROR("\r\n");
		return FALSE;
	}

	if (!KDrv_SRCCAP_HDMIRx_mac_deinit()) {
		HDMIRX_MSG_ERROR("\r\n");
		return FALSE;
	}

	if (!KDrv_SRCCAP_HDMIRx_deinit(pstHDMIRxBank)) {
		HDMIRX_MSG_ERROR("\r\n");
		return FALSE;
	}
	return TRUE;
	...
	MS_U8 ucMHLSupportPath)
{
	...
	if (!KDrv_SRCCAP_HDMIRx_mux_sw_init()) {
		ret_val &= FALSE;
		HDMIRX_MSG_ERROR("\r\n");
	}
	...
	const stHDMIRx_Bank *pstHDMIRxBank)
{
	...
	if (!KDrv_SRCCAP_HDMIRx_mux_sw_deinit()) {
		HDMIRX_MSG_ERROR("\r\n");
		return FALSE;
	}
	...
	stHDMIRx_Bank * pstHDMIRxBank)
{
	...
	for (u32_i = 0 ; u32_i < KDrv_SRCCAP_HDMIRx_query_dvi2miu_n();
		u32_i++) {
// ioremap free, dma stop.
		if (!KDrv_SRCCAP_HDMIRx_dvi2miu_deinit(
			DVI2MIU_S0 + u32_i)) {
			ret_val &= FALSE;
			HDMIRX_MSG_ERROR("\r\n");
		}
	...
	if (!KDrv_SRCCAP_HDMIRx_mux_suspend(INPUT_PORT_DVI0)) {
		HDMIRX_MSG_ERROR("\r\n");
		ret_val &= FALSE;
	}
	...
	for (u32_i = 0 ; u32_i < KDrv_SRCCAP_HDMIRx_query_dsc_n();
		u32_i++) {
		if (!KDrv_SRCCAP_HDMIRx_dsc_suspend(E_DSC_D0 + u32_i)) {
			ret_val &= FALSE;
			HDMIRX_MSG_ERROR("%s,%d\n",
			__func__, __LINE__);
		}
	...
	for (u32_i = 0;
		u32_i < KDrv_SRCCAP_HDMIRx_a_query_max_port_n();
		u32_i++) {
		enInputPortType =
			(E_MUX_INPUTPORT)(INPUT_PORT_DVI0 + u32_i);

		if (!KDrv_SRCCAP_HDMIRx_hpd_suspend(enInputPortType)) {
			HDMIRX_MSG_ERROR("\r\n");
			ret_val &= FALSE;
		}

		if (!KDrv_SRCCAP_HDMIRx_hdcp_suspend(enInputPortType)) {
			HDMIRX_MSG_ERROR("\r\n");
			ret_val &= FALSE;
		}

		if (!KDrv_SRCCAP_HDMIRx_pkt_suspend(enInputPortType)) {
			HDMIRX_MSG_ERROR("\r\n");
			ret_val &= FALSE;
		}
		...
		if (KDrv_SRCCAP_HDMIRx_a_query_2p1_port(
			enInputPortType))
			if (!KDrv_SRCCAP_HDMIRx_frl_suspend(
				enInputPortType)) {
				HDMIRX_MSG_ERROR("\r\n");
				ret_val &= FALSE;
			}

		if (!KDrv_SRCCAP_HDMIRx_tmds_suspend(enInputPortType)) {
			HDMIRX_MSG_ERROR("\r\n");
			ret_val &= FALSE;
		}

		if (!KDrv_SRCCAP_HDMIRx_phy_suspend(enInputPortType)) {
			HDMIRX_MSG_ERROR("\r\n");
			ret_val &= FALSE;
		}

		if (!KDrv_SRCCAP_HDMIRx_mac_suspend(enInputPortType)) {
			HDMIRX_MSG_ERROR("\r\n");
			ret_val &= FALSE;
		}

		if (!KDrv_SRCCAP_HDMIRx_irq_suspend(enInputPortType)) {
			HDMIRX_MSG_ERROR("\r\n");
			ret_val &= FALSE;
		}
	}
	...
	stHDMIRx_Bank * pstHDMIRxBank)
{
	...
	for (u32_i = 0;
		...
		KDrv_SRCCAP_HDMIRx_hpd_pull(
		FALSE, enInputPortType,
		pHDMIRes->bHPDInvertFlag[HDMI_GET_PORT_SELECT(enInputPortType)],
		pHDMIRes->ucMHLSupportPath);

		if (!KDrv_SRCCAP_HDMIRx_phy_resume(enInputPortType)) {
			HDMIRX_MSG_ERROR("\r\n");
			ret_val &= FALSE;
		}
	}
...
static MS_BOOL _mdrv_HDMI_sw_module_suspend(
	stHDMIRx_Bank * pstHDMIRxBank)
{
	pHDMIRes->bHDMITaskProcFlag_Park = TRUE;
	return TRUE;
}
	...
	stHDMIRx_Bank * pstHDMIRxBank)
{
	...
	pHDMIRes->bHDMITaskProcFlag_Park = FALSE;
	return ret_val;
}
...
void MDrv_HDCP22_IRQEnable(MS_BOOL ubIRQEnable)
{
	...
	if (pHDMIRes != NULL) {
		for (ucPortIdx = 0; ucPortIdx <= (INPUT_PORT_DVI_END - INPUT_PORT_DVI0);
			...
			KDrv_SRCCAP_HDMIRx_hdcp_writedone_irq_enable(
...
void MDrv_HDCP14_ReadR0IRQEnable(MS_BOOL ubR0IRQEnable)
{
	...
	if (pHDMIRes != NULL) {
		for (ucPortIdx = 0; ucPortIdx <= (INPUT_PORT_DVI_END - INPUT_PORT_DVI0);
			...
			KDrv_SRCCAP_HDMIRx_hdcp_readr0_irq_enable(
...
void MDrv_HDMI_init(int hw_version, stHDMIRx_Bank *pstHDMIRxBank)
{
	...
	if (pHDMIRes != NULL) {
		...
		if (!KDrv_SRCCAP_HDMIRx_ioremap(pstHDMIRxBank)) {
			ret_val &= FALSE;
			HDMIRX_MSG_ERROR("\r\n");
		}
		...
		if (pHDMIRes->ucInitialIndex != HDMI_INITIAL_DONE_INDEX) {
			for (enInputPortType = INPUT_PORT_DVI0;
				...
				pHDMIRes->stHDMIPollingInfo[ucPortIndex].ucHDMIInfoSource =
					KDrv_SRCCAP_HDMIRx_mux_inport_2_src(enInputPortType);
				pHDMIRes->stHDMIPollingInfo[
					ucPortIndex].u32MuteEvent = 0;
				pHDMIRes->stHDMIPollingInfo[
					ucPortIndex].u32MuteHwStop_t1 = 0;
				pHDMIRes->stHDMIPollingInfo[
					ucPortIndex].u8MutedebounceFlag = 0;
				pHDMIRes->stHDMIPollingInfo[
					ucPortIndex].u16DEStableCnt = 0;
				pHDMIRes->stHDMIPollingInfo[
					ucPortIndex].u8ActivePort =
					(1<<HDMI_BYTE_BITS) - 1;
				pHDMIRes->stHDMIPollingInfo[
					ucPortIndex].u16ActiveStableCnt = 0;
			}
...
// sim mode is applied when there is no hdmi ip at current bit file.
		if (ret_val == FALSE)
			KDrv_SRCCAP_HDMIRx_setSimMode(TRUE);
//#endif
	...
	stHDMIRx_Bank *pstHDMIRxBank)
{
	...
	for (u32_i = 0; u32_i < KDrv_SRCCAP_HDMIRx_a_query_max_port_n(); u32_i++)
		pHDMIRes->stHDMIPollingInfo[u32_i].u8RegenFsm = 0;
...
static void MDrv_HDMI_WriteEDID(E_XC_DDCRAM_PROG_TYPE eDDCRamType, MS_U8 *u8EDID,
								MS_U16 u16EDIDSize)
{
	KDrv_SRCCAP_HDMIRx_edid_writeedid(eDDCRamType, u8EDID, u16EDIDSize);
}
...
static void MDrv_HDMI_ReadEDID(E_XC_DDCRAM_PROG_TYPE eDDCRamType, MS_U8 *u8EDID,
							   MS_U16 u8EDIDSize)
{
	KDrv_SRCCAP_HDMIRx_edid_readedid(eDDCRamType, u8EDID, u8EDIDSize);
}

static void MDrv_HDMI_DDCRAM_Enable(E_XC_DDCRAM_PROG_TYPE eDDCRamType)
{
	KDrv_SRCCAP_HDMIRx_edid_ddcram_en(eDDCRamType);
}
								   ...
								   E_MUX_INPUTPORT enInputPortType)
{
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			...
			return KDrv_SRCCAP_HDMIRx_mac_get_datainfo(
...
MS_BOOL MDrv_HDCP_Enable(E_MUX_INPUTPORT enInputPortType, MS_BOOL bEnable)
{
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX)) {
			KDrv_SRCCAP_HDMIRx_hdcp_ddc_en(enInputPortType, bEnable);
			bRet = TRUE;
		}
	}
								 ...
								 E_MUX_INPUTPORT enInputPortType)
{
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX)) {
			KDrv_SRCCAP_HDMIRx_phy_set_eq(enInputPortType, enEq, u8EQValue);
			bRet = TRUE;
		} else
						  ...
						  MS_BOOL bInverse)
{
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX)) {
			HDMIRX_MSG_INFO("[%x] HPD Inverse : %d, HighLow Flag %d !!\n",
				enInputPortType, bInverse, bHighLow);
			KDrv_SRCCAP_HDMIRx_hpd_pull(bHighLow, enInputPortType, bInverse, 0);
			bRet = TRUE;
		} else
			HDMIRX_MSG_ERROR("Wrong Input Port Index !!\n");
	}
...
MS_BOOL MDrv_DVI_Software_Reset(E_MUX_INPUTPORT enInputPortType, MS_U16 u16Reset)
{
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX)) {
			KDrv_SRCCAP_HDMIRx_mac_rst_software(enInputPortType, u16Reset);
			bRet = TRUE;
		} else
			HDMIRX_MSG_ERROR("Wrong Input Port Index !!\n");
	}
...
MS_BOOL MDrv_DVI_ClkPullLow(MS_BOOL bPullLow, E_MUX_INPUTPORT enInputPortType)
{
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX)) {
			HDMIRX_MSG_INFO("[%x] Clk PullLow Flag %d !!\n",
				enInputPortType, bPullLow);
			KDrv_SRCCAP_HDMIRx_phy_set_clkpulllow(bPullLow, enInputPortType);
			bRet = TRUE;
		} else
			HDMIRX_MSG_ERROR("Wrong Input Port Index !!\n");
	}
								 ...
								 MS_BOOL bDrivingHigh)
{
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX)) {
			KDrv_SRCCAP_HDMIRx_arc_pinctrl(enInputPortType, bEnable,
				bDrivingHigh);
			bRet = TRUE;
		} else
			HDMIRX_MSG_ERROR("Wrong Input Port Index !!\n");
	...
	if (pHDMIRes != NULL) {
		...
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			...
			if (u8HDMIInfoSource < HDMI_PORT_MAX_NUM) {
				u16GCPacketInfo = KDrv_SRCCAP_HDMIRx_pkt_gcontrol_info(
								  ...
								  MS_BOOL bReadFlag)
{
	...
	if (pHDMIRes != NULL) {
		...
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX)) {
			if (u8HDMIInfoSource < HDMI_PORT_MAX_NUM) {
				u8ErrorStatus = KDrv_SRCCAP_HDMIRx_pkt_get_errstatus(
					u8HDMIInfoSource, u8Value, bReadFlag,
					&pHDMIRes->stHDMIPollingInfo[u8HDMIInfoSource]);

				if (u8ErrorStatus > 0) {
					if (KDrv_SRCCAP_HDMIRx_pkt_get_errstatus(
...
MS_U8 MDrv_HDMI_audio_cp_hdr_info(E_MUX_INPUTPORT enInputPortType)
{
	...
	if (pHDMIRes != NULL) {
		...
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX)) {
			if (u8HDMIInfoSource < HDMI_PORT_MAX_NUM) {
				u8ContentProtectInfo = KDrv_SRCCAP_HDMIRx_pkt_acp_info(
					u8HDMIInfoSource,
					&pHDMIRes->stHDMIPollingInfo[u8HDMIInfoSource]);
			}
		} else
		...
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			...
			if (u8HDMIInfoSource < HDMI_PORT_MAX_NUM) {
				if (pHDMIRes->ulPacketStatus[u8HDMIInfoSource] &
					HDMI_STATUS_AVI_PACKET_RECEIVE_FLAG) {
					enInfoFrameVersion =
						KDrv_SRCCAP_HDMIRx_pkt_avi_infoframe_info_ver(
						u8HDMIInfoSource,
						&pHDMIRes->stHDMIPollingInfo[u8HDMIInfoSource]);
				}
			}
	...
	if (pHDMIRes != NULL) {
		...
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			...
			if (u8HDMIInfoSource < HDMI_PORT_MAX_NUM) {
				if (pHDMIRes->ulPacketStatus[u8HDMIInfoSource] &
					HDMI_STATUS_AVI_PACKET_RECEIVE_FLAG) {
					if (KDrv_SRCCAP_HDMIRx_pkt_avi_infoframe_info(
		...
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			...
			if (u8HDMIInfoSource < HDMI_PORT_MAX_NUM) {
				if (pHDMIRes->ulPacketStatus[u8HDMIInfoSource] &
					...
					bCheckFlag = KDrv_SRCCAP_HDMIRx_mac_islarger166mhz(
						u8HDMIInfoSource,
						&pHDMIRes->stHDMIPollingInfo[u8HDMIInfoSource]);
				}
			}
		...
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			...
			if (u8HDMIInfoSource < HDMI_PORT_MAX_NUM) {
				if (pHDMIRes->ulPacketStatus[u8HDMIInfoSource] &
					HDMI_STATUS_VS_PACKET_RECEIVE_FLAG) {
					enAdditionalFormat =
						KDrv_SRCCAP_HDMIRx_pkt_check_additional_format(
						u8HDMIInfoSource,
						&pHDMIRes->stHDMIPollingInfo[u8HDMIInfoSource]);
				}
			}
			...
			if (u8HDMIInfoSource < HDMI_PORT_MAX_NUM) {
				if (pHDMIRes->ulPacketStatus[u8HDMIInfoSource] &
					...
					if (KDrv_SRCCAP_HDMIRx_pkt_check_additional_format(
						...
						E_HDMI_3D_FORMAT) {
						en3DStructure =
						KDrv_SRCCAP_HDMIRx_pkt_get_3d_structure(
						u8HDMIInfoSource,
						&pHDMIRes->stHDMIPollingInfo[u8HDMIInfoSource]);
					}
				}
				...
				if (pHDMIRes->ulPacketStatus[u8HDMIInfoSource] &
					...
					if (KDrv_SRCCAP_HDMIRx_pkt_check_additional_format(
						...
						&pHDMIRes->stHDMIPollingInfo[u8HDMIInfoSource]) ==
							E_HDMI_3D_FORMAT) {
						en3DExtendData =
							KDrv_SRCCAP_HDMIRx_pkt_get_3d_ext_data(
							u8HDMIInfoSource,
						&pHDMIRes->stHDMIPollingInfo[u8HDMIInfoSource]);
					}
				}
...
MS_BOOL SYMBOL_WEAK MDrv_HDMI_GetDEStableStatus(E_MUX_INPUTPORT enInputPortType)
{
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX))
			bRet = KDrv_SRCCAP_HDMIRx_mac_get_destable(enInputPortType);
		else
			HDMIRX_MSG_ERROR("Wrong Input Port Index !!\n");
	}
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			...
			if (!pHDMIRes->stHDMIPollingInfo[ucPortIndex].bIsRepeater) {
				...
				if (KDrv_SRCCAP_HDMIRx_hdcp_get_encryptionflag(
						enInputPortType) &&
					(!KDrv_SRCCAP_HDMIRx_phy_get_clklosedet(
						enInputPortType))) {
					// [13] Ri: HDCP1.4
						...
						MS_U8 ucData)
{
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX))
			KDrv_SRCCAP_HDMIRx_hdcp_writex74(enInputPortType, ucOffset, ucData);
		else
			HDMIRX_MSG_ERROR("Wrong Input Port Index !!\n");
	}
}
...
MS_U8 MDrv_HDCP_ReadX74(E_MUX_INPUTPORT enInputPortType, MS_U8 ucOffset)
{
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX))
			ucRetData =
				KDrv_SRCCAP_HDMIRx_hdcp_readx74(enInputPortType, ucOffset);
		else
			HDMIRX_MSG_ERROR("Wrong Input Port Index !!\n");
	}
...
void MDrv_HDCP_SetRepeater(E_MUX_INPUTPORT enInputPortType, MS_BOOL bIsRepeater)
{
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			...
			ucBcaps = KDrv_SRCCAP_HDMIRx_hdcp_readx74(
				enInputPortType,
			...
			KDrv_SRCCAP_HDMIRx_hdcp_writex74(enInputPortType,
...
void MDrv_HDCP_SetBstatus(E_MUX_INPUTPORT enInputPortType, MS_U16 usBstatus)
{
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			...
			// [6:0]: Device cnt
			KDrv_SRCCAP_HDMIRx_hdcp_writex74(
...
void MDrv_HDCP_SetHDMIMode(E_MUX_INPUTPORT enInputPortType, MS_BOOL bHDMIMode)
{
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX)) {
			ucBstatusHighByte = KDrv_SRCCAP_HDMIRx_hdcp_readx74(
...
MS_U8 MDrv_HDCP_GetInterruptStatus(E_MUX_INPUTPORT enInputPortType)
{
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX)) {
			ucRetIntStatus =
				(KDrv_SRCCAP_HDMIRx_hdcp_getstatus(enInputPortType) &
				 BMASK(_BYTE_15
					   : _BYTE_8)) >>
				_BYTE_8;
			...
			if (ucRetIntStatus & HDMI_BIT0) {
				...
				KDrv_SRCCAP_HDMIRx_hdcp_setready(enInputPortType,
						FALSE); // Clear Ready
			}

			KDrv_SRCCAP_HDMIRx_hdcp_clearstatus(
							...
							MS_U32 ulDataLen)
{
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX)) {
			KDrv_SRCCAP_HDMIRx_hdcp_write_ksv_list(enInputPortType, pucKSV,
					ulDataLen);
		} else
			HDMIRX_MSG_ERROR("Wrong Input Port Index !!\n");
	}
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			...
			for (ucCnt = 0; ucCnt < HDMI_HDCP_VPRIME_LENGTH; ucCnt++) {
				KDrv_SRCCAP_HDMIRx_hdcp_writex74(enInputPortType,
					DEF_VPRIME_OFFSET + ucCnt, *(pucVPrime + ucCnt));
			}
...
MDrv_HDMI_CheckHDCPENCState(E_MUX_INPUTPORT enInputPortType)
{
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX)) {
			// HDCP RX detection ENC_EN status
			if (KDrv_SRCCAP_HDMIRx_hdcp_get_encryptionflag(enInputPortType) &&
				(!KDrv_SRCCAP_HDMIRx_phy_get_clklosedet(enInputPortType))) {
				// [13] Ri: HDCP1.4
				if (pHDMIRes->bHDCP14RiFlag[ucPortIndex]) {
	...
	MS_BOOL bDataRtermEnable)
{
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX)) {
			HDMIRX_MSG_INFO("[%x] DataRterm Enable %d !!\n",
				enInputPortType, bDataRtermEnable);
			KDrv_SRCCAP_HDMIRx_phy_set_datarterm_ctrl(enInputPortType,
					bDataRtermEnable);
...
MS_HDCP_STATUS_INFO_t MDrv_HDMI_GetHDCPStatusPort(E_MUX_INPUTPORT enInputPortType)
{
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX)) {
			u16HDCPStatus = KDrv_SRCCAP_HDMIRx_hdcp_getstatus(enInputPortType);
...
MS_U8 MDrv_HDMI_GetSCDCValue(E_MUX_INPUTPORT enInputPortType, MS_U8 ucOffset)
{
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX))
			ucRetData =
				KDrv_SRCCAP_HDMIRx_mac_get_scdcval(enInputPortType, ucOffset);
		else
			HDMIRX_MSG_ERROR("Wrong Input Port Index !!\n");
	}
		...
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			...
			if (!pHDMIRes->stHDMIPollingInfo[ucPortIndex].bNoInputFlag) {
				...
				case E_HDMI_GET_TMDS_BIT_RATES:
					ulRetRates =
						KDrv_SRCCAP_HDMIRx_mac_getratekhz(
				...
				case E_HDMI_GET_TMDS_CHARACTER_RATES:
					ulRetRates = KDrv_SRCCAP_HDMIRx_mac_getratekhz(
						enInputPortType, E_HDMI_CHAR,
					pHDMIRes->stHDMIPollingInfo[ucPortIndex].bHDMI20Flag ==
							HDMI_SOURCE_VERSION_HDMI21
							? TRUE
...
MDrv_HDMI_GetSCDCCableDetectFlag(E_MUX_INPUTPORT enInputPortType)
{
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX))
			bReValue = KDrv_SRCCAP_HDMIRx_mac_get_scdccabledet(enInputPortType);
		else
			HDMIRX_MSG_ERROR("Wrong Input Port Index !!\n");
	}
				...
				MS_BOOL bSwitchContentFlag, MS_U8 *pu8PacketContent)
{
	...
	for (u8temp = 0; u8temp < u8PacketLength; u8temp++) {
		if (KDrv_SRCCAP_HDMIRx_pkt_get_packet_value(
	...
	do {
		...
		case PKT_VS:
			if (ulPacketStatus & HDMI_STATUS_VS_PACKET_RECEIVE_FLAG) {
				...
				// Due to byte 3 NULL Padding
				if (u8PacketLength > HDMI_VS_PAD_ONE_PACKET_LENGTH)
					u8PacketLength = HDMI_VS_PAD_ONE_PACKET_LENGTH;
			}
			break;
		...
		case PKT_GM:
			if (ulPacketStatus & HDMI_STATUS_GM_PACKET_RECEIVE_FLAG) {
				bGetContentFlag = TRUE;

				if (u8PacketLength > HDMI_GM_PACKET_LENGTH)
					u8PacketLength = HDMI_GM_PACKET_LENGTH;
			}
			break;
			...
			if (ulPacketStatus & HDMI_STATUS_AUDIO_SAMPLE_PACKET_RECEIVE_FLAG) {
				...
				for (u8temp = 0; u8temp < u8PacketLength; u8temp++) {
					pu8PacketContent[u8temp] =
						KDrv_SRCCAP_HDMIRx_pkt_audio_channel_status(
		...
		case PKT_MULTI_VS:
			if (ulPacketStatus & HDMI_STATUS_VS_PACKET_RECEIVE_FLAG) {
				...
				pu8PacketContent[HDMI_MULTI_VS_PACKET_LENGTH] =
					KDrv_SRCCAP_HDMIRx_mac_get_datainfo(
							   ...
							   E_HDMI_GET_DATA_INFO enInfoType)
{
	...
	if (pHDMIRes != NULL) {
		u8HDMIInfoSource = KDrv_SRCCAP_HDMIRx_mux_inport_2_src(enInputPortType);
		...
		if (u8HDMIInfoSource < HDMI_PORT_MAX_NUM) {
			...
			case E_HDMI_GET_HTT:
			case E_HDMI_GET_VTT:
			case E_HDMI_GET_HDE_DEC:
			case E_HDMI_GET_VDE_DEC:
			case E_HDMI_GET_CHIP_HDCP_CAPABILITY: {
				usInfoData = KDrv_SRCCAP_HDMIRx_mac_get_datainfo(
					enInfoType, u8HDMIInfoSource,
					&pHDMIRes->stHDMIPollingInfo[u8HDMIInfoSource]);
			} break;

			case E_HDMI_GET_AUDIO_PROTECT_INFO: {
				usInfoData = (MS_U16)KDrv_SRCCAP_HDMIRx_pkt_acp_info(
					u8HDMIInfoSource,
					&pHDMIRes->stHDMIPollingInfo[u8HDMIInfoSource]);
			} break;

			case E_HDMI_GET_ERROR_STATUS: {
				usInfoData = KDrv_SRCCAP_HDMIRx_pkt_get_errstatus(
			...
			case E_HDMI_GET_ISRC1_HEADER_INFO: {
				...
				if (ulPacketStatus & HDMI_STATUS_ISRC1_PACKET_RECEIVE_FLAG) {
					usInfoData =
					(MS_U16)KDrv_SRCCAP_HDMIRx_pkt_getisrc1headerinfo(
						u8HDMIInfoSource,
						&pHDMIRes->stHDMIPollingInfo[u8HDMIInfoSource]);
				}
			} break;
...
MDrv_HDCP_GetAuthVersion(E_MUX_INPUTPORT enInputPortType)
{
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX))
			enHDCPAuthVersion =
				KDrv_SRCCAP_HDMIRx_hdcp_get_authver(enInputPortType, TRUE);
		else
			HDMIRX_MSG_ERROR("Wrong Input Port Index !!\n");
	}
									 ...
									 MS_BOOL bPowerDown)
{
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX)) {
			KDrv_SRCCAP_HDMIRx_phy_set_powerdown(
				enInputPortType,
				&pHDMIRes->stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)],
				bPowerDown);
			bDoneFlag = TRUE;
			...
			MS_DVI_CHANNEL_TYPE u8Channel, MS_U16 *pu16CRCVal)
{
	...
	if ((pHDMIRes != NULL) && (pu16CRCVal != 0)) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX)) {
			bRetValue = KDrv_SRCCAP_HDMIRx_mac_get_crcval(
...
MS_BOOL MDrv_HDMI_Get_Dither_Value(E_MUX_INPUTPORT enInputPortType)
{
	MS_BOOL bret = FALSE;
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX)) {
			bret = gu32_dither_value[enInputPortType - INPUT_PORT_DVI0];
		} else
			HDMIRX_MSG_ERROR("Wrong Input Port Index !!\n");
	}
			...
			E_DITHER_TYPE enDitherType, MS_BOOL ubRoundEnable)
{
	...
	if (pHDMIRes != NULL) {
		if ((enInputPortType >= INPUT_PORT_DVI0) &&
			(enInputPortType < INPUT_PORT_DVI_MAX)) {
			KDrv_SRCCAP_HDMIRx_mac_set_ditheren(enInputPortType, enDitherType,
				ubRoundEnable);
			gu32_dither_value[enInputPortType - INPUT_PORT_DVI0] = ubRoundEnable;
			bret = TRUE;
		} else
			HDMIRX_MSG_ERROR("Wrong Input Port Index !!\n");
	}
...
MS_BOOL MDrv_HDCP22_PollingReadDone(MS_U8 ucPortIdx)
{
	...
	if (pHDMIRes != NULL) {
		if (ucPortIdx <= HDMI_GET_PORT_SELECT(INPUT_PORT_DVI_END)) {
			if (KDrv_SRCCAP_HDMIRx_hdcp_22_polling_readdone(ucPortIdx))
				bReadDoneFlag = TRUE;
...
void MDrv_HDCP22_PortInit(MS_U8 ucPortIdx)
{
	if (pHDMIRes != NULL) {
		if (ucPortIdx <= HDMI_GET_PORT_SELECT(INPUT_PORT_DVI_END)) {
			KDrv_SRCCAP_HDMIRx_hdcp_22_init(ucPortIdx);
...
void MDrv_HDCP22_RecvMsg(MS_U8 ucPortIdx, MS_U8 *ucMsgData)
{
	...
	if ((pHDMIRes != NULL) && (ucMsgData != NULL)) {
		if ((ucPortIdx >= 0) &&
			...
			KDrv_SRCCAP_HDMIRx_hdcp_22_recvmsg(ucPortIdx, ucMsgData);
		else
			HDMIRX_MSG_ERROR("Wrong Input Port Index !!\n");
	}
}
						 ...
						 MS_U32 dwDataLen, void *pDummy)
{
	...
	if ((pHDMIRes != NULL) && (pucData != NULL)) {
		if (ucPortIdx <= HDMI_GET_PORT_SELECT(INPUT_PORT_DVI_END)) {
			KDrv_SRCCAP_HDMIRx_hdcp_22_sendmsg(ucPortType, ucPortIdx, pucData,
				dwDataLen, pDummy,
				&ubRecIDListSetDone);
...
MS_BOOL MDrv_HDMI_IOCTL(MS_U32 ulCommand, void *pBuffer, MS_U32 ulBufferSize)
{
	...
	case E_HDMI_INTERFACE_CMD_GET_VS_PARSING_INFO: {
		MS_U8 u8PacketContent[HDMI_GET_MULTIVS_PACKET_LENGTH] = {0};
		...
		if (ulBufferSize == sizeof(stCMD_HDMI_GET_VS_PARSING_INFO)) {
			...
			if (MDrv_HDMI_GetPacketContent(pParameters->enInputPortType,
				PKT_MULTI_VS, HDMI_GET_MULTIVS_PACKET_LENGTH,
					u8PacketContent)) {
				_mdrv_HDMI_ParseMultiVSInfoFrame(u8PacketContent,
...
void MDrv_HDMI_ISR_PKT_QUE(void)
{
	...
	if (pHDMIRes != NULL) {
		for (enInputPortType = INPUT_PORT_DVI0;
			 enInputPortType <= INPUT_PORT_DVI_END; enInputPortType++) {
			KDrv_SRCCAP_HDMIRx_irq_pkt_que(
				enInputPortType,
				&pHDMIRes->stHDMIPollingInfo[enInputPortType - INPUT_PORT_DVI0]);
		}
	}
...
void MDrv_HDMI_ISR_SQH_ALL_WK(void)
{
	...
	if (pHDMIRes != NULL) {
		for (enInputPortType = INPUT_PORT_DVI0;
			 enInputPortType <= INPUT_PORT_DVI_END; enInputPortType++) {
			KDrv_SRCCAP_HDMIRx_irq_sqh_all_wk(
				enInputPortType,
				&pHDMIRes->stHDMIPollingInfo[enInputPortType - INPUT_PORT_DVI0]);
		}
	}
...
{
	if (pHDMIRes != NULL) {
		KDrv_SRCCAP_HDMIRx_irq_clk_det(INPUT_PORT_DVI0,
...
{
	if (pHDMIRes != NULL) {
		KDrv_SRCCAP_HDMIRx_irq_clk_det(INPUT_PORT_DVI1,
...
{
	if (pHDMIRes != NULL) {
		KDrv_SRCCAP_HDMIRx_irq_clk_det(INPUT_PORT_DVI2,
...
{
	if (pHDMIRes != NULL) {
		KDrv_SRCCAP_HDMIRx_irq_clk_det(INPUT_PORT_DVI3,
...
{
	return KDrv_SRCCAP_HDMIRx_irq_mask_save_global(e_int);
}

void MDrv_HDMI_IRQ_MASK_RESTORE(EN_KDRV_HDMIRX_INT e_int, unsigned int val)
{
	KDrv_SRCCAP_HDMIRx_irq_mask_restore_global(e_int, val);
}
...
MS_BOOL MDrv_HDMI_deinit(int hw_version, const stHDMIRx_Bank *pstHDMIRxBank)
{
	...
	if (!KDrv_SRCCAP_HDMIRx_iounmap(pstHDMIRxBank)) {
		HDMIRX_MSG_ERROR("\r\n");
		return FALSE;
	}
...
void MDrv_HDMI_Set_UnMute_Debounce_ms(MS_U32 u32_val)
{
	gu32_unmute_debounce_ms = u32_val;
}

MS_U32 MDrv_HDMI_Get_UnMute_Debounce_ms(void)
{
	return gu32_unmute_debounce_ms;
}

void MDrv_HDMI_Set_Mute_Msk(MS_U32 u32_val)
{
	gu32_mute_msk = u32_val;
}

MS_U32 MDrv_HDMI_Get_Mute_Msk(void)
{
	return gu32_mute_msk;
}
 ...
 * FALSE: unmute
 */
MS_BOOL MDrv_HDMI_sw_mute(
	E_MUX_INPUTPORT enInputPortType,
	MS_U32 *ret_u32p_evt)
{
	MS_BOOL ret_val = FALSE;
	stHDMI_POLLING_INFO *p_stHDMIPollingInfo =
	&pHDMIRes->stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)];

	MS_U32 u32_t1 = MsOS_GetSystemTime();

	MS_U32 u32_pre_mute_evt = 0;
	MS_U32 u32_cur_mute_evt = 0;

	if (p_stHDMIPollingInfo == NULL)
		return FALSE;

	u32_pre_mute_evt = p_stHDMIPollingInfo->u32MuteEvent;

	ret_val = KDrv_SRCCAP_HDMIRx_mac_sw_mute_notify(
		enInputPortType, p_stHDMIPollingInfo, gu32_mute_msk);

	u32_cur_mute_evt = p_stHDMIPollingInfo->u32MuteEvent;

	if (u32_pre_mute_evt != u32_cur_mute_evt) {
		if (ret_val == FALSE && gu32_unmute_debounce_ms > 0) {
		// report unmute, debounce enterring.
			p_stHDMIPollingInfo->u8MutedebounceFlag = 1;
			p_stHDMIPollingInfo->u32MuteHwStop_t1 = u32_t1;
		} else {
			p_stHDMIPollingInfo->u8MutedebounceFlag = 0;
		}

		if (u32_pre_mute_evt == 0) {
			// mute
			HDMIRX_MSG_INFO(
		"[%x] mute, evt=%x/%x, flag=%x, msk=%x, t1=%d/%d/%d, ret=%x\n",
				enInputPortType,
				u32_pre_mute_evt,
				u32_cur_mute_evt,
				p_stHDMIPollingInfo->u8MutedebounceFlag,
				gu32_mute_msk,
				u32_t1,
				p_stHDMIPollingInfo->u32MuteHwStop_t1,
				gu32_unmute_debounce_ms,
				ret_val);
		}
	}

	if (p_stHDMIPollingInfo->u8MutedebounceFlag) {
		if (_mdrv_hdmi_time_diff_u32(u32_t1,
			p_stHDMIPollingInfo->u32MuteHwStop_t1)
			>= gu32_unmute_debounce_ms) {
			// debounce timeout, unmute
			p_stHDMIPollingInfo->u8MutedebounceFlag = 0;
			ret_val = FALSE;
			HDMIRX_MSG_INFO(
		"[%x] unmute, evt=%x/%x, flag=%x, msk=%x, t1=%d/%d/%d, ret=%x\n",
				enInputPortType,
				u32_pre_mute_evt,
				u32_cur_mute_evt,
				p_stHDMIPollingInfo->u8MutedebounceFlag,
	...
	if (ret_u32p_evt)
		*ret_u32p_evt = u32_cur_mute_evt;

	if (u32_pre_mute_evt != u32_cur_mute_evt) {
		HDMIRX_MSG_DBG("[%x] evt=%x/%x, flag=%x, msk=%x, t1=%d/%d/%d, ret=%x\n",
			enInputPortType,
			u32_pre_mute_evt,
			u32_cur_mute_evt,
			p_stHDMIPollingInfo->u8MutedebounceFlag,
...
MS_U32 MDrv_HDMI_Get_Audio_Fs(E_MUX_INPUTPORT enInputPortType)
{
	stHDMI_POLLING_INFO *p_stHDMIPollingInfo =
	&pHDMIRes->stHDMIPollingInfo[HDMI_GET_PORT_SELECT(enInputPortType)];
	MS_U32 u32_f1 = 0;
	EN_HDMI_SIGNAL_TYPE e_sgl = E_SIG_TMDS;

	if (p_stHDMIPollingInfo == NULL)
		return 0;

	e_sgl =
	(p_stHDMIPollingInfo->ucSourceVersion == HDMI_SOURCE_VERSION_HDMI21) ?
		E_SIG_FRL : E_SIG_TMDS;

	if (p_stHDMIPollingInfo->u16ActiveStableCnt
		> HDMI_AUD_FS_GET_API_SWITCH_CNT)
		u32_f1 = KDrv_SRCCAP_HDMIRx_pkt_get_audio_fs_hz(
		enInputPortType,
...
void MDrv_HDMI_Set_Init_Stub(void)
{
	MS_BOOL ret = FALSE;
	MS_BOOL isSim = FALSE;

	isSim = KDrv_SRCCAP_HDMIRx_isSimMode();
	if (isSim) {
		ret = KDrv_SRCCAP_HDMIRx_mac_proc_stablepolling(
			pHDMIRes->ucMHLSupportPath,
			pHDMIRes->ulPacketStatus,
			pHDMIRes->stHDMIPollingInfo);
	}
	...
	if (ret)
		HDMIRX_MSG_INFO("[%s] OK, isSim:%d\n", __func__, isSim);
	else
		HDMIRX_MSG_ERROR("[%s] NG, isSim:%d\n", __func__, isSim);
}
