...
#define SRCCAP_DV_DBG_LEVEL_SVP       SRCCAP_DV_BIT(6)
	...
	if (((_dbg_level_) & g_dv_debug_level) != 0) { \
		if ((_dbg_level_) & SRCCAP_DV_DBG_LEVEL_ERR) { \
			...
			pr_err(SRCCAP_DV_LABEL "[PR][%s,%5d] "_fmt, \
				__func__, __LINE__, ##_args); \
		} else if ((_dbg_level_) & SRCCAP_DV_DBG_LEVEL_SVP) { \
			pr_err(SRCCAP_DV_LABEL "[SVP][%s,%5d] "_fmt, \
				__func__, __LINE__, ##_args); \
		} else { \
...
#define SRCCAP_DV_DEFAULT_LOG_TRACE(_fmt, _args...) \
{ \
	pr_err(SRCCAP_DV_LABEL "[DV_DEFAULT][%s,%5d] "_fmt, \
	__func__, __LINE__, ##_args); \
}
...
enum srccap_dv_debug_force_game_mode {
	SRCCAP_DV_DEBUG_FORCE_GAME_MODE_NONE = 0,
	SRCCAP_DV_DEBUG_FORCE_GAME_MODE_ON,
	SRCCAP_DV_DEBUG_FORCE_GAME_MODE_OFF,

	SRCCAP_DV_DEBUG_FORCE_GAME_MODE_NUM,
};
...
extern bool pqu_descrb_irq_status;
