...
static inline bool _mtk_memout_svp_check_scnsize(int size, int max_size)
{
	if ((size < 0) || (size >= max_size)) {
		SRCCAP_MSG_ERROR("[SVP] scan size %d overflow!\n", size);
		return false;
	}
	return true;
}
...
static int _mtk_memout_auth_buf(struct mtk_srccap_dev *srccap_dev, int buf_fd)
{
	...
	SRCCAP_MSG_DEBUG("[SVP] buf iova = 0x%llx, size = 0x%x\n", iova, size);

	/* authorize source buf */
	buf_tag = SRCCAP_SOURCE_BUF_TAG << SRCCAP_BUF_TAG_OFFSET;
...
static bool _mtk_memout_svp_get_dbg_value_from_string(char *buf, char *name,
	...
	*value = 0;
	...
	for (string = buf; string < (buf + len); ) {
		...
		if (strncmp(tmp_name, name, strlen(name) + 1) == 0) {
			...
			find = true;
			SRCCAP_MSG_DEBUG("name = %s, value = %d\n", name, *value);
			return find;
		}
	}
...
int mtk_memout_svp_init(struct mtk_srccap_dev *srccap_dev)
{
	...
	srccap_dev->memout_info.secure_info.optee_version = MTK_OPTEE_VER_3;
	...
	struct v4l2_memout_secure_info *secureinfo)
{
	...
	if ((secureinfo == NULL) || (srccap_dev == NULL)) {
		SRCCAP_MSG_POINTER_CHECK();
		return -EINVAL;
	}

	if ((srccap_dev->srccap_info.src < V4L2_SRCCAP_INPUT_SOURCE_NONE)
		|| (srccap_dev->srccap_info.src >= V4L2_SRCCAP_INPUT_SOURCE_NUM)) {
		SRCCAP_MSG_ERROR("[SVP] invalid source %d\n",
	...
	input = (u32)srccap_dev->srccap_info.src;
	/* enable secure mode */
	if (secureinfo->bEnable) {
		if (srccap_dev->memout_info.secure_info.svp_enable) {
			SRCCAP_MSG_ERROR("[SVP] Error!! %s\n",
				"Already enable secure mode, please disable first.");
			return -EINVAL;
		}
		...
		case SRCCAP_INPUT_PORT_HDMI3:
			...
			break;
		default:
			SRCCAP_MSG_ERROR("[SVP] Unsupported SVP extin source %d\n",
		...
		if (srccap_dev->memout_info.secure_info.svp_enable) {
			...
			if ((optee_handler.u32VdoPipelineID != 0) &&
				...
				ret = _mtk_memout_svp_smc_call(srccap_dev,
						...
						&op);
				if (ret) {
					SRCCAP_MSG_ERROR("[SVP] %s\n",
						"smc call: DESTROY_VIDEO_PIPELINE fail");
					return ret;
				}
			}
			pipeline_id = 0;
	...
	srccap_dev->memout_info.secure_info.force_svp = secureinfo->bForceSVP;
	...
	s32 meta_fd)
{
	...
	SRCCAP_MSG_DEBUG("[SVP] aid=%d, video pipeline ID=0x%x\n",
		svp_meta.aid, svp_meta.pipelineid);

	ret = mtk_memout_write_metadata_mmap_fd(meta_fd,
	...
	char *buf)
{
	...
	memset(&hwreg_sstinfo, 0, sizeof(struct reg_srccap_memout_sstinfo));
	if (drv_hwreg_srccap_memout_get_sst(&hwreg_sstinfo)) {
		SRCCAP_MSG_ERROR("[SVP] HWREG get sst fail/n");
		return -EINVAL;
	}

	string_size += scnprintf(buf + string_size, max_size - string_size,
		"===== SRCCAP SVP debug command start =====\n"
		" SRCCAP Device ID  :               %02u\n"
		" SRCCAP Secure Mode:          %7s\n"
		" Force  Secure Mode:          %7s\n"
		" SRCCAP AID Status :               %02u\n"
		" SST HDMI P0 Status:               %02u\n"
		" SST HDMI P1 Status:               %02u\n"
		" SST HDMI P2 Status:               %02u\n"
		" SST HDMI P3 Status:               %02u\n"
		" Video Pipeline ID :       0x%08x\n"
		" Dolby Pipeline ID :       0x%08x\n"
		" Dolby Buf Status  :           %6s\n"
		"--------------------------------------\n"
		"Force SVP cmd example:\n"
		"echo force_svp=1 > /sys/devices/platform/mtk_srccap0/mtk_dbg/svp\n"
		"echo force_svp=1 > /sys/devices/platform/mtk_srccap1/mtk_dbg/svp\n"
		"===== SRCCAP SVP debug command end =======\n",
		srccap_dev->dev_id,
		srccap_dev->memout_info.secure_info.svp_enable?"Enable":"Disable",
		srccap_dev->memout_info.secure_info.force_svp?"Enable":"Disable",
		srccap_dev->memout_info.secure_info.aid,
		hwreg_sstinfo.secure_hdmi_0,
		hwreg_sstinfo.secure_hdmi_1,
		hwreg_sstinfo.secure_hdmi_2,
		hwreg_sstinfo.secure_hdmi_3,
		srccap_dev->memout_info.secure_info.video_pipeline_ID,
		srccap_dev->dv_info.dma_info.secure.svp_id,
		srccap_dev->dv_info.dma_info.secure.svp_buf_authed?"Locked":"Unlock");

	if (!_mtk_memout_svp_check_scnsize(string_size, max_size - string_size))
		SRCCAP_MSG_ERROR("_mtk_memout_svp_check_scnsize fail!\n");
...
int mtk_memout_svp_store(
	...
	const char *buf)
{
	int ret = 0, len = 0, string_size = 0;
	u32 force_svp = 0;
	u16 max_size = SRCCAP_STRING_SIZE_65535;
	bool find = false;
	char *cmd = NULL;
	struct v4l2_memout_secure_info sec_inf;
	...
	cmd = MALLOC(sizeof(char) * max_size);
	if (cmd == NULL) {
		SRCCAP_MSG_ERROR("malloc cmd fail!\n");
		return -ENOMEM;
	}

	memset(cmd, 0, sizeof(char) * max_size);
	memset(&sec_inf, 0, sizeof(struct v4l2_memout_secure_info));

	len = strlen(buf);
	string_size = snprintf(cmd, len + 1, "%s", buf);
	if (!_mtk_memout_svp_check_scnsize(string_size, len + 1)) {
		SRCCAP_MSG_ERROR("_mtk_memout_svp_check_scnsize fail!\n");
		FREE_MEM(cmd, sizeof(char) * max_size);
		return -ENOMEM;
	}

	find = _mtk_memout_svp_get_dbg_value_from_string(cmd, "force_svp", len, &force_svp);
	if (!find) {
		SRCCAP_MSG_ERROR("Cmdline format error, "
			"force_svp should be set, please echo help!\n");
		FREE_MEM(cmd, sizeof(char) * max_size);
		return -EINVAL;
	}
	...
	if (force_svp) {
		if (!srccap_dev->memout_info.secure_info.svp_enable) {
			sec_inf.bEnable = true;
			sec_inf.bForceSVP = true;
			ret = mtk_memout_set_secure_mode(srccap_dev, &sec_inf);
			if (ret) {
				SRCCAP_MSG_ERROR("mtk_memout_set_secure_mode fail!\n");
				FREE_MEM(cmd, sizeof(char) * max_size);
				return ret;
			}
		}
	} else {
		if (srccap_dev->memout_info.secure_info.force_svp) {
			ret = mtk_memout_set_secure_mode(srccap_dev, &sec_inf);
			if (ret) {
				SRCCAP_MSG_ERROR("mtk_memout_set_secure_mode fail!\n");
				FREE_MEM(cmd, sizeof(char) * max_size);
