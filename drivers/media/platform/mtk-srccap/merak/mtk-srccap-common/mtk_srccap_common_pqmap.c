...
static int _mtk_srccap_common_map_fd(int fd, void **va, u64 *size)
{
	...
	if (IS_ERR_OR_NULL(db)) {
		SRCCAP_MSG_ERROR("dma_buf_get fail, db=0x%lx\n", (unsigned long)db);
		return -EPERM;
	}
...

{
	...
	if (tag == EXTMEM_TAG_REGISTER_SETTINGS) {
		...
		reg_func *regfunc = (reg_func *)data;
		int i = 0;
		struct ST_PQ_SWREG_INFO stSWRegInfo;
		struct ST_PQ_SWREG_STATUS stSWRegStatus;

		SRCCAP_MSG_DEBUG("VRREG num = %u\n", regfunc->padded_data.fn_num);
		for (i = 0; i < regfunc->padded_data.fn_num; ++i) {
			uint32_t fnidx = regfunc->fn_tbl[i].fn_idx;
			stSWRegInfo.u32RegIdx = regfunc->fn_tbl[i].fn_idx;
			stSWRegInfo.u16Value = regfunc->fn_tbl[i].fn_para;
			SRCCAP_MSG_DEBUG("VR[0x%x]:%u\n", fnidx, regfunc->fn_tbl[i].fn_para);
			MApi_PQ_SetSWReg(pCtx, &stSWRegInfo, &stSWRegStatus);
		}
...
char srccap_common_set_pim_reg(
	int tag, const char *data,
	const size_t datalen, void *pCtx)
{
	struct ST_PQ_NTS_HWREG_INFO stHWRegInfo;
	...
	if (tag == EXTMEM_TAG_REGISTER_SETTINGS) {
		...
		SRCCAP_MSG_DEBUG("HWREG num = %u\n", regfunc->padded_data.fn_num);
		for (i = 0; i < regfunc->padded_data.fn_num; ++i) {
			u32 fn_idx = regfunc->fn_tbl[i].fn_idx;
			u32 fn_para = regfunc->fn_tbl[i].fn_para;

			_mtk_srccap_common_store_data(fn_idx, regfunc->fn_tbl[i].fn_para);
			stHWRegInfo.u32RegIdx = regfunc->fn_tbl[i].fn_idx;
			stHWRegInfo.u16Value = regfunc->fn_tbl[i].fn_para;
			stHWRegInfo.eDomain = E_PQ_DOMAIN_SOURCE;
			ret = MApi_PQ_SetHWReg_NonTs_Imm(&stHWRegInfo, &stHWRegStatus);
			if (ret != E_PQAPI_RC_SUCCESS)
				SRCCAP_MSG_ERROR("MApi_PQ_SetHWReg_NonTs fail, ret:0x%x\n", ret);
...
static int _mtk_srccap_common_deinit_pqmap(struct mtk_srccap_dev *srccap_dev)
{
	...
	if (!srccap_dev->common_info.pqmap_info.pqmap_va)
		FREE_MEM(srccap_dev->common_info.pqmap_info.pqmap_va,
			srccap_dev->common_info.pqmap_info.va_size);
...
static int _mtk_srccap_common_deinit_vd_pqmap(struct mtk_srccap_dev *srccap_dev)
{
	...
	if (!srccap_dev->common_info.vdpqmap_info.pqmap_va)
		FREE_MEM(srccap_dev->common_info.vdpqmap_info.pqmap_va,
			srccap_dev->common_info.vdpqmap_info.va_size);
	...
	struct mtk_srccap_dev *srccap_dev)
{
	...
	srccap_dev->common_info.pqmap_info.pqmap_va = MALLOC(size);
	if (srccap_dev->common_info.pqmap_info.pqmap_va == NULL) {
		SRCCAP_MSG_ERROR("srccap pqmap malloc failed\n");
		return -EPERM;
	}
	memcpy(srccap_dev->common_info.pqmap_info.pqmap_va, va, size);
	_mtk_srccap_common_unmap_fd(info->pqmap_fd, va);

	srccap_dev->common_info.pqmap_info.va_size = size;
	...
	struct mtk_srccap_dev *srccap_dev)
{
	...
	srccap_dev->common_info.vdpqmap_info.pqmap_va = MALLOC(size);
	if (srccap_dev->common_info.vdpqmap_info.pqmap_va == NULL) {
		SRCCAP_AVD_MSG_ERROR("srccap pqmap malloc failed\n");
		return -EPERM;
	}
	memcpy(srccap_dev->common_info.vdpqmap_info.pqmap_va, va, size);
	_mtk_srccap_common_unmap_fd(info->pqmap_fd, va);

	srccap_dev->common_info.vdpqmap_info.va_size = size;
...
cleanup:
	_mtk_srccap_common_deinit_vd_pqmap(srccap_dev);
	return ret;
}
...
{
	int ret = 0;
	struct ST_PQ_ML_CTX_INFO ml_ctx_info = {0};
	struct ST_PQ_ML_CTX_STATUS ml_ctx_status = {0};
	...
	if (srccap_dev->dev_id == 0) {
		ml_ctx_info.pSrc0Addr = (void *)ml_script_info->start_addr;
		ml_ctx_info.pSrc0MaxAddr = (void *)ml_script_info->max_addr;
	} else {
		ml_ctx_info.pSrc1Addr = (void *)ml_script_info->start_addr;
		ml_ctx_info.pSrc1MaxAddr = (void *)ml_script_info->max_addr;
	}

	SRCCAP_MSG_DEBUG("dev_id:%d, mem_index:%d, src0Addr:0x%lx, src0MaxAddr:0x%lx\n",
		srccap_dev->dev_id, ml_script_info->mem_index,
		(unsigned long)ml_ctx_info.pSrc0Addr, (unsigned long)ml_ctx_info.pSrc0MaxAddr);
	SRCCAP_MSG_DEBUG("src1Addr:0x%lx, src1MaxAddr:0x%lx\n",
		(unsigned long)ml_ctx_info.pSrc1Addr, (unsigned long)ml_ctx_info.pSrc1MaxAddr);
...
static int srccap_common_pqmap_get_ml_result(
			...
			struct srccap_ml_script_info *ml_script_info)
{
	struct ST_PQ_ML_PARAM ml_param = {0};
	struct ST_PQ_ML_INFO ml_status = {0};
	int ret = 0;
...
static int srccap_common_load_pqmap_pim(
	struct mtk_srccap_dev *srccap_dev,
	struct srccap_ml_script_info *ml_script_info)
{
	void *pim = NULL;
	...
	if (srccap_dev->dev_id >= SRCCAP_DEV_NUM) {
		SRCCAP_MSG_ERROR("invalid device id\n");
		return -EPERM;
	}

	pim = srccap_dev->common_info.pqmap_info.pPim;
	...
	node_array = srccap_dev->common_info.rm_info.p.node_array;
	...
	if (srccap_dev->streaming) {
		...
		for (index = 0; index < array_size; ++index)
			pqmap_pim_load_setting(pim, node_array[index], 0,
				_mtk_srccap_common_pim_callback,
	...
	struct srccap_ml_script_info *ml_script_info)
{
	...
	SRCCAP_ATRACE_BEGIN("mtk_srccap_common_load_pqmap");
	ret = srccap_common_load_pqmap_pim(srccap_dev, ml_script_info);
	SRCCAP_ATRACE_END("mtk_srccap_common_load_pqmap");
