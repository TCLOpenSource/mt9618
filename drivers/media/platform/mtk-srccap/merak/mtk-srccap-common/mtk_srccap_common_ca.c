...
static int _mtk_common_ca_get_optee_version(struct mtk_srccap_dev *srccap_dev)
{
	...
	if (srccap_dev->memout_info.secure_info.optee_version == -1) {
		...
		if (verfile == NULL || IS_ERR_OR_NULL(verfile)) {
			...
			srccap_dev->memout_info.secure_info.optee_version = MTK_OPTEE_VER_1;
			set_fs(cur_mm_seg);
			kfree(sVer);
			goto final;
		}
		...
		if (strncmp(sVer, "2.4", BOOTARG_CMP_SIZE) == 0)
			srccap_dev->memout_info.secure_info.optee_version = MTK_OPTEE_VER_2;
...
static void _mtk_common_ca_teec_free_temp_refs(TEEC_Operation *operation, TEEC_SharedMemory *shms)
{
	size_t n = 0;
...
int _mtk_common_ca_teec_alloc_shm(TEEC_Context *ctx, TEEC_SharedMemory *shm)
{
...
static void _mtk_common_ca_teec_post_proc_tmpref(uint32_t param_type,
...
static void _mtk_common_ca_teec_post_proc_whole(TEEC_RegisteredMemoryReference *memref,
			struct tee_param *param)
{
...
static void _mtk_common_ca_teec_post_proc_partial(uint32_t param_type,
...
static void _mtk_common_ca_teec_post_proc_operation(TEEC_Operation *operation,
			...
			TEEC_SharedMemory *shms)
{
	...
	for (n = 0; n < TEEC_CONFIG_PAYLOAD_REF_COUNT; n++) {
...
#if (MULTI_INVOKE_TYPE_SUPPORT)
		case TEEC_MEMREF_WHOLE:
			_mtk_common_ca_teec_post_proc_whole(&operation->params[n].memref,
						params + n);
			break;
		case TEEC_MEMREF_PARTIAL_INPUT:
		case TEEC_MEMREF_PARTIAL_OUTPUT:
		case TEEC_MEMREF_PARTIAL_INOUT:
			_mtk_common_ca_teec_post_proc_partial(param_type,
		...
		case TEEC_MEMREF_TEMP_OUTPUT:
		case TEEC_MEMREF_TEMP_INOUT:
			_mtk_common_ca_teec_post_proc_tmpref(param_type,
...
static int _mtk_common_ca_teec_pre_proc_tmpref(TEEC_Context *ctx,
			...
			struct tee_param *param, TEEC_SharedMemory *shm)
{
	...
	ret = _mtk_common_ca_teec_alloc_shm(ctx, shm);
	if (ret) {
...
#if (MULTI_INVOKE_TYPE_SUPPORT)
static int _mtk_common_ca_teec_pre_proc_whole(
			TEEC_RegisteredMemoryReference *memref,
			struct tee_param *param)
{
...
static int _mtk_common_ca_teec_pre_proc_partial(uint32_t param_type,
			TEEC_RegisteredMemoryReference *memref,
			struct tee_param *param)
{
...
static int _mtk_common_ca_teec_pre_proc_operation(
			TEEC_Context *ctx,
			...
			TEEC_SharedMemory *shms)
{
	...
	for (n = 0; n < TEEC_CONFIG_PAYLOAD_REF_COUNT; n++) {
...
#if (MULTI_INVOKE_TYPE_SUPPORT)
		case TEEC_MEMREF_WHOLE:
			ret = _mtk_common_ca_teec_pre_proc_whole(
					&operation->params[n].memref, params + n);
			break;
		case TEEC_MEMREF_PARTIAL_INPUT:
		case TEEC_MEMREF_PARTIAL_OUTPUT:
		case TEEC_MEMREF_PARTIAL_INOUT:
			ret = _mtk_common_ca_teec_pre_proc_partial(param_type,
				&operation->params[n].memref, params + n);
			break;
#endif
		case TEEC_MEMREF_TEMP_INPUT:
		case TEEC_MEMREF_TEMP_OUTPUT:
		case TEEC_MEMREF_TEMP_INOUT:
			ret = _mtk_common_ca_teec_pre_proc_tmpref(ctx, param_type,
				&(operation->params[n].tmpref), params + n, shms + n);
			break;
		default:
			ret = -EINVAL;
...
{
	int otpee_version = _mtk_common_ca_get_optee_version(srccap_dev);
...
int mtk_common_ca_open_session(struct mtk_srccap_dev *srccap_dev,
...
{
	if (_mtk_common_ca_get_optee_version(srccap_dev) == MTK_OPTEE_VER_3) {
	...
	SRCCAP_MSG_ERROR("[SVP] Not support OPTEE version (%d)\n",
				_mtk_common_ca_get_optee_version(srccap_dev));
	return -EPERM;
}

int mtk_common_ca_teec_invoke_cmd(struct mtk_srccap_dev *srccap_dev,
			TEEC_Session *session, u32 cmd_id,
			TEEC_Operation *operation, u32 *error_origin)
{
	...
	if (otpee_version == MTK_OPTEE_VER_3) {
		...
		ret = _mtk_common_ca_teec_pre_proc_operation(&(session->ctx),
		...
		_mtk_common_ca_teec_post_proc_operation(operation, params, shm);

out_free_temp_refs:
		_mtk_common_ca_teec_free_temp_refs(operation, shm);
