...
#define VPS_DATA_1 0x0D
#define VPS_DATA_2 0x0E
#define VPS_DATA_3 0x05
#define VPS_DATA_4 0x0B
...
static int _vbi_g_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VBI_TTX_DATA:
		...
		memset(&phyPacketPointer, 0, sizeof(MS_PHY));
	...
	case V4L2_CID_VBI_VPS_ALL_DATA:
		...
		_u8VPSData[VPS_DATA_1 - 1] = (MS_U8)VPSdata1;
		_u8VPSData[VPS_DATA_2 - 1] = (MS_U8)VPSdata2;
		_u8VPSData[VPS_DATA_3 - 1] = (MS_U8)VPSdata3;
		_u8VPSData[VPS_DATA_4 - 1] = (MS_U8)VPSdata4;
...
static int _vbi_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VBI_INIT_TTX_SLICER:
		...
		memset((void *)pVBICCvirAddress,
			VBI_TTX_BUFFER_INIT, MTK_SRCCAP_VBI_TTX_BUFFER_LENGTH);
		SRCCAP_VBI_MSG_INFO
		    ("[V4L2] V4L2_CID_VBI_INIT_TTX_SLICER bufAddr=0x%llx\n",
...
void mtk_vbi_SetBufferAddr(void *Kernelvaddr, struct srccap_vbi_info vbi_info)
{
	SRCCAP_VBI_MSG_INFO("[STI][VBI] SetBufferAddr IN, vaddr = 0x%lx\n",
		(unsigned long)Kernelvaddr);

	pVBICCvirAddress = (unsigned long)Kernelvaddr;
	...
	SRCCAP_VBI_MSG_INFO("[VBI] vbiBufSize = 0x%lX\n", (unsigned long)vbiBufSize);
	SRCCAP_VBI_MSG_INFO("[VBI] vbiBufHeapId = %d\n", (int)vbiBufHeapId);
}
