...
enum V4L2_AVD_RESULT mtk_avd_InitHWinfo(struct mtk_srccap_dev *dev)
{
	...
	SRCCAP_AVD_MSG_INFO("Comb3dBufAddr = 0x%llX\n", (unsigned long long)Comb3dBufAddr);
	SRCCAP_AVD_MSG_INFO("Comb3dBufSize = 0x%lX\n", (unsigned long)Comb3dBufSize);
	SRCCAP_AVD_MSG_INFO("Comb3dBufHeapId = %d\n", (int)Comb3dBufHeapId);
...
enum V4L2_AVD_RESULT mtk_avd_vbi_CGMS_detect(bool *bDetect)
{
...
enum V4L2_AVD_RESULT mtk_avd_macro_vision_detect(bool *bDetect)
{
	...
	*bDetect = Api_AVD_GetMacroVisionDetect();
#endif
	SRCCAP_AVD_MSG_DEBUG("out\n");
...
{
	SRCCAP_AVD_MSG_INFO("in\n");
	SRCCAP_AVD_MSG_INFO("\t sizeof(init)={%zu}\n", sizeof(pParam));
	...
	if (Api_AVD_Init((struct VD_INITDATA *)&pParam, sizeof(pParam)) == 0) {
		SRCCAP_AVD_MSG_ERROR("\t sizeof(init)={%zu}\n", sizeof(pParam));
		show_v4l2_ext_avd_init_para(&pParam);
			...
			enum v4l2_ext_avd_freerun_freq *pFreerunFreq)
{
	...
	Api_AVD_SetFreerunFreq((int)*pFreerunFreq);
#endif
...
enum V4L2_AVD_RESULT mtk_avd_set_factory_para(
			struct v4l2_ext_avd_factory_data *pFactoryData)
{
	...
	Api_AVD_SetFactoryPara((int)pFactoryData->eFactoryPara,
...
enum V4L2_AVD_RESULT mtk_avd_get_factory_para(
			struct v4l2_ext_avd_factory_data *pFactoryData)
{
	SRCCAP_AVD_MSG_INFO("in\n");
	...
	Api_AVD_GetFactoryPara((int)pFactoryData->eFactoryPara,
					&(pFactoryData->u8Value));
#endif
...
enum V4L2_AVD_RESULT mtk_avd_75ire_setup_cancel(bool *bEnable)
{
	SRCCAP_AVD_MSG_INFO("in\n");
	...
	Api_AVD_75IRE_Setup_Cancel(*bEnable);
#endif
...
enum V4L2_AVD_RESULT mtk_avd_set_crop_screen_info(struct mtk_srccap_dev *srccap_dev,
	struct v4l2_ext_avd_crop_screen_info *pstCropScreen)
{
	SRCCAP_AVD_MSG_INFO("in\n");
	...
	if (pstCropScreen == NULL) {
		SRCCAP_AVD_MSG_POINTER_CHECK();
		return -EINVAL;
	}
	SRCCAP_AVD_MSG_INFO("[FV][cropscreen] X = [%d],Y = [%d], W = [%d], H = [%d]\n",
		pstCropScreen->cropScreen_x,
		pstCropScreen->cropScreen_y,
		pstCropScreen->cropScreen_width,
		pstCropScreen->cropScreen_height);

	srccap_dev->avd_info.fv_crop.CropWinX =
		(__u16)pstCropScreen->cropScreen_x;
	srccap_dev->avd_info.fv_crop.CropWinY =
		(__u16)pstCropScreen->cropScreen_y;
	srccap_dev->avd_info.fv_crop.CropWinWidth =
		(__u16)pstCropScreen->cropScreen_width;
	srccap_dev->avd_info.fv_crop.CropWinHeight =
		(__u16)pstCropScreen->cropScreen_height;
...
enum V4L2_AVD_RESULT mtk_avd_get_crop_screen_info(struct mtk_srccap_dev *srccap_dev,
	struct v4l2_ext_avd_crop_screen_info *pstCropScreen)
{
	SRCCAP_AVD_MSG_INFO("in\n");
