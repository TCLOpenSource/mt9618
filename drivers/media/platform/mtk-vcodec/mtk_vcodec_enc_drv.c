					...
					unsigned long action, void *data)
{
	...
	struct mtk_vcodec_dev *dev =
		container_of(nb, struct mtk_vcodec_dev, enc_pm_notifier);
...
#endif
	dev->enc_pm_notifier.notifier_call = mtk_vcodec_enc_suspend_notifier;
	register_pm_notifier(&dev->enc_pm_notifier);
	dev->is_codec_suspending = 0;
