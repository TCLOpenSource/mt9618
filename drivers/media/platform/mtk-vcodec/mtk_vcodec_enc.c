...
#define TIME_SEC_TO_MICRO_SEC (1000000)
#define TIME_NANO_SEC_TO_MICRO_SEC (1000)

#ifdef TV_ENCODE_INTEGRATION
#define MTK_VENC_TSVC_MAX_LAYER (2)
#endif
...
// TODO: Change to original flow in M6.
	...
	case V4L2_CID_MPEG_MTK_COLOR_DESC:
		mtk_v4l2_debug(2,
			...
			ctrl->val);
		memcpy(&p->color_desc, ctrl->p_new.p,
	...
	case V4L2_CID_MPEG_MTK_VIDEO_QUALITY:
		...
		break;
#ifdef TV_ENCODE_INTEGRATION
	case V4L2_CID_MPEG_MTK_HIER_P_LAYER_COUNT:
		mtk_v4l2_debug(2,
			"V4L2_CID_MPEG_MTK_HIER_P_LAYER_COUNT: 0x%x",
			ctrl->val);
		p->hier_p_layer_count = ctrl->val;
		break;
#endif
#ifndef TV_ENCODE_INTEGRATION
...
#ifdef TV_ENCODE_INTEGRATION
	param->hier_p_layer_count = enc_params->hier_p_layer_count;
#endif
}
			    ...
			    struct v4l2_buffer *buf)
{
	...
	ktime_get_real_ts64(&p->frmencodestart);
...
static void mtk_venc_worker(struct work_struct *work)
{
	...
	ktime_get_real_ts64(&p->frmencodeend);
	mtk_v4l2_debug(1, "<=== time[%lld us] src_buf[%d] dst_buf[%d] venc_if_encode ret=%d Size=%u===>",
		((p->frmencodeend.tv_sec - p->frmencodestart.tv_sec) * TIME_SEC_TO_MICRO_SEC +
		(p->frmencodeend.tv_nsec - p->frmencodestart.tv_nsec) / TIME_NANO_SEC_TO_MICRO_SEC),
...
int mtk_vcodec_enc_ctrls_setup(struct mtk_vcodec_ctx *ctx)
{
...
#ifdef TV_ENCODE_INTEGRATION
	memset(&cfg, 0, sizeof(cfg));
	cfg.id = V4L2_CID_MPEG_MTK_HIER_P_LAYER_COUNT;
	cfg.type = V4L2_CTRL_TYPE_INTEGER;
	cfg.flags = V4L2_CTRL_FLAG_WRITE_ONLY;
	cfg.name = "Video encode hier p layer count";
	cfg.min = 0;
	cfg.max = MTK_VENC_TSVC_MAX_LAYER;
	cfg.step = 1;
	cfg.def = 0;
	cfg.ops = ops;
	v4l2_ctrl_new_custom(handler, &cfg, NULL);
	...
	cfg.type = V4L2_CTRL_TYPE_INTEGER;
	cfg.flags = V4L2_CTRL_FLAG_VOLATILE;
	cfg.name = "Video encode average QP";
	cfg.min = 0;
	cfg.max = 2048;
	cfg.step = 1;
	cfg.def = 0;
