...
#define OSTL_LARB 10
#define OSTL_PORT 1
#define OSTL_LARB_210 2
#define OSTL_PORT_210 4
#define OSTL_LARB_21C 3
#define OSTL_PORT_21C 7
#define OSTL_LARB_220 3
#define OSTL_PORT_220 8
...
static bool vcu_msg_with_hw_id(struct vdec_vcu_ipi_ack *msg)
{
	...
	case VCU_IPIMSG_DEC_SMI_LARB_OSTDL_210:
	case VCU_IPIMSG_DEC_SMI_LARB_OSTDL_21C:
	case VCU_IPIMSG_DEC_SMI_LARB_OSTDL_220:
		return true;
	default:
		return false;
	}
...
int vcu_dec_ipi_handler(void *data, unsigned int len, void *priv)
{
	...
	if (msg->msg_id == VCU_IPIMSG_DEC_WAITISR) {
		...
		case VCU_IPIMSG_DEC_SMI_LARB_OSTDL: {
			int smi_larb_value = msg->status;

			if (vcu->ctx->dv_ostl_larb == NULL)
				vcu->ctx->dv_ostl_larb = mtk_smi_larb_find_node(OSTL_LARB);

			mtk_smi_larb_node_ostdl(vcu->ctx->dv_ostl_larb, OSTL_LARB,
				OSTL_PORT, smi_larb_value);
			ret = 1;
			msg->status = 0;
			break;
		}
		case VCU_IPIMSG_DEC_SMI_LARB_OSTDL_210: {
			...
			if (vcu->ctx->dv_ostl_larb == NULL)
				vcu->ctx->dv_ostl_larb = mtk_smi_larb_find_node(OSTL_LARB_210);

			mtk_smi_larb_node_ostdl(vcu->ctx->dv_ostl_larb, OSTL_LARB_210,
				OSTL_PORT_210, smi_larb_value);
			ret = 1;
			msg->status = 0;
			break;
		}
		case VCU_IPIMSG_DEC_SMI_LARB_OSTDL_21C: {
			...
			if (vcu->ctx->dv_ostl_larb == NULL)
				vcu->ctx->dv_ostl_larb = mtk_smi_larb_find_node(OSTL_LARB_21C);

			mtk_smi_larb_node_ostdl(vcu->ctx->dv_ostl_larb, OSTL_LARB_21C,
				OSTL_PORT_21C, smi_larb_value);
			ret = 1;
			msg->status = 0;
			break;
		}
		case VCU_IPIMSG_DEC_SMI_LARB_OSTDL_220: {
			...
			if (vcu->ctx->dv_ostl_larb == NULL)
				vcu->ctx->dv_ostl_larb = mtk_smi_larb_find_node(OSTL_LARB_220);

			mtk_smi_larb_node_ostdl(vcu->ctx->dv_ostl_larb, OSTL_LARB_220,
				OSTL_PORT_220, smi_larb_value);
			ret = 1;
			msg->status = 0;
			break;
		}
...
{
	if (vcu->ctx->dv_ostl_larb != NULL)
		of_node_put(vcu->ctx->dv_ostl_larb);
