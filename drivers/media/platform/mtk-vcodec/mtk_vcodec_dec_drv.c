					...
					unsigned long action, void *data)
{
	...
	struct mtk_vcodec_dev *dev =
		container_of(nb, struct mtk_vcodec_dev, dec_pm_notifier);
...
static int mtk_vcodec_dec_probe(struct platform_device *pdev)
{
...
#ifndef TV_INTEGRATION // temp solution
	...
	spin_lock_init(&dev->dec_counter_lock);
	...
	dev->dec_pm_notifier.notifier_call = mtk_vcodec_dec_suspend_notifier;
	register_pm_notifier(&dev->dec_pm_notifier);
	dev->is_codec_suspending = 0;
