...
#define RETRY_CNT		1UL  //AN also retry once. Totally send 1(origin)+3(retry) times
...
#define TRANSMIT_ATTEMPTS_MAX			7
#define TRANSMIT_RETRIES_MAX_MS			400
#define TRANSMIT_RETRIES_MONITOR_PERIOD_MS	10
#define TRANSMIT_RETRIES_PER_MS	        25  // 400ms/16
...
void mtk_cec_enable(struct mtk_cec_dev *pdev)
{
	...
	W2BYTEMSK(pdev->reg_base + REG_0008_PM_CEC, (pdev->xtal/100000UL)<<8, BMASK(15:8));
	...
	W2BYTEMSK(pdev->reg_base + REG_0048_PM_CEC, 0x1F00, BMASK(12:8));
	W2BYTEMSK(pdev->reg_base + REG_0048_PM_CEC, 0x0000, BMASK(12:8));

	// CEC_13[7:0]: CEC irq mask control
	W2BYTEMSK(pdev->reg_base + REG_004C_PM_CEC, 0x00, BMASK(7:0));
...
void mtk_cec_factory_pin_enable_store(struct mtk_cec_dev *pdev, bool enable)
{
	// CEC_03
	if (enable)
		W2BYTEMSK(pdev->reg_base + REG_000C_PM_CEC, 0x0400, BIT(10));
	else
		W2BYTEMSK(pdev->reg_base + REG_000C_PM_CEC, 0, BIT(10));
	dev_dbg(pdev->dev, "[%s] CEC factory pin enable = %X\n", __func__, enable);
}
...
bool mtk_cec_sendframe(struct mtk_cec_dev *pdev, u8 *data, u8 len, u8 attempts)
{
	u32 i = 0, count = 0;
	u8 opcode = 0, header = 0;
	u8 retries = 0, max_retries = 0;

	mtk_cec_clear_tx_event_status(pdev);

	header = data[0];
	opcode = data[1];

	// set header and opcode. CEC_18[7:0]: reg_tx_data0
	...
	dev_info(pdev->dev, "[%s] len = %d\n", __func__, len);
	dev_info(pdev->dev, "[%s] header = %x\n", __func__, header);
	dev_info(pdev->dev, "[%s] opcode = %x\n", __func__, opcode);
	...
	if (attempts > TRANSMIT_ATTEMPTS_MAX)
		attempts = TRANSMIT_ATTEMPTS_MAX;  //max attempts is 7

	retries = 0;
	// max_retries = (25ms * len * attempts)/10ms
	max_retries = (TRANSMIT_RETRIES_PER_MS * len * attempts)/TRANSMIT_RETRIES_MONITOR_PERIOD_MS;

	while ((R2BYTE(pdev->reg_base + REG_0014_PM_CEC) != 0x0001)) {
		dev_dbg(pdev->dev, "CEC busy!attempt %d\r\n", attempts);

		msleep(TRANSMIT_RETRIES_MONITOR_PERIOD_MS);
		retries += 1;

		if (retries == max_retries) {
			dev_info(pdev->dev, "cec busy! max_retries: %d ms.\r\n", max_retries*TRANSMIT_RETRIES_MONITOR_PERIOD_MS);
			return false;
		}
	...
	if ((opcode == 0x00) && (len == 1)) {
		pdev->retrycnt = 0;
		W2BYTEMSK(pdev->reg_base + REG_0000_PM_CEC,
							(0x10 | pdev->retrycnt)<<8, BMASK(15:8));
...
u16 mtk_cec_get_event_status(struct mtk_cec_dev *pdev)
{
	u16 ret = 0;

	// CEC_11[14:8]: reg_cec_event_int
	ret = R2BYTE(pdev->reg_base + REG_0044_PM_CEC);
	ret = ret >> 8;
	return ret;
}
...
void mtk_cec_clear_tx_event_status(struct mtk_cec_dev *pdev)
{
	// CEC_12[15:8]: CEC irq clear
	W2BYTEMSK(pdev->reg_base + REG_0048_PM_CEC, 0x1E00, BMASK(12:8));
	W2BYTEMSK(pdev->reg_base + REG_0048_PM_CEC, 0x0000, BMASK(12:8));
}

void mtk_cec_clear_rx_event_status(struct mtk_cec_dev *pdev)
{
	// CEC_12[15:8]: CEC irq clear
	W2BYTEMSK(pdev->reg_base + REG_0048_PM_CEC, 0x1100, BMASK(12:8));
	W2BYTEMSK(pdev->reg_base + REG_0048_PM_CEC, 0x0000, BMASK(12:8));
...
void mtk_cec_config_wakeup(struct mtk_cec_dev *pdev)
{
	...
	//clear tx length and data0/data1
	W2BYTEMSK(pdev->reg_base +REG_0060_PM_CEC, 0x00, BMASK(15:0));
	W2BYTEMSK(pdev->reg_base +REG_0000_PM_CEC, 0x00, BMASK(3:0));
	...
	// write Vendor ID from low address to high address
	vendor_id_0 = (u16)(((pdev->adap->log_addrs.vendor_id >> 16) & 0xFF) |
					(pdev->adap->log_addrs.vendor_id & 0xFF00));
	vendor_id_1 = (u8)(pdev->adap->log_addrs.vendor_id & 0xFF);
