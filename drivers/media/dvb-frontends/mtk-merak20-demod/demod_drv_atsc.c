...
if (ioctl(piodata, DMD_ATSC_HAL_CMD_VSB_FEC_LOCK, NULL)) {
	...
	if (pinfo->atsclockstatus & DMD_ATSC_LOCK_VSB_PRE_LOCK) {
		...
		DMD_ATSC_HAL_CMD_VSB_PRELOCK, NULL)) {
			...
			#if DMD_ATSC_UTOPIA_EN || DMD_ATSC_UTOPIA2_EN
			if (((MsOS_GetSystemTime()-
			pinfo->atscscantimestart) < timeout) && ioctl(piodata,
...
#endif
{
	...
	pres->sdmd_atsc_pridata.virt_dram_base_addr =
	pres->sdmd_atsc_initdata.virt_dram_base_addr;

	pres->sdmd_atsc_pridata.utdistartaddr32 =
	pres->sdmd_atsc_initdata.utdistartaddr32;

	pres->sdmd_atsc_pridata.virt_mppl_base_addr =
	pres->sdmd_atsc_initdata.virt_mppl_base_addr;

	pres->sdmd_atsc_pridata.virt_efuse_base_addr =
	pres->sdmd_atsc_initdata.virt_efuse_base_addr;

	pres->sdmd_atsc_pridata.virt_demod_base_addr =
	pres->sdmd_atsc_initdata.virt_demod_base_addr;
			...
			struct dtv_frontend_properties *p)
{
	...
	ret_tmp = _mdrv_dmd_atsc_md_getreg(0, 0x20C5, &data2);
	ret_tmp = _mdrv_dmd_atsc_md_getreg(0, 0x20C6, &data3);
...
ssize_t atsc_get_information(char *buf)
{
	...
	"%s%s%s%s %s%s%d%s %s%s%d%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%s%s %s%s%s%s %s%s%s%s %s%s%s%s %s%s%d%s %s%s%d%s%d%s %s%s%d%s",
	...
	SM_BER_BEFORE_RS, SM_COMMA, postber, SM_END,
