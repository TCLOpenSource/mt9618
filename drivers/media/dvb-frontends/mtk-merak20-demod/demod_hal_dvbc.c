...
#define NUMBER_OF_QAM 5
#define NUMBER_OF_LOCK 2
#define MAX_STRING_SIZE 10

#define NO_CHANNEL_OFFSET 0xC3
#define NO_CHANNEL_FLAG 0x04

char qam_arr[NUMBER_OF_QAM][MAX_STRING_SIZE] = {
	"16QAM",
	"32QAM",
	"64QAM",
	"128QAM",
...
char lock_arr[NUMBER_OF_LOCK][MAX_STRING_SIZE] = {
	"UNLOCK",
	"LOCK"
};
...
u8 dvbc_table_MT5873[] = {
    #include "fw_dmd_dvbc_MT5873.dat"
};
...
static int intern_dvbc_get_ssi(u16 *ssi, int rf_power_dbm)
{
	u8 QAM;
	s32 s32tmp = 0;
	s16 ch_power_db = 0;
...
int dmd_hal_dvbc_load_code(void)
{
	...
	if ((ip_version & 0xff00) == 0x0500) {
		memcpy(dram_code_virt_addr, &dvbc_table_MT5873[0],
				sizeof(dvbc_table_MT5873));
	} else {
 ...
 */
	...
	intern_dvbc_get_ssi(&ssi, signal_power_level);
	stats->len = 2;
		...
		*status = FE_HAS_SIGNAL | FE_HAS_CARRIER;
		...
		if (tr_lock == 1) {
			...
			if ((mtk_demod_get_time() - scan_start_time)
				> DMD_DVBC_TR_TIMEOUT) {
				*status = FE_TIMEDOUT;
				pr_notice(">>>>>>>> DVBC no channel !!!\n");
			} else {
				*status = 0;
			}

			ret |= mtk_demod_mbx_dvb_read(TOP_REG_BASE + NO_CHANNEL_OFFSET, &reg);
			if ((reg & NO_CHANNEL_FLAG) == NO_CHANNEL_FLAG) {
				pr_notice(">>>>>>>> No DVBC signal detected!!!\n");
				*status = FE_TIMEDOUT;
			}
...
ssize_t demod_hal_dvbc_get_information(struct device_driver *driver, char *buf)
{
	...
	u16 demod_rev = 0, cnr = 0, block_error = 0, quality = 0, ssi = 0;
	...
	//fix array index out of range bug, index should be 0~4.
	switch (reg) {
	case 0:
	case 1:
	case 2:
	...
	case 4:
		...
		break;
	default:
		modulation = QAM_256 - 1; //out of range, should not happen
		break;
	}
	...
	// total 17 items (SM 11, SD 8, Common 3)
	pr_info("Chip Rev.(Demod) = 0x%x\n", demod_rev);
	...
	pr_info("BER before RS = %d\n", post_ber);
	pr_info("UEC (Accumulate) = %d\n", pkterr_accu);
	pr_info("UEC (Instant) = %d\n", block_error);
	pr_info("Modulation = %s\n", qam_arr[modulation]);
	pr_info("TS Rate = %d\n", ts_rate);
	pr_info("SQI = %d\n", quality);
	pr_info("Signal Strength = %d\n", ssi);

	return scnprintf(buf, PAGE_SIZE,
		"%s%s%s%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%s%s %s%s%s%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%s%s %s%s%d%s %s%s%d%s %s%s%d%s",
		SM_CHIP_REV, SM_COMMA, SM_NONE, SM_END,
		SM_FREQUENCY, SM_COMMA, Cur_freq, SM_END,
		SM_FREQ_OFFSET, SM_COMMA, frequency_offset, SM_END,
		SM_AGC, SM_COMMA, agc_val, SM_END, // SD_SM
		SM_DEMOD_LOCK, SM_COMMA, lock_arr[fec_lock], SM_END,
		SM_TS_LOCK, SM_COMMA, lock_arr[fec_lock], SM_END,
		...
		SM_MODULATION, SM_COMMA, qam_arr[modulation], SM_END, //SD
