...
#endif
{
	...
	pres->sdmd_isdbt_pridata.virt_mpll_base_addr =
	pres->sdmd_isdbt_initdata.virt_mpll_base_addr;

	pres->sdmd_isdbt_pridata.virt_efuse_base_addr =
	pres->sdmd_isdbt_initdata.virt_efuse_base_addr;

	pres->sdmd_isdbt_pridata.virt_demod_base_addr =
	pres->sdmd_isdbt_initdata.virt_demod_base_addr;
	#endif
...
"\033[35mFunction = %s, Line = %d, get heap_miu_start_offset is 0x%llX\033[m\n",
		...
		DMD_DBG(PRINT(
		"#######pRes->sDMD_ISDBT_InitData.u32TdiStartAddr = [0x%llx]\n",
		(pres->sdmd_isdbt_initdata.tdi_start_addr)));
	}
	#else
...
ssize_t isdbt_get_information(char *buf)
{
	...
	memset(&isdbt_cfo_data, 0, sizeof(struct dmd_isdbt_cfo_data));
	memset(&isdbt_snr, 0, sizeof(struct dmd_isdbt_snr_data));
	memset(&sisdbt_modulation_mode, 0, sizeof(struct isdbt_modulation_mode));
	memset(&sisdbt_get_ber_value, 0, sizeof(struct dmd_isdbt_get_ber_value));
...
(sisdbt_get_ber_value.bervalue*5194) / (sisdbt_get_ber_value.berperiod*100);
	...
	return scnprintf(buf, PAGE_SIZE,
		"%s%s%x%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s",
		...
		SM_UEC, SM_COMMA, packet_err, SM_END,
		SM_TMCC_DET, SM_COMMA, isdbt_tmcc_det_flag, SM_END,
		SM_MODE, SM_COMMA, transmission_mode, SM_END,
		SM_GUARD, SM_COMMA, guard_interval, SM_END,
		SM_LAYER_A_MODULATION, SM_COMMA, layerA_modulation, SM_END,
		SM_LAYER_A_CODERATE, SM_COMMA, layerA_fec, SM_END,
		SM_LAYER_A_INTERLEAVE, SM_COMMA, layerA_interleaving, SM_END,
		SM_LAYER_A_SEGMENT, SM_COMMA, layerA_segment_count, SM_END,
		SM_LAYER_A_BER_BEFORE_RS, SM_COMMA, layerA_post_ber, SM_END,
		SM_LAYER_B_MODULATION, SM_COMMA, layerB_modulation, SM_END,
		SM_LAYER_B_CODERATE, SM_COMMA, layerB_fec, SM_END,
		SM_LAYER_B_INTERLEAVE, SM_COMMA, layerB_interleaving, SM_END,
		SM_LAYER_B_SEGMENT, SM_COMMA, layerB_segment_count, SM_END,
		SM_LAYER_B_BER_BEFORE_RS, SM_COMMA, layerB_post_ber, SM_END,
		SM_LAYER_C_MODULATION, SM_COMMA, layerC_modulation, SM_END,
		SM_LAYER_C_CODERATE, SM_COMMA, layerC_fec, SM_END,
		SM_LAYER_C_INTERLEAVE, SM_COMMA, layerC_interleaving, SM_END,
		SM_LAYER_C_SEGMENT, SM_COMMA, layerC_segment_count, SM_END,
		SM_LAYER_C_BER_BEFORE_RS, SM_COMMA, layerC_post_ber, SM_END,
		SM_EMERGENCY_FLAG, SM_COMMA, isdbt_ewbs_flag, SM_END,
		SM_LOCK_STATUS, SM_COMMA, isdbt_lock_flag, SM_END,
		SM_LOCK_STATUS, SM_COMMA, isdbt_ts_lock_flag, SM_END);
}
...
struct dvb_frontend *fe, struct dtv_frontend_properties *p)
{
	...
	pr_info("[%s][%d][127:64 - 63:0]:0x%llx 0x%llx\n", __func__, __LINE__, ((u64*)(p))[1], ((u64*)(p))[0]);
	for(i= 0; i < 64; i++)
	{
		// command 0 ~ 63
		if(((u64)1 << i) & ((u64*)(p))[0])
		{
			pr_info("[%s][%d] cmd = %3d [063-000] \n", __func__, __LINE__, i);
			switch(i) {
			...
			case DTV_ISDBT_LAYERC_MODULATION:
				ret = _mdrv_dmd_isdbt_md_get_modulation_mode(0, E_ISDBT_Layer_C, &sisdbt_modulation_mode);
				switch (sisdbt_modulation_mode.eisdbttdi) {
				case E_ISDBT_2K_TDI_0:
				case E_ISDBT_4K_TDI_0:
				case E_ISDBT_8K_TDI_0:
			...
			case DTV_ISDBT_LAYERB_SEGMENT_COUNT:
				...
				break;
				pr_info("[%s][%d] DTV_ISDBT_LAYERB_SEGMENT_COUNT: %d\n", __func__, __LINE__, p->layer[1].segment_count);
			case DTV_ISDBT_LAYERB_MODULATION: // eisdbtconstellation
			case DTV_ISDBT_LAYERB_FEC: // eisdbtcoderate
			case DTV_ISDBT_LAYERB_TIME_INTERLEAVING: // eisdbttdi
				ret = _mdrv_dmd_isdbt_md_get_modulation_mode(0, E_ISDBT_Layer_B, &sisdbt_modulation_mode);
				switch (sisdbt_modulation_mode.eisdbttdi) {
				case E_ISDBT_2K_TDI_0:
				case E_ISDBT_4K_TDI_0:
				case E_ISDBT_8K_TDI_0:
			...
			case DTV_ISDBT_LAYERA_SEGMENT_COUNT:
				...
				if ((u8temp != 0x0F) && (u8temp != 0x00))
					...
					break;
				pr_info("[%s][%d] DTV_ISDBT_LAYERA_SEGMENT_COUNT: %d\n", __func__, __LINE__, p->layer[0].segment_count);
			case DTV_ISDBT_PARTIAL_RECEPTION:
				ret = _mdrv_dmd_isdbt_md_getreg(0, 0x1508, &u8temp);
				p->isdbt_partial_reception = ((u8temp&0x02) == 0x02) ? 1:0;
				break;
			case DTV_INNER_FEC: // eisdbtcoderate
			case DTV_TRANSMISSION_MODE: // eisdbtfft
			case DTV_GUARD_INTERVAL: // eisdbtgi
			case DTV_ISDBT_LAYERA_TIME_INTERLEAVING: // eisdbttdi
			case DTV_ISDBT_LAYERA_MODULATION: // eisdbtconstellation
			case DTV_ISDBT_LAYERA_FEC: // eisdbtcoderate
			case DTV_MODULATION: // eisdbtconstellation
				ret = _mdrv_dmd_isdbt_md_get_modulation_mode(0, E_ISDBT_Layer_A, &sisdbt_modulation_mode);
				switch (sisdbt_modulation_mode.eisdbttdi) {
				case E_ISDBT_2K_TDI_0:
				case E_ISDBT_4K_TDI_0:
				case E_ISDBT_8K_TDI_0:
			...
			case DTV_STAT_CNR:
				...
				pr_info("[%s][%d] DTV_STAT_CNR: %d\n", __func__, __LINE__, isdbt_stats->stat[0].svalue);
				break;
			}
		}
		// command 64 - 127
		if(((u64)1 << i) & ((u64*)(p))[1])
		{
			pr_info("[%s][%d] cmd = %2d [127-064]\n", __func__, __LINE__, i + 64);
			switch((i+64)) {
			case DTV_STAT_SIGNAL_QUALITY:
				/* SQI */
				_mdrv_dmd_isdbt_md_getreg(0, 0x150A, &u8temp);
				u8temp &= 0x0F;
				layerA_segment_count = _isdbt_get_information_segment_count(u8temp);
				...
				pr_info("[%s][%d] DTV_STAT_SIGNAL_QUALITY: %d\n", __func__, __LINE__, sqi);
				...
				break;
			case DTV_ISDBT_LAYERC_POST_BER:
				sisdbt_get_ber_value.eisdbtlayer = E_ISDBT_Layer_C;
				ret = _mdrv_dmd_isdbt_md_get_postviterbi_ber(0, &sisdbt_get_ber_value);
				p->layer[2].post_ber = (sisdbt_get_ber_value.bervalue*BER_ERR_CNT_R) / (sisdbt_get_ber_value.berperiod*BER_ERR_WIN_R);
				break;
			case DTV_POST_BER:
			case DTV_ISDBT_LAYERA_POST_BER:
				sisdbt_get_ber_value.eisdbtlayer = E_ISDBT_Layer_A;
				ret = _mdrv_dmd_isdbt_md_get_postviterbi_ber(0, &sisdbt_get_ber_value);
				p->layer[0].post_ber = (sisdbt_get_ber_value.bervalue*BER_ERR_CNT_R) / (sisdbt_get_ber_value.berperiod*BER_ERR_WIN_R);
				// Temp : LayerA_BER
				p->post_ber = p->layer[0].post_ber;
				break;
			case DTV_STAT_POST_ERROR_BIT_COUNT:// post_bit_error
			case DTV_STAT_POST_TOTAL_BIT_COUNT: // post_bit_count
				sisdbt_get_ber_value.eisdbtlayer = E_ISDBT_Layer_A;
				ret = _mdrv_dmd_isdbt_md_get_postviterbi_ber(0, &sisdbt_get_ber_value);
			...
			case DTV_STAT_ERROR_BLOCK_COUNT:
				// UEC : Get Packet Error
				ret = _mdrv_dmd_isdbt_md_read_pkt_err(0, E_ISDBT_Layer_A, &packet_err);
				isdbt_stats2 = &p->block_error;
				isdbt_stats2->len = 1;
				isdbt_stats2->stat[0].uvalue = packet_err;
				isdbt_stats2->stat[0].scale = FE_SCALE_COUNTER;
				break;
			case DTV_FREQUENCY_OFFSET:
				// Get freq offset
				...
				pr_info("[%s][%d] DTV_FREQUENCY_OFFSET:%d Hz\n", __func__, __LINE__, p->frequency_offset);
				break;
			case DTV_AGC:
				...
				p->agc_val = ifagc_val;
				break;
			case DTV_ISDBT_EWBS_FLAG:
				if (_mdrv_dmd_isdbt_md_getlock(0, DMD_ISDBT_GETLOCK_EWBS_LOCK) == DMD_ISDBT_LOCK)
					p->isdbt_ewbs_flag = 1;
				else
					p->isdbt_ewbs_flag = 0;
                                pr_info("[%s][%d] DTV_ISDBT_EWBS_FLAG:%d \n", __func__, __LINE__, p->isdbt_ewbs_flag);
				break;
			case DTV_ISDBT_LAYERB_POST_BER:
				sisdbt_get_ber_value.eisdbtlayer = E_ISDBT_Layer_B;
				ret = _mdrv_dmd_isdbt_md_get_postviterbi_ber(0, &sisdbt_get_ber_value);
				p->layer[1].post_ber = (sisdbt_get_ber_value.bervalue*BER_ERR_CNT_R) / (sisdbt_get_ber_value.berperiod*BER_ERR_WIN_R);
				break;
			}
		}
	}
