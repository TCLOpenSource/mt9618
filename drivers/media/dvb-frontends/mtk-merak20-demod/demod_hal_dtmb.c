...
const u8 INTERN_MT5879_DTMB_table[] = {
	#include	"dmd_mt5879_intern_dtmb.dat"
};

const u8 INTERN_MT5873_DTMB_table[] = {
	#include	"dmd_mt5873_intern_dtmb.dat"
};
...
u16 demod_dtmb_top[0x80] = { // 0x20, for check patch column100
...
u16 demod_dtmb_top_ext[0x80] = { // 0x121
...
u16 demod_dtmb_clkgen[0x80] = { // 0x35
	0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
	0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
	0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
	0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
...
u16 demod_dtmb_clkgen_ext[0x80] = { // 0x122
...
u16 demod_dtmb_sram_share[0x80] = { // 0x126
	0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
	0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
	0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
	0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
...
u16 demod_dtmb_dvbs2[0x80] = { // 0x3a
...
static void _hal_intern_dtmb_initclk(void)
{
	...
	if (pRes->sDMD_DTMB_PriData.m6_version < MT5876_IP_VER) {//5896/5897
...
///pr_emerg("\npRes->sDMD_DTMB_PriData.m6_version=0x%x\n",pRes->sDMD_DTMB_PriData.m6_version);
	if (pRes->sDMD_DTMB_PriData.m6_version < MT5897_IP_VER) {
	...
	else if (pRes->sDMD_DTMB_PriData.m6_version < MT5876_IP_VER)	{
		...
		switch (pRes->sDMD_DTMB_PriData.m6_version) {
		case MT5876_IP_VER:
			HAL_INTERN_DTMB_DBINFO(PRINT("------DMD_DTMB_CHIP_MT5876------\n"));
			break;
		case MT5879_IP_VER:
			HAL_INTERN_DTMB_DBINFO(PRINT("------DMD_DTMB_CHIP_MT5879------\n"));
			break;
		case MT5873_IP_VER:
			HAL_INTERN_DTMB_DBINFO(PRINT("------DMD_DTMB_CHIP_MT5873------\n"));
			break;
		}

		if (pRes->sDMD_DTMB_PriData.m6_version == MT5879_IP_VER) {
			temp1 = readl(pRes->sDMD_DTMB_PriData.virt_clk_base_addr + 0x1F84)
			& 0xFFFFFEEF;
			...
			for (i = 0; i < 0x80; i++) {
				writew(demod_dtmb_top[i], pRes->sDMD_DTMB_PriData.virtDMDBaseAddr
				+ 0x2000 + i*4); //demod_top
				writew(demod_dtmb_top_ext[i], //demod_top_ext
				pRes->sDMD_DTMB_PriData.virtDMDBaseAddr + 0x24200 + i*4);
				writew(demod_dtmb_clkgen[i], pRes->sDMD_DTMB_PriData.virtDMDBaseAddr
				+ 0x6A00 + i*4); //demod_clkgen
				writew(demod_dtmb_clkgen_ext[i], //demod_clkgen_ext
				pRes->sDMD_DTMB_PriData.virtDMDBaseAddr + 0x24400 + i*4);
				writew(demod_dtmb_sram_share[i], //demod_sram_share
				pRes->sDMD_DTMB_PriData.virtDMDBaseAddr + 0x24C00 + i*4);
				writew(demod_dtmb_ana[i], pRes->sDMD_DTMB_PriData.virtDMDBaseAddr
				+ 0x5C00 + i*4); //demod_ana
				writew(demod_dtmb_dvbs2[i], pRes->sDMD_DTMB_PriData.virtDMDBaseAddr
		...
		if (pRes->sDMD_DTMB_PriData.m6_version >= MT5879_IP_VER) {
			...
			writew(temp, pRes->sDMD_DTMB_PriData.virt_efuse_base_addr + (0x28<<2));
			temp = readw(pRes->sDMD_DTMB_PriData.virt_efuse_base_addr + (0x2d<<2));
			temp1 = (temp<<16);
			temp = readw(pRes->sDMD_DTMB_PriData.virt_efuse_base_addr + (0x2c<<2));
			temp1 |= temp;
			check_val = (temp1>>15) & 0x01;
			if (check_val != 0) {
				reg = mtk_demod_read_byte(TOPRegBase + 0x01*2 + 1);// release bit13
				reg &= (~0x20);
				mtk_demod_write_byte(TOPRegBase + 0x01*2 + 1, reg);
				reg = mtk_demod_read_byte(ANA_REG_BASE + 0x47*2);
				reg &= ~(0x1f);
				reg |= (u8)((temp1>>8)&0x1f);
				mtk_demod_write_byte(ANA_REG_BASE + 0x47*2, reg);
			}
		}

		if (pRes->sDMD_DTMB_PriData.m6_version < MT5873_IP_VER) {
			reg = readb(pRes->sDMD_DTMB_PriData.virt_sram_base_addr);
			reg &= 0x00;
			writeb(reg, pRes->sDMD_DTMB_PriData.virt_sram_base_addr);
		}
	...
	#else
	if (pRes->sDMD_DTMB_PriData.m6_version >= MT5876_IP_VER)
	...
	if (pRes->sDMD_DTMB_PriData.m6_version == MT5879_IP_VER) {
		temp1 = readl(pRes->sDMD_DTMB_PriData.virt_mpll_base_addr + 0xC8) & 0xFFFF00FC;
		temp1 |= 0x00001100;
		writel(temp1, (pRes->sDMD_DTMB_PriData.virt_mpll_base_addr + 0xC8));
		temp1 = readl(pRes->sDMD_DTMB_PriData.virt_mpll_base_addr + 0x154) & 0xFFFFFEFF;
		writel(temp1, (pRes->sDMD_DTMB_PriData.virt_mpll_base_addr + 0x154));
	} else if (pRes->sDMD_DTMB_PriData.m6_version == MT5873_IP_VER) {
		temp1 = readl(pRes->sDMD_DTMB_PriData.virt_demod_base_addr + 0x04) & 0xFFFFFFFE;
		writel(temp1, (pRes->sDMD_DTMB_PriData.virt_demod_base_addr + 0x04));
	}
	...
	while (_hal_dmd_riu_readbyte(DMDMcuRstState+0x04) != 0xf) {
		...
		HAL_INTERN_DTMB_DBINFO(PRINT(">MCU is ready!\n\r"));
...
//	#endif
	...
	if (pRes->sDMD_DTMB_PriData.m6_version == MT5873_IP_VER) {
		DTMB_table = &INTERN_MT5873_DTMB_table[0];
		ulib_size16 = sizeof(INTERN_MT5873_DTMB_table);
	} else if (pRes->sDMD_DTMB_PriData.m6_version == MT5879_IP_VER) {
		DTMB_table = &INTERN_MT5879_DTMB_table[0];
		ulib_size16 = sizeof(INTERN_MT5879_DTMB_table);
	} else {
...
//	#endif
	...
	if (pRes->sDMD_DTMB_PriData.bDownloaded) {
		for (i = 0; i < 1024; i++) {
			...
			if (udata != DTMB_table[i]) {
				HAL_INTERN_DTMB_DBINFO(PRINT(">DSP code not exist!"));
	...
	_hal_dmd_riu_writebyte(DMDMcuBase+0x0C, utmpdata8); // write data to VD MCU 51 code sram
	...
	_hal_dmd_riu_writebyte(DMDMcuBase+0x03, 0x50); // disable auto-increase
...
//	#endif
	...
	HAL_INTERN_DTMB_DBINFO(PRINT(">>DTMB_FW_DRAM_START_ADDR=%llu\n", ((
	...
	utmpdata8 = (u8)pRes->sDMD_DTMB_InitData.utdistartaddr32;
	HAL_INTERN_DTMB_DBINFO(PRINT("utdistartaddr32=%llu\n",
	pRes->sDMD_DTMB_InitData.utdistartaddr32));
...
static u8 _hal_intern_dtmb_exit(void)
{
	...
	if (pRes->sDMD_DTMB_PriData.m6_version == MT5873_IP_VER) {
		temp1 = readl(pRes->sDMD_DTMB_PriData.virt_demod_base_addr) & 0xFFFFFFEE;
		writel(temp1, pRes->sDMD_DTMB_PriData.virt_demod_base_addr);
		sys.DelayMS(1);
		temp1 |= 0x00000011;
		writel(temp1, pRes->sDMD_DTMB_PriData.virt_demod_base_addr);
	} else if (pRes->sDMD_DTMB_PriData.m6_version == MT5879_IP_VER) {
		temp1 = readl(pRes->sDMD_DTMB_PriData.virt_clk_base_addr + 0x1F84) & 0xFFFFFEEF;
		writel(temp1, (pRes->sDMD_DTMB_PriData.virt_clk_base_addr + 0x1F84));
		sys.DelayMS(1);
		temp1 |= 0x00000110;
		writel(temp1, (pRes->sDMD_DTMB_PriData.virt_clk_base_addr + 0x1F84));
		...
		for (i = 0; i < 0x80; i++) {
			writew(demod_dtmb_top[i], pRes->sDMD_DTMB_PriData.virtDMDBaseAddr
			+ 0x2000 + i*4); //demod_top
			writew(demod_dtmb_top_ext[i], pRes->sDMD_DTMB_PriData.virtDMDBaseAddr
			+ 0x24200 + i*4); //demod_top_ext
			writew(demod_dtmb_clkgen[i], pRes->sDMD_DTMB_PriData.virtDMDBaseAddr
			+ 0x6A00 + i*4); //demod_clkgen
			writew(demod_dtmb_clkgen_ext[i], pRes->sDMD_DTMB_PriData.virtDMDBaseAddr
			+ 0x24400 + i*4); //demod_clkgen_ext
			writew(demod_dtmb_sram_share[i], pRes->sDMD_DTMB_PriData.virtDMDBaseAddr
			+ 0x24C00 + i*4); //demod_sram_share
			writew(demod_dtmb_ana[i], pRes->sDMD_DTMB_PriData.virtDMDBaseAddr
			+ 0x5C00 + i*4); //demod_ana
			writew(demod_dtmb_dvbs2[i], pRes->sDMD_DTMB_PriData.virtDMDBaseAddr
			+ 0x7400 + i*4); //demod_dvbs2
