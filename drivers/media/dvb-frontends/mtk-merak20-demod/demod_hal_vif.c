...
#define VIF_FW_RUN_ON_DRAM_MODE     1 //0
...
#else
#define SRAM_OFFSET 0x0000
#endif

static u8 unlock_cnt;
static u8 final_lock_status;
static u8 force_unlock;
...
#define MT5876_IP_VERSION  0x0300
#define MT5879_IP_VERSION  0x0400
#define MT5873_IP_VERSION  0x0500

#define DRIVER_VER_L    0x17   // 23 (dd)
#define DRIVER_VER_H    0xC6  // 12/22 (mm/yy)

#define DMDTOP_EXT_REG_BASE  0x24200
#define DMD_CLKGEN_REG_BASE    0x6A00
#define DMD_CLKGEN_EXT_REG_BASE  0x24400
#define DMD_SRAM_SHARE_REG_BASE  0x24C00
#define DMD_REMAP_REG_BASE  0x0200
#define DMD_DVBS2_REG_BASE  0x7400
...
void _recovery_to_default(void)
{
	u8 i;

	pr_info("\r\n ===== _recovery_to_default_Start ===== \r\n");

	/* [0]: reset demod 0 */
	/* [4]: reset demod 1 */
	writeb(0x01, _pvif_in->virt_reset_base_addr);// only reset demod 1
	hal_vif_delayms(1);
	writeb(0x11, _pvif_in->virt_reset_base_addr);
	hal_vif_delayms(1);

	for (i = 0; i < 0x80; i++) {
		writew(dmd_top[i], (_pvif_in->virt_riu_base_addr + DMDTOP_REG_BASE + i*4));
		writew(dmd_top_ext[i], (_pvif_in->virt_riu_base_addr + DMDTOP_EXT_REG_BASE + i*4));
		writew(dmd_clkgen[i], (_pvif_in->virt_riu_base_addr + DMD_CLKGEN_REG_BASE + i*4));
		writew(dmd_clkgen_ext[i], (_pvif_in->virt_riu_base_addr +
			DMD_CLKGEN_EXT_REG_BASE + i*4));
		writew(dmd_sram_share[i], (_pvif_in->virt_riu_base_addr +
			DMD_SRAM_SHARE_REG_BASE + i*4));
		writew(dmd_ana[i], (_pvif_in->virt_riu_base_addr + ADC_REG_BASE + i*4));
		//writew(dmd_remap[i], (_pvif_in->virt_riu_base_addr + DMD_REMAP_REG_BASE + i*4));
		writew(dmd_dvbs2[i], (_pvif_in->virt_riu_base_addr + DMD_DVBS2_REG_BASE + i*4));
	}

	_riu_write_byte((0x0200 + 0x57*2), 0x02); // remapping reset
	_riu_write_byte((0x0200 + 0x57*2), 0x00);

	pr_info("\r\n ===== _recovery_to_default_End ===== \r\n");
}
...
void hal_vif_adc_init(void)
{
	...
	if (!g_vif_downloaded) {
		if (_pvif_in->ip_version == MT5879_IP_VERSION) {
			writel(0x00000001, (_pvif_in->virt_clk_base_addr + 0x1F84));
			hal_vif_delayms(1);
			writel(0x00000111, (_pvif_in->virt_clk_base_addr + 0x1F84));
			hal_vif_delayms(1);
		} else if (_pvif_in->ip_version < MT5879_IP_VERSION) {
			_recovery_to_default();
		}
	}
	...
	if ((_pvif_in->ip_version & 0xFF00) == 0x0100) {
		if ((_pvif_in->ip_version & 0x00FF) == 0x0003) {
			temp2 = readl(_pvif_in->virt_clk_base_addr + 0x14F0) & 0xFFFFE0FF;
			temp2 |= 0x00001f00;
			writel(temp2, (_pvif_in->virt_clk_base_addr + 0x14F0));
		}
	}
	...
	temp2 = readl(_pvif_in->virt_clk_base_addr + 0x19D8) & 0xFFFFFFFE;
	temp2 |= 0x00000001;
	writel(temp2, (_pvif_in->virt_clk_base_addr + 0x19D8));

	if (_pvif_in->ip_version >= MT5873_IP_VERSION) {
		temp2 = readl(_pvif_in->virt_clk_base_addr + 0x1A3C) & 0xFFFFFFFE;
		temp2 |= 0x00000001;
		writel(temp2, (_pvif_in->virt_clk_base_addr + 0x1A3C));
	}

	// Start IMI SRAM initial setting
	//[0:0]
	if (_pvif_in->ip_version >= MT5873_IP_VERSION) {
		temp2 = readl(_pvif_in->virt_demod_base_addr + 0x04) & 0xFFFFFFFE;
		writel(temp2, (_pvif_in->virt_demod_base_addr + 0x04));
	} else {
	...
	// MPLL setting
	temp2 = readl(_pvif_in->virt_mpll_base_addr + 0xC8) & 0xFFFF00FC;
	temp2 |= 0x00001100;
	writel(temp2, (_pvif_in->virt_mpll_base_addr + 0xC8));

	temp2 = readl(_pvif_in->virt_mpll_base_addr + 0x154) & 0xFFFFFEFF;
	writel(temp2, (_pvif_in->virt_mpll_base_addr + 0x154));
	...
	temp2 |= _BIT2; // 172MHz
	#else
	if (_pvif_in->ip_version >= MT5876_IP_VERSION) {
		//temp2 |= 0x00000010;
		temp2 |= _BIT4; // 108MHz
	} else {
		//temp2 |= 0x00000014;
		temp2 |= (_BIT2 | _BIT4); // 108MHz
	}
	#endif
	writel(temp2, (_pvif_in->virt_clk_base_addr + 0x3110));
	...
	_riu_write_byte(DMDMCU2_BASE + 0x04, (SRAM_OFFSET & 0xFF));
	_riu_write_byte(DMDMCU2_BASE + 0x05, (SRAM_OFFSET >> 8));
	...
	temp = SRAM_OFFSET - 1;
	_riu_write_byte(DMDMCU2_BASE + 0x0E, (temp & 0xFF));
	_riu_write_byte(DMDMCU2_BASE + 0x0F, (temp >> 8));
	...
	if (_pvif_in->ip_version >= MT5873_IP_VERSION)
		_riu_write_byte(MBREG_BASE + 0x1A, 0x03);
	else
		_riu_write_byte(MBREG_BASE + 0x1A, 0x02);
}
...
bool hal_vif_filter_coef_download(void)
{
	...
	if (_pvif_filters[0] != 0) {
		...
		address_offset = ((intern_vif_table[address_offset] << 8)
				|intern_vif_table[(u16)(address_offset+1)]);
...
bool hal_vif_download(void)
{
	...
	//when ready == false
	{
		...
		pr_info("\r\n == hal_vif_download_virt_dram_base_addr = 0x%p == \r\n",
			virt_dram_base_addr);
...
void hal_vif_set_sound_sys(enum vif_sound_sys sound_sys, u32 if_freq)
{
	...
	if (_pvif_in->clamp_gain_b == 0) {
		_pvif_in->clamp_gain_b = _pvif_in->clampgain_gain_ov_negative;
		_pvif_in->clamp_gain_gh = _pvif_in->clampgain_gain_ov_negative;
		_pvif_in->clamp_gain_dk = _pvif_in->clampgain_gain_ov_negative;
		_pvif_in->clamp_gain_i = _pvif_in->clampgain_gain_ov_negative;
		_pvif_in->clamp_gain_l = _pvif_in->clampgain_gain_ov_positive;
		_pvif_in->clamp_gain_lp = _pvif_in->clampgain_gain_ov_positive;
		_pvif_in->clamp_gain_mn = _pvif_in->clampgain_gain_ov_negative;
	}
	...
	if (if_freq != 0) {
		...
		case VIF_SOUND_B_NICAM:
			...
			//clamp_gain = 0x05A0;
			clamp_gain = _pvif_in->clamp_gain_b;
		...
		case VIF_SOUND_GH_NICAM:
			...
			//clamp_gain = 0x05A0;
			clamp_gain = _pvif_in->clamp_gain_gh;
		...
		case VIF_SOUND_DK_NICAM:
			...
			//clamp_gain = 0x05CE;
			clamp_gain = _pvif_in->clamp_gain_dk;
		...
		case VIF_SOUND_I:
			...
			//clamp_gain = 0x065E;
			clamp_gain = _pvif_in->clamp_gain_i;
		...
		case VIF_SOUND_MN:
			...
			//clamp_gain = 0x05AE;
			clamp_gain = _pvif_in->clamp_gain_mn;
		...
		case VIF_SOUND_L:
			...
			//clamp_gain = 0x0772;
			clamp_gain = _pvif_in->clamp_gain_l;
		...
		case VIF_SOUND_LL:
			...
			//clamp_gain = 0x0600;
			clamp_gain = _pvif_in->clamp_gain_lp;
...
u8 hal_vif_exit(void)
{
	...
	if (_pvif_in->ip_version >= MT5873_IP_VERSION) {
		writel(0x00000000, _pvif_in->virt_demod_base_addr);
		hal_vif_delayms(1);
		writel(0x00000011, _pvif_in->virt_demod_base_addr);
	} else if (_pvif_in->ip_version >= MT5879_IP_VERSION) {
		writel(0x00000001, (_pvif_in->virt_clk_base_addr + 0x1F84));
		hal_vif_delayms(1);
		writel(0x00000111, (_pvif_in->virt_clk_base_addr + 0x1F84));
	} else if (_pvif_in->ip_version >= MT5876_IP_VERSION) {
		_recovery_to_default();
	} else {
...
u8 hal_vif_read_foe(void)
{
	...
	lock_status = _riu_read_byte(MBREG_BASE + 0x1D) & 0x01;

	if (lock_status != 0)
		foe = _riu_read_byte(MBREG_BASE + 0x1F) & 0xFF;
	else
		foe = 0;
