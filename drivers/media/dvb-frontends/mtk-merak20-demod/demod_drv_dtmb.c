...
/*MS_VIRT*/
	...
	pRes->sDMD_DTMB_PriData.virt_mpll_base_addr =
	pRes->sDMD_DTMB_InitData.virt_mpll_base_addr;

	pRes->sDMD_DTMB_PriData.virt_efuse_base_addr =
	pRes->sDMD_DTMB_InitData.virt_efuse_base_addr;

	pRes->sDMD_DTMB_PriData.virt_demod_base_addr =
	pRes->sDMD_DTMB_InitData.virt_demod_base_addr;
...
//	ret &= _mdrv_dmd_dtmb_md_getreg(0, 0x20C1, &udata81);
	...
	if ((sDtmbModulationMode.upnc8) == 1) {//CONTSANT phase
		...
		switch (sDtmbModulationMode.upnm16) {
		case 420:
			p->guard_interval = (u8) GUARD_INTERVAL_PN420_VAR;
		break;

		case 945:
		p->guard_interval = (u8) GUARD_INTERVAL_PN945_VAR;
		break;
	...
	p->transmission_mode = TRANSMISSION_MODE_C1; break;
	  ...
	  //!TS Rate porting
	   ...
	   et2tsrate = (u32) (2842560000U/(3780 + sDtmbModulationMode.upnm16)
...
char *buf)
{
	...
	u8 ifagc = 0, qrate = 0;
	s32 err_flg[20];
	u32 et2tsrate = 0, icp = 0;//uber32 = 0,
	s16 freqoffset = 0;
	struct DMD_DTMB_CFO_DATA cfo;
	struct DMD_DTMB_MODULATION_INFO sDtmbModulationMode;
	//struct DMD_DTMB_BER_DATA berr;
	...
	switch (sDtmbModulationMode.upnm16) {
	case 420:
			err_flg[12] = GUARD_INTERVAL_PN420;
		break;

	case 595:
		err_flg[12] = GUARD_INTERVAL_PN595;
		break;

	case 945:
		err_flg[12] = GUARD_INTERVAL_PN945;
		break;
	...
	switch (sDtmbModulationMode.usiinterleaver16) {
	case 240:
		err_flg[13] = INTERLEAVING_240; break;

	case 720:
		err_flg[13] = INTERLEAVING_720; break;
	...
	switch (sDtmbModulationMode.fsicoderate)	{
	case 4:
		err_flg[14] = FEC_2_5;   break;

	case 6:
		err_flg[14] = FEC_3_5;   break;

	case 8:
		err_flg[14] = FEC_4_5;   break;
	...
	switch (sDtmbModulationMode.usicarriermode8) {
	case 0/*CarrierMode_MC*/:
	err_flg[15] = TRANSMISSION_MODE_C3780; break;

	case 1/*CarrierMode_SC*/:
	err_flg[15] = TRANSMISSION_MODE_C1; break;
...
//	err_flg[16] = (s32) uber32*2*10000;// format: original-ber*10^9
	...
	udata16 = (u16) ((udata82<<8)|udata81);
	err_flg[16] = (s32) (udata16 * 1953125);//BER*1000000000/512;
	//pr_emerg("AGC = %d\n", err_flg[0]);
	//pr_emerg("CNR = %d\n", err_flg[1]);
	//pr_emerg("UEC = %d\n", err_flg[2]);
	...
	"%s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%sd %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s",
	SM_AGC, SM_COMMA, err_flg[0], SM_END,
	SM_CNR, SM_COMMA, err_flg[1], SM_END,
	SM_C_N, SM_COMMA, err_flg[1], SM_END,
	SM_UEC, SM_COMMA, err_flg[2], SM_END,
	SM_CHIP_REV, SM_COMMA, err_flg[3], SM_END,
	SM_DEMOD_LOCK, SM_COMMA, err_flg[4], SM_END,
	SM_TS_LOCK, SM_COMMA, err_flg[5], SM_END,
	SD_SIGNAL_QAULITY, SM_COMMA, err_flg[6], SM_END,
	SD_SIGNAL_LEVEL, SM_COMMA, err_flg[7], SM_END,
	SM_TS_RATE, SM_COMMA, err_flg[8], SM_END,
	SM_PN_PHASE, SM_COMMA, err_flg[9], SM_END,
	SM_FREQ_OFFSET, SM_COMMA, err_flg[10], SM_END,
	SM_MODULATION_TYPE, SM_COMMA, err_flg[11], SM_END,
	SM_GUARD_INTERVAL, SM_COMMA, err_flg[12], SM_END,
	SM_TIME_INTERLEAVE, SM_COMMA, err_flg[13], SM_END,
	SM_CODE_RATE, SM_COMMA, err_flg[14], SM_END,
	SM_TRANSMISSION_MODE, SM_COMMA, err_flg[15], SM_END,
	SM_BER_BEFORE_BCH, SM_COMMA, err_flg[16], SM_END,
