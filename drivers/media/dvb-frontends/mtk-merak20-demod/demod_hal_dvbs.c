...
#define DVBS2_DISEQC_TX_DATA_MT5873      (DVBS2_REG_BASE*2+0x62*2)
...
#define DVBS2_DISEQC_TX_DATA_MODE_MT5873 (DVBS2_REG_BASE*2+0x66*2+1) //bit 15
...
#define DVBS2_SW_USE0 (DVBS2_REG_BASE*2+0x78*2)
#define DVBS2_SW_USE6 (DVBS2_REG_BASE*2+0x7B*2)
#define DVBS2_DISEQC_TX_TONE_MODE        (DVBS2_REG_BASE*2+0x6B*2+1)
#define DVBS2_DISEQC_TX_LENGTH_MT5873        (DVBS2_REG_BASE*2+0x5e*2)
...
u8 INTERN_DVBS_table_MT5873[] = {
#include "fw_dmd_dvbs_MT5873.dat"
};
...
int intern_dvbs_exit(struct dvb_frontend *fe)
{
	...
	if (ip_version > 0x0400) { // for mt5873 & after
	...
	} else if (ip_version > 0x0300) { // for mt5879
		/* [4]: reset demod 0 */
		/* [8]: reset demod 1 */
		...
		u16tmp |= 0x0110;
		writew(u16tmp, demod_prvi->virt_ts_base_addr + 0x04);
	} else if (ip_version > 0x0200) { // for mt5876
...
int intern_dvbs_load_code(void)
{
	...
	if ((ip_version & 0xff00) == 0x0500) {
		memcpy(dram_code_virt_addr, &INTERN_DVBS_table_MT5873[0],
			sizeof(INTERN_DVBS_table_MT5873));
	} else
...
void intern_dvbs_init_clk(struct dvb_frontend *fe)
{
	...
	if (((ip_version & 0xff00) == 0x0500) || ((ip_version & 0xff00) == 0x0400)) {
...
int intern_dvbs_config(struct dvb_frontend *fe)
{
	...
	Cur_freq = c->frequency * transfer_unit;
	ret = intern_dvbs_reset();
	if (tone22k_Flag) {
...
static int intern_dvbs_get_pkterr(u16 *pkterr)//V
{
	...
	if (!u8Data) {
		...
		mtk_demod_mbx_dvb_read(ISDBTOUTER_UNCRT_PKT_NUM_PCKTPRD_15_8, &u8Data);
		u16PktErr = u8Data;
		mtk_demod_mbx_dvb_read(ISDBTOUTER_UNCRT_PKT_NUM_PCKTPRD_7_0, &u8Data);
		u16PktErr = (u16PktErr << 8)|u8Data;
	}
	*pkterr = u16PktErr;
...
int intern_dvbs_get_ber_window(u32 *err_period, u32 *err)
{
	...
	if (_bDemodType) {
		...
		*err = (*err << 8) | reg;
		...
		mtk_demod_mbx_dvb_read(ISDBTOUTER_BITERR_NUM_PCKTPRD_FREEZE, &reg_frz);
		mtk_demod_mbx_dvb_write(ISDBTOUTER_BITERR_NUM_PCKTPRD_FREEZE, reg_frz|0x01);

		mtk_demod_mbx_dvb_read(ISDBTOUTER_BITERR_PCKTPRD_H, &reg);
		*err_period = reg;

		mtk_demod_mbx_dvb_read(ISDBTOUTER_BITERR_PCKTPRD_L, &reg);
		*err_period = (*err_period << 8)|reg;

		mtk_demod_mbx_dvb_read(ISDBTOUTER_BITERR_NUM_PCKTPRD+3, &reg);
		*err = reg;
		mtk_demod_mbx_dvb_read(ISDBTOUTER_BITERR_NUM_PCKTPRD+2, &reg);
		*err = (*err << 8)|reg;
		mtk_demod_mbx_dvb_read(ISDBTOUTER_BITERR_NUM_PCKTPRD+1, &reg);
		*err = (*err << 8)|reg;
		mtk_demod_mbx_dvb_read(ISDBTOUTER_BITERR_NUM_PCKTPRD, &reg);
		*err = (*err << 8)|reg;

		reg_frz = reg_frz&(~0x01);
		mtk_demod_mbx_dvb_write(ISDBTOUTER_BITERR_NUM_PCKTPRD_FREEZE, reg_frz);
	}
	if (*err_period == 0)
		*err_period = 1;
...
int intern_dvbs_diseqc_set_tone(enum fe_sec_mini_cmd minicmd)
{
	...
	u16  u16WaitCount = 0, u16WaitCount1 = 0;
	u8   mask = 0;
	u32  address = 0;
	bool machine_en_flag = 0;
	...
	if ((u8Data & 0x03) == 0x01)
	{
		...
		do {
			mtk_demod_mbx_dvb_read_dsp(E_DMD_S2_STATE_FLAG, &u8Data);//State
                        pr_info("[%s] E_DMD_S2_STATE_FLAG= %d\n", __func__, u8Data);
			u16WaitCount1++;
			mtk_demod_delay_ms(1);
		} while ((u8Data != 0x01) && (u16WaitCount1 < 50));

		mtk_demod_mbx_dvb_read((TOP_REG_BASE + 0x60*2), &u8Data);
		pr_info("[%s] TOP u8Data= %d\n", __func__, u8Data);
	}
	diseqc_init_clk();
...
int intern_dvbs_diseqc_send_cmd(struct dvb_diseqc_master_cmd *cmd)
{
	...
	if (ip_version == 0x0300) {
		mtk_demod_mbx_dvb_read(0x3500+(0x00*2), &u8Data);
		if (u8Data == 0x0D) {
			mtk_demod_mbx_dvb_write(TOP_REG_BASE + 0x79*2+1, cmd->msg_len);
			for (u8Index = 0; u8Index < (cmd->msg_len); u8Index++) {
				u8Data = cmd->msg[u8Index];
				mtk_demod_mbx_dvb_write(TOP_REG_BASE + 0x7A*2 + u8Index, u8Data);
			}
			//Write_Reg(DVBS2_DISEQC_EN,0x01, 6, 6);
			mtk_demod_mbx_dvb_write(TOP_REG_BASE+(0x7F*2), 0x6C);
			/* assert interrupt to VD MCU51 */
			mtk_demod_write_byte(DMD51_REG_BASE + 0x03, 0x02);
			/* de-assert interrupt to VD MCU51 */
			mtk_demod_write_byte(DMD51_REG_BASE + 0x03, 0x00);
			...
			u16WaitCount = 0;
			intern_dvbs_diseqc_wait_tx_done(&u16WaitCount);
			if (u16WaitCount >= INTERN_DVBS_DEMOD_WAIT_TIMEOUT) {
				pr_err("INTERN_DVBS_Customized_DiSEqC_SendCmd Busy!!!\n");
				return -ETIMEDOUT;
			}
		} else {
			mtk_demod_mbx_dvb_write(TOP_REG_BASE + 0x69*2+1,  cmd->msg_len);
			for (u8Index = 0; u8Index <  (cmd->msg_len); u8Index++) {
				u8Data = cmd->msg[u8Index];
				mtk_demod_mbx_dvb_write(TOP_REG_BASE + 0x6A*2 + u8Index, u8Data);
			}
			//Write_Reg(DVBS2_DISEQC_EN,0x01, 6, 6);
			mtk_demod_mbx_dvb_write(TOP_REG_BASE+(0x7F*2), 0x6C);
		...
		intern_dvbs_diseqc_length_check_type(Tx_Len);
		intern_dvbs_diseqc_check_type(cmd);
		for (u8Index = 0; u8Index < cmd->msg_len; u8Index++) {
			pr_info("DiseqcData[%d] 0x%x ", u8Index, cmd->msg[u8Index]);
			if (ip_version == 0x0400) { //MT5879
				//Diseqc ECO Tx data 0~5
				address = DVBS2_SW_USE0;
				u8Data = cmd->msg[u8Index];
				mtk_demod_write_byte(address+u8Index, u8Data);
			} else if (ip_version == 0x0500) { //MT5873
				//Diseqc  Tx data 0~5
				address = DVBS2_DISEQC_TX_DATA_MT5873;
				u8Data = cmd->msg[u8Index];
				mtk_demod_write_byte(address+u8Index, u8Data);
			} else {
		...
		if (ip_version == 0x0400) { //MT5879
			//Diseqc ECO Enable
			address = DVBS2_SW_USE6;
			u8Data = mtk_demod_read_byte(address);
			u8Data |= BIT0;
			mtk_demod_write_byte(address, u8Data);
		} else if (ip_version == 0x0500) { //MT5873
			//Diseqc  Enable
			address = DVBS2_DISEQC_TX_DATA_MODE_MT5873;
			u8Data = mtk_demod_read_byte(address);
			u8Data &= (~BIT7);
			mtk_demod_write_byte(address, u8Data);
		}
...
void intern_dvbs_diseqc_wait_tx_done(u16  *u16WaitCount)
{
	u8   u8Data = 0, u8Data2 = 0;
	u16  WaitCount = 0;
	u32  address = 0;
	if (ip_version == 0x0300) {
		do {
			mtk_demod_mbx_dvb_read(TOP_REG_BASE+(0x7F*2), &u8Data);
			mtk_demod_delay_ms(1);
			WaitCount++;
		} while ((u8Data != 0x00) &&
			(WaitCount < INTERN_DVBS_DEMOD_WAIT_TIMEOUT));
	} else {
		do {
			address = DVBS2_DISEQC_STATUS;
			u8Data = mtk_demod_read_byte(address);
	...
	*u16WaitCount = WaitCount;
}

void intern_dvbs_diseqc_length_check_type(u8 Tx_Len)
{
	u8   u8Data = 0, mask = 0;
	u32  address = 0;

	if (ip_version == 0x0500) { //MT5873
		address = DVBS2_DISEQC_TX_LENGTH_MT5873;
		u8Data = mtk_demod_read_byte(address);
		mask = (u8)(~BIT7);
		u8Data &= (~mask);
		u8Data |= Tx_Len;
