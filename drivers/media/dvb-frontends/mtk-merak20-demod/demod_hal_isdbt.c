...
#define DMD_ISDBT_CHIP_MT5896       0x26

#define DMD_ISDBT_CHIP_VERSION     DMD_ISDBT_CHIP_MT5896
...
#elif DVB_STI
 #define MBRegBase              0x5E00 // 0x2F00
#else
...
#elif DVB_STI
 #define DMDMcuBase             0x62300 //0x31180
#else
...
#if ((DMD_ISDBT_CHIP_VERSION == DMD_ISDBT_CHIP_K5AP) || \
(defined MTK_PROJECT))
 // 0: FW download to SRAM, 1: FW download to DRAM
 #define FW_RUN_ON_DRAM_MODE     1
#else
 #define FW_RUN_ON_DRAM_MODE     1//VT 0
#endif
...
//  Local Variables
//------------------------------------------------------------------
#define DMDMcuRstState 0x62800
#define MT5896_IP_VER  0x0100
#define MT5897_IP_VER  0x0200
#define MT5876_IP_VER  0x0300
#define MT5879_IP_VER  0x0400
#define MT5873_IP_VER  0x0500
static u8 id;
...
const u8 intern_isdbt_table[] = {
	#include "dmd_isdbt.dat"
};
const u8 intern_isdbt_8M_table[] = {
	#include "dmd_isdbt_8m.dat"
};

const u8 intern_mt5879_isdbt_table[] = {
	#include "dmd_5879_isdbt.dat"
};
const u8 intern_mt5879_isdbt_8M_table[] = {
	#include "dmd_5879_isdbt_8m.dat"
};

const u8 intern_mt5873_isdbt_table[] = {
	#include "dmd_5873_isdbt.dat"
};
const u8 intern_mt5873_isdbt_8M_table[] = {
	#include "dmd_5873_isdbt_8m.dat"
};
...
u16 demod_isdbt_top[0x80] = { // 0x20, for check patch column100
...
u16 demod_isdbt_top_ext[0x80] = { // 0x121
...
u16 demod_isdbt_clkgen[0x80] = { // 0x35
	0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
	0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
	0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
	0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
...
u16 demod_isdbt_clkgen_ext[0x80] = { // 0x122
...
u16 demod_isdbt_sram_share[0x80] = { // 0x126
	0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
	0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
	0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
	0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
...
u16 demod_isdbt_dvbs2[0x80] = { // 0x3a
...
static void _hal_intern_isdbt_initclk(void)
{
	...
	pres->sdmd_isdbt_initdata.isdbt_fec_lock_check_time = DEFAULT_FEC_CHECK_TIME;
	pres->sdmd_isdbt_initdata.isdbt_icfo_ch_exist_check_time = DEFAULT_ICFO_CHECK_TIME;
	#endif
...
#elif (DMD_ISDBT_CHIP_VERSION == DMD_ISDBT_CHIP_MT5896)
static void _hal_intern_isdbt_initclk(void)
{
	...
	ip_version = (u16)pres->sdmd_isdbt_initdata.ip_version;

	switch (ip_version) {
	case MT5876_IP_VER:
		HAL_INTERN_ISDBT_DBINFO(PRINT("--DMD_ISDBT_CHIP_MT5876--\n"));
		break;
	case MT5879_IP_VER:
		HAL_INTERN_ISDBT_DBINFO(PRINT("--DMD_ISDBT_CHIP_MT5879--\n"));
		break;
	case MT5873_IP_VER:
		HAL_INTERN_ISDBT_DBINFO(PRINT("--DMD_ISDBT_CHIP_MT5873--\n"));
		break;
	default:
		HAL_INTERN_ISDBT_DBINFO(PRINT("--DMD_ISDBT_CHIP_MT5896--\n"));
		break;
	}

	// for hang issue
	/* [4][8]: reset demod 0/1 */
	if (ip_version == MT5879_IP_VER) {
		temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x1F84) & 0xFFFFFEEF;
		writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x1F84));
		sys.delayms(1);
		temp1 |= 0x00000110;
		writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x1F84));
		sys.delayms(1);
	} else if (ip_version < MT5879_IP_VER) {
		writel(0x00000000, pres->sdmd_isdbt_pridata.virt_mpll_base_addr + 0x104);
		sys.delayms(1);
		writel(0x00000011, pres->sdmd_isdbt_pridata.virt_mpll_base_addr + 0x104);
		sys.delayms(1);

		for (i = 0; i < 0x80; i++) {
			writew(demod_isdbt_top[i], pres->sdmd_isdbt_pridata.virt_dmd_base_addr
			+ 0x2000 + i*4); //demod_top
			writew(demod_isdbt_top_ext[i], pres->sdmd_isdbt_pridata.virt_dmd_base_addr
			+ 0x24200 + i*4); //demod_top_ext
			writew(demod_isdbt_clkgen[i], pres->sdmd_isdbt_pridata.virt_dmd_base_addr
			+ 0x6A00 + i*4); //demod_clkgen
			writew(demod_isdbt_clkgen_ext[i], //demod_clkgen_ext
			pres->sdmd_isdbt_pridata.virt_dmd_base_addr + 0x24400 + i*4);
			writew(demod_isdbt_sram_share[i], //demod_sram_share
			pres->sdmd_isdbt_pridata.virt_dmd_base_addr + 0x24C00 + i*4);
			writew(demod_isdbt_ana[i], pres->sdmd_isdbt_pridata.virt_dmd_base_addr
			+ 0x5C00 + i*4); //demod_ana
			writew(demod_isdbt_dvbs2[i], pres->sdmd_isdbt_pridata.virt_dmd_base_addr
			+ 0x7400 + i*4); //demod_dvbs2
		}
		_hal_dmd_riu_writebyte(0x0200 + 0x57*2, 0x02); // remapping reset
		_hal_dmd_riu_writebyte(0x0200 + 0x57*2, 0x00);
	}

	// ana trim
	if (ip_version >= MT5879_IP_VER) {
		temp = (0x1a<<2) | 0x2000;// [13] read trigger, [9:2] addr
		writew(temp, pres->sdmd_isdbt_pridata.virt_efuse_base_addr + (0x28<<2));

		temp = readw(pres->sdmd_isdbt_pridata.virt_efuse_base_addr + (0x2d<<2));
		temp1 = (temp<<16);
		temp = readw(pres->sdmd_isdbt_pridata.virt_efuse_base_addr + (0x2c<<2));
		temp1 |= temp;

		check_val = (temp1>>15) & 0x01;
		if (check_val != 0) {
			u8tmp = _hal_dmd_riu_readbyte(TOPRegBase + 0x01*2 + 1);// release bit13
			u8tmp &= (~0x20);
			_hal_dmd_riu_writebyte(TOPRegBase + 0x01*2 + 1, u8tmp);

			u8tmp = _hal_dmd_riu_readbyte(ANARegBase + 0x47*2);
			u8tmp &= ~(0x1f);
			u8tmp |= (u8)((temp1>>8)&0x1f);
			_hal_dmd_riu_writebyte(ANARegBase + 0x47*2, u8tmp);
		}
	}

	// demod ckgen enable
	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x14E4) & 0xFFFFFDBF;
	temp1 |= 0x00000240;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x14E4));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x1D8) & 0xFFFFFCF8;
	temp1 |= 0x00000004;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x1D8));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x2B8) & 0xFFFFFFF8;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x2B8));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x2C0) & 0xFFFFFFFC;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x2C0));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x2C8) & 0xFFFFFFFC;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x2C8));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x2D8) & 0xFFFFFFFC;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x2D8));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x510) & 0xFFFFFFFC;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x510));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0xAB8) & 0xFFFFFFFC;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0xAB8));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x1598) & 0xFFFFFFFE;
	temp1 |= 0x00000001;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x1598));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x1740) & 0xFFFFFFFE;
	temp1 |= 0x00000001;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x1740));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x19C8) & 0xFFFFFFFE;
	temp1 |= 0x00000001;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x19C8));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x19CC) & 0xFFFFFFFE;
	temp1 |= 0x00000001;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x19CC));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x19D0) & 0xFFFFFFFE;
	temp1 |= 0x00000001;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x19D0));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x19D4) & 0xFFFFFFFE;
	temp1 |= 0x00000001;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x19D4));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x3040) & 0xFFFFFCE0;
	temp1 |= 0x00000004;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x3040));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x3110) & 0xFFFFFCE0;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x3110));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x31C0) & 0xFFFFFFFC;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x31C0));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x3B4C) & 0xFFFFFFDF;
	temp1 |= 0x00000020;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x3B4C));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x3B5C) & 0xFFFFFFFE;
	temp1 |= 0x00000001;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x3B5C));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x3B74) & 0xFFFFFFFB;
	temp1 |= 0x00000004;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x3B74));

	if (ip_version == MT5873_IP_VER) {
		temp1 = readl(pres->sdmd_isdbt_pridata.virt_demod_base_addr + 0x04) & 0xFFFFFFFE;
		writel(temp1, (pres->sdmd_isdbt_pridata.virt_demod_base_addr + 0x04));
	} else {
		temp1 = readl(pres->sdmd_isdbt_pridata.virt_mpll_base_addr + 0x1EC) & 0xFFFFFFFE;
		writel(temp1, (pres->sdmd_isdbt_pridata.virt_mpll_base_addr + 0x1EC));
	}

	if (ip_version == MT5876_IP_VER || ip_version == MT5879_IP_VER) {
		temp1 = readl(pres->sdmd_isdbt_pridata.virt_mpll_base_addr + 0xC8) & 0xFFFF00FC;
		temp1 |= 0x00001100;
		writel(temp1, (pres->sdmd_isdbt_pridata.virt_mpll_base_addr + 0xC8));

		temp1 = readl(pres->sdmd_isdbt_pridata.virt_mpll_base_addr + 0x154) & 0xFFFFFEFF;
		writel(temp1, (pres->sdmd_isdbt_pridata.virt_mpll_base_addr + 0x154));
	}

	if (pres->sdmd_isdbt_initdata.ip_version < MT5876_IP_VER) {
		temp1 = readl(pres->sdmd_isdbt_pridata.virt_pmux_base_addr) & 0xFFFFFFF0;
		temp1 |= 0x00000003;
		writel(temp1, (pres->sdmd_isdbt_pridata.virt_pmux_base_addr));

		temp1 = readl(pres->sdmd_isdbt_pridata.virt_vagc_base_addr) & 0xFFFFF0FF;
		temp1 |= 0x00000300;
		writel(temp1, (pres->sdmd_isdbt_pridata.virt_vagc_base_addr));
	}

	// Start TOP CLKGEN initial setting
	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x3110) & 0xFFFFFCE0;
	#if (FW_RUN_ON_DRAM_MODE == 1)
	temp1 |= 0x00000004;   //172M
	#else
	if (pres->sdmd_isdbt_initdata.ip_version >= MT5876_IP_VER)
		temp1 |= 0x00000010;   //108M
	else
		temp1 |= 0x00000014;   //108M
	#endif
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x3110));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x1F80) & 0xFFFFFF00;
	temp1 |= 0x00000011;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x1F80));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x1F88) & 0xFFFFFFF0;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x1F88));

	temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x1F8C) & 0xFFFFF0FF;
	writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x1F8C));

	if (pres->sdmd_isdbt_initdata.ip_version == MT5879_IP_VER) {
		temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x1F84) & 0xFFFFFFFE;
		temp1 |= 0x00000111;
	} else {
		temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x1F84) & 0xFFFFFFFE;
		temp1 |= 0x00000001;
	...
	temp = (u16)(((pres->sdmd_isdbt_initdata.tdi_start_addr * 16) + (4 * 1024 * 1024))>>16);
	_hal_dmd_riu_writebyte(MCU2RegBase + 0x1A, (u8)temp);
	_hal_dmd_riu_writebyte(MCU2RegBase + 0x1B, (u8)(temp>>8));

	temp = (u16)(((pres->sdmd_isdbt_initdata.tdi_start_addr * 16) + (4 * 1024 * 1024))>>32);
...
static u8 _hal_intern_isdbt_download(void)
{
	...
	if (pres->sdmd_isdbt_pridata.bdownloaded) {
		...
		if (pres->sdmd_isdbt_pridata.elast_type == DMD_ISDBT_DEMOD_8M) {
			if (ip_version == MT5879_IP_VER) {
				isdbt_table = &intern_mt5879_isdbt_8M_table[0];
				lib_size = sizeof(intern_mt5879_isdbt_8M_table);
			} else if (ip_version == MT5873_IP_VER) {
				isdbt_table = &intern_mt5873_isdbt_8M_table[0];
				lib_size = sizeof(intern_mt5873_isdbt_8M_table);
			} else {
			...
			if (ip_version == MT5879_IP_VER) {
				isdbt_table = &intern_mt5879_isdbt_table[0];
				lib_size = sizeof(intern_mt5879_isdbt_table);
			} else if (ip_version == MT5873_IP_VER) {
				isdbt_table = &intern_mt5873_isdbt_table[0];
				lib_size = sizeof(intern_mt5873_isdbt_table);
			} else {
	...
	HAL_INTERN_ISDBT_DBINFO(PRINT(">>> ISDBT_FW_DRAM_START_ADDR=0x%llx\n",
	...
	#elif DVB_STI
	dramcode_virtual_addr =
	(pres->sdmd_isdbt_initdata.virt_dram_base_addr + (4 * 1024 * 1024));
	#else
	...
	#if DVB_STI
	MEMCPY(dramcode_virtual_addr, &isdbt_table[0], lib_size);
	#else
	MEMCPY((void *)(size_t)dramcode_virtual_addr, &isdbt_table[0], lib_size);
	#endif
	...
	MEMCPY((dramcode_virtual_addr + addess_offset), &u8tmp_data, 1);
	u8tmp_data = (u8)(pres->sdmd_isdbt_initdata.if_khz >> 8);
	MEMCPY((dramcode_virtual_addr + addess_offset + 1), &u8tmp_data, 1);
	u8tmp_data = (u8)pres->sdmd_isdbt_initdata.iq_swap;
	HAL_INTERN_ISDBT_DBINFO(PRINT(
	"bIQSwap=%d\n", pres->sdmd_isdbt_initdata.iq_swap));
	MEMCPY((dramcode_virtual_addr + addess_offset + 2), &u8tmp_data, 1);
	u8tmp_data = (u8)pres->sdmd_isdbt_initdata.agc_reference_value;
	HAL_INTERN_ISDBT_DBINFO(PRINT("u16AGC_REFERENCE=%X\n",
	pres->sdmd_isdbt_initdata.agc_reference_value));
	MEMCPY((dramcode_virtual_addr + addess_offset + 3), &u8tmp_data, 1);
	u8tmp_data =
	(u8)(pres->sdmd_isdbt_initdata.agc_reference_value >> 8);
	MEMCPY((dramcode_virtual_addr + addess_offset + 4), &u8tmp_data, 1);
	u8tmp_data = (u8)pres->sdmd_isdbt_initdata.tdi_start_addr;
	HAL_INTERN_ISDBT_DBINFO(PRINT(
	"u32TdiStartAddr=%llX\n", pres->sdmd_isdbt_initdata.tdi_start_addr));
	MEMCPY((dramcode_virtual_addr + addess_offset + 5), &u8tmp_data, 1);
	u8tmp_data = (u8)(pres->sdmd_isdbt_initdata.tdi_start_addr >> 8);
	MEMCPY((dramcode_virtual_addr + addess_offset + 6), &u8tmp_data, 1);
	u8tmp_data = (u8)(pres->sdmd_isdbt_initdata.tdi_start_addr >> 16);
	MEMCPY((dramcode_virtual_addr + addess_offset + 7), &u8tmp_data, 1);
	u8tmp_data = (u8)(pres->sdmd_isdbt_initdata.tdi_start_addr >> 24);
	MEMCPY((dramcode_virtual_addr + addess_offset + 8), &u8tmp_data, 1);
	u8tmp_data = (u8)pres->sdmd_isdbt_initdata.tuner_gain_invert;
	MEMCPY((dramcode_virtual_addr + addess_offset + 9), &u8tmp_data, 1);
	#else
	u8tmp_data = (u8)pres->sdmd_isdbt_initdata.if_khz;
	HAL_INTERN_ISDBT_DBINFO(
	PRINT("u16IF_KHZ=%d\n", pres->sdmd_isdbt_initdata.if_khz));
...
static u8 _hal_intern_isdbt_exit(void)
{
	...
	#elif DVB_STI
	if (ip_version == MT5873_IP_VER) {
		temp1 = readl(pres->sdmd_isdbt_pridata.virt_demod_base_addr) & 0xFFFFFFEE;
		writel(temp1, pres->sdmd_isdbt_pridata.virt_demod_base_addr);
		sys.delayms(1);
		temp1 |= 0x00000011;
		writel(temp1, pres->sdmd_isdbt_pridata.virt_demod_base_addr);
	} else if (ip_version == MT5879_IP_VER) {
