...
#define DMD_ATSC_CHIP_MT5889    0x26
#define DMD_ATSC_CHIP_MT5896    0x27
#define DMD_ATSC_CHIP_MT5897    0x28
#define DMD_ATSC_CHIP_MT5876    0x29
#define DMD_ATSC_CHIP_MT5879    0x3A
#define DMD_ATSC_CHIP_MT5873    0x3B
...
#elif	defined(CC_MT5876)
 #define DMD_ATSC_CHIP_VERSION		DMD_ATSC_CHIP_MT5876
#elif	defined(CC_MT5879)
 #define DMD_ATSC_CHIP_VERSION		DMD_ATSC_CHIP_MT5879
#elif	defined(CC_MT5873)
 #define DMD_ATSC_CHIP_VERSION		DMD_ATSC_CHIP_MT5873
...
#undef DMD_ATSC_CHIP_VERSION
#define DMD_ATSC_CHIP_VERSION		DMD_ATSC_CHIP_MT5876 //mokona

#if DMD_ATSC_CHIP_VERSION == DMD_ATSC_CHIP_MT5876
...
#define	J83ABC_ENABLE			 0
 ...
 #if DMD_ATSC_CHIP_VERSION == DMD_ATSC_CHIP_MT5876
 #define DO_NOT_RST_MOD          1
 #else
 #define DO_NOT_RST_MOD          0
 #endif

 #if DMD_ATSC_CHIP_VERSION == DMD_ATSC_CHIP_MT5879
 #define SEPARATE_MOD_RST        1
 #else
 #define SEPARATE_MOD_RST        0
 #endif

 #if DMD_ATSC_CHIP_VERSION >= DMD_ATSC_CHIP_MT5873
 #define DMD_INDPEND_RST         1
 #else
 #define DMD_INDPEND_RST         0
 #endif
...
#if	(!J83ABC_ENABLE) && !FW_RUN_ON_DRAM_MODE
...
static void _hal_intern_atsc_initclk(void)
{
	...
	if ((pres->sdmd_atsc_pridata.m6_version & 0xFF0F) > 0x0100) {
		temp1 = readl(pres->sdmd_atsc_pridata.virt_clk_base_addr + 0x14F0) & 0xFFFFFFE0;
		temp1 |= 0x0000001F;
		writel(temp1, (pres->sdmd_atsc_pridata.virt_clk_base_addr + 0x14F0));
	}
	...
	if ((pres->sdmd_atsc_pridata.m6_version & 0x00F0) > 0)
...
#elif ((DMD_ATSC_CHIP_VERSION == DMD_ATSC_CHIP_MT5876) \
	|| (DMD_ATSC_CHIP_VERSION == DMD_ATSC_CHIP_MT5879) \
	|| (DMD_ATSC_CHIP_VERSION == DMD_ATSC_CHIP_MT5873))
static void _hal_intern_atsc_initclk(void)
{
	//u8 u8Val = 0;
	u8 reg = 0;
	...
	u16version = pres->sdmd_atsc_pridata.m6_version & 0xff00;

	if (u16version == 0x0300) {
		PRINT("--------------DMD_ATSC_CHIP_MT5876--------------\n");
	} else if (u16version == 0x0400) {
		PRINT("--------------DMD_ATSC_CHIP_MT5879--------------\n");
	} else {
		PRINT("--------------DMD_ATSC_CHIP_MT5873--------------\n");
	}

	// for hang issue
	/* [4][8]: reset demod 0/1 */
	if (u16version == 0x0400) {
		temp = readl(pres->sdmd_atsc_pridata.virt_ts_base_addr + 0x04);
		temp &= 0xfeef;
		writel(temp, pres->sdmd_atsc_pridata.virt_ts_base_addr + 0x04);
		sys.delayms(1);
		temp = readl(pres->sdmd_atsc_pridata.virt_ts_base_addr + 0x04);
		temp |= 0x0110;
		writew(temp, pres->sdmd_atsc_pridata.virt_ts_base_addr + 0x04);
		sys.delayms(1);
	} else if (u16version == 0x0500) {
		writel(0x00000000, pres->sdmd_atsc_pridata.virt_demod_base_addr);
		sys.delayms(1);
		writel(0x00000011, pres->sdmd_atsc_pridata.virt_demod_base_addr);
		sys.delayms(1);
	}
	...
	if (u16version >= 0x0400) {
		...
		writel(temp, pres->sdmd_atsc_pridata.virt_efuse_base_addr + (0x28<<2));

		temp = readl(pres->sdmd_atsc_pridata.virt_efuse_base_addr + (0x2d<<2));
		u32_efuse_value = (temp<<16);
		temp = readl(pres->sdmd_atsc_pridata.virt_efuse_base_addr + (0x2c<<2));
		u32_efuse_value |= temp;

		check_val = (u32_efuse_value>>15) & 0x01;
		if (check_val != 0) {
			reg = _hal_dmd_riu_readbyte(DEMOD_TOP_REG_BASE + 0x01*2 + 1);
			reg &= (~0x20);
			mtk_demod_write_byte(DEMOD_TOP_REG_BASE + 0x01*2 + 1, reg);

			reg = _hal_dmd_riu_readbyte(ANA_REG_BASE + 0x47*2);
			reg &= ~(0x1f);
			reg |= (u8)((u32_efuse_value>>8)&0x1f);
			_riu_write_byte(ANA_REG_BASE + 0x47*2, reg);
		} //if (check_val != 0)

	}

	#if FW_RUN_ON_DRAM_MODE
	if (u16version < 0x0500) {
		temp1 = readl(pres->sdmd_atsc_pridata.virt_mppl_base_addr + 0x0c8) & 0xFFFF0000;
		temp1 |= 0x1100;
		writel(temp1, pres->sdmd_atsc_pridata.virt_mppl_base_addr + 0x0c8);

		temp1 = readl(pres->sdmd_atsc_pridata.virt_mppl_base_addr + 0x154) & 0xFFFFF0FF;
		writel(temp1, pres->sdmd_atsc_pridata.virt_mppl_base_addr + 0x154);
	}
	...
	// Start IMI SRAM initial setting
	if (u16version == 0x0500) {
		temp1 = readl(pres->sdmd_atsc_pridata.virt_demod_base_addr + 0x0004) & 0xFFFFFFFE;
		writel(temp1, pres->sdmd_atsc_pridata.virt_demod_base_addr + 0x0004);
	} else {
		temp1 = readl(pres->sdmd_atsc_pridata.virt_sram_base_addr) & 0xFFFFFFFE;
		writel(temp1, pres->sdmd_atsc_pridata.virt_sram_base_addr);
	}
	/* [3:0] */
	// IFAGC_DATA digital pad GPIO setting for overlap UART(from mt5876)
	if (u16version < 0x0300) {
		temp1 = readl(pres->sdmd_atsc_pridata.virt_pmux_base_addr) & 0xFFFFFFF0;
		temp1 |= 0x00000003;
		writel(temp1, pres->sdmd_atsc_pridata.virt_pmux_base_addr);
	}
	/* [11:8] Tuner GPIO setting for overlap headphone GPIO(mt5879), USB(mt5876) */
	//temp1 = readl(pres->sdmd_atsc_pridata.virt_vagc_base_addr) & 0xFFFFF0FF;
	//temp1 |= 0x00000300;
	//writel(temp1, pres->sdmd_atsc_pridata.virt_vagc_base_addr);
	...
	if (u16version == 0x0400) {
		temp1 = readl(pres->sdmd_atsc_pridata.virt_clk_base_addr + 0x1F84) & 0xFFFFFEEE;
		temp1 |= 0x00000111;
		writel(temp1, (pres->sdmd_atsc_pridata.virt_clk_base_addr + 0x1F84));
	...
	#endif //#if SUPPORT_IO_REMAP

	#if FW_RUN_ON_DRAM_MODE
	_riu_write_byte(MCU2RegBase + 0xE0, 0x23);
	_riu_write_byte(MCU2RegBase + 0xE1, 0x21);
	_riu_write_byte(MCU2RegBase + 0xE4, 0x01);
	_riu_write_byte(MCU2RegBase + 0xE6, 0x11);
	_riu_write_byte(MCU2RegBase + 0x00, 0x00);
	_riu_write_byte(MCU2RegBase + 0x01, 0x00);
	_riu_write_byte(MCU2RegBase + 0x02, 0x00);
	_riu_write_byte(MCU2RegBase + 0x03, 0x00);
	_riu_write_byte(MCU2RegBase + 0x04, (SRAM_OFFSET & 0xFF));
	_riu_write_byte(MCU2RegBase + 0x05, (SRAM_OFFSET >> 8));
	_riu_write_byte(MCU2RegBase + 0x06, 0x00);
	_riu_write_byte(MCU2RegBase + 0x07, 0x00);
	_riu_write_byte(MCU2RegBase + 0x08, 0x00);
	_riu_write_byte(MCU2RegBase + 0x09, 0x00);
	_riu_write_byte(MCU2RegBase + 0x0A, 0x01);
	_riu_write_byte(MCU2RegBase + 0x0B, 0x00);
	_riu_write_byte(MCU2RegBase + 0x0C, 0x00);
	_riu_write_byte(MCU2RegBase + 0x0D, 0x00);
	u16_temp_val = SRAM_OFFSET - 1;
	_riu_write_byte(MCU2RegBase + 0x0E, (u16_temp_val & 0xFF));
	_riu_write_byte(MCU2RegBase + 0x0F, (u16_temp_val >> 8));
	_riu_write_byte(MCU2RegBase + 0x18, 0x04);
	//temp = (u16)(((pres->sdmd_atsc_initdata.utdistartaddr32 * 16))>>16);
	temp = (u16)(pres->sdmd_atsc_pridata.utdistartaddr32>>16);
	_riu_write_byte(MCU2RegBase + 0x1A, (u8)temp);
	_riu_write_byte(MCU2RegBase + 0x1B, (u8)(temp>>8));
	_riu_write_byte(MCU2RegBase + 0x1C, 0x01);
	_riu_write_byte(MCU2RegBase + 0x20, 0x00);
	reg = _riu_read_byte(TOPRegBase + 0x01);
	reg &= (~0x10);
	_riu_write_byte(TOPRegBase + 0x01, reg);

	//after m6l, it should check MCU ready or not
	while (_hal_dmd_riu_readbyte(DMDMcuRstState+0x04) != 0xf) {
		_hal_dmd_riu_writebyteMask(DMD_MCU_BASE+0x00, 0x01, 0x01); // reset VD_MCU
		HAL_INTERN_ATSC_DBINFO(PRINT(">MCU is ready!\n\r"));
		_hal_dmd_riu_writebyteMask(DMD_MCU_BASE+0x00, 0x00, 0x03); // release VD_MCU
	}
...
static u8/*bool*/ _hal_intern_atsc_download(void)
{
	...
	ATSC_table = &INTERN_ATSC_TABLE[0];
	lib_size = sizeof(INTERN_ATSC_TABLE);
	...
	#if defined MTK_PROJECT
	va_dramcodeaddr = (MS_PHY)BSP_MapReservedMem(
	(void *)(pres->sdmd_atsc_pridata.utdistartaddr32 * 16), 1024*1024);
	#elif DMD_ATSC_3PARTY_EN
	// Please transform DRAM virtual address according to 3 party platform
	//va_dramcodeaddr = ;
	#else
	va_dramcodeaddr = (pres->sdmd_atsc_pridata.virt_dram_base_addr);
	#endif
	va_dramcodeaddr = (pres->sdmd_atsc_pridata.virt_dram_base_addr);
	//PRINT(">>> va_dramcodeaddr=0x%lx\n", va_dramcodeaddr);

	MEMCPY(va_dramcodeaddr, &ATSC_table[0], lib_size);

	uaddressOffset16 = (ATSC_table[0x400] << 8) | ATSC_table[0x401];

	tmpdata = (u8)pres->sdmd_atsc_initdata.if_khz;
	MEMCPY((va_dramcodeaddr + uaddressOffset16), &tmpdata, 1);
	tmpdata = (u8)(pres->sdmd_atsc_initdata.if_khz >> 8);
	MEMCPY((va_dramcodeaddr + uaddressOffset16 + 1), &tmpdata, 1);
	tmpdata = (u8)pres->sdmd_atsc_initdata.biqswap;
	MEMCPY((va_dramcodeaddr + uaddressOffset16 + 2), &tmpdata, 1);
	tmpdata = (u8)pres->sdmd_atsc_initdata.agc_reference;
	MEMCPY((va_dramcodeaddr + uaddressOffset16 + 3), &tmpdata, 1);
	tmpdata = (u8)(pres->sdmd_atsc_initdata.agc_reference >> 8);
	MEMCPY((va_dramcodeaddr + uaddressOffset16 + 4), &tmpdata, 1);
	tmpdata = (u8)pres->sdmd_atsc_pridata.utdistartaddr32;
	MEMCPY((va_dramcodeaddr + uaddressOffset16 + 5), &tmpdata, 1);
	tmpdata = (u8)(pres->sdmd_atsc_pridata.utdistartaddr32 >> 8);
	MEMCPY((va_dramcodeaddr + uaddressOffset16 + 6), &tmpdata, 1);
	tmpdata = (u8)(pres->sdmd_atsc_pridata.utdistartaddr32 >> 16);
	MEMCPY((va_dramcodeaddr + uaddressOffset16 + 7), &tmpdata, 1);
	tmpdata = (u8)(pres->sdmd_atsc_pridata.utdistartaddr32 >> 24);
	MEMCPY((va_dramcodeaddr + uaddressOffset16 + 8), &tmpdata, 1);
	tmpdata = (u8)pres->sdmd_atsc_initdata.btunergaininvert;
	MEMCPY((va_dramcodeaddr + uaddressOffset16 + 9), &tmpdata, 1);
	tmpdata = (u8)(pres->sdmd_atsc_initdata.m6_version >> 8);
	MEMCPY((va_dramcodeaddr + uaddressOffset16 + 10), &tmpdata, 1);

	#if defined MTK_PROJECT
	BSP_UnMapReservedMem((void *)(pres->sdmd_atsc_pridata.utdistartaddr32 * 16)
		+ (3 * 1024 * 1024));
	#endif
	HAL_INTERN_ATSC_DBINFO(PRINT(">Load DRAM code done...\n"));
...
static u8/*bool*/ _hal_intern_atsc_exit(void)
{
	...
	TZ_DEMOD_RESET(0);
	#elif DVB_STI
	if ((pres->sdmd_atsc_pridata.m6_version & 0xFF0F) < 0x0400) {
		writel(0x00000000, pres->sdmd_atsc_pridata.virt_reset_base_addr);
		sys.delayms(1);
		writel(0x00000011, pres->sdmd_atsc_pridata.virt_reset_base_addr);
	} else if ((pres->sdmd_atsc_pridata.m6_version & 0xff00) == 0x0400) {
		temp1 = readl(pres->sdmd_atsc_pridata.virt_clk_base_addr + 0x1F84) & 0xFFFFFFFE;
		temp1 |= 0x00000001;
		writel(temp1, (pres->sdmd_atsc_pridata.virt_clk_base_addr + 0x1F84));
		sys.delayms(1);
...
static u8/*bool*/ _hal_intern_atsc_setvsbmode(void)
{
	...
	#else
	if (_mbx_writereg(0x20C2, 0x02) == FALSE)
		return FALSE;
	_mbx_readreg(0x20C2, &data_8);

	while	(!(data_8	& 0x20)) {
	PRINT("kavana check demod control bit = %d!!\n\r",data_8);
		//_mbx_writereg(0x20C2, 0x02);
		_mbx_readreg(0x20C2, &data_8);
	}
	PRINT("kavana check demod atcive VSB mode = %d!!\n\r",data_8);
...
static u8/*bool*/ _hal_intern_atsc_set64qammode(void)
{
	...
	#endif
	PRINT("kavana check demod atcive 64Q mode!!\n\r");
	return _mbx_writereg(0x20C0, 0x04);
}
...
static u8/*bool*/ _hal_intern_atsc_set256qammode(void)
{
	...
	#endif
	PRINT("kavana check demod atcive 256Q mode!!\n\r");
	return _mbx_writereg(0x20C0, 0x04);
}
