...
static void _hal_intern_isdbt_initclk(void)
{
	...
	HAL_INTERN_ISDBT_DBINFO(PRINT("--DMD_ISDBT_CHIP_MERAK--\n"));
	ip_version = (u16)pres->sdmd_isdbt_initdata.ip_version;

	writel(0x00000000, pres->sdmd_isdbt_pridata.virt_reset_base_addr);
	sys.delayms(1);
	writel(0x00000011, pres->sdmd_isdbt_pridata.virt_reset_base_addr);
	sys.delayms(1);
	...
	if (ip_version < MT5876_IP_VER) {//5896/5897
	...
	if (ip_version < MT5876_IP_VER) {
		temp1 = readl(pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x14F0);// & 0xFFFFE0FF;
		temp1 |= 0x00001F00;
		writel(temp1, (pres->sdmd_isdbt_pridata.virt_clk_base_addr + 0x14F0));
	}
	...
	if (ip_version == MT5896_IP_VER) {
		PRINT("------DMD_ISDBT_CHIP_MT5896-E1E2-----\n");
		writel(0x000000FC, pres->sdmd_isdbt_pridata.virt_sram_base_addr);
		_hal_dmd_riu_writebyte(MBRegBase + 0x1A, 0x00);
	} else if (ip_version < MT5876_IP_VER) {
		if (ip_version < MT5897_IP_VER) {
			PRINT("------DMD_ISDBT_CHIP_MT5896E3-----\n");
			_hal_dmd_riu_writebyte(MBRegBase + 0x1A, 0x00);
		} else {
			PRINT("------DMD_ISDBT_CHIP_MT5897-----\n");
			_hal_dmd_riu_writebyte(MBRegBase + 0x1A, 0x01);
		}

		temp1 = readl(pres->sdmd_isdbt_pridata.virt_sram_base_addr) & 0xFFFFFFFE;
		writel(temp1, (pres->sdmd_isdbt_pridata.virt_sram_base_addr));
