...
ssize_t demod_hal_dvbc_get_information(struct device_driver *driver, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE,
		"%s%s%s%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%s%s %s%s%s%s %s%s%d.%d%s %s%s%d.%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%s%s %s%s%d%s %s%s%d%s %s%s%d%s",
		...
		SM_C_N, SM_COMMA, cnr/TEN_FOR_DECIMAL, cnr%TEN_FOR_DECIMAL, SM_END, // SD_SM
