		...
		*msf_snr = mtk_demod_float_op(*msf_snr,
		...
		if (snr_cali == 1) {
			if (u8_gi == 0) {
				...
				msf_snr_offset.data = 0;//For customer Low SQI problem
				msf_snr_offset.exp = 0;
...
ssize_t demod_hal_dvbt_t2_get_information(struct device_driver *driver, char *buf)
{
	...
	if (lock_signal == T2_SIGNAL) {
		...
		switch ((reg>>3)&0x03) {
		case 0:
			strscpy(modulation, DVBT2_MODULATION[0], sizeof(modulation));//QPSK;
			break;
		case 1:
			strscpy(modulation, DVBT2_MODULATION[1], sizeof(modulation));//QAM_16;
			break;
		case 2:
			strscpy(modulation, DVBT2_MODULATION[2], sizeof(modulation));//QAM_64;
			break;
		case 3:
		default:
			strscpy(modulation, DVBT2_MODULATION[3], sizeof(modulation));//QAM_256;
			break;
		}
