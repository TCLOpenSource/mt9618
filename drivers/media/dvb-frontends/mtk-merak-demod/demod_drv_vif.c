...
#define EXTEND_FOE_HB 850000
#define EXTEND_FOE_LB 749000
...
u16 vif_coef_custom[EQ_COEF_LEN] = {0};

bool is_atv;

const u16 vif_coef_ori[EQ_COEF_LEN] = {
...
s32 drv_vif_foe2hz(u8 foe, int tuner_quarter_step)
{
	...
	// support bigger locking range using fakke offset
	if ((ret > EXTEND_FOE_LB) && (ret < EXTEND_FOE_HB))
		ret = EXTEND_FOE_LB;
	else if ((ret < (-1*EXTEND_FOE_LB)) && (ret > (-1*EXTEND_FOE_HB)))
		ret = (-1*EXTEND_FOE_LB);
...
u8 _ascii2dec(u8 ascii_num)
{
	u8 val = 0;

	if (((ascii_num >= ASCII_A) && (ascii_num <= ASCII_F)) ||
		((ascii_num >= ASCII_a) && (ascii_num <= ASCII_f)))
		val = (ascii_num & 0x0F) + 0x09;
	else if ((ascii_num >= ASCII_0) && (ascii_num <= ASCII_9))
		val = ascii_num & 0x0F;
	else
		pr_emerg("[%s][%d] Input data is wrong\n", __func__, __LINE__);

	return val;
}

void _region_model_selected(void)
{
	u16 tmp = 0;
	u16 coeff_cnt = 0;
	...
	for (coeff_cnt = 0; coeff_cnt < EQ_COEF_LEN; coeff_cnt++) {
		if (vif_init_inst.custom_file_open != 0)
			tmp = vif_coef_custom[coeff_cnt];
		else
			tmp = vif_coef_ori[coeff_cnt];

		vif_coef[coeff_cnt] = tmp;
	}
}
...
bool drv_vif_set_filter_coef(void)
{
	bool ret = FALSE;

	//pr_debug("vif_coef[0] = %x\n", vif_coef[0]);
	//pr_debug("vif_coef[786] = %x\n", vif_coef[786]);

	_region_model_selected();

	ret = hal_vif_filter_coef_download();

	return ret;
}

void drv_vif_coef_mapping(const struct firmware *fw)
{
	u16 i = 0, j = 0;

	// check if "end" is exist or not
	while (!((*(fw->data + i) == ASCII_e) && (*(fw->data + i + 1) == ASCII_n) &&
						(*(fw->data + i + 2) == ASCII_d))) {
		// check if "0x" is exist or not
		if ((*(fw->data + i) == ASCII_0) && (*(fw->data + i + 1) == ASCII_x)) {
			vif_coef_custom[j] = ((_ascii2dec(*(fw->data + i + 2)) << 12) +
					(_ascii2dec(*(fw->data + i + 3)) << 8) +
					(_ascii2dec(*(fw->data + i + 4)) << 4) +
						_ascii2dec(*(fw->data + i + 5)));
			j++;
		}
		i++;
	}
...
struct dvb_frontend *fe, struct dtv_frontend_properties *p)
{
	...
	// support bigger locking range using fakke offset
	if ((freq_offset_hz > EXTEND_FOE_LB) && (freq_offset_hz < EXTEND_FOE_HB))
		freq_offset_hz = EXTEND_FOE_LB;
	else if ((freq_offset_hz < (-1*EXTEND_FOE_LB)) && (freq_offset_hz > (-1*EXTEND_FOE_HB)))
		freq_offset_hz = (-1*EXTEND_FOE_LB);
...
ssize_t drv_vif_fetch_information(char *buf)
{
	...
	u8 demod_rev = 0;
	s32 freqoffset_hz = 0;
	s32 freqoffset_khz = 0;
	char in_khz[8] = " kHz";
	char lock_msg[2][8] = {"Unlock", "Lock"};
	...
	if ((foe&0x80) != 0) {
		...
		freqoffset_hz = foe * 15625;
	freqoffset_khz = freqoffset_hz/1000;
	...
	return scnprintf(buf, PAGE_SIZE, "%s%s%s%s %s%s%d%s %s%s%d%s%s %s%s%d%s %s%s%s%s",
	SM_CHIP_REV, SM_COMMA, SM_NONE, SM_END,
	SM_SYSTEM, SM_COMMA, system, SM_END,
	SM_VISION_CARRIER_OFFSET, SM_COMMA, freqoffset_khz, in_khz, SM_END,
	SM_AGC, SM_COMMA, ifagc, SM_END,
	SM_DEMOD_LOCK, SM_COMMA, lock_msg[lock_status], SM_END);
}
