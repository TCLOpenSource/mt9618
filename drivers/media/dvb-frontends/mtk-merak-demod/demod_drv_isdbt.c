...
#define SNR_COREFFICIENT 100
#define BER_ERR_WINDOW_COE 192512
#define BER_ERR_CNT_R 5194
#define BER_ERR_WIN_R 100
#define SQI_NORMAL_WIN 325
#define SQI_QPSK_WIN 270
#define SQI_COREFFICIENT 100
//#define T_STABLE 3000
#define MAX_SEG 13
#define UNKNOWN_MOD 7
#define MAX_SQI 100
#define CHAR_LEN 8
//-----------------------------------------------------------------
//  Local Structurs
...
static u16 u16isdbttotalerrpkt_a, u16isdbttotalerrpkt_b, u16isdbttotalerrpkt_c;
...
#endif
{
	...
	pres->sdmd_isdbt_initdata.virt_riu_base_addr;

	pres->sdmd_isdbt_pridata.virt_reset_base_addr =
	pres->sdmd_isdbt_initdata.virt_reset_base_addr;

	pres->sdmd_isdbt_pridata.virt_ts_base_addr =
	pres->sdmd_isdbt_initdata.virt_ts_base_addr;

	pres->sdmd_isdbt_pridata.virt_clk_base_addr =
	pres->sdmd_isdbt_initdata.virt_clk_base_addr;

	pres->sdmd_isdbt_pridata.virt_sram_base_addr =
	pres->sdmd_isdbt_initdata.virt_sram_base_addr;

	pres->sdmd_isdbt_pridata.virt_pmux_base_addr =
	pres->sdmd_isdbt_initdata.virt_pmux_base_addr;

	pres->sdmd_isdbt_pridata.virt_vagc_base_addr =
	pres->sdmd_isdbt_initdata.virt_vagc_base_addr;
	#endif
	...
	if (_isdbt_cma_pool_init_param.heap_id != 0x7F) {
		...
		if (MApi_CMA_Pool_Init(&_isdbt_cma_pool_init_param) == FALSE)
			DMD_DBG(PRINT(
			"\033[35mFunction = %s, Line = %d, get MApi_CMA_Pool_Init ERROR!!!\n",
			__PRETTY_FUNCTION__, __LINE__));
		else {
			DMD_DBG(PRINT(
	"\033[35mFunction = %s, Line = %d, get pool_handle_id is %u\033[m\n",
			__PRETTY_FUNCTION__, __LINE__,
			_isdbt_cma_pool_init_param.pool_handle_id));
			DMD_DBG(PRINT(
			"\033[35mFunction = %s, Line = %d, get miu is %u\033[m\n",
			__PRETTY_FUNCTION__, __LINE__,
			_isdbt_cma_pool_init_param.miu));
			DMD_DBG(PRINT(
"\033[35mFunction = %s, Line = %d, get heap_miu_start_offset is 0x%llX\033[m\n",
			__PRETTY_FUNCTION__, __LINE__,
			_isdbt_cma_pool_init_param.heap_miu_start_offset));
			DMD_DBG(PRINT(
		"\033[35mFunction = %s, Line = %d, get heap_length is 0x%X\033[m\n",
			__PRETTY_FUNCTION__, __LINE__,
			_isdbt_cma_pool_init_param.heap_length));
		}
		...
		if (MApi_CMA_Pool_GetMem(&_isdbt_cma_alloc_param) == FALSE)
			DMD_DBG(PRINT(
		"\033[35mFunction = %s, Line = %d, CMA_Pool_GetMem ERROR!!\033[m\n",
		__PRETTY_FUNCTION__, __LINE__));
		else {
			DMD_DBG(PRINT(
	...
	u16isdbttotalerrpkt_a = 0;
	u16isdbttotalerrpkt_b = 0;
	u16isdbttotalerrpkt_c = 0;
...
s32 _isdbt_get_information_getfreqoffset(struct dmd_isdbt_cfo_data *isdbt_cfo_data)
{
...
u8 _isdbt_get_information_segment_count(u8 u8temp)
{
	u8 layer_segment_count = 0;

	if ((u8temp != 0x0F) && (u8temp != 0x00))
		layer_segment_count = u8temp;
	else
		layer_segment_count = 0;

	return layer_segment_count;
}

u8 _isdbt_get_information_tmcc_lock(u8 u8temp, u8 reg)
{
	u8 isdbt_tmcc_det_flag = 0;

	if ((u8temp == 0x20) && (reg == 0x01))
		isdbt_tmcc_det_flag = 1;
	else
		isdbt_tmcc_det_flag = 0;

	return isdbt_tmcc_det_flag;
}
...
static void _get_ber_string(u32 u32berdata, char *charberdata)
{
	u8 u8berdigi_1 = 0, u8berdigi_2 = 0, u8berdigi_3 = 0, u8temp = 0;

	for (u8temp = 0; u32berdata >= 100; u8temp++)
		u32berdata = u32berdata/10;
	u8berdigi_3 = u32berdata/10;
	u8berdigi_2 = u32berdata - u8berdigi_3 * 10;
	u8berdigi_1 = 7 - u8temp - 1;

if ((u8berdigi_3 == 0) && (u8berdigi_2 == 0))
	strncpy(charberdata, "0.0e+0", CHAR_LEN);
else if (snprintf(charberdata, CHAR_LEN, "%d.%de-%d", u8berdigi_3, u8berdigi_2, u8berdigi_1) < 0)
	DMD_DBG(PRINT("[%s] snprintf Fail\n", __func__));

}

ssize_t isdbt_get_information(char *buf)
{
	u8 reg = 0, system, ifagc_val = 0, u8temp = 0, isdbt_ewbs_flag = 0;
	u8 isdbt_lock_flag = 0, isdbt_tmcc_det_flag = 0;
	u8 isdbt_ts_lock_flag = 0;
	u16 u16packet_err_a, u16packet_err_b, u16packet_err_c, u16tmp = 0;
	char chfftsize[CHAR_LEN] = "Invalid";
	char chisdbt_ts_lock_flag[CHAR_LEN] = "Unlock";
	char chisdbt_lock_flag[CHAR_LEN] = "Unlock";
	char chguard_interval[CHAR_LEN] = "Invalid";
	char chlayerA_modulation[CHAR_LEN] = "Invalid";
	char chlayerB_modulation[CHAR_LEN] = "Invalid";
	char chlayerC_modulation[CHAR_LEN] = "Invalid";
	char chlayerA_coderate[CHAR_LEN] = "Invalid";
	char chlayerB_coderate[CHAR_LEN] = "Invalid";
	char chlayerC_coderate[CHAR_LEN] = "Invalid";
	char chsnr[CHAR_LEN] = "Invalid";
	char chlayerA_interleaving[CHAR_LEN] = "0";
	char chlayerB_interleaving[CHAR_LEN] = "0";
	char chlayerC_interleaving[CHAR_LEN] = "0";
	char chlayerA_post_ber[CHAR_LEN] = "Invalid";
	char chlayerB_post_ber[CHAR_LEN] = "Invalid";
	char chlayerC_post_ber[CHAR_LEN] = "Invalid";

	u8 u8flag = 0;

	u32 demod_rev, snr, u32temp = 0;
	s32 freq_offset_khz = 0;

	u8 transmission_mode = 0, guard_interval = 0;
	u8 layerA_modulation = 0, layerB_modulation = 0, layerC_modulation = 0;
	u8 layerA_fec = 0, layerB_fec = 0, layerC_fec = 0;
	u8 layerA_interleaving = 0, layerB_interleaving = 0, layerC_interleaving = 0;
	u8 layerA_segment_count = 0, layerB_segment_count = 0, layerC_segment_count = 0;
	//u8 isdbt_partial_reception = 0;

	u32 layerA_post_ber = 0, layerB_post_ber = 0, layerC_post_ber = 0;

	struct dmd_isdbt_cfo_data isdbt_cfo_data = {0, 0, 0, 0};
	struct dmd_isdbt_snr_data isdbt_snr = {0, 0};
	struct isdbt_modulation_mode sisdbt_modulation_mode = {0, 0, 0, 0, 0};
	struct dmd_isdbt_get_ber_value sisdbt_get_ber_value = {0, 1, 1};

	pr_info("[%s] is called\n", __func__);

    /* Demod Rev. */
	_mdrv_dmd_isdbt_md_getreg(0, 0x20C4, &reg);//_mbx_readreg(0x20C4, &data1);
	demod_rev = reg;
	_mdrv_dmd_isdbt_md_getreg(0, 0x20C5, &reg);//_mbx_readreg(0x20C5, &data2);
	demod_rev = (demod_rev<<8) | reg;
	_mdrv_dmd_isdbt_md_getreg(0, 0x20C6, &reg);//_mbx_readreg(0x20C6, &data3);
	demod_rev = (demod_rev<<8) | reg;
	/* System */
	system = SYS_ISDBT;
    /* Freq. Offset */
	_mdrv_dmd_isdbt_md_getfreqoffset(0, &isdbt_cfo_data);
	freq_offset_khz = _isdbt_get_information_getfreqoffset(&isdbt_cfo_data);

    /* AGC */
	_mdrv_dmd_isdbt_md_setreg(0, 0x2822, 0x03);
	_mdrv_dmd_isdbt_md_getreg(0, 0x2825, &reg);
	ifagc_val = reg & 0xFF;
	/* CNR */
	if (_mdrv_dmd_isdbt_md_getlock(0, DMD_ISDBT_GETLOCK) == DMD_ISDBT_LOCK) {
		u32temp = 0;
		for (u8temp = 0; u8temp < 2; ++u8temp) {
			_mdrv_dmd_isdbt_md_getsnr(0, &isdbt_snr);
			u32temp += (s32)(isdbt_snr.reg_snr);
			//_delayms(1);
		}
		u32temp /= 10;
		u32temp >>= 1;
	...
	//_delayms(T_STABLE);
	_mdrv_dmd_isdbt_md_read_pkt_err(0, E_ISDBT_Layer_A, &u16tmp);
	u16packet_err_a = u16tmp;
	_mdrv_dmd_isdbt_md_read_pkt_err(0, E_ISDBT_Layer_B, &u16tmp);
	u16packet_err_b = u16tmp;
	_mdrv_dmd_isdbt_md_read_pkt_err(0, E_ISDBT_Layer_C, &u16tmp);
	u16packet_err_c = u16tmp;
	...
	u8flag = (((u32)u16isdbttotalerrpkt_a + (u32)u16packet_err_a) >= 65535);

	u16isdbttotalerrpkt_a = u8flag*65535 + (1-u8flag)*(u16isdbttotalerrpkt_a + u16packet_err_a);

	u8flag = (((u32)u16isdbttotalerrpkt_b + (u32)u16packet_err_b) >= 65535);

	u16isdbttotalerrpkt_b = u8flag*65535 + (1-u8flag)*(u16isdbttotalerrpkt_b + u16packet_err_b);

	u8flag = (((u32)u16isdbttotalerrpkt_c + (u32)u16packet_err_c) >= 65535);

	u16isdbttotalerrpkt_c = u8flag*65535 + (1-u8flag)*(u16isdbttotalerrpkt_c + u16packet_err_c);

	snr = u32temp;
	if (snprintf(chsnr, CHAR_LEN, "%d%d.%d", u32temp/100, (u32temp/10)%10, u32temp%10) < 0)
		DMD_DBG(PRINT("[%s] snprintf Fail\n", __func__));

    /* TMCC Det */
	_mdrv_dmd_isdbt_md_getreg(0, 0x1518, &u8temp);
	_mdrv_dmd_isdbt_md_getreg(0, 0x1519, &reg);
	u8temp &= 0x20;
	reg &= 0x01;
	isdbt_tmcc_det_flag = _isdbt_get_information_tmcc_lock(u8temp, reg);

	/* Mode */
	/* Guard */
	_mdrv_dmd_isdbt_md_get_modulation_mode(0, E_ISDBT_Layer_A, &sisdbt_modulation_mode);

	switch (sisdbt_modulation_mode.eisdbtfft) {
	case E_ISDBT_FFT_2K:
		strncpy(chfftsize, "mode1", CHAR_LEN);
		transmission_mode = 0;
		break;
	case E_ISDBT_FFT_4K:
		strncpy(chfftsize, "mode2", CHAR_LEN);
		transmission_mode = 1;
		break;
	case E_ISDBT_FFT_8K:
		strncpy(chfftsize, "mode3", CHAR_LEN);
		transmission_mode = 2;
		break;
	case E_ISDBT_FFT_INVALID:
	default:
		strncpy(chfftsize, "Invalid", CHAR_LEN);
		transmission_mode = 3;
		break;
	}
	...
	case E_ISDBT_GUARD_INTERVAL_1_32:
		strncpy(chguard_interval, "1/32", CHAR_LEN);
		guard_interval = 0;
		break;
	case E_ISDBT_GUARD_INTERVAL_1_16:
		strncpy(chguard_interval, "1/16", CHAR_LEN);
		guard_interval = 1;
		break;
	case E_ISDBT_GUARD_INTERVAL_1_8:
		strncpy(chguard_interval, "1/8", CHAR_LEN);
		guard_interval = 2;
		break;
	case E_ISDBT_GUARD_INTERVAL_1_4:
		strncpy(chguard_interval, "1/4", CHAR_LEN);
		guard_interval = 3;
		break;
	case E_ISDBT_GUARD_INTERVAL_INVALID:
	default:
		strncpy(chguard_interval, "Invalid", CHAR_LEN);
		guard_interval = 4;
		break;
	}
    /* A/B/C_Modulation */
	...
	case E_ISDBT_DQPSK:
		strncpy(chlayerA_modulation, "DQPSK", CHAR_LEN);
		layerA_modulation = 0;
		break;
	case E_ISDBT_QPSK:
		strncpy(chlayerA_modulation, "QPSK", CHAR_LEN);
		layerA_modulation = 1;
		break;
	case E_ISDBT_16QAM:
		strncpy(chlayerA_modulation, "16QAM", CHAR_LEN);
		layerA_modulation = 2;
		break;
	case E_ISDBT_64QAM:
		strncpy(chlayerA_modulation, "64QAM", CHAR_LEN);
		layerA_modulation = 3;
		break;
	...
	case E_ISDBT_CODERATE_1_2:
		strncpy(chlayerA_coderate, "1/2", CHAR_LEN);
		layerA_fec = 1;
		break;
	case E_ISDBT_CODERATE_2_3:
		strncpy(chlayerA_coderate, "2/3", CHAR_LEN);
		layerA_fec = 2;
		break;
	case E_ISDBT_CODERATE_3_4:
		strncpy(chlayerA_coderate, "3/4", CHAR_LEN);
		layerA_fec = 3;
		break;
	case E_ISDBT_CODERATE_5_6:
		strncpy(chlayerA_coderate, "5/6", CHAR_LEN);
		layerA_fec = 4;
		break;
	case E_ISDBT_CODERATE_7_8:
		strncpy(chlayerA_coderate, "7/8", CHAR_LEN);
		layerA_fec = 5;
		break;
	...
	case E_ISDBT_8K_TDI_0:
		strncpy(chlayerA_interleaving, "0", CHAR_LEN);
		layerA_interleaving = 0;
		break;
	case E_ISDBT_8K_TDI_1:
		strncpy(chlayerA_interleaving, "1", CHAR_LEN);
		layerA_interleaving = 1;
		break;
	case E_ISDBT_4K_TDI_2:
	case E_ISDBT_8K_TDI_2:
		strncpy(chlayerA_interleaving, "2", CHAR_LEN);
		layerA_interleaving = 2;
		break;
	case E_ISDBT_2K_TDI_4:
	case E_ISDBT_4K_TDI_4:
	case E_ISDBT_8K_TDI_4:
		strncpy(chlayerA_interleaving, "4", CHAR_LEN);
		layerA_interleaving = 4;
		break;
	case E_ISDBT_2K_TDI_8:
	case E_ISDBT_4K_TDI_8:
		strncpy(chlayerA_interleaving, "8", CHAR_LEN);
		layerA_interleaving = 8;
		break;
	case E_ISDBT_2K_TDI_16:
		strncpy(chlayerA_interleaving, "16", CHAR_LEN);
		layerA_interleaving = 16;
		break;
	...
	//_mdrv_dmd_isdbt_md_getreg(0, 0x1508, &u8temp);
	//isdbt_partial_reception = ((u8temp&0x02) == 0x02) ? 1:0;

	_mdrv_dmd_isdbt_md_getreg(0, 0x150A, &u8temp);
	u8temp &= 0x0F;

	layerA_segment_count = _isdbt_get_information_segment_count(u8temp);

	sisdbt_get_ber_value.eisdbtlayer = E_ISDBT_Layer_A;
	_mdrv_dmd_isdbt_md_get_postviterbi_ber(0, &sisdbt_get_ber_value);
	layerA_post_ber =
(sisdbt_get_ber_value.bervalue*5194) / (sisdbt_get_ber_value.berperiod*100);

	_get_ber_string(layerA_post_ber, chlayerA_post_ber);

	// layerB information
	_mdrv_dmd_isdbt_md_get_modulation_mode(0, E_ISDBT_Layer_B, &sisdbt_modulation_mode);

	switch (sisdbt_modulation_mode.eisdbtconstellation) {
	case E_ISDBT_DQPSK:
		strncpy(chlayerB_modulation, "DQPSK", CHAR_LEN);
		layerB_modulation = 0;
		break;
	case E_ISDBT_QPSK:
		strncpy(chlayerB_modulation, "QPSK", CHAR_LEN);
		layerB_modulation = 1;
		break;
	case E_ISDBT_16QAM:
		strncpy(chlayerB_modulation, "16QAM", CHAR_LEN);
		layerB_modulation = 2;
		break;
	case E_ISDBT_64QAM:
		strncpy(chlayerB_modulation, "64QAM", CHAR_LEN);
		layerB_modulation = 3;
		break;
	case E_ISDBT_QAM_INVALID:
	default:
	...
	switch (sisdbt_modulation_mode.eisdbtcoderate) {
	case E_ISDBT_CODERATE_1_2:
		strncpy(chlayerB_coderate, "1/2", CHAR_LEN);
		layerB_fec = 1;
		break;
	case E_ISDBT_CODERATE_2_3:
		strncpy(chlayerB_coderate, "2/3", CHAR_LEN);
		layerB_fec = 2;
		break;
	case E_ISDBT_CODERATE_3_4:
		strncpy(chlayerB_coderate, "3/4", CHAR_LEN);
		layerB_fec = 3;
		break;
	case E_ISDBT_CODERATE_5_6:
		strncpy(chlayerB_coderate, "5/6", CHAR_LEN);
		layerB_fec = 4;
		break;
	case E_ISDBT_CODERATE_7_8:
		strncpy(chlayerB_coderate, "7/8", CHAR_LEN);
		layerB_fec = 5;
		break;
	case E_ISDBT_CODERATE_INVALID:
	default:
	...
	case E_ISDBT_4K_TDI_0:
	case E_ISDBT_8K_TDI_0:
		strncpy(chlayerB_interleaving, "0", CHAR_LEN);
		layerB_interleaving = 0;
		break;
	case E_ISDBT_8K_TDI_1:
		strncpy(chlayerB_interleaving, "1", CHAR_LEN);
		layerB_interleaving = 1;
		break;
	case E_ISDBT_4K_TDI_2:
	case E_ISDBT_8K_TDI_2:
		strncpy(chlayerB_interleaving, "2", CHAR_LEN);
		layerB_interleaving = 2;
		break;
	case E_ISDBT_2K_TDI_4:
	case E_ISDBT_4K_TDI_4:
	case E_ISDBT_8K_TDI_4:
		strncpy(chlayerB_interleaving, "4", CHAR_LEN);
		layerB_interleaving = 4;
		break;
	case E_ISDBT_2K_TDI_8:
	case E_ISDBT_4K_TDI_8:
		strncpy(chlayerB_interleaving, "8", CHAR_LEN);
		layerB_interleaving = 8;
		break;
	case E_ISDBT_2K_TDI_16:
		strncpy(chlayerB_interleaving, "16", CHAR_LEN);
		layerB_interleaving = 16;
		break;
	case E_ISDBT_TDI_INVALID:
	default:
		strncpy(chlayerB_interleaving, "0", CHAR_LEN);
	...
	_mdrv_dmd_isdbt_md_getreg(0, 0x150C, &u8temp);
	u8temp &= 0x0F;

	layerB_segment_count = _isdbt_get_information_segment_count(u8temp);
...
(sisdbt_get_ber_value.bervalue*5194) / (sisdbt_get_ber_value.berperiod*100);
	...
	_get_ber_string(layerB_post_ber, chlayerB_post_ber);

	// layerC information
	_mdrv_dmd_isdbt_md_get_modulation_mode(0, E_ISDBT_Layer_C, &sisdbt_modulation_mode);

	switch (sisdbt_modulation_mode.eisdbtconstellation) {
	case E_ISDBT_DQPSK:
		strncpy(chlayerC_modulation, "DQPSK", CHAR_LEN);
		layerC_modulation = 0;
		break;
	case E_ISDBT_QPSK:
		strncpy(chlayerC_modulation, "QPSK", CHAR_LEN);
		layerC_modulation = 1;
		break;
	case E_ISDBT_16QAM:
		strncpy(chlayerC_modulation, "16QAM", CHAR_LEN);
		layerC_modulation = 2;
		break;
	case E_ISDBT_64QAM:
		strncpy(chlayerC_modulation, "64QAM", CHAR_LEN);
		layerC_modulation = 3;
		break;
	case E_ISDBT_QAM_INVALID:
	default:
	...
	switch (sisdbt_modulation_mode.eisdbtcoderate) {
	case E_ISDBT_CODERATE_1_2:
		strncpy(chlayerC_coderate, "1/2", CHAR_LEN);
		layerC_fec = 1;
		break;
	case E_ISDBT_CODERATE_2_3:
		strncpy(chlayerC_coderate, "2/3", CHAR_LEN);
		layerC_fec = 2;
		break;
	case E_ISDBT_CODERATE_3_4:
		strncpy(chlayerC_coderate, "3/4", CHAR_LEN);
		layerC_fec = 3;
		break;
	case E_ISDBT_CODERATE_5_6:
		strncpy(chlayerC_coderate, "5/6", CHAR_LEN);
		layerC_fec = 4;
		break;
	case E_ISDBT_CODERATE_7_8:
		strncpy(chlayerC_coderate, "7/8", CHAR_LEN);
		layerC_fec = 5;
		break;
	case E_ISDBT_CODERATE_INVALID:
	default:
	...
	case E_ISDBT_4K_TDI_0:
	case E_ISDBT_8K_TDI_0:
		strncpy(chlayerC_interleaving, "0", CHAR_LEN);
		layerC_interleaving = 0;
		break;
	case E_ISDBT_8K_TDI_1:
		strncpy(chlayerC_interleaving, "1", CHAR_LEN);
		layerC_interleaving = 1;
		break;
	case E_ISDBT_4K_TDI_2:
	case E_ISDBT_8K_TDI_2:
		strncpy(chlayerC_interleaving, "2", CHAR_LEN);
		layerC_interleaving = 2;
		break;
	case E_ISDBT_2K_TDI_4:
	case E_ISDBT_4K_TDI_4:
	case E_ISDBT_8K_TDI_4:
		strncpy(chlayerC_interleaving, "4", CHAR_LEN);
		layerC_interleaving = 4;
		break;
	case E_ISDBT_2K_TDI_8:
	case E_ISDBT_4K_TDI_8:
		strncpy(chlayerC_interleaving, "8", CHAR_LEN);
		layerC_interleaving = 8;
		break;
	case E_ISDBT_2K_TDI_16:
		strncpy(chlayerC_interleaving, "16", CHAR_LEN);
		layerC_interleaving = 16;
		break;
	case E_ISDBT_TDI_INVALID:
	default:
		strncpy(chlayerC_interleaving, "0", CHAR_LEN);
	...
	_mdrv_dmd_isdbt_md_getreg(0, 0x150E, &u8temp);
	u8temp &= 0x0F;

	layerC_segment_count = _isdbt_get_information_segment_count(u8temp);
...
(sisdbt_get_ber_value.bervalue*5194) / (sisdbt_get_ber_value.berperiod*100);
	...
	_get_ber_string(layerC_post_ber, chlayerC_post_ber);
	...
	//	isdbt_ewbs_flag = 0;
	isdbt_ewbs_flag =
	(_mdrv_dmd_isdbt_md_getlock(0, DMD_ISDBT_GETLOCK_EWBS_LOCK) == DMD_ISDBT_LOCK);

    /* Lock Status */
	if (_mdrv_dmd_isdbt_md_getlock(0,
	DMD_ISDBT_GETLOCK) == DMD_ISDBT_LOCK){
		strncpy(chisdbt_lock_flag, "Lock", CHAR_LEN);
		strncpy(chisdbt_ts_lock_flag, "Lock", CHAR_LEN);
		isdbt_lock_flag = 1;
		isdbt_ts_lock_flag = 1;
	} else {
		strncpy(chisdbt_lock_flag, "Unlock", CHAR_LEN);
		strncpy(chisdbt_ts_lock_flag, "Unlock", CHAR_LEN);
		strncpy(chlayerA_post_ber, "1.0e+0", CHAR_LEN);
		strncpy(chlayerB_post_ber, "1.0e+0", CHAR_LEN);
		strncpy(chlayerC_post_ber, "1.0e+0", CHAR_LEN);
		isdbt_lock_flag = 0;
		isdbt_ts_lock_flag = 0;
	}

	return scnprintf(buf, PAGE_SIZE,
		"%s%s%s%s %s%s%d%s %s%s%d%s %s%s%s%s %s%s%d%s %s%s%s%s %s%s%d%s %s%s%s%s %s%s%s%s %s%s%d%s %s%s%d%s %s%s%s%s %s%s%s%s %s%s%s%s %s%s%d%s %s%s%s%s %s%s%d%s %s%s%d%s %s%s%s%s %s%s%s%s %s%s%s%s %s%s%d%s %s%s%s%s %s%s%d%s %s%s%d%s %s%s%s%s %s%s%s%s %s%s%s%s %s%s%d%s %s%s%s%s %s%s%d%s %s%s%s%s %s%s%s%s",
		SM_CHIP_REV, SM_COMMA, SM_NONE, SM_END,
		SM_SYSTEM, SM_COMMA, system, SM_END,
		SM_FREQ_OFFSET, SM_COMMA, freq_offset_khz, SM_END,
		SD_SIGNAL_QAULITY, SM_COMMA, SM_NONE, SM_END,
		SM_AGC, SM_COMMA, ifagc_val, SM_END,
		SM_CNR, SM_COMMA, chsnr, SM_END,
		SM_TMCC_DET, SM_COMMA, isdbt_tmcc_det_flag, SM_END,
		SD_FFT_MODE, SM_COMMA, chfftsize, SM_END,
		SM_GUARD_INTERVAL, SM_COMMA, chguard_interval, SM_END,
		SM_LAYER_A_UEC_ACCU, SM_COMMA, u16isdbttotalerrpkt_a, SM_END,
		SM_LAYER_A_UEC, SM_COMMA, u16packet_err_a, SM_END,
		SM_LAYER_A_MODULATION, SM_COMMA, chlayerA_modulation, SM_END,
		SM_LAYER_A_CODERATE, SM_COMMA, chlayerA_coderate, SM_END,
		SM_LAYER_A_INTERLEAVE, SM_COMMA, chlayerA_interleaving, SM_END,
		SM_LAYER_A_SEGMENT, SM_COMMA, layerA_segment_count, SM_END,
		SM_LAYER_A_BER_BEFORE_RS, SM_COMMA, chlayerA_post_ber, SM_END,
		SM_LAYER_B_UEC_ACCU, SM_COMMA, u16isdbttotalerrpkt_b, SM_END,
		SM_LAYER_B_UEC, SM_COMMA, u16packet_err_b, SM_END,
		SM_LAYER_B_MODULATION, SM_COMMA, chlayerB_modulation, SM_END,
		SM_LAYER_B_CODERATE, SM_COMMA, chlayerB_coderate, SM_END,
		SM_LAYER_B_INTERLEAVE, SM_COMMA, chlayerB_interleaving, SM_END,
		SM_LAYER_B_SEGMENT, SM_COMMA, layerB_segment_count, SM_END,
		SM_LAYER_B_BER_BEFORE_RS, SM_COMMA, chlayerB_post_ber, SM_END,
		SM_LAYER_C_UEC_ACCU, SM_COMMA, u16isdbttotalerrpkt_c, SM_END,
		SM_LAYER_C_UEC, SM_COMMA, u16packet_err_c, SM_END,
		SM_LAYER_C_MODULATION, SM_COMMA, chlayerC_modulation, SM_END,
		SM_LAYER_C_CODERATE, SM_COMMA, chlayerC_coderate, SM_END,
		SM_LAYER_C_INTERLEAVE, SM_COMMA, chlayerC_interleaving, SM_END,
		SM_LAYER_C_SEGMENT, SM_COMMA, layerC_segment_count, SM_END,
		SM_LAYER_C_BER_BEFORE_RS, SM_COMMA, chlayerC_post_ber, SM_END,
		SM_EMERGENCY_FLAG, SM_COMMA, isdbt_ewbs_flag, SM_END,
		SM_DEMOD_LOCK, SM_COMMA, chisdbt_lock_flag, SM_END,
		SM_TS_LOCK, SM_COMMA, chisdbt_ts_lock_flag, SM_END);
}
...
struct dvb_frontend *fe, struct dtv_frontend_properties *p)
{
	...
	u8 layerA_segment_count = 0, layerB_segment_count = 0, layerC_segment_count = 0;
	u8 layerA_modulation = 0, layerB_modulation = 0, layerC_modulation = 0;
	u32 layerA_BER_biterr = 0, layerB_BER_biterr = 0, layerC_BER_biterr = 0;
	u16 sqi;
	u32 u32temp;
	...
	if (_mdrv_dmd_isdbt_md_getlock(0, DMD_ISDBT_GETLOCK) == DMD_ISDBT_LOCK) {
		_mdrv_dmd_isdbt_md_getreg(0, 0x1508, &u8temp);
		layerA_modulation = (u8temp >> 4) & 0x07;
		_mdrv_dmd_isdbt_md_getreg(0, 0x150A, &u8temp);
		layerB_modulation = (u8temp >> 4) & 0x07;
		_mdrv_dmd_isdbt_md_getreg(0, 0x150C, &u8temp);
		layerC_modulation = (u8temp >> 4) & 0x07;
	} else {
		layerA_modulation = 0;
		layerB_modulation = 0;
		layerC_modulation = 0;
	...
	u32temp = (u32)s32temp;
	s32temp = s32temp*SNR_COREFFICIENT;
	...
	isdbt_stats2->len = 4;
	isdbt_stats2->stat[1].uvalue = packet_err; //Fix the isdbt layer rule for linux dvb
	//isdbt_stats2->stat[0].scale = FE_SCALE_COUNTER;
	switch (layerA_segment_count) {
	case 0:
		isdbt_stats2->stat[1].scale = FE_SCALE_NOT_AVAILABLE;
		break;
	default:
		isdbt_stats2->stat[1].scale = FE_SCALE_COUNTER;
		break;
	}
	...
	p->layer[0].post_ber =
(sisdbt_get_ber_value.bervalue*BER_ERR_CNT_R) / (sisdbt_get_ber_value.berperiod*BER_ERR_WIN_R);

	layerA_BER_biterr = (sisdbt_get_ber_value.bervalue);

	isdbt_stats2 = &p->post_bit_error;
	isdbt_stats2->len = 1;
	isdbt_stats2->stat[0].uvalue = (sisdbt_get_ber_value.bervalue);
	isdbt_stats2->stat[0].scale = FE_SCALE_COUNTER;

	isdbt_stats2 = &p->post_bit_count;
	isdbt_stats2->len = 1;
	isdbt_stats2->stat[0].uvalue = ((u64)sisdbt_get_ber_value.berperiod)*(BER_ERR_WINDOW_COE);
	isdbt_stats2->stat[0].scale = FE_SCALE_COUNTER;
	...
	// UEC : Get Packet Error
	ret = _mdrv_dmd_isdbt_md_read_pkt_err(0, E_ISDBT_Layer_B,
	&packet_err);

	isdbt_stats2 = &p->block_error;
	isdbt_stats2->len = 4;
	isdbt_stats2->stat[2].uvalue = packet_err;//Fix the isdbt layer rule for linux dvb
	//isdbt_stats2->stat[1].scale = FE_SCALE_COUNTER;
	switch (layerB_segment_count) {
	case 0:
		isdbt_stats2->stat[2].scale = FE_SCALE_NOT_AVAILABLE;
		break;
	default:
		isdbt_stats2->stat[2].scale = FE_SCALE_COUNTER;
		break;
	}
...
(sisdbt_get_ber_value.bervalue*BER_ERR_CNT_R) / (sisdbt_get_ber_value.berperiod*BER_ERR_WIN_R);
	...
	layerB_BER_biterr = (sisdbt_get_ber_value.bervalue);

	isdbt_stats2 = &p->post_bit_error;
	isdbt_stats2->len = 1;
	isdbt_stats2->stat[1].uvalue = (sisdbt_get_ber_value.bervalue);
	isdbt_stats2->stat[1].scale = FE_SCALE_COUNTER;

	isdbt_stats2 = &p->post_bit_count;
	isdbt_stats2->len = 1;
	isdbt_stats2->stat[1].uvalue = ((u64)sisdbt_get_ber_value.berperiod)*(BER_ERR_WINDOW_COE);
	isdbt_stats2->stat[1].scale = FE_SCALE_COUNTER;
	...
	&sisdbt_modulation_mode);
		...
		// UEC : Get Packet Error
	ret = _mdrv_dmd_isdbt_md_read_pkt_err(0, E_ISDBT_Layer_C,
	&packet_err);

	isdbt_stats2 = &p->block_error;
	isdbt_stats2->len = 4;
	isdbt_stats2->stat[3].uvalue = packet_err;//Fix the isdbt layer rule for linux dvb
	//isdbt_stats2->stat[2].scale = FE_SCALE_COUNTER;
	switch (layerC_segment_count) {
	case 0:
		isdbt_stats2->stat[3].scale = FE_SCALE_NOT_AVAILABLE;
		break;
	default:
		isdbt_stats2->stat[3].scale = FE_SCALE_COUNTER;
		break;
	}
...
(sisdbt_get_ber_value.bervalue*BER_ERR_CNT_R) / (sisdbt_get_ber_value.berperiod*BER_ERR_WIN_R);
	...
	layerC_BER_biterr = (sisdbt_get_ber_value.bervalue);

	isdbt_stats2 = &p->post_bit_error;
	isdbt_stats2->len = 1;
	isdbt_stats2->stat[2].uvalue = (sisdbt_get_ber_value.bervalue);
	isdbt_stats2->stat[2].scale = FE_SCALE_COUNTER;

	isdbt_stats2 = &p->post_bit_count;
	isdbt_stats2->len = 1;
	isdbt_stats2->stat[2].uvalue = ((u64)sisdbt_get_ber_value.berperiod)*(BER_ERR_WINDOW_COE);
	isdbt_stats2->stat[2].scale = FE_SCALE_COUNTER;
	...
	*/
	...
	if (((layerA_segment_count == MAX_SEG) && (layerA_modulation == 1))
		|| ((layerA_modulation == 1) && (layerB_modulation == 1)
		&& (layerC_segment_count == 0))
		|| ((layerA_modulation == 1) && (layerC_modulation == 1)
		&& (layerB_segment_count == 0))
		|| ((layerA_modulation == 1) && (layerB_modulation == 1)
		&& (layerC_modulation == 1))) {
		sqi = (u16)(u32temp*(SQI_COREFFICIENT))/(SQI_QPSK_WIN);
		//PRINT("1:sqi:%d\n", sqi);
	} else {
		sqi = (u16)(u32temp*(SQI_COREFFICIENT))/(SQI_NORMAL_WIN);
		//PRINT("2:sqi:%d\n", sqi);
	}

	if ((layerA_modulation != UNKNOWN_MOD)
		&& (((layerA_segment_count > layerB_segment_count)
		&& (layerC_segment_count == 0))
		|| ((layerA_segment_count > layerC_segment_count)
		&& (layerB_segment_count == 0))
		|| ((layerA_segment_count > layerB_segment_count)
		&& (layerA_segment_count > layerC_segment_count)))) {
		if (layerA_BER_biterr == 0)
			sqi = MAX_SQI;
	}

	if ((layerB_modulation != UNKNOWN_MOD)
		&& (((layerB_segment_count > layerC_segment_count)
		&& (layerA_segment_count == 0))
		|| ((layerB_segment_count > layerA_segment_count)
		&& (layerC_segment_count == 0))
		|| ((layerB_segment_count > layerA_segment_count)
		&& (layerB_segment_count > layerC_segment_count)))) {
		if (layerB_BER_biterr == 0)
			sqi = MAX_SQI;
	}

	if ((layerC_modulation != UNKNOWN_MOD)
		&& (((layerC_segment_count > layerB_segment_count)
		&& (layerA_segment_count == 0))
		|| ((layerC_segment_count > layerA_segment_count)
		&& (layerB_segment_count == 0))
		|| ((layerC_segment_count > layerA_segment_count)
		&& (layerC_segment_count > layerB_segment_count)))) {
		if (layerC_BER_biterr == 0)
			sqi = MAX_SQI;
	}
	...
	//PRINT("QQ_GetSignalQQ:%d\n", sqi);

	isdbt_stats = &p->quality;
	isdbt_stats->len = 1;
	isdbt_stats->stat[0].uvalue = sqi;
	isdbt_stats->stat[0].scale = FE_SCALE_COUNTER;
