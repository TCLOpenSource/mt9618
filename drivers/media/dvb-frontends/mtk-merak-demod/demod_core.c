...
static int mtk_demod_set_frontend(struct dvb_frontend *fe)
{
	...
	if (demod_prvi->previous_system != c->delivery_system) {
		...
		case SYS_ATSC:
			...
			break;
		case SYS_ATSC30:
			pr_info("ATSC3_exit in remove is called\n");
			ret = _mdrv_dmd_atsc_md_setpowerstate(1, E_POWER_SUSPEND);
			if (ret) {
				pr_err("[austin_demod_dd][%s][%d] _mdrv_dmd_atsc_md_exit fail !\n",
				__func__, __LINE__);
			return ret;
			}
			break;
		...
		case SYS_ATSC30:
			...
			if (c->delivery_system == SYS_ATSC ||
				...
				pr_err("plp num = %d, plp_id - [0,1,2,3]= %d %d %d %d\n",
 ...
 */
	...
	if (c->delivery_system == SYS_UNDEFINED && fe->exit != DVB_FE_DEVICE_RESUME) {
		pr_warn("[mdbgerr_merak_demod_dd][%s][%d]SYS_UNDEFINED!\n", __func__, __LINE__);
		demod_prvi->fw_downloaded = 0;
	}
	return ret;
...
static int mtk_demod_sleep(struct dvb_frontend *fe)
{
	...
	c->delivery_system = delivery_sys_rec;
/*
 *	// [For 5896] release DRAM addr before return by fw_down == 0
 *	if ((c->delivery_system == SYS_UNDEFINED) && (demod_init_get_memory_flag == 1)) {
 *		mtk_demod_release_memory_buffer(demod_prvi);
 *		demod_init_get_memory_flag = 0;
 *	}
 */
					...
					enum fe_status *status)
{
	...
	case SYS_ATSC:
	case SYS_ATSC30:
		if (atsclockstatus == DMD_ATSC_NULL)
			*status = 0;
 ...
 */
	...
	case SYS_ATSC:
		...
		break;
	case SYS_ATSC30:
		pr_err("[%s][%d] dmd_drv_atsc_init fail !\n", __func__, __LINE__);
		// Init Parameter
...
static int cfm_property_process_get(struct cfm_property *tvp)
{
	...
	case CFM_STREAM_PIN_MUX_INFO:
		if (delivery_sys_rec == SYS_ATSC30)
			pinctrl_ts = "ext_ts_para_demod_pad_mux";
...
static int demod_probe(struct platform_device *pdev)
{
	...
	pr_info("[%s][%d] AUSTIN = %d\n", __func__, __LINE__, p_austin_dev->austin_exist);
	if (p_austin_dev->austin_exist) {
		demod_dev->dmd_ops.chip_delsys.element[(u8)demod_dev->dmd_ops.chip_delsys.size] =
		(u8)(SYS_ATSC30);
