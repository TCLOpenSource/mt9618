...
#define BCHCTRL1   0x3f62
#define BCHCTRL2   0x3f63
#define BCHDBSLT   0x3f60
#define BCHDBGOUTL 0x3f74
#define BCHDBGOUTH 0x3f75
#define BCHBITCOUNT 762
//-------------------------------------------------
//	Local	Structurs
//-------------------------------------------------
...
/*MS_VIRT*/
	...
	pRes->sDMD_DTMB_PriData.virtDMDBaseAddr =
	pRes->sDMD_DTMB_InitData.virt_riu_base_addr;

	pRes->sDMD_DTMB_PriData.virt_reset_base_addr =
	pRes->sDMD_DTMB_InitData.virt_reset_base_addr;

	pRes->sDMD_DTMB_PriData.virt_ts_base_addr =
	pRes->sDMD_DTMB_InitData.virt_ts_base_addr;

	pRes->sDMD_DTMB_PriData.virt_clk_base_addr =
	pRes->sDMD_DTMB_InitData.virt_clk_base_addr;

	pRes->sDMD_DTMB_PriData.virt_sram_base_addr =
	pRes->sDMD_DTMB_InitData.virt_sram_base_addr;

	pRes->sDMD_DTMB_PriData.m6_version =
	pRes->sDMD_DTMB_InitData.m6_version;

	if (pRes->sDMD_DTMB_PriData.m6_version > 0x0100) {
		pRes->sDMD_DTMB_PriData.virt_vagc_base_addr =
		pRes->sDMD_DTMB_InitData.virt_vagc_base_addr;

		pRes->sDMD_DTMB_PriData.virt_pmux_base_addr =
		pRes->sDMD_DTMB_InitData.virt_pmux_base_addr;
	}
	#endif
			...
			struct dtv_frontend_properties *p)
{
	...
	//u8 ifagc = 0;
	u8 ret = 0, udata81 = 0, udata82 = 0;
	s16 freqoffset = 0;
	u16 icp = 0, snrqty = 0, udata16 = 0;
...
//	struct DMD_DTMB_BER_DATA berr;
	memset(&cfo, 0, sizeof(struct DMD_DTMB_CFO_DATA));
	memset(&sDtmbModulationMode, 0, sizeof(struct DMD_DTMB_MODULATION_INFO));
...
//!SNR porting
	...
	snrqty = (u16) snrqty*300; //doing "/1000" upper layer;
	//pr_emerg("\n[demod_drv_dtmb.c]0: snrqty(*10^-3dB) = %d\n", snrqty);
...
/////////////////////////////////////////////////////////
		...
		//!IFAGC porting
		p->agc_val = _mdrv_dmd_dtmb_md_getagc(0);
...
//	p->post_ber = uber32;
//	ret &= _mdrv_dmd_dtmb_md_getreg(0, 0x20C1, &udata81);

	ret &= _mdrv_dmd_dtmb_md_setreg(0, BCHCTRL1, 0);
	ret &= _mdrv_dmd_dtmb_md_setreg(0, BCHCTRL2, 2);
	ret &= _mdrv_dmd_dtmb_md_getreg(0, BCHDBSLT, &udata81);
	ret &= _mdrv_dmd_dtmb_md_setreg(0, BCHDBSLT, (udata81|0x01));
	ret &= _mdrv_dmd_dtmb_md_getreg(0, BCHDBGOUTL, &udata81);
	ret &= _mdrv_dmd_dtmb_md_getreg(0, BCHDBGOUTH, &udata82);
	udata16 = (u16) ((udata82<<8)|udata81);
	uber32 = (u32) (udata16 * 2563);//BER*1000000000/512/BCHBITCOUNT;
	p->post_ber = uber32;
	p->pre_ber = uber32;
	...
	stats = &p->post_bit_error;//DTV_STAT_POST_ERROR_BIT_COUNT;
	stats->len = 1;
	stats->stat[0].uvalue = udata16;
	stats->stat[0].scale = FE_SCALE_COUNTER;

	//pr_emerg("\n[demod_drv_dtmb.c]1: post_bit_error = %d\n", stats->stat[0].uvalue);
	ret &= _mdrv_dmd_dtmb_md_getreg(0, BCHCTRL1, &udata81);
	ret &= _mdrv_dmd_dtmb_md_getreg(0, BCHCTRL2, &udata82);
	udata16 = (u16) ((udata82<<8)|udata81);
	stats = &p->post_bit_count;//DTV_STAT_POST_TOTAL_BIT_COUNT;
	stats->len = 1;
	stats->stat[0].uvalue = udata16 * BCHBITCOUNT;
	stats->stat[0].scale = FE_SCALE_COUNTER;
	//pr_emerg("\n[demod_drv_dtmb.c]2: post_bit_count = %d\n", stats->stat[0].uvalue);
	...
	if ((sDtmbModulationMode.upnc8) == 1) {//CONTSANT phase
		...
		switch (sDtmbModulationMode.upnm16) {
		case 420:
			p->guard_interval = GUARD_INTERVAL_PN420_VAR;
		break;

		case 945:
		p->guard_interval = GUARD_INTERVAL_PN945_VAR;
		break;
...
char *buf)
{
	...
	u8 ifagc = 0, qrate = 0;
	s32 err_flg[23];
	u32 et2tsrate = 0, uber32 = 0, icp = 0;
	s16 freqoffset = 0;
	char dmd_status[8] = "Unlock";
	char ts_status[8] = "Unlock";
	char pnp[8] = "V";
	char coderate[8] = "0.8";
	char modulation[8] = "4QAM-NR";
	char order_ber_b4_bch[8] = "e+";
	s32 stemp32 = 0;
	struct DMD_DTMB_CFO_DATA cfo;
	struct DMD_DTMB_MODULATION_INFO sDtmbModulationMode;
	//struct DMD_DTMB_BER_DATA berr;
	...
	err_flg[1] = (s32) (_mdrv_dmd_dtmb_md_getsignalquality(0)*300);//VT adds
	err_flg[17] = (s32) (err_flg[1]/1000);
	err_flg[18] = (s32) ((err_flg[1]-err_flg[17]*1000)/100);
	//CNR:doing "/1000" on upper layer;

	//Incorrect packets
	ret_tmp = _mdrv_dmd_dtmb_md_getreg(0, 0x20C1, &data3);
	if (data3 == 0xF0)
		icp = (u32) _mdrv_dmd_dtmb_md_geticp(0);
	else
		icp = 0;

	udtmbtotalerrpkt32 = udtmbtotalerrpkt32 + icp;
	err_flg[2] = udtmbtotalerrpkt32;
	err_flg[19] = (s32) icp;
	...
	if (dtmblockstatus == DMD_DTMB_LOCK)
		strncpy(dmd_status, "Lock", sizeof(dmd_status)); //dmdstatus = TRUE;
	else if (dtmblockstatus == DMD_DTMB_UNLOCK)
		strncpy(dmd_status, "Unlock", sizeof(dmd_status)); //dmdstatus = FALSE;
	//err_flg[4] = (s32) dmdstatus;
	//PRINT("(in DTMB_DRV)DEMOD_LOCK: %d\r\n", err_flg[4]);

	ret_tmp = _mdrv_dmd_dtmb_md_getreg(0, 0x20C1, &data3);
	if (data3 == 0xF0 || data3 > 0x62) {
		strncpy(ts_status, "Lock", sizeof(ts_status));//err_flg[5] = 1;
		err_flg[5] = 1;
	}	else {
		strncpy(ts_status, "Unlock", sizeof(ts_status));//err_flg[5] = 0;
		err_flg[5] = 0;//PRINT("(in DTMB_DRV)TS_LOCK: %d\r\n", err_flg[5]);
	}
	 ...
	 et2tsrate = (u32) (2842560000/(3780+sDtmbModulationMode.upnm16)
		*qrate*sDtmbModulationMode.fsicoderate);
		...
		if ((sDtmbModulationMode.upnc8) == 0)//var phase
			strncpy(pnp, "V", sizeof(pnp));//err_flg[9] = 1;//PNP: 0 = conts-phase
		else
			strncpy(pnp, "C", sizeof(pnp));//err_flg[9] = 0;//PNP: 0 = conts phase

	switch (sDtmbModulationMode.usiqammode8) {
	case 4/*constellation_qam4*/:
		strncpy(modulation, "4QAM", sizeof(modulation));//err_flg[11] = QPSK;
	break;
	//case constellation_qam4NR:  _pSignal->QAMSize = 5;   break;
	case 16/*constellation_qam16*/:
		strncpy(modulation, "16QAM", sizeof(modulation));//err_flg[11] = QAM_16;
	break;

	case 32/*constellation_qam32*/:
		strncpy(modulation, "32QAM", sizeof(modulation));//err_flg[11] = QAM_32;
	break;

	case 64/*constellation_qam64*/:
		strncpy(modulation, "64QAM", sizeof(modulation));//err_flg[11] = QAM_64;
	break;
	...
	break;
		...
		if (sDtmbModulationMode.usinr8)
			strncpy(modulation, "4QAM-NR", sizeof(modulation));//err_flg[11] = QAM_4_NR;

	//((sDtmbModulationMode.upnc8) == 0)//var phase
	switch (sDtmbModulationMode.upnm16) {
	case 420:
			err_flg[12] = 420;//GUARD_INTERVAL_PN420;
		break;

	case 595:
		err_flg[12] = 595;//GUARD_INTERVAL_PN595;
		break;

	case 945:
		err_flg[12] = 945;//GUARD_INTERVAL_PN945;
		break;
	...
	switch (sDtmbModulationMode.usiinterleaver16) {
	case 240:
		err_flg[13] = 240;   break;//INTERLEAVING_240; break;

	case 720:
		err_flg[13] = 720;   break;//INTERLEAVING_720; break;
	...
	switch (sDtmbModulationMode.fsicoderate)	{
	case 4:
		strncpy(coderate, "0.4", sizeof(coderate));   break;//err_flg[14]=4;//FEC_2_5;break;

	case 6:
		strncpy(coderate, "0.6", sizeof(coderate));   break;////FEC_3_5;   break;

	case 8:
		strncpy(coderate, "0.8", sizeof(coderate));   break;//;//FEC_4_5;   break;

	default:
		strncpy(coderate, "0.4", sizeof(coderate));   break;//;//FEC_2_5;   break;
	}
//pr_emerg("VT: sDtmbModulationMode.usicarriermode8=%d\n",sDtmbModulationMode.usicarriermode8);
	switch (sDtmbModulationMode.usicarriermode8) {
	case 0/*CarrierMode_MC*/:
	err_flg[15] = 3780; break;//TRANSMISSION_MODE_C3780; break;

	case 1/*CarrierMode_SC*/:
	err_flg[15] = 1; break;//TRANSMISSION_MODE_C1; break;

	default:
	err_flg[15] = 0; break;//TRANSMISSION_MODE_C1; break;
	}
/////
	...
	ret_tmp = _mdrv_dmd_dtmb_md_readfrequencyoffset(0, &cfo);
	freqoffset = (short)((((s32)cfo.fftfirstcfo*(s32)cfo.sr/0x10000
	+(s32)cfo.fftsecondcfo*(s32)cfo.sr/0x20000)));

	ret_tmp = _mdrv_dmd_dtmb_md_getreg(0, 0x2857, &data3);
	//pr_emerg("Mix-Rate-MSB = %d\n", data3);
	//pr_emerg("freqoffset = %d, -freqoffset = %d\n", freqoffset, -freqoffset);
	if (data3 < 4)
		err_flg[10] = (s32) freqoffset;
	else
		err_flg[10] = (s32) -1 * freqoffset;

	//!ber porting: format=10^9???
//	 ret_tmp =  _mdrv_dmd_dtmb_md_getpreldpcber(0, &berr);
	//_MDrv_DMD_DTMB_MD_GetPreLdpcBer(u8 id, DMD_DTMB_BER_DATA *pber)
	//u32 BitErr;
	//u16 Error_window;
	//BER = (UINT16)(((double)berr.BitErr/(double)7488.0/
...
//		))/7488/berr.error_window);
	//mcSHOW_DBG_MSG(("Pre-LDPC BER*50000 = %d\n",BER));
//	err_flg[16] = (s32) uber32*2*10000;// format: original-ber*10^9

	ret_tmp &= _mdrv_dmd_dtmb_md_setreg(0, BCHCTRL1, 0);
	ret_tmp &= _mdrv_dmd_dtmb_md_setreg(0, BCHCTRL2, 2);
	ret_tmp &= _mdrv_dmd_dtmb_md_getreg(0, BCHDBSLT, &udata81);
	ret_tmp &= _mdrv_dmd_dtmb_md_setreg(0, BCHDBSLT, (udata81|0x01));
	ret_tmp &= _mdrv_dmd_dtmb_md_getreg(0, BCHDBGOUTL, &udata81);
	ret_tmp &= _mdrv_dmd_dtmb_md_getreg(0, BCHDBGOUTH, &udata82);
	udata16 = (u16) ((udata82<<8)|udata81);
	err_flg[16] = (s32) (udata16 * 2563);//BER*1000000000/512/BCHBITCOUNT;
//pr_emerg("VT: ber_b4_bch(10e-9)=%d\n",err_flg[16]);
	if (err_flg[5] && err_flg[16] == 0) {
		strncpy(order_ber_b4_bch, "e+", sizeof(order_ber_b4_bch));
		err_flg[20] = 0;
		err_flg[21] = 0;
		err_flg[22] = 0;
	}	else if (err_flg[5] == 0) {
	  ...
	  err_flg[20] = 1;
		...
		err_flg[20] = (s32) (stemp32/10);
		err_flg[21] = stemp32 - err_flg[20] * 10;
		err_flg[22] = 9 - data3 - 1;
		strncpy(order_ber_b4_bch, "e-", sizeof(order_ber_b4_bch));
	}
	...
	//pr_emerg("CNR = %d\n", err_flg[1]);
	//pr_emerg("UEC = %d\n", err_flg[2]);
	//pr_emerg("Chip rev.(Demod) = %d\n", err_flg[3]);
	//pr_emerg("Demod Lock=%d\n", err_flg[4]);

	//pr_emerg("TS Lock = %d\n", err_flg[5]);
	//pr_emerg("Signal quality = %d\n", err_flg[6]);
	//pr_emerg("Signal level = %d\n", err_flg[7]);
	//pr_emerg("TS Rate = %d\n", err_flg[8]);
	//pr_emerg("PN Phase = %d\n", err_flg[9]);

	//pr_emerg("Freq. Offset = %d\n", err_flg[10]);
	//pr_emerg("Modulation Type = %d\n", err_flg[11]);
	//pr_emerg("Guard Interval = %d\n", err_flg[12]);
	//pr_emerg("Time Interleave = %d\n", err_flg[13]);
	//pr_emerg("Code Rate = %d\n", err_flg[14]);

	//pr_emerg("Transmission Mode = %d\n", err_flg[15]);
	//pr_emerg("BER before BCH/Post LDPC(10^9) = %d\n", err_flg[16]);

	return scnprintf(buf, PAGE_SIZE,
	"%s%s%s%s %s%s%d%s %s%s%d%s %s%s%d%s %s%s%d%s%d%s %s%s%s%s %s%s%s%s %s%s%d%s %s%s%d%s%s %s%s%d%s %s%s%d%s %s%s%s%s %s%s%s%s %s%s%d%s %s%s%d%s%d%s%d%s %s%s%d%s",
	SM_CHIP_REV, SM_COMMA, SM_NONE, SM_END,
	SM_FREQ_OFFSET, SM_COMMA, err_flg[10], SM_END,
	SD_SIGNAL_QAULITY, SM_COMMA, err_flg[6], SM_END,
	SM_AGC, SM_COMMA, err_flg[0], SM_END,
	SM_CNR, SM_COMMA, err_flg[17], SM_DOT, err_flg[18], SM_END,
	SM_DEMOD_LOCK, SM_COMMA, dmd_status, SM_END,
	SM_TS_LOCK, SM_COMMA, ts_status, SM_END,
	SM_CARRIER_MODE, SM_COMMA, err_flg[15], SM_END,
	SM_GUARD_INTERVAL, SM_COMMA, err_flg[12], pnp, SM_END,
	SM_UEC_ACCU, SM_COMMA, err_flg[2], SM_END,
	SM_UEC, SM_COMMA, err_flg[19], SM_END,
	SM_MODULATION, SM_COMMA, modulation, SM_END,
	SM_CODE_RATE, SM_COMMA, coderate, SM_END,
	SM_TIME_INTERLEAVE, SM_COMMA, err_flg[13], SM_END,
	SM_BER_BEFORE_BCH, SM_COMMA, err_flg[20], SM_DOT, err_flg[21],
	order_ber_b4_bch, err_flg[22], SM_END,
	SM_TS_RATE, SM_COMMA, err_flg[8], SM_END);
	//SD_POST_LDPC, SM_COMMA, err_flg[16], SM_END);
}

ssize_t dtmb_get_information(char *buf)
{
	struct device_driver driver;

	return dtmb_get_information_show(&driver, buf);
}
