...
static void *dmx_usercopy_ex(void *parg, unsigned int cmd)
{
	struct dmx2_controls *pctrl = (struct dmx2_controls *) parg;
	u32 i, count;
	...
	case DMX2_SET_CTRLS:
	case DMX2_GET_CTRLS:
		count = ((struct dmx2_controls *)parg)->count;
		base = temp = kzalloc(count * sizeof(struct dmx2_control), GFP_KERNEL);
		if (!base)
			return NULL;

		for (i = 0; i < count; i++, temp++)
			if (copy_from_user(temp, (pctrl->controls + i),
				sizeof(struct dmx2_control))) {
				pctrl->controls = NULL;
				kfree(base);
				return NULL;
			}
...
static int dmx_usercopy(struct file *file,
		     ...
		     long (*func)(struct file *file, unsigned int cmd, unsigned long arg),
		     void * (*extension)(void *parg, unsigned int cmd))
{
#define BUF_SIZE 128
...
static long dvb_demux2_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case DMX2_SET_CTRLS:
	case DMX2_GET_CTRLS:
		ret = dmx_usercopy(file, cmd, arg,
			get_op(dmx2dev, dmx, unlocked_ioctl), &dmx_usercopy_ex);
		break;
	default:
		ret = dmx_usercopy(file, cmd, arg,
			get_op(dmx2dev, dmx, unlocked_ioctl), NULL);
		break;
	}
...
static long dvb_demux2_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case DMX2_GET_CTRLS32:
		ret = do_dvb_demux2_compat_ioctl(file, cmd, arg);
		break;
	default:
		ret = dmx_usercopy(
				file,
				cmd,
				arg,
				get_op(dmx2dev, dmx, compat_ioctl),
				NULL);
		break;
	}
...
static long dvb_dvr2_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case DMX2_GET_CTRLS:
		ret = dmx_usercopy(file, cmd, arg,
			get_op(dmx2dev, dvr, unlocked_ioctl), &dmx_usercopy_ex);
		break;
	default:
		ret = dmx_usercopy(file, cmd, arg,
			get_op(dmx2dev, dvr, unlocked_ioctl), NULL);
		break;
	}
...
static long dvb_dvr2_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case DMX2_SET_CTRLS32:
	case DMX2_GET_CTRLS32:
		ret = do_dvb_dvr2_compat_ioctl(file, cmd, arg);
		break;
	default:
		ret = dmx_usercopy(
				file,
				cmd,
				arg,
				get_op(dmx2dev, dvr, compat_ioctl),
				NULL);
		break;
	}
