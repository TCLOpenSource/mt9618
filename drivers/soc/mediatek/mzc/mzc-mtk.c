...
{
	phys_addr_t in_addr2 = ((in_addr & ZDEC_PAGE_MASK) + PAGE_SIZE);
...
int __mtk_dtv_lenc_cmdq_set(phys_addr_t in_addr,
	...
	unsigned int process_id)
{
	unsigned long long msb = (unsigned long long)(out_addr);
	...
	if (out_addr > BIT_MASK_32)
		MZC_REG_WRITE_32(LENC_INQ_DATA,
			...
			| (msb >> SHITF_35) << SHIFT_4 | process_id);
	else
		MZC_REG_WRITE_32(LENC_INQ_DATA,
	...
	int *free_cycle, int *page_count)
{
	...
	MZC_REG_WRITE(LDEC_TIMEOUT_EN, mzc_pdata->comm_reg.reg24);
	orig_active_cycle = MZC_REG_READ_32(RO_LDEC_CYC_INFO);
	*active_cycle = orig_active_cycle;
	...
	orig_free_cycle = MZC_REG_READ_32(RO_LDEC_CYC_INFO);
	*free_cycle = orig_free_cycle;
	...
	orig_page_count = MZC_REG_READ_32(RO_LDEC_CYC_INFO);
	...
	int *free_cycle, int *page_count)
{
	...
	*active_cycle = orig_active_cycle;
	...
	orig_free_cycle = MZC_REG_READ_32(RO_LENC_CYC_INFO);
	*free_cycle = orig_free_cycle;
	...
	orig_page_count = MZC_REG_READ_32(RO_LENC_CYC_INFO);
	*page_count = orig_page_count;
}
...
static s32 mtk_mzc_resume(struct platform_device *pdev)
{
	...
	res = clk_prepare_enable(mzc_mux_gate);
