...
static ssize_t tee_boot_write(struct file *file, const char __user *buffer,
		size_t count, loff_t *f_pos)
{
	int ret = 0;
	char c[2] = { '\0' };

	ret = get_user(c[0], buffer);
	pr_info("MTK clear retry count : %s\n", c);
	if (ret == 0 && count == 1 && c[0] == '1') {
		pr_info("%s: MTK clear retry count\n", __func__);
...
static int __init mtk_ab_boot_init(void)
{
	...
	retry_count_addr = ioremap(RIU_BUS_ADDR + (RETRY_COUNT_BANK<<REGISTER_OFFSET), PAGE_SIZE);
	value = readw((void __iomem *)(retry_count_addr));
	pr_info("mtk tee init done, retry count:%x", value);
	iounmap(retry_count_addr);
