				...
				size_t count, loff_t *f_pos)
{
	...
	if (count < strlen(version_3_2) + 1) {
		pr_err("%s(%d): invalid count\n", __func__, __LINE__);
		return count;
	}

	if (optee_version == 3)
		ret = copy_to_user(buf, version_3_2, strlen(version_3_2) + 1);
	else if (optee_version == 2)
		ret = copy_to_user(buf, version_2_4, strlen(version_2_4) + 1);
	else
		return -EINVAL;
...
static int mtk_prepare_dyn_ramlog_buffer(void)
{
	...
	if (optee->sec_caps & OPTEE_SMC_SEC_CAP_DYNAMIC_SHM) {
		...
		if (!ramlog_vaddr) {
			pr_err("%s: prepare ramlog buffer failed!\n", __func__);
			return -ENOMEM;
		}

		mtk_ramlog_buf = tee_shm_register(mtk_tee_ctx, ramlog_vaddr,
			RAMLOG_SIZE, TEE_SHM_DMA_BUF | TEE_SHM_KERNEL_MAPPED);
		if (IS_ERR(mtk_ramlog_buf)) {
			pr_err("%s: tee_shm_register failed with %ld\n",
