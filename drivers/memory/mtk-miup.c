			...
			struct mtk_miup_para miup)
{
	...
	//end address version 1&3 for normal protect need -1
	//end address version 2 for normal protect no need -1
	if (pdata->end_addr_ver == END_ADDR_VER_1 || pdata->end_addr_ver == END_ADDR_VER_3)
		eaddr_align -= 1;
				...
				struct mtk_miup_para *miup)
{
	...
	if (pdata->end_addr_ver == END_ADDR_VER_1 ||
	   (pdata->end_addr_ver == END_ADDR_VER_3 && prot_type == 0))
		end_addr += 1;
...
static int mtk_miup_clean_hit_log(struct mtk_miup *pdata)
{
	int ret = 1;
	void __iomem *base = NULL;
...
static void mtk_miup_suspend_protect(struct device *dev)
{
	...
	//end address version 1 for out of area need +1
	//end address version 2&3 for out of area no need +1
	if (pdata->end_addr_ver == END_ADDR_VER_1)
	...
	if (ob_black_client_cnt &&
		...
		for (i = 0; i < ob_black_client_cnt; i++) {
			...
			if ((hit_log.cid & cid_en) == (cid & cid_en)) {
				pr_warn_ratelimited(
				"ID : 0x%llX %s address: 0x%llX - 0x%llX SW patch by pass panic\n",
				hit_log.cid, (hit_log.attr == 0) ? "read" : "write",
