	...
	struct device_attribute *attr, char *buf)
{
	...
	case MIUP_LOG_NONE:
		count += scnprintf(buf + count, PAGE_SIZE, "%s", "no log");
		break;
	case MIUP_LOG_ERR:
		count += scnprintf(buf + count, PAGE_SIZE, "%s", "error log");
		break;
	case MIUP_LOG_WARN:
		count += scnprintf(buf + count, PAGE_SIZE, "%s", "warning log");
		break;
	case MIUP_LOG_INFO:
		count += scnprintf(buf + count, PAGE_SIZE, "%s", "info log");
		break;
	default:
		count += scnprintf(buf + count, PAGE_SIZE, "%s", "all log");
		break;
	}
	count += scnprintf(buf + count, PAGE_SIZE,
