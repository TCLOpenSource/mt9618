...
/*
 * Provide for smi larb ostdl dynamic setting
 * @node, larb. port: target larb device node/larb/port
 * @ostdl: target ostdl value, 0 is not allowed and then reset to default
 */
int mtk_smi_larb_node_ostdl(struct device_node *np, u32 larb, u32 port, u32 ostdl)
{
	int ret = 0;
	void __iomem *base;
	struct platform_device *pdev;
	struct mtk_smi_larb *pst_larb;
	struct mtk_smi_larb_gen *larb_data;
	u8 ostdl_ori;
	...
	if (!np) {
		pr_err("[MTK-SMI]larb node parameter is null: %u-%u\n", larb, port);
		return -ENODEV;
	}

	pdev = of_find_device_by_node(np);
	if (!pdev) {
		pr_err("[MTK-SMI]larb dev not found by node parameter: %u-%u\n", larb, port);
		return -ENODEV;
	}

	pst_larb = dev_get_drvdata(&pdev->dev);
	larb_data = (struct mtk_smi_larb_gen *)pst_larb->larb_gen;
	if (!larb_data || port >= larb_data->port_in_larb_gen2[larb]) {
		pr_err("[MTK-SMI]larb port not found: %u-%u\n", larb, port);
		return ret;
	}
...
EXPORT_SYMBOL(mtk_smi_larb_node_ostdl);

struct device_node *mtk_smi_larb_find_node(u32 larb)
{
	int count;
	...
	if (larb >= SMI_LARB_PORT_NR_MAX) {
		pr_err("[MTK-SMI]find_node bad larb: %u\n", larb);
		return NULL;
	}

	np = of_find_node_by_name(NULL, dev_name);
	if (!np) {
		pr_err("[MTK-SMI]find_node larb node not found:%u\n", larb);
		return NULL;
	}

	return np;
}
EXPORT_SYMBOL(mtk_smi_larb_find_node);
