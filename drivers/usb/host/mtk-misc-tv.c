...
static int mtk_gpio_init(struct xhci_hcd_mtk *mtk, struct platform_device *pdev)
{
	...
	for (i = 0; i < count; i++) {
		...
		/* get vbus gpio from dts. set NULL if empty.
		 * in normal case, power on vbus (OUT_HIGH) by default.
		 */
		m_port->vbus =
			devm_gpiod_get_index_optional(
				dev, "vbus", i, m_port->flag & DIS_INIT ?
		...
		if (IS_ERR(m_port->vbus)) {
			...
			dev_info(m_port->mtk->dev,
				...
				m_port->p_num, m_port->flag & DIS_INIT ? "off" : "on");
		}
...
static void mtk_user_api_create(struct xhci_hcd_mtk *mtk, struct platform_device *pdev)
{
	...
	struct device_node *node = dev->of_node;
	struct mtk_misc *m_misc = &mtk->m_misc;
	struct proc_dir_entry *fool_dir, *vctrl_dir, *port_dir, *pde_file;
	char name[BUF_SIZE];
	int i, ret;
	...
	for (i = 0; i < m_misc->nports; i++) {
		...
		pde_file = proc_create_single_data(MTK_VBUS_CTRL_MODE_FILE,
			0440, port_dir, c_mode_show, m_misc->ports[i]);
		if (pde_file != NULL)
			proc_set_user(pde_file,
					GLOBAL_ROOT_UID, MTK_USB_VBUS_GID);

		/* create /proc/mtk_usb/v_ctrl/p#/p_state */
		pde_file = proc_create_data(MTK_VBUS_POWER_STATE_FILE,
			0660, port_dir, &p_state_fops, m_misc->ports[i]);
		if (pde_file != NULL)
			proc_set_user(pde_file,
					GLOBAL_ROOT_UID, MTK_USB_VBUS_GID);
	}
...
int mtk_misc_probe_end(struct xhci_hcd_mtk *mtk, struct platform_device *pdev)
{
	int ret;
