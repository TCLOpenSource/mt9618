...
static int prd_u3p;
static int prd_u2p;
static void __iomem *prd_ippc_base;

#define IS_PRD_HOST(ssusb) (!ssusb->standbypower)

static void mtu3_prd_port_init(struct ssusb_mtk *ssusb)
{
	if (!IS_PRD_HOST(ssusb))
		return;
	prd_u3p = -1;
	prd_u2p = -1;
	prd_ippc_base = ssusb->ippc_base;
}

void mtu3_prd_port_conf(int u2p, int u3p)
{
	prd_u2p = u2p;
	prd_u3p = u3p;
	pr_info("[PRD][MTU3] port conf u2p %d u3p %d\n", prd_u2p, prd_u3p);
}
EXPORT_SYMBOL_GPL(mtu3_prd_port_conf);

int mtu3_prd_port_en(void)
{
	void __iomem *ibase = prd_ippc_base;
	u32 value;

	if (!ibase || (prd_u2p < 0) || (prd_u3p < 0)) {
		pr_err("[PRD][MTU3] port enable err: %p u2p %d u3p %d\n",
			ibase, prd_u2p, prd_u3p);
		return -EINVAL;
	}

	pr_info("[PRD][MTU3] port enable u2p %d u3p %d\n", prd_u2p, prd_u3p);
	value = mtu3_readl(ibase, SSUSB_U3_CTRL(prd_u3p));
	value &= ~(SSUSB_U3_PORT_PDN | SSUSB_U3_PORT_DIS);
	value |= SSUSB_U3_PORT_HOST_SEL;
	mtu3_writel(ibase, SSUSB_U3_CTRL(prd_u3p), value);

	value = mtu3_readl(ibase, SSUSB_U2_CTRL(prd_u2p));
	value &= ~(SSUSB_U2_PORT_PDN | SSUSB_U2_PORT_DIS);
	value |= SSUSB_U2_PORT_HOST_SEL;
	mtu3_writel(ibase, SSUSB_U2_CTRL(prd_u2p), value);
	return 0;
}
EXPORT_SYMBOL_GPL(mtu3_prd_port_en);
#endif
...
int ssusb_host_enable(struct ssusb_mtk *ssusb)
{
...
#ifdef CONFIG_USB_MTK_DTV_XHCI
		if (IS_PRD_HOST(ssusb) && (i == prd_u3p)) {
			pr_info("[PRD][MTU3] don't enable u3p %d\n", i);
			continue;
		}
#endif
	...
	for (i = 0; i < num_u2p; i++) {
#ifdef CONFIG_USB_MTK_DTV_XHCI
		if (IS_PRD_HOST(ssusb) && (i == prd_u2p)) {
			pr_info("[PRD][MTU3] don't enable u2p %d\n", i);
			continue;
		}
#endif
