...
static int mtk_tv_plane_atomic_set_property(struct drm_plane *plane,
					  ...
					  uint64_t val)
{
	static __s64 start_atomic_begin;
	int ret = -EINVAL;
	__s64 cur, diff;
	struct mtk_drm_plane *mplane = to_mtk_plane(plane);
	...
	if (start_atomic_begin == 0)
		start_atomic_begin = ktime_get();
	else {
		cur = ktime_get();
		diff = ktime_ms_delta(cur, start_atomic_begin);
		if (diff)
			mtk_drm_atrace_int("drm_atomic_diff", (int32_t)diff);

		start_atomic_begin = cur;
	}
