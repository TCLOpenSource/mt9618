...
#define PANEL_8K_W	(7680)
#define PANEL_8K_H	(4320)

#define VG_REQ_DIV_RATIO_2 (2)
...
#define DEGAMMA_SIZE	(0x3000)
#define GAMMA_SIZE	(0x810)
#define DEGAMMA_ENTRY	(0x400)
#define DEGAMMA_ENTRY_SIZE	(0x4)
#define GAMMA_ENTRY	(0x158)
#define GAMMA_ENTRY_SIZE	(0x2)

#define MTK_GOP_DEBUG_MODE_ENG_EN	(0x1)
#define MTK_GOP_DEBUG_MODE_ALPHAVAL	(0x2)
#define MTK_GOP_DEBUG_MODE_PQEN	(0x4)
...
struct gop_capability {
	...
	uint32_t u32SupportMaxWidth[MAX_GOP_PLANE_NUM];
	uint8_t u8bNeedDualGOP;
	uint8_t u8SupportAutoDetect;
	uint8_t u8SupportBlend2OSDB0;
	uint8_t u8SupportOSDBLocSwitch;
	uint8_t u8SupportBlendVideoIrq;
	uint8_t u8SupportGOPMultiApha;
	uint8_t u8SupportGOPStep2HVSP;
};
...
bool mtk_gop_check_plane_IsUpdated(struct mtk_tv_drm_crtc *mcrtc,
	struct drm_crtc_state *crtc_state);
int mtk_gop_crtc_begin(struct mtk_tv_drm_crtc *mcrtc,
	struct drm_crtc_state *crtc_state);
int  mtk_gop_crtc_flush(struct mtk_tv_drm_crtc *mcrtc,
	struct drm_crtc_state *crtc_state);
...
EN_GOP_WRAP_ML_SYNC mtk_gop_get_ml_syncId(struct mtk_tv_drm_crtc *mcrtc);
