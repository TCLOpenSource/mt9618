...
int gop_use_ml = 1;	// default use ML
...
static void _KdrvCheckGOPRet(MS_BOOL bRet, char *cmd)
{
	if (bRet != TRUE)
		UTP_GOP_LOGE(LOG_TAG, "%s failed\n", cmd);
}
...
static EN_GOP_OPDST WrpOsdb2Utp(EN_GOP_WRAP_OPOSDB_DST eOpOsdbDst)
{
	EN_GOP_OPDST eDst;

	switch (eOpOsdbDst) {
	case E_GOP_WARP_OPDST_OSDB0:
		eDst = E_GOP_OPDST_OSDB0;
		break;
	case E_GOP_WARP_OPDST_OSDB1:
		eDst = E_GOP_OPDST_OSDB1;
		break;
	default:
		eDst = E_GOP_OPDST_INVALID;
		break;
	}
...
static enum EN_HW_GOP_CSC WrpCsc2Hw(enum EN_GOP_WRAP_CSC eWrapCsc)
{
	enum EN_HW_GOP_CSC eCsc;

	switch (eWrapCsc) {
	case E_GOP_WRAP_CSC_Y2R_709_F2F:
		eCsc = E_HW_GOP_CSC_Y2R_709_F2F;
		break;
	case E_GOP_WRAP_CSC_Y2R_709_F2L:
		eCsc = E_HW_GOP_CSC_Y2R_709_F2L;
		break;
	case E_GOP_WRAP_CSC_Y2R_709_L2F:
		eCsc = E_HW_GOP_CSC_Y2R_709_L2F;
		break;
	case E_GOP_WRAP_CSC_Y2R_709_L2L:
		eCsc = E_HW_GOP_CSC_Y2R_709_L2L;
		break;
	case E_GOP_WRAP_CSC_Y2R_601_F2F:
		eCsc = E_HW_GOP_CSC_Y2R_601_F2F;
		break;
	case E_GOP_WRAP_CSC_Y2R_601_F2L:
		eCsc = E_HW_GOP_CSC_Y2R_601_F2L;
		break;
	case E_GOP_WRAP_CSC_Y2R_601_L2F:
		eCsc = E_HW_GOP_CSC_Y2R_601_L2F;
		break;
	case E_GOP_WRAP_CSC_Y2R_601_L2L:
		eCsc = E_HW_GOP_CSC_Y2R_601_L2L;
		break;
	case E_GOP_WRAP_CSC_R2Y_709_F2F:
		eCsc = E_HW_GOP_CSC_R2Y_709_F2F;
		break;
	case E_GOP_WRAP_CSC_R2Y_709_F2L:
		eCsc = E_HW_GOP_CSC_R2Y_709_F2L;
		break;
	case E_GOP_WRAP_CSC_R2Y_709_L2F:
		eCsc = E_HW_GOP_CSC_R2Y_709_L2F;
		break;
	case E_GOP_WRAP_CSC_R2Y_709_L2L:
		eCsc = E_HW_GOP_CSC_R2Y_709_L2L;
		break;
	case E_GOP_WRAP_CSC_R2Y_601_F2F:
		eCsc = E_HW_GOP_CSC_R2Y_601_F2F;
		break;
	case E_GOP_WRAP_CSC_R2Y_601_F2L:
		eCsc = E_HW_GOP_CSC_R2Y_601_F2L;
		break;
	case E_GOP_WRAP_CSC_R2Y_601_L2F:
		eCsc = E_HW_GOP_CSC_R2Y_601_L2F;
		break;
	case E_GOP_WRAP_CSC_R2Y_601_L2L:
		eCsc = E_HW_GOP_CSC_R2Y_601_L2L;
		break;
	default:
		eCsc = E_HW_GOP_CSC_NONE;
		break;
	}
...
int utp_gop_InitByGop(UTP_GOP_INIT_INFO *utp_init_info, MTK_GOP_ML_INFO *pGopMlInfo)
{
	...
	GopInit.u32ML_Fd = pGopMlInfo->fd[E_GOP_WRAP_ML_DISP_SYNC];
	GopInit.u32ML_FdVGSep = pGopMlInfo->fd[E_GOP_WRAP_ML_OSD_SYNC];
...
int utp_gop_set_enable(UTP_GOP_SET_ENABLE *en_info, __u8 gop_ml_resIdx)
{
	...
	if (en_info->GopChangeStatus & BIT(GOP_BYPASS_STATE_CHANGE)) {
		utp_ret = MApi_GOP_SetGOPBypassPath(u8gop, en_info->bypass_path);
...
int utp_gop_SetWindow(UTP_GOP_SET_WINDOW *p_win)
{
	...
	MS_U64 gopAidType;
	EN_GOP_OPDST eOpOSDBDst = E_GOP_OPDST_INVALID;
	enum EN_HW_GOP_CSC eCsc = E_HW_GOP_CSC_NONE;
	...
	bEnableMultiAlpha = (MS_BOOL)p_win->stGopAlphaMode.bEnMultiAlpha;
	...
	eOpOSDBDst = WrpOsdb2Utp(p_win->eOpOsdbDst);
	eCsc = WrpCsc2Hw(p_win->eCSC);
	...
	UTP_GOP_LOGD(LOG_TAG, "bEnableMultiAlpha %d\n", bEnableMultiAlpha);
	...
	if (p_win->GopChangeStatus & BIT(GOP_BYPASS_STATE_CHANGE)) {
	...
	if (p_win->GopChangeStatus & BIT(GOP_OVERSCAN_STATE_CHANGE)) {
		utp_ret = MApi_GOP_SetOverScanInfo(u8gop, p_win->bOverscanEn);
		_WrpCheckGOPRet(utp_ret, "MApi_GOP_SetOverScanInfo");
	}

	utp_ret = MApi_GOP_SetPanelSize(u8gop, p_win->panel_w, p_win->panel_h);
	_WrpCheckGOPRet(utp_ret, "MApi_GOP_SetPanelSize");
	utp_ret = MApi_GOP_SetMixer4Size(p_win->gop_mixer4_w, p_win->gop_mixer4_h);
	_WrpCheckGOPRet(utp_ret, "MApi_GOP_SetMixer4Size");
	utp_ret = MApi_GOP_SetMixer2Size(p_win->gop_mixer2_w, p_win->gop_mixer2_h);
	_WrpCheckGOPRet(utp_ret, "MApi_GOP_SetMixer2Size");
	bRet = KDrv_HW_GOP_SetMixerOSDB0OutSize(p_win->gop_mixer4_w, p_win->gop_mixer4_h);
	_KdrvCheckGOPRet(bRet, "KDrv_HW_GOP_SetMixerOSDB0OutSize");
	...
	memset(&stretchWinInfo, 0x0, sizeof(GOP_StretchInfo));
	...
	if (p_win->GopChangeStatus & BIT(GOP_ALPHA_MODE_STATE_CHANGE)) {
		...
		_WrpCheckGOPRet(utp_ret, "MApi_GOP_GWIN_SetBlending");
		bRet = KDrv_HW_GOP_EnableMultiAlpha(u8gop, bEnableMultiAlpha);
		_KdrvCheckGOPRet(bRet, "KDrv_HW_GOP_EnableMultiAlpha");
	}
	...
	if (p_win->GopChangeStatus & BIT(GOP_OPBLEND_POS_STATE_CHANGE)) {
		utp_ret = MApi_GOP_SetOsdb_Dst(u8gop, eOpOSDBDst);
		_WrpCheckGOPRet(utp_ret, "MApi_GOP_SetOsdb_Dst");
	}

	if (p_win->GopChangeStatus & BIT(GOP_CSC_STATE_CHANGE)) {
		bRet = KDrv_HW_GOP_SetCSC(u8gop, eCsc);
		_KdrvCheckGOPRet(bRet, "KDrv_HW_GOP_CSC");
	}
	...
	utp_ret = MApi_GOP_TriggerAutoDetect(u8gop);
	_WrpCheckGOPRet(utp_ret, "MApi_GOP_TriggerAutoDetect");
...
int gop_wrapper_set_testpattern(MTK_GOP_TESTPATTERN *pGopTestPatternInfo)
{
	...
	stGOPTestPatternInfo.MoveDir = (EN_GOP_TESTPATTERN_MOVE_DIR)pGopTestPatternInfo->MoveDir;
...
enum EN_GOP_RESULT gop_wrapper_ml_fire(__u8 u8MlResIdx)
{
	E_GOP_API_Result ret;

	ret = MApi_GOP_Ml_Fire(u8MlResIdx);
	if (ret == GOP_API_SUCCESS)
		return E_GOP_WRAP_SUCCESS;
	else if (ret == GOP_API_MENULOAD_RETRY)
		return E_GOP_WRAP_MENULOAD_RETRY;
	else
		return E_GOP_WRAP_FAIL;
}
...
int gop_wrapper_setLayer(MS_U32 GopIdx, MS_U32 MlResIdx)
{
	...
	if (bLayerChange) {
		...
		if (likely(gop_use_ml))
			utp_ret |= MApi_GOP_GWIN_UpdateRegOnceEx2((MS_U8)GopIdx, TRUE,
								TRUE, MlResIdx);
		else
			utp_ret |= MApi_GOP_GWIN_UpdateRegOnceEx2((MS_U8)GopIdx, TRUE,
								FALSE, MlResIdx);
		...
		if (likely(gop_use_ml))
			utp_ret |= MApi_GOP_GWIN_UpdateRegOnceEx2((MS_U8)GopIdx,
							FALSE, TRUE, MlResIdx);
		else
			utp_ret |= MApi_GOP_GWIN_UpdateRegOnceEx2((MS_U8)GopIdx,
							FALSE, FALSE, MlResIdx);
		_WrpCheckGOPRet(utp_ret, "MApi_GOP_GWIN_UpdateRegOnceEx2");
...
int gop_wrapper_get_ROI(MTK_GOP_GETROI *pRoiInfo)
{
	...
	if (MApi_GOP_GetRoi(&stGOPRoi) == GOP_API_SUCCESS) {
		pRoiInfo->u64CntOSDB0 = stGOPRoi.u64CntOSDB0;
		pRoiInfo->u64CntOSDB1 = stGOPRoi.u64CntOSDB1;
		ret = 0;
	} else {
		pRoiInfo->u64CntOSDB0 = 0;
		pRoiInfo->u64CntOSDB1 = 0;
		ret = -1;
	}
...
int gop_wrapper_set_vg_order(EN_GOP_WRAP_VIDEO_ORDER_TYPE eVGOrder, __u8 u8MlResIdx)
{
	...
	if (likely(gop_use_ml))
		...
		MApi_GOP_GWIN_UpdateRegOnceEx2(0, TRUE, FALSE, u8MlResIdx);
	...
	if (likely(gop_use_ml))
		...
		MApi_GOP_GWIN_UpdateRegOnceEx2(0, FALSE, FALSE, u8MlResIdx);

	return ret;
}

int gop_wrapper_set_osdb_loc(EN_GOP_WRAP_OSDB_LOCATION_TYPE eOsdbloc, __u8 u8MlResIdx)
{
	int ret = 0;
	EN_GOP_OSDB_LOCATION eGOPOsdbLoc;
	...
	switch (eOsdbloc) {
	case E_GOP_WRAP_PQGAMMA_OSDB_LD_PADJ:
		eGOPOsdbLoc = E_GOP_PQGAMMA_OSDB_LD_PADJ;
		break;
	case E_GOP_WRAP_PQGAMMA_LD_OSDB_PADJ:
		eGOPOsdbLoc = E_GOP_PQGAMMA_LD_OSDB_PADJ;
		break;
	case E_GOP_WRAP_OSDB_PQGAMMA_LD_PADJ:
		eGOPOsdbLoc = E_GOP_OSDB_PQGAMMA_LD_PADJ;
		break;
	case E_GOP_WRAP_OSDB_LD_PQGAMMA_PADJ:
		eGOPOsdbLoc = E_GOP_OSDB_LD_PQGAMMA_PADJ;
		break;
	case E_GOP_WRAP_LD_PQGAMMA_OSDB_PADJ:
		eGOPOsdbLoc = E_GOP_LD_PQGAMMA_OSDB_PADJ;
		break;
	case E_GOP_WRAP_LD_OSDB_PQGAMMA_PADJ:
		eGOPOsdbLoc = E_GOP_LD_OSDB_PQGAMMA_PADJ;
		break;
	default:
		eGOPOsdbLoc = E_GOP_LOCATION_MAX;
		break;
	}

	if (MApi_GOP_Set_osdb_loc(eGOPOsdbLoc) != GOP_API_SUCCESS) {
		UTP_GOP_LOGE(LOG_TAG, "MApi_GOP_Set_osdb_loc failed\n");
		ret = -1;
	}
