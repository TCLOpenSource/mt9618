...
struct mtk_tv_drm_crtc_ops {
	...
	int (*timeline_inc)(struct mtk_tv_drm_crtc *mcrtc, uint32_t planeIdx);
	int (*set_graphic_pq_buf)(struct mtk_tv_drm_crtc *mcrtc,
		struct drm_mtk_tv_graphic_pq_setting *PqBufInfo);
	int (*get_graphic_roi)(struct drm_mtk_tv_graphic_roi_info *RoiInfo);
	int (*set_graphic_roi)(uint32_t threshold);
	int (*pnlgamma_get_enable)(struct drm_mtk_tv_pnlgamma_enable *enableInfo);
	int (*ldm_set_led_check)(struct mtk_tv_drm_crtc *crtc,
		struct drm_mtk_tv_ldm_led_check *ledcheckInfo);
	int (*ldm_set_black_insert_enable)(struct mtk_tv_drm_crtc *crtc,
		struct drm_mtk_tv_ldm_black_insert_enable *blackInsertInfo);
	int (*ldm_set_pq_param)(struct mtk_tv_drm_crtc *crtc,
		struct drm_mtk_tv_ldm_pq_param *ldparamInfo);
	int (*ldm_set_VCOM_enable)(struct mtk_tv_drm_crtc *crtc,
		struct drm_mtk_tv_ldm_VCOM_enable *ldVCOMInfo);
...
int mtk_tv_drm_graphic_get_roi_ioctl(struct drm_device *dev, void *data,
						struct drm_file *file_priv);
int mtk_tv_drm_graphic_set_roi_ioctl(struct drm_device *dev, void *data,
						struct drm_file *file_priv);
int mtk_tv_drm_get_pnlgamma_enable_ioctl(struct drm_device *dev, void *data,
						struct drm_file *file_priv);
