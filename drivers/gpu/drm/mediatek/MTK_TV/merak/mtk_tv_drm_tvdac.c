...
static int mtk_tvdac_set_cvbso_swen_clock(struct device *dev, struct mtk_tvdac_context *pctx_tvdac)
{
	...
	if (pctx_tvdac->status == TRUE) {
		if (MTK_DRM_CVBSO_INIT_IDX == REG_RENDER_CVBSO_VIF_VIF)
			cvbso_swen_type = REG_RENDER_CVBSO_SWEN_VIF_VIF;
		else if (MTK_DRM_CVBSO_INIT_IDX == REG_RENDER_CVBSO_CVBS_ON)
			cvbso_swen_type = REG_RENDER_CVBSO_SWEN_CVBS_ON;
	} else
...
static int mtk_tvdac_disable_clock(
	struct device *dev,
	char *clk_name)
{
	int ret = 0;
	...
	if (__clk_is_enabled(target_clk) == true)
		clk_disable_unprepare(target_clk);
...
static int mtk_tvdac_set_cvbso_clock(struct device *dev, struct mtk_tvdac_context *pctx_tvdac)
{
	...
	if (pctx_tvdac->status == TRUE) {
		if (MTK_DRM_CVBSO_INIT_IDX == REG_RENDER_CVBSO_VIF_VIF)
			...
			cvbso_clk_type = REG_RENDER_CVBSO_CLK_SETTING_CVBS_ON;
	} else
	...
	for (idx = 0; idx < count; idx++) {
		...
		if (cvbso_clk_type == REG_RENDER_CVBSO_CLK_SETTING_OFF)
			ret = mtk_tvdac_disable_clock(dev, clk_info[idx].clk_name);
		else
	...
	struct drm_file *file_priv)
{
	...
	args = (struct drm_mtk_tv_dac *)data;
...
int mtk_tvdac_init(struct device *dev)
{
	...
	struct mtk_tv_kms_context *pctx = 0;
	struct mtk_tvdac_context *pctx_tvdac = 0;
	struct mtk_video_context *pctx_video = 0;
	struct mtk_video_hw_version *video_hw_version =	0;
	...
	ret = drv_hwreg_render_cvbso_set_hw_version(video_hw_version->video_blending);
	if (ret) {
		TVDAC_MSG_RETURN_CHECK(ret);
		return ret;
	}
	...
	if (ret) {
		TVDAC_MSG_RETURN_CHECK(ret);
		ret = 0;  // Do NOT return fail to avoid error check causes bringup stuck
	}
