...
#define PQML_MAX_CMD	(800)
#define CFDML_MAX_CMD	(100)
#define ADL_MAX_BUFSIZE	(0x3840)
...
#define GOP_CAPS_OFFSET_GOP0_AFBC	(6)
...
#define GOP_CAPS_OFFSET_GOP_AFBC_COMMON	(0)
...
#define GOP_M6_SERIES_MINOR	(0)
#define GOP_M6L_SERIES_MINOR	(1)
...
#define GOP_CONST_ALPHA_DEF	(0xFF)
...
#define GOP_ML_CMD_SIZE	(8)
#define CFD_ML_BUF_ENTRY_SIZE	(6)
...
#define GOP_DTS_IORESOURCE_NUM	(3)
#define GOP_DTS_IORESOURCE_SIZE	(4)
#define GOP_BUS_OFFSET	(0x1c000000)
#define GOP_DTS_IORESOURCE_PA0_IDX	(1)
#define GOP_DTS_IORESOURCE_PA1_IDX	(5)
#define GOP_DTS_IORESOURCE_PA2_IDX	(9)
#define GOP_DTS_IORESOURCE_SIZE0_IDX	(3)
#define GOP_DTS_IORESOURCE_SIZE1_IDX	(7)
#define GOP_DTS_IORESOURCE_SIZE2_IDX	(11)
#define GOP_HWREG_ADDR0_IDX	(0)
#define GOP_HWREG_ADDR1_IDX	(1)
#define GOP_HWREG_ADDR2_IDX	(2)
//-------------------------------------------------------------------------------------------------
//  Type and Structure
//-------------------------------------------------------------------------------------------------
...
/// Define GWIN color format.
	typedef enum {
		...
		E_DRV_GOP_COLOR_A2RGB10 = E_GOP_COLOR_A2RGB10,
		/// Color format A2BGR10.
		E_DRV_GOP_COLOR_A2BGR10 = E_GOP_COLOR_A2BGR10,
		/// Color format RGB10A2.
		E_DRV_GOP_COLOR_RGB10A2 = E_GOP_COLOR_RGB10A2,
		/// Color format BGR10A2.
		E_DRV_GOP_COLOR_BGR10A2 = E_GOP_COLOR_BGR10A2,
		/// Color format RGB888.
		E_DRV_GOP_COLOR_RGB888 = E_GOP_COLOR_RGB888,
...
/// Define GOP chip property for different chip characteristic.
	typedef struct __attribute__((__packed__)) {
		...
		MS_U32 u32GOPNum;
		MS_U8 u8OpMuxNum;
		void __iomem *VAdr[GOP_MAX_SUPPORT_DEFAULT];
...
///GOP Ignore init value
	typedef enum {
		...
		E_DRV_GOP_SET_DST_OP_IGNORE = 0x0008,
		E_DRV_GOP_ENABLE_TRANSCLR_IGNORE = 0x0010,
		E_DRV_GOP_SWRST_IGNORE = 0x0020,
...
///the GOP config
	...
	typedef struct DLL_PACKED {
		GOP_HWREG_INFO stCFD[CFDML_MAX_CMD];
	} GOP_CFD_INFO;

	typedef struct DLL_PACKED {
		GOP_HWREG_INFO stPQ[PQML_MAX_CMD];
	} GOP_PQ_INFO;

	typedef struct DLL_PACKED {
		MS_U8 AdlBuf[ADL_MAX_BUFSIZE];
	} GOP_ADL_INFO;
	...
	typedef struct __attribute__((__packed__)) {
		...
		MS_BOOL bMixer2RmaEn;
		MS_BOOL bMixer4RmaEn;
		MS_BOOL bBypassEn;
		EN_GOP_OUTPUT_MODE GOPOutMode;
		MS_U32 u32ML_Fd[E_GOP_ML_MAX];
		enum sm_return_type eMLStatus[E_GOP_ML_MAX];
		MS_U16 u16ML_CmdCnd[SHARED_GOP_MAX_COUNT];
		MS_U16 u16ML_PQCmdCnt[SHARED_GOP_MAX_COUNT];
		MS_U8 u8ML_MemIdx[SHARED_GOP_MAX_COUNT];
		MS_U64 ml_va_address[SHARED_GOP_MAX_COUNT];
		struct sm_ml_info stMlInfo[SHARED_GOP_MAX_COUNT];
		ST_GOP_PQ_CFD_ML_INFO stGopPqCfdMlInfo[SHARED_GOP_MAX_COUNT];
		ST_GOP_ADL_INFO stGOPAdlInfo[SHARED_GOP_MAX_COUNT];
		MS_BOOL bIsGOPAdlUpdated[SHARED_GOP_MAX_COUNT];
		ST_GOP_PQCTX_INFO stGopPQCtx[SHARED_GOP_MAX_COUNT];
		GOP_ADL_INFO GopAdl[SHARED_GOP_MAX_COUNT];
		GOP_CFD_INFO GopCFDMl[SHARED_GOP_MAX_COUNT];
		GOP_PQ_INFO GopPQMl[SHARED_GOP_MAX_COUNT];
		MS_BOOL GopOverScanEn[SHARED_GOP_MAX_COUNT];
	} MS_GOP_CTX_SHARED;
...
//======================================================================================
void MDrv_GOP_GWIN_SetStretchWin(MS_U8 u8GOP, MS_U16 x,
					 MS_U16 y, MS_U16 width, MS_U16 height);
void MDrv_GOP_GWIN_Get_StretchWin(MS_U8 u8GOP, MS_U16 *x,
					  MS_U16 *y, MS_U16 *width, MS_U16 *height);
...
GOP_Result MDrv_GOP_ml_fire(MS_GOP_CTX_LOCAL *pGOPCtx, MS_U8 u8MlResIdx);
