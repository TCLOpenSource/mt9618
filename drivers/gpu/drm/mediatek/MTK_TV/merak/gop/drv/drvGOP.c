	...
	enum EN_PQAPI_RESULT_CODES enRet)
{
	if ((enRet != E_PQAPI_RC_SUCCESS) &&
		(enRet != E_PQAPI_RC_FAIL_HW_NOT_SUPPORT)) {
		    ...
		    int ret_array_length)
{
	...
	if (u32TmpArray == NULL) {
		DRM_ERROR("[GOP][%s, %d]: %s kmalloc_array failed!\r\n",
			__func__, __LINE__, name);
...
MS_U32 readChipCaps_DTB(void)
{
	...
	if (np != NULL) {
		...
		if (((u32Tmp & GOP_MAJOR_IPVERSION_OFFSET_MSK) >>
			...
			name = MTK_DRM_GOP_REGBASE_TAG;
			pu32array = kmalloc(sizeof(MS_U32) * GOP_DTS_IORESOURCE_NUM *
				GOP_DTS_IORESOURCE_SIZE, GFP_KERNEL);
			if (!pu32array) {
				DRM_ERROR("[GOP][%s, %d]: kmalloc failed\n",
					__func__, __LINE__);
				return -ENOSPC;
			}
			ret = DTBArray(np, name, pu32array, GOP_DTS_IORESOURCE_NUM *
				GOP_DTS_IORESOURCE_SIZE);
			if (ret != 0) {
				DRM_ERROR("[GOP][%s, %d]: read DTS failed\n",
					__func__, __LINE__);
				kfree(pu32array);
				return -EACCES;
			}
			if (pu32array[GOP_DTS_IORESOURCE_SIZE0_IDX] != 0) {
				RIUBase1 = ioremap(pu32array[GOP_DTS_IORESOURCE_PA0_IDX],
					pu32array[GOP_DTS_IORESOURCE_SIZE0_IDX]);
				if (!RIUBase1) {
					DRM_ERROR("[GOP][%s, %d]: ioremap failed\n",
						__func__, __LINE__);
					kfree(pu32array);
					return -EACCES;
				}
				g_GopChipPro.VAdr[GOP_HWREG_ADDR0_IDX] = (MS_VIRT *)RIUBase1;
				g_GopChipPro.PAdr[GOP_HWREG_ADDR0_IDX] =
					(pu32array[GOP_DTS_IORESOURCE_PA0_IDX] - GOP_BUS_OFFSET);
				g_GopChipPro.u32RegSize[GOP_HWREG_ADDR0_IDX] =
					pu32array[GOP_DTS_IORESOURCE_SIZE0_IDX];
			}

			if (pu32array[GOP_DTS_IORESOURCE_SIZE1_IDX] != 0) {
				RIUBase1 = ioremap(pu32array[GOP_DTS_IORESOURCE_PA1_IDX],
					pu32array[GOP_DTS_IORESOURCE_SIZE1_IDX]);
				if (!RIUBase1) {
					DRM_ERROR("[GOP][%s, %d]: ioremap failed\n",
						__func__, __LINE__);
					kfree(pu32array);
				...
				g_GopChipPro.VAdr[GOP_HWREG_ADDR1_IDX] = (MS_VIRT *)RIUBase1;
				g_GopChipPro.PAdr[GOP_HWREG_ADDR1_IDX] =
					(pu32array[GOP_DTS_IORESOURCE_PA1_IDX] - GOP_BUS_OFFSET);
				g_GopChipPro.u32RegSize[GOP_HWREG_ADDR1_IDX] =
					pu32array[GOP_DTS_IORESOURCE_SIZE1_IDX];
			}

			if (pu32array[GOP_DTS_IORESOURCE_SIZE2_IDX] != 0) {
				RIUBase1 = ioremap(pu32array[GOP_DTS_IORESOURCE_PA2_IDX],
					pu32array[GOP_DTS_IORESOURCE_SIZE2_IDX]);
				if (!RIUBase1) {
					DRM_ERROR("[GOP][%s, %d]: ioremap failed\n",
						__func__, __LINE__);
					kfree(pu32array);
				...
				g_GopChipPro.VAdr[GOP_HWREG_ADDR2_IDX] = (MS_VIRT *)RIUBase1;
				g_GopChipPro.PAdr[GOP_HWREG_ADDR2_IDX] =
					(pu32array[GOP_DTS_IORESOURCE_PA2_IDX] - GOP_BUS_OFFSET);
				g_GopChipPro.u32RegSize[GOP_HWREG_ADDR2_IDX] =
					pu32array[GOP_DTS_IORESOURCE_SIZE2_IDX];
			}
...
MS_U8 MDrv_DumpGopByGwinId(MS_GOP_CTX_LOCAL *pGOPCtx, MS_U32 GwinID)
{
	...
	if (u8MaxGOPNum >= SHARED_GOP_MAX_COUNT) {
		DRM_ERROR("[GOP][%s, %d] gop index out of bound\n", __func__, __LINE__);
		return INVALID_GOP_ID;
	}
	...
	if (pGOPDrvLocalCtx != NULL) {
		for (i = 0; i < GOP_MAX_SUPPORT_DEFAULT; i++) {
			if (pGOPDrvLocalCtx->pGopChipPro->PAdr[i] != 0) {
				...
				(uint64_t)(uintptr_t)pGOPDrvLocalCtx->pGopChipPro->VAdr[i]);
			}
		}
...
void MDrv_GOP_Init(MS_GOP_CTX_LOCAL *pGOPCtx, MS_U8 u8GOPNum)
{
	...
	if (u16PnlWidth == GOP_8K_WIDTH && u16PnlHeight == GOP_8K_HEIGHT) {
		...
		MDrv_GOP_SetMixer2Size(GOP_8K_WIDTH, GOP_8K_HEIGHT);
		_MDrv_GOP_SetPxmSize(GOP_8K_WIDTH, GOP_8K_HEIGHT);
		MDrv_GOP_SetMixer4Size(GOP_4K_WIDTH, GOP_4K_HEIGHT);
	} else {
		KDrv_HW_GOP_SetStep2HVSPHscale(FALSE, 0x0);
		KDrv_HW_GOP_SetStep2HVSPVscale(FALSE, 0x0);
		MDrv_GOP_SetMixer2Size(u16PnlWidth, u16PnlHeight);
		_MDrv_GOP_SetPxmSize(u16PnlWidth, u16PnlHeight);
		MDrv_GOP_SetMixer4Size(u16PnlWidth, u16PnlHeight);
		KDrv_HW_GOP_SetMixerVGOutSize(u16PnlWidth, u16PnlHeight);
		KDrv_HW_GOP_SetMixerOSDB0OutSize(u16PnlWidth, u16PnlHeight);
	}
...
{
	GOP_CTX_DRV_LOCAL *pGOPDrvLocalCtx = (GOP_CTX_DRV_LOCAL *)pGOPCtx;
	MS_GOP_CTX_SHARED *pGOPCtxShared = NULL;
	MS_U16 u16PnlWidth = 0;
	MS_U32 u32Hratio = 0;
	MS_U16 u16Hin = 0, u16Hout = 0;
	MS_U16 u16DispX = 0, u16DispW = 0;
	MS_U16 u16DispY = 0, u16DispH = 0;

	if (u8GOP >= SHARED_GOP_MAX_COUNT ||
		_IsGopNumVaild(pGOPCtx, u8GOP) == FALSE) {
		DRM_ERROR("[%s] not support gop%d in this chip version!!\n", __func__, u8GOP);
		return;
	}

	if (!pGOPDrvLocalCtx) {
		DRM_ERROR("[%s] Null pointer of GOP_CTX_DRV_LOCAL\n", __func__);
		return;
	}

	pGOPCtxShared = pGOPDrvLocalCtx->apiCtxLocal.pGOPCtxShared;
	if (!pGOPCtxShared) {
		DRM_ERROR("[%s] Null pointer of MS_GOP_CTX_SHARED\n", __func__);
		return;
	}

	u16PnlWidth = pGOPCtxShared->u16PnlWidth[u8GOP];

	if (pGOPCtxShared->GopOverScanEn[u8GOP] == FALSE && dst > u16PnlWidth) {
	...
	if (u16PnlWidth == GOP_8K_WIDTH) {
		if (bEnable == TRUE) {
			if ((dst/HVSP2_RATIO_DEFAULT) > src) {
				...
				if ((dst/HVSP2_RATIO_DEFAULT) != 0) {
					u32Hratio = (MS_U32)(src*SCALING_MULITPLIER) /
						(dst/HVSP2_RATIO_DEFAULT);
					u16Hout = (dst/HVSP2_RATIO_DEFAULT);
				} else {
					u32Hratio = 0;
				}
			} else if ((dst/HVSP2_RATIO_DEFAULT) == src) {
				u32Hratio = 0;
				bEnable = FALSE;
			} else {
				u32Hratio = 0;
				bEnable = FALSE;
				DRM_ERROR("[%s]GOP:%d,src_w:%d x 2 > crtc_w:%d,src_w align to %d\n",
					__func__, u8GOP, src, dst, (dst/HVSP2_RATIO_DEFAULT));
				KDrv_HW_GOP_GetGopStretchWin(u8GOP, &u16DispX, &u16DispY,
					&u16DispW, &u16DispH);
				KDrv_HW_GOP_SetGopStretchWin(u8GOP, u16DispX, u16DispY,
					(dst/HVSP2_RATIO_DEFAULT), u16DispH);
			}
		} else {
			u32Hratio = 0;
		}
	...
	KDrv_HW_GOP_SetStep1HVSPHscale(u8GOP, bEnable, u32Hratio, u16Hin, u16Hout);
}
			      ...
			      MS_U16 dst)
{
	GOP_CTX_DRV_LOCAL *pGOPDrvLocalCtx = (GOP_CTX_DRV_LOCAL *)pGOPCtx;
	MS_GOP_CTX_SHARED *pGOPCtxShared = NULL;
	MS_U16 u16PnlHeight = 0;
	MS_U32 u32Vratio = 0;
	MS_U16 u16Vin = 0, u16Vout = 0;
	MS_U16 u16DispX = 0, u16DispW = 0;
	MS_U16 u16DispY = 0, u16DispH = 0;
	...
	u16PnlHeight = pGOPCtxShared->u16PnlHeight[u8GOP];

	if (pGOPCtxShared->GopOverScanEn[u8GOP] == FALSE && dst > u16PnlHeight) {
	...
	if (u16PnlHeight == GOP_8K_HEIGHT) {
		if (bEnable == TRUE) {
			if ((dst/HVSP2_RATIO_DEFAULT) > src) {
				...
				//others size use step2 HVSP when 8k timing
				if ((dst/HVSP2_RATIO_DEFAULT) != 0) {
					u32Vratio = (MS_U32)(src*SCALING_MULITPLIER) /
						(dst/HVSP2_RATIO_DEFAULT);
					u16Vout = (dst/HVSP2_RATIO_DEFAULT);
				} else {
					u32Vratio = 0;
				}
			} else if ((dst/HVSP2_RATIO_DEFAULT) == src) {
				...
				u32Vratio = 0;
				bEnable = FALSE;
				DRM_ERROR("[%s]GOP:%d,src_h:%d x 2 > crtc_h:%d,src_h align to %d\n",
					__func__, u8GOP, src, dst, (dst/HVSP2_RATIO_DEFAULT));
				KDrv_HW_GOP_GetGopStretchWin(u8GOP, &u16DispX, &u16DispY,
					&u16DispW, &u16DispH);
				KDrv_HW_GOP_SetGopStretchWin(u8GOP, u16DispX, u16DispY,
					u16DispW, (dst/HVSP2_RATIO_DEFAULT));
			}
		} else {
			u32Vratio = 0;
		}
	...
	KDrv_HW_GOP_SetStep1HVSPVscale(u8GOP, bEnable, u32Vratio, u16Vin, u16Vout);
}
...
{
	E_HW_GOPStrchVMode eHWVMode = E_HW_GOP_VSTRCH_BILINEAR;
...
GOP_Result MDrv_GOP_SetTestPattern(ST_GOP_TESTPATTERN *pstGOPTestPatInfo)
{
	...
	HWGOPPatInfo.MoveDir = (EN_HW_GOP_TESTPATTERN_MOVE_DIR)pstGOPTestPatInfo->MoveDir;
...
GOP_Result MDrv_GOP_SetMLScript(MS_GOP_CTX_LOCAL *pGOPCtx, MS_U8 u8GOP, MS_U8 MlResIdx)
{
	...
	enum EN_PQAPI_RESULT_CODES enRet;
	MS_GOP_CTX_SHARED *pGOPShare;
	MS_U16 MlCmdCnt, MlPqCmdCnt;
	MS_U64 MlVa, Ml_MaxAddr = 0;
	...
	pGOPShare = pGOPCtx->pGOPCtxShared;
	...
	u32_cfd_ml_bufsize = pGOPShare->stGopPqCfdMlInfo[u8GOP].u32_cfd_ml_bufsize;
	u32_pq_ml_bufsize = pGOPShare->stGopPqCfdMlInfo[u8GOP].u32_pq_ml_bufsize;
	pCtx = pGOPShare->stGopPQCtx[u8GOP].pCtx;
	bCFDUpdated = pGOPShare->stGopPqCfdMlInfo[u8GOP].bCFDUpdated;
	bPQUpdated = pGOPShare->stGopPqCfdMlInfo[u8GOP].bPQUpdated;
	Ml_MaxAddr = pGOPShare->stMlInfo[MlResIdx].start_va +
					pGOPShare->stMlInfo[MlResIdx].mem_size;

	if (Ml_MaxAddr == 0) {
		DRM_ERROR("[GOP][%s, %d] menuload max address is 0\n", __func__, __LINE__);
		KDrv_HW_GOP_MlTblFlush(u8GOP);
		return GOP_FAIL;
	}

	if ((pGOPShare->eMLStatus[MlResIdx] == E_SM_RET_OK)
		&& (bCFDUpdated || bPQUpdated)) {
		...
		PQMLCtxInfo.pGopAddr = (void *)(uintptr_t)pGOPShare->ml_va_address[MlResIdx];
		PQMLCtxInfo.pGopMaxAddr = (void *)(uintptr_t)Ml_MaxAddr;
		...
		if (bCFDUpdated) {
			...
			for (i = 0; i < (u32_cfd_ml_bufsize/CFD_ML_BUF_ENTRY_SIZE); i++) {
				PQRegInfo.u32RegIdx = pGOPShare->GopCFDMl[u8GOP].stCFD[i].u32RegIdx;
				PQRegInfo.u16Value = pGOPShare->GopCFDMl[u8GOP].stCFD[i].u16Value;
		...
		if (bPQUpdated) {
			...
			for (i = 0; i < (u32_pq_ml_bufsize/CFD_ML_BUF_ENTRY_SIZE); i++) {
				PQRegInfo.u32RegIdx = pGOPShare->GopPQMl[u8GOP].stPQ[i].u32RegIdx;
				PQRegInfo.u16Value = pGOPShare->GopPQMl[u8GOP].stPQ[i].u16Value;
				enRet = MApi_PQ_SetHWReg_NonTs(pCtx, &PQRegInfo, &PQRegState);
				Check_GOP_CfdPQ_ML_Ret("MApi_PQ_SetHWReg_NonTs", enRet);
			}
		}
		...
		pGOPShare->stGopPqCfdMlInfo[u8GOP].bCFDUpdated = FALSE;
		pGOPShare->stGopPqCfdMlInfo[u8GOP].bPQUpdated = FALSE;
	}
	...
	write_cmd_info.va_address = pGOPShare->ml_va_address[MlResIdx] +
		(MlPqCmdCnt * GOP_ML_CMD_SIZE);
	write_cmd_info.va_max_address = Ml_MaxAddr;
	write_cmd_info.add_32_support = FALSE;

	pGOPShare->u16ML_PQCmdCnt[MlResIdx] += MlPqCmdCnt;
	if (pGOPShare->u16ML_PQCmdCnt[MlResIdx] > PIM_GOP_DOMAIN_ML_MAX_SIZE) {
		DRM_ERROR("[GOP]PQ cmd:%d > max size: %d\n",
			pGOPShare->u16ML_PQCmdCnt[MlResIdx],
			PIM_GOP_DOMAIN_ML_MAX_SIZE);
	}

	if ((write_cmd_info.va_address + (write_cmd_info.cmd_cnt*GOP_ML_CMD_SIZE)) > Ml_MaxAddr) {
		DRM_ERROR("[GOP]ML Over buffer bound, GOP:%d, max addr=0x%llx\n",
			u8GOP, Ml_MaxAddr);
	}
	...
	if (pGOPShare->eMLStatus[MlResIdx] == E_SM_RET_OK) {
		...
		pGOPShare->u16ML_CmdCnd[MlResIdx] += (MlCmdCnt + MlPqCmdCnt);
		pGOPShare->ml_va_address[MlResIdx] = write_cmd_info.va_address;
	}
...
GOP_Result MDrv_GOP_SetVGOrder(EN_GOP_VG_ORDER eVGOrder)
{
	...
	if (!KDrv_HW_GOP_SetVGOrder(eHWVGOrder)) {
...
MS_U32 MDrv_GOP_ml_fire(MS_GOP_CTX_LOCAL *pGOPCtx, MS_U8 u8MlResIdx)
{
	...
	if (pGOPCtx->pGOPCtxShared->eMLStatus[u8MlResIdx] == E_SM_RET_OK) {
		if (sm_ml_fire(fd, &fire_info) != E_SM_RET_OK) {
			DRM_ERROR("[GOP][%s, %d]: sm_ml_fire failed, mem_index:%d\n",
				__func__, __LINE__, fire_info.mem_index);
			ret = GOP_MENULOAD_RETRY;
		}
	} else {
		DRM_ERROR("[GOP][%s, %d]: skip menuload fire\n", __func__, __LINE__);
		ret = GOP_FAIL;
	}

	pGOPCtx->pGOPCtxShared->u16ML_CmdCnd[u8MlResIdx] = 0;
	pGOPCtx->pGOPCtxShared->u16ML_PQCmdCnt[u8MlResIdx] = 0;
...
GOP_Result MDrv_GOP_ml_get_mem_info(MS_GOP_CTX_LOCAL *pGOPCtx, MS_U8 u8MlResIdx)
{
	...
	if (sm_ml_get_mem_index(fd, &u8MemIndex, 0) != E_SM_RET_OK) {
		DRM_ERROR("[GOP][%s, %d]: sm_ml_get_mem_index failed\n", __func__, __LINE__);
		pGOPCtx->pGOPCtxShared->eMLStatus[u8MlResIdx] = E_SM_RET_FAIL;
		return GOP_FAIL;
	}

	if (sm_ml_get_info(fd, u8MemIndex,
	&pGOPCtx->pGOPCtxShared->stMlInfo[u8MlResIdx]) != E_SM_RET_OK) {
		DRM_ERROR("[GOP][%s, %d]: sm_ml_get_info failed, mem_index:%d\n",
			__func__, __LINE__, u8MemIndex);
		pGOPCtx->pGOPCtxShared->eMLStatus[u8MlResIdx] = E_SM_RET_FAIL;
		return GOP_FAIL;
	}
	...
	pGOPCtx->pGOPCtxShared->u16ML_PQCmdCnt[u8MlResIdx] = 0;
	pGOPCtx->pGOPCtxShared->eMLStatus[u8MlResIdx] = E_SM_RET_OK;
...
GOP_Result MDrv_GOP_GetRoi(ST_GOP_GETROI *pGOPRoi)
{
	...
	if (KDrv_HW_GOP_GetROI(&stGOPRoi)) {
		pGOPRoi->u64CntOSDB0 = stGOPRoi.u64CntOSDB0;
		pGOPRoi->u64CntOSDB1 = stGOPRoi.u64CntOSDB1;
	} else {
		DRM_ERROR("[GOP][%s, %d]: KDrv_HW_GOP_GetROI failed\n",
				__func__, __LINE__);
		return GOP_FAIL;
	}
...
GOP_Result MDrv_GOP_TriggerAutoDetect(MS_U8 u8GOP)
{
	GOP_Result ret = GOP_SUCCESS;

	if (!KDrv_HW_GOP_TriggerAutoDetect(u8GOP)) {
		DRM_ERROR("[GOP][%s, %d]: KDrv_HW_GOP_TriggerAutoDetect failed\n",
				__func__, __LINE__);
		ret = GOP_FAIL;
	}
...
GOP_Result MDrv_GOP_SetOsdb_Dst(MS_U8 u8GOP, EN_GOP_OPDST eOpDstPos)
{
	GOP_Result ret = GOP_SUCCESS;
	EN_HW_GOP_OPDST eDstOSDB;

	switch (eOpDstPos) {
	case E_GOP_OPDST_OSDB0:
		eDstOSDB = E_HW_GOP_OPDST_OSDB0;
		break;
	case E_GOP_OPDST_OSDB1:
		eDstOSDB = E_HW_GOP_OPDST_OSDB1;
		break;
	default:
		eDstOSDB = E_HW_GOP_OPDST_INVALID;
		DRM_ERROR("[GOP][%s, %d]: Invalid OSDB dst %d\n",
				__func__, __LINE__, eOpDstPos);
		break;
	}

	if (!KDrv_HW_GOP_SetOsdb_Dst(u8GOP, eDstOSDB)) {
		DRM_ERROR("[GOP][%s, %d]: KDrv_HW_GOP_SetOsdb_Dst failed\n",
				__func__, __LINE__);
		ret = GOP_FAIL;
	}
...
GOP_Result MDrv_GOP_SetOsdbLoc(EN_GOP_OSDB_LOCATION eOsdbLoc)
{
	GOP_Result ret = GOP_SUCCESS;
	EN_HW_GOP_OSDB_LOCATION eHwOsdbLoc;

	switch (eOsdbLoc) {
	case E_GOP_PQGAMMA_OSDB_LD_PADJ:
		eHwOsdbLoc = E_HW_GOP_PQGAMMA_OSDB_LD_PADJ;
		break;
	case E_GOP_PQGAMMA_LD_OSDB_PADJ:
		eHwOsdbLoc = E_HW_GOP_PQGAMMA_LD_OSDB_PADJ;
		break;
	case E_GOP_OSDB_PQGAMMA_LD_PADJ:
		eHwOsdbLoc = E_HW_GOP_OSDB_PQGAMMA_LD_PADJ;
		break;
	case E_GOP_OSDB_LD_PQGAMMA_PADJ:
		eHwOsdbLoc = E_HW_GOP_OSDB_LD_PQGAMMA_PADJ;
		break;
	case E_GOP_LD_PQGAMMA_OSDB_PADJ:
		eHwOsdbLoc = E_HW_GOP_LD_PQGAMMA_OSDB_PADJ;
		break;
	case E_GOP_LD_OSDB_PQGAMMA_PADJ:
		eHwOsdbLoc = E_HW_GOP_LD_OSDB_PQGAMMA_PADJ;
		break;
	default:
		eHwOsdbLoc = E_HW_GOP_LOCATION_MAX;
		break;
	}

	if (!KDrv_HW_GOP_SetOsdbLoc(eHwOsdbLoc)) {
		DRM_ERROR("[GOP][%s, %d]: KDrv_HW_GOP_SetOsdbLoc failed\n",
				__func__, __LINE__);
		ret = GOP_FAIL;
	}
...
GOP_Result MDrv_GOP_SetMixer4Size(MS_U32 u32Width, MS_U32 u32Height)
{
	GOP_Result ret = GOP_SUCCESS;

	if (!KDrv_HW_GOP_SetMixer4OutSize(u32Width, u32Height)) {
		DRM_ERROR("[GOP][%s, %d]: KDrv_HW_GOP_SetMixer4OutSize failed\n",
				__func__, __LINE__);
		ret = GOP_FAIL;
	}
...
GOP_Result MDrv_GOP_SetMixer2Size(MS_U32 u32Width, MS_U32 u32Height)
{
	GOP_Result ret = GOP_SUCCESS;

	if (!KDrv_HW_GOP_SetMixer2OutSize(u32Width, u32Height)) {
		DRM_ERROR("[GOP][%s, %d]: KDrv_HW_GOP_SetMixer2OutSize failed\n",
				__func__, __LINE__);
		ret = GOP_FAIL;
	}
