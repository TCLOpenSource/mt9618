...
#define GOP_M_ERR(args...) { pr_err("MAPI_GOP" args); }
...
static MS_U8 _GOP_GWIN_IsGwinExistInClient(void *pInstance, MS_U8 gId)
{
	...
	if (result0 || (result1 && (result2 || result3 || result4))) {
		GOP_M_ERR("result0 = %d,result1 = %d,result2 = %d,result3 = %d,result4 = %d\n",
...
MS_U32 Ioctl_GOP_Init(void *pInstance, MS_U8 u8GOP, GOP_InitInfo *pGopInit)
{
	...
	if (pGopInit == NULL) {
		...
		g_pGOPCtxLocal->pGOPCtxShared->u32ML_Fd[E_GOP_ML_DISPSYNC] = pGopInit->u32ML_Fd;
		g_pGOPCtxLocal->pGOPCtxShared->u32ML_Fd[E_GOP_ML_OSDSYNC] = pGopInit->u32ML_FdVGSep;
...
MS_U32 GOP_SetAdlnfo(void *pInstance, MS_U8 u8GOP, ST_GOP_ADL_INFO *pGopAdlInfo)
{
	...
	if (pGopAdlInfo->u64_user_adl_buf != NULL) {
		if (pGopAdlInfo->adl_bufsize > ADL_MAX_BUFSIZE) {
			GOP_M_ERR("[%s, %d]adl_bufsize:0x%x > ADL_MAX_BUFSIZE\n",
	...
	if (pHwRegInfo != NULL) {
		for (i = 0; i < sizeof(MS_U32); i++) {
			if (pData != NULL) {
				...
				GOP_M_ERR("[GOP][%s, %d] pData is NULL\n", __func__, __LINE__);
...
MS_U32 GOP_SetPqMlnfo(void *pInstance, MS_U8 u8GOP, ST_GOP_PQ_CFD_ML_INFO *pGopMlInfo)
{
	...
	UtopiaInstanceGetPrivate(pInstance, (void *)&psGOPInstPri);
#endif
	u32_cfd_ml_bufsize = pGopMlInfo->u32_cfd_ml_bufsize;
	u32_pq_ml_bufsize = pGopMlInfo->u32_pq_ml_bufsize;
	pGopCtx = g_pGOPCtxLocal->pGOPCtxShared;
...
MS_U32 GOP_TriggerAutoDetect(MS_U8 u8GOP)
{
	return MDrv_GOP_TriggerAutoDetect(u8GOP);
}

MS_U32 GOP_SetOsdb_Dst(MS_U8 u8GOP, EN_GOP_OPDST eOpDstPos)
{
	return MDrv_GOP_SetOsdb_Dst(u8GOP, eOpDstPos);
}

MS_U32 GOP_SetOsdbLoc(EN_GOP_OSDB_LOCATION eOsdbLoc)
{
	return MDrv_GOP_SetOsdbLoc(eOsdbLoc);
}

MS_U32 GOP_SetOverScanInfo(void *pInstance, MS_U8 u8GOP, MS_BOOL bOverScanEn)
{
#ifdef INSTANT_PRIVATE
	GOP_INSTANT_PRIVATE	* psGOPInstPri = NULL;
	...
	g_pGOPCtxLocal->pGOPCtxShared->GopOverScanEn[u8GOP] = bOverScanEn;

	return GOP_API_SUCCESS;
}

MS_U32 GOP_SetMixer4Size(MS_U32 u32Width, MS_U32 u32Height)
{
	return MDrv_GOP_SetMixer4Size(u32Width, u32Height);
}

MS_U32 GOP_SetMixer2Size(MS_U32 u32Width, MS_U32 u32Height)
{
	return MDrv_GOP_SetMixer2Size(u32Width, u32Height);
}

MS_U32 GOP_SetPanelSize(void *pInstance, MS_U8 u8GOP, MS_U32 u32Width, MS_U32 u32Height)
{
#ifdef INSTANT_PRIVATE
	GOP_INSTANT_PRIVATE	* psGOPInstPri = NULL;
	...
	g_pGOPCtxLocal->pGOPCtxShared->u16PnlWidth[u8GOP] = u32Width;
	g_pGOPCtxLocal->pGOPCtxShared->u16PnlHeight[u8GOP] = u32Height;
