					 ...
					 struct drm_crtc_state *old_crtc_state)
{
	...
	if (crtc->state->event) {
		spin_lock_irqsave(&mtk_tv_crtc->event_lock, flags);
		mtk_tv_crtc->event = crtc->state->event;
		mtk_tv_crtc->pending_needs_vblank = true;
		crtc->state->event = NULL;
		spin_unlock_irqrestore(&mtk_tv_crtc->event_lock, flags);
	}
}
			   ...
			   const struct mtk_tv_drm_crtc_ops *ops)
{
	...
	spin_lock_init(&mtk_tv_crtc->event_lock);
						...
						struct drm_file *file_priv)
{
	...
	drm_for_each_crtc(crtc, dev) {
		...
		if (mtk_tv_crtc->ops->get_graphic_roi) {
			ret = mtk_tv_crtc->ops->get_graphic_roi(args);
			break;
		}
	}
						...
						struct drm_file *file_priv)
{
	...
	drm_for_each_crtc(crtc, dev) {
		...
		if (mtk_tv_crtc->ops->set_graphic_roi) {
			ret = mtk_tv_crtc->ops->set_graphic_roi(threshold);
			break;
		}
	}
						...
						struct drm_file *file_priv)
{
	...
	drm_for_each_crtc(crtc, dev) {
		mtk_tv_crtc = to_mtk_tv_crtc(crtc);
		if (mtk_tv_crtc->ops->pnlgamma_get_enable) {
			ret = mtk_tv_crtc->ops->pnlgamma_get_enable(args);
			break;
		}
	}
	return ret;
