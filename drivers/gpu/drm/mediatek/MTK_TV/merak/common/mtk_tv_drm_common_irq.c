	...
	uint32_t u32IRQ_Version)
{
	...
	ret = drv_hwreg_render_common_irq_set_mask(paramIn, &paramOut, u32IRQ_Version, false);
	...
	uint32_t u32IRQ_Version)
{
	...
	ret = drv_hwreg_render_common_irq_set_clr(paramIn, &paramOut, u32IRQ_Version, false);
	...
	uint32_t u32IRQ_Version)
{
	...
	ret = drv_hwreg_render_common_irq_get_status(paramIn, &paramOut, u32IRQ_Version, false);
...
int mtk_common_irq_suspend(void)
{
	...
	if (np != NULL) {
		...
		irq_mod_detect = of_irq_get(np, 2);
		if (irq_mod_detect < 0) {
			DRM_INFO("of_irq_get failed\n");
			return -EINVAL;
		}
