...
void _vb1_gfx_4k_setting(struct mtk_drm_panel_context *pStPanelCtx)
{
	...
	if (pStPanelCtx->pnlInfo.vbo_byte == E_VBO_4BYTE_MODE) {
		if (pStPanelCtx->cfg.format == E_OUTPUT_ARGB8888_MODE0)
			drv_hwreg_render_pnl_set_vby1_gfx_4k_60HZ_MODE0();
		if (pStPanelCtx->cfg.format == E_OUTPUT_ARGB8888_MODE1)
			drv_hwreg_render_pnl_set_vby1_gfx_4k_60HZ_MODE1();
	}
...
void _mtk_gfxdev_vby1_lane_order(struct mtk_drm_panel_context *pStPanelCtx)
{
	if (!pStPanelCtx) {
		DRM_ERROR("[%s, %d]: Panel context is NULL.\n",
			__func__, __LINE__);
	...
	if (pStPanelCtx->cfg.linkIF != E_LINK_NONE) {
		drv_hwreg_render_pnl_set_vby1_lane_order_gfx(
			pStPanelCtx->cfg.lanes,
			pStPanelCtx->pnlInfo.hsync_pol,
			pStPanelCtx->pnlInfo.vsync_pol);
	}
}
...
static void _mtk_gfx_vby1_setting(struct mtk_drm_panel_context *pStPanelCtx)
{
	...
	if (pStPanelCtx->cfg.linkIF != E_LINK_NONE) {
		...
		case E_4K2K_60HZ:
			...
			_mtk_gfxdev_vby1_lane_order(pStPanelCtx);
		break;
	...
	//_vby1_set_mft_gfx to call drv_set_vby1_mft
	drv_hwreg_render_pnl_set_vby1_mft_gfx((en_drv_pnl_link_if)pStPanelCtx->cfg.linkIF,
...
static void _mtk_gfx_mft_hbkproch_protect_init(struct mtk_drm_panel_context *pStPanelCtx)
{
	...
	drv_hwreg_render_pnl_gfx_hbkproch_protect_init(&stHprotectInfoTmp, &sthbproch_protect_ans);
}
...
bool mtk_render_gfx_cfg_connector(struct mtk_drm_panel_context *pStPanelCtx)
{
	...
	en_drv_pnl_output_format eColorformat = E_DRV_OUTPUT_RGB;
	...
	if (pStPanelCtx->cfg.linkIF != E_LINK_NONE) {
		if (pStPanelCtx->cfg.timing == E_4K2K_60HZ)
			...
			out_clk_if = 0;
		if ((pStPanelCtx->cfg.format == E_OUTPUT_ARGB8888_W_DITHER) ||
			(pStPanelCtx->cfg.format == E_OUTPUT_ARGB8888_W_ROUND) ||
			(pStPanelCtx->cfg.format == E_OUTPUT_ARGB8888_MODE0) ||
			(pStPanelCtx->cfg.format == E_OUTPUT_ARGB8888_MODE1))
			eColorformat = E_DRV_OUTPUT_RGBA8888;
		else if (pStPanelCtx->cfg.format == E_OUTPUT_ARGB8101010)
			eColorformat = E_DRV_OUTPUT_RGBA1010108;
		else
			eColorformat = (en_drv_pnl_output_format)pStPanelCtx->cfg.format;
	...
	if (pStPanelCtx->out_upd_type != E_NO_UPDATE_TYPE) {
		...
		drv_hwreg_render_pnl_set_osd_vby1_fifo_reset();
