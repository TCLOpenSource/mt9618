...
static void _mtk_delta_mft_hbkproch_protect_init(struct mtk_drm_panel_context *pStPanelCtx)
{
	...
	stHprotectInfoTmp.op_format =  (en_drv_pnl_output_format)pStPanelCtx->pnlInfo.op_format;
	sthbproch_protect_ans.verify_enable = false;

	drv_hwreg_render_pnl_delta_hbkproch_protect_init(
		&stHprotectInfoTmp, &sthbproch_protect_ans);
}
...
static void _mtk_extdev_vby1_setting(struct mtk_drm_panel_context *pStPanelCtx)
{
	...
	//_vby1_set_mft_exdev to call drv_set_vby1_mft
	drv_hwreg_render_pnl_set_vby1_mft_deltav((en_drv_pnl_link_if)pStPanelCtx->cfg.linkIF,
							pStPanelCtx->cfg.div_sec,
							pStPanelCtx->cfg.lanes,
							pStPanelCtx->pnlInfo.de_width);
}
...
void _mtk_video_display_set_extdev_panel_color_format(struct mtk_drm_panel_context *pStPanelCtx)
{
	...
	if (drm_con == NULL) {
		DRM_INFO("[%s][%d]drm_con is NULL\n", __func__, __LINE__);
		return;
	}

	mtk_connector = to_mtk_tv_connector(drm_con);
	if (mtk_connector == NULL) {
		DRM_INFO("[%s][%d]mtk_connector is NULL\n", __func__, __LINE__);
		return;
	}

	pctx_kms = (struct mtk_tv_kms_context *)mtk_connector->connector_private;
	if (pctx_kms == NULL) {
		DRM_INFO("[%s][%d]pctx_kms is NULL\n", __func__, __LINE__);
		return;
	}

	pctx_pnl = &pctx_kms->panel_priv;
	if (pctx_pnl == NULL) {
		DRM_INFO("[%s][%d]pctx_pnl is NULL\n", __func__, __LINE__);
		return;
	}
	...
	if (mi_render_csc_input == NULL) {
		...
		cfd_csc_input.u8Data_Format = pctx_pnl->cus.render_in_color_format;
		cfd_csc_input.u8ColorSpace = pctx_pnl->cus.render_in_color_space;
		cfd_csc_input.u8Range = pctx_pnl->cus.render_in_color_range;
	} else {
	...
	cfd_csc_video_output.u8Data_Format = pctx_pnl->info.op_format;
	cfd_csc_video_output.u8ColorSpace = pctx_pnl->info.op_color_space;
	cfd_csc_video_output.u8Range = pctx_pnl->info.op_color_range;

	//set video2 color format
	cfd_csc_delta_output.u8Data_Format = pStPanelCtx->pnlInfo.op_format;
	cfd_csc_delta_output.u8ColorSpace = pStPanelCtx->pnlInfo.op_color_space;
	cfd_csc_delta_output.u8Range = pStPanelCtx->pnlInfo.op_color_range;
	...
	mtk_video_display_SetCFD_Setting(pctx_pnl->hw_info.pnl_lib_version,
		&cfd_csc_input, &cfd_csc_video_output, &cfd_csc_delta_output);

	mtk_video_display_set_defaultBackgroundColor(&pctx_kms->video_priv);
}
...
bool mtk_render_extdev_cfg_connector(struct mtk_drm_panel_context *pStPanelCtx)
{
	...
	if (pStPanelCtx->out_upd_type != E_NO_UPDATE_TYPE) {
		...
		drv_hwreg_render_pnl_set_video2_vby1_fifo_reset();
		mtk_extdev_render_output_en(pStPanelCtx, true);
		_mtk_video_display_set_extdev_panel_color_format(pStPanelCtx);
...
static int mtk_tv_drm_extdev_panel_update_kms_pnl_info(struct drm_panel *panel)
{
	...
	mtk_tv_drm_panel_checkDTSPara(&pStPanelCtx->pnlInfo);

	drm_con = pStPanelCtx->drmPanel.connector;
	mtk_connector = to_mtk_tv_connector(drm_con);
	pctx = (struct mtk_tv_kms_context *)mtk_connector->connector_private;
	memcpy(&pctx->panel_priv.extdev_info, &pStPanelCtx->pnlInfo, sizeof(drm_st_pnl_info));
	return 0;
}
...
static int mtk_tv_drm_extdev_panel_prepare(struct drm_panel *panel)
{
	...
	if (drmCrtcState->mode_changed
		...
		if (ret) {
			...
			mtk_tv_drm_extdev_panel_update_kms_pnl_info(panel);
...
static int mtk_tv_drm_extdev_panel_probe(struct platform_device *pdev)
{
	...
	if (!pStPanelCtx) {
		DRM_ERROR("[%s, %d]: pStPanelCtx allocated memopry failed\n",
			__func__, __LINE__);
		return -ENOMEM;
	}
