...
static void mtk_tv_sync_timeline_fence_release(struct dma_fence *fence)
{
	...
	DRM_INFO("[%s][%d][drm]fence free,parent:%s, pt: 0x%p, fence: 0x%p\n",
					      ...
					      int size)
{
	...
	length_needed = snprintf(str, size, "%llu", fence->seqno);
	if (length_needed < 0)
		DRM_ERROR("[%s, %d]snprintf fail\r\n", __func__, __LINE__);
}
						       ...
						       char *str, int size)
{
	...
	length_needed = snprintf(str, size, "%d", parent->value);
	if (length_needed < 0)
		DRM_ERROR("[%s, %d]snprintf fail\r\n", __func__, __LINE__);
}
				     ...
				     unsigned int inc)
{
	...
	current_timeline = obj->value;

	spin_lock_irq(&obj->lock);

	if ((current_timeline + inc) > INT_MAX) {
		DRM_INFO("[%s][%d][drm]timeline:%s, value++ %lld > INT_MAX,clear 0\n",
			__func__, __LINE__, obj->name, (current_timeline + 1));
		obj->value = 0;
	} else {
		obj->value += inc;
		DRM_INFO("[%s][%d][drm]timeline:%s, value=%lld -> %d\n", __func__, __LINE__,
			obj->name, current_timeline, obj->value);
	}

	list_for_each_entry_safe(pt, next, &obj->pt_list, link) {
		DRM_INFO("[%s][%d][drm]pt =  0x%p\n", __func__, __LINE__, pt);
		 ...
		 */
		DRM_INFO("[%s][%d][drm]signal fence: 0x%p, fence ref cnt %d\n",
...
int mtk_tv_sync_fence_create(struct sync_timeline *obj, struct mtk_fence_data *data)
{
	...
	DRM_INFO("[%s][%d][drm]create fence: 0x%p\n", __func__, __LINE__, &pt->base);
	data->fence = fd;
